# -*- coding: utf-8 -*-
# =============================================================================
# 019_Script_BESS_Simulacao_018_4bus_0700_2220_2380_1250_duas_simulacoes_conjugado_5000_ano_2047.py
#
# Regras deste script:
...
#     Simulacao 018 segmentos_pass1_sem_hvac_int_ano_2047
#   PASSO 2 (simulacao completa final):
#     Simulacao 018 segmentos_ano_2047
# =============================================================================
...
DIR_PASS1 = WORK_DIR / "Simulacao 018 segmentos_pass1_sem_hvac_int_ano_2047"
...
def build_circuit_pass1(
    ...
    dss.text(
        ...
        f"kWrated=673 kWhrated=2841 Effcurve=Eff %EffCharge=95 %EffDischarge=95 "
...
DIR_PASS2 = WORK_DIR / "Simulacao 018 segmentos_ano_2047"
...
# =============================================================================
# Entrada:
#   Simulacao 018 segmentos_ano_2047/UFSM_Mon_<KEY>_s*.csv
# Saida:
#   Simulacao 018 segmentos_ano_2047/Monitor_<KEY>_anual.csv
# Remove:
...
# =============================================================================
# Entrada:
#   Simulacao 018 segmentos_ano_2047/EXP_MTR_GERAL_*.csv
# Saida:
#   Simulacao 018 segmentos_ano_2047/EXP_MTR_GERAL_anual.csv
# Remove:
#   EXP_MTR_GERAL_*.csv
# =============================================================================
...
#   Simulacao 018 segmentos_ano_2047/Monitor_<KEY>_anual.csv
# Saidas:
#   Simulacao 018 segmentos_ano_2047/Curva_de_Carga_<KEY>_sim_18.csv
# Remove:
...
#   Simulacao 018 segmentos_ano_2047/EXP_MTR_GERAL_anual.csv
# Saida:
#   Simulacao 018 segmentos_ano_2047/Resumo_EnergyMeter_Geral_Sim18.csv
# =============================================================================
...
# %% Celula 010 - ENERGIA e DEMANDA MAX por mes e posto (SIM18 - ANO 2047, 4 BESS degradados)
# =============================================================================
# Entradas:
#   META: 00_dados_de_entrada_ano_letivo_2025_15min.csv
#         Usado apenas como mapa anual de Hora_decimal, Mes e Posto.
#   SIM18: curva GERAL com 4 BESS em operacao conjugada para o ano 2047
# Saida:
#   Simulacao 018 segmentos_ano_2047/01_Integralizacao_Mensal_SIM18_2047.csv
#
# Observacao:
#   Esta celula nao compara com SIM2/RGE nem com outra simulacao.
#   Ela apenas integraliza a simulacao conjugada com 4 BESS do ano 2047 por mes e posto.
# =============================================================================
...
# %% Celula 011 - SIMULACAO 018: tabela de custos (SIM18 - ANO 2047, 4 BESS degradados)
# =============================================================================
# Entrada:
#   Simulacao 018 segmentos_ano_2047/01_Integralizacao_Mensal_SIM18_2047.csv
#
# Saida:
#   Simulacao 018 segmentos_ano_2047/Tabela_demanda_consumo_por_posto_SIM18_2047.csv
#
# Demandas contratadas do ano 2047:
#   DCFP = 5550 kW
#   DCP sem BESS = 3000 kW * (1 + 22 * 0,5%) = 3330 kW
#   DCP com 4 BESS = 3330 - (4 x 673) = 638 kW
#
# Observacao:
...
# - Os 4 BESS foram dimensionados como 800 kW / 3375 kWh cada no ano inicial.
# - Em 2047, cada banco opera com kWrated=673 kW e kWhrated=2841 kWh.
# - Assim, o conjunto reduz a DCP em 4 x 673 = 2692 kW e totaliza 11364 kWh.
# - O percentual de reserva para ponta e mantido igual ao caso original: 12000/13500 = 88,8888889%.
DCP_BASE_2025 = 3000.0
DCFP_BASE_2025 = 5000.0
CRESC_LINEAR_ANUAL = 0.005

NUM_BESS = 4
BESS_P_UNITARIA_KW = 673.0      # potencia operacional degradada usada para reduzir a DCP
BESS_E_UNITARIA_KWH = 2841.0    # energia operacional degradada usada no OpenDSS

BESS_P_TOTAL_KW = NUM_BESS * BESS_P_UNITARIA_KW       # 2692 kW em 2047
BESS_E_TOTAL_KWH = NUM_BESS * BESS_E_UNITARIA_KWH     # 11364 kWh em 2047

# Separacao operacional da energia:
# - mesmo percentual de reserva da ponta do caso original: 12000 kWh / 13500 kWh;
# - o valor absoluto da reserva reduz proporcionalmente com a degradacao energetica.
BESS_RESERVA_PONTA_PCT = 100.0 * 12000.0 / 13500.0
...
DCP_COM_BESS = max(0.0, DCP_SEM_BESS - BESS_P_TOTAL_KW)       # 638 kW em 2047
...
if not IN_FP.exists():
    raise FileNotFoundError(
        f"Arquivo de entrada nao encontrado: {IN_FP}. "
        f"Execute primeiro a Celula 010 para gerar a integralizacao mensal."
    )
...
print("DONE. Saved (Simulacao 018 - 4 BESS - 2047 - conjugado degradado):")
print(" -", OUT_SIM18)
...
print(f" - Potencia operacional unitaria dos BESS = {BESS_P_UNITARIA_KW:.0f} kW")
print(f" - Potencia operacional total dos BESS = {BESS_P_TOTAL_KW:.0f} kW")
print(f" - Energia operacional unitaria dos BESS = {BESS_E_UNITARIA_KWH:.0f} kWh")
print(f" - Energia operacional total dos BESS = {BESS_E_TOTAL_KWH:.0f} kWh")
