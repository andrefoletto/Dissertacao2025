# -*- coding: utf-8 -*-
# =============================================================================
# 013_Script_BESS_Simulacao_012_4bus_0700_2220_2380_1250_duas_simulacoes_ano_2028_2034_2037_2039.py
#
# Regras deste script:
...
#     Simulacao 012 segmentos_pass1_sem_hvac_int_ano_2039
#   PASSO 2 (simulacao completa final):
#     Simulacao 012 segmentos_ano_2039
# =============================================================================
...
TARGETS = {
    ...
    "FORA_PONTA": {"kWtarget": 9999, "kWtargetLow": 5350},
}

DIR_PASS1 = WORK_DIR / "Simulacao 012 segmentos_pass1_sem_hvac_int_ano_2039"
...
def build_circuit_pass1(
    ...
    dss.text(
        ...
        "kWrated=727 kWhrated=2725 Effcurve=Eff %EffCharge=95 %EffDischarge=95 "
        "%reserve=2.5 dispmode=external"
    )
    dss.text(
        "New Storage.BESSM2 phases=3 bus1=B023 kv=0.38 "
        "kWrated=767 kWhrated=2875 Effcurve=Eff %EffCharge=95 %EffDischarge=95 "
        "%reserve=2.5 dispmode=external"
    )
    dss.text(
        "New Storage.BESSM3 phases=3 bus1=B033 kv=0.38 "
        "kWrated=787 kWhrated=2950 Effcurve=Eff %EffCharge=95 %EffDischarge=95 "
        "%reserve=2.5 dispmode=external"
    )
    dss.text(
...
# %% Celula 003 - PASSO 1: monitores -> Curva_de_Carga_BESSM1_IN_sim_12.csv, BESSM2_IN, BESSM3_IN e BESSM4_IN (SHIFT DIREITA)
# =============================================================================
# Entradas:
#   PASSO1/Monitor_BESSM1_IN_anual.csv
#   PASSO1/Monitor_BESSM2_IN_anual.csv
...
#   PASSO1/Monitor_BESSM4_IN_anual.csv
# Saidas:
#   PASSO1/Curva_de_Carga_BESSM1_IN_sim_12.csv
#   PASSO1/Curva_de_Carga_BESSM2_IN_sim_12.csv
#   PASSO1/Curva_de_Carga_BESSM3_IN_sim_12.csv
#   PASSO1/Curva_de_Carga_BESSM4_IN_sim_12.csv
# Remove:
#   PASSO1/Monitor_BESSM1_IN_anual.csv
#   PASSO1/Monitor_BESSM2_IN_anual.csv
#   PASSO1/Monitor_BESSM3_IN_anual.csv
...
for mon_base in ["BESSM1_IN", "BESSM2_IN", "BESSM3_IN", "BESSM4_IN"]:
    ...
    out_fp = DIR_PASS1 / f"Curva_de_Carga_{mon_base}_sim_12.csv"
...
for mon_base, prefix in [("BESSM1_IN", "BESSM1"), ("BESSM2_IN", "BESSM2"), ("BESSM3_IN", "BESSM3"), ("BESSM4_IN", "BESSM4")]:
    in_fp = DIR_PASS1 / f"Curva_de_Carga_{mon_base}_sim_12.csv"
...
DIR_PASS2 = WORK_DIR / "Simulacao 012 segmentos_ano_2039"
...
# =============================================================================
# Entrada:
#   Simulacao 012 segmentos_ano_2039/UFSM_Mon_<KEY>_s*.csv
# Saida:
#   Simulacao 012 segmentos_ano_2039/Monitor_<KEY>_anual.csv
# Remove:
...
# =============================================================================
# Entrada:
#   Simulacao 012 segmentos_ano_2039/EXP_MTR_GERAL_*.csv
# Saida:
#   Simulacao 012 segmentos_ano_2039/EXP_MTR_GERAL_anual.csv
# Remove:
#   EXP_MTR_GERAL_*.csv
# =============================================================================
...
#   Simulacao 012 segmentos_ano_2039/Monitor_<KEY>_anual.csv
# Saidas:
#   Simulacao 012 segmentos_ano_2039/Curva_de_Carga_<KEY>_sim_12.csv
# Remove:
...
for in_fp in files:
    ...
    out_fp = DIR_PASS2 / f"Curva_de_Carga_{mon_key}_sim_12.csv"
...
#   Simulacao 012 segmentos_ano_2039/EXP_MTR_GERAL_anual.csv
# Saida:
#   Simulacao 012 segmentos_ano_2039/Resumo_EnergyMeter_Geral_Sim12.csv
# =============================================================================
...
OUT_FP = DIR_PASS2 / "Resumo_EnergyMeter_Geral_Sim12.csv"
...
# %% Celula 010 - ENERGIA e DEMANDA MAX por mes e posto (SIM12 - ANO 2039, 4 BESS)
# =============================================================================
# Entradas:
#   META: 00_dados_de_entrada_ano_letivo_2025_15min.csv
#         Usado apenas como mapa anual de Hora_decimal, Mes e Posto.
#   SIM12: curva GERAL com 4 BESS para o ano 2039
# Saida:
#   Simulacao 012 segmentos_ano_2039/01_Integralizacao_Mensal_SIM12_2039.csv
#
# Observacao:
#   Esta celula nao compara com SIM2/RGE nem com outra simulacao.
#   Ela apenas integraliza a simulacao com 4 BESS do ano 2039 por mes e posto.
# =============================================================================
...
SIM12_DIR = WORK_DIR / f"Simulacao 012 segmentos_ano_{ANO_ANALISE}"

META_FP = WORK_DIR / "00_dados_de_entrada_ano_letivo_2025_15min.csv"
SIM12_FP = SIM12_DIR / "Curva_de_Carga_GERAL_sim_12.csv"
OUT_FP = SIM12_DIR / f"01_Integralizacao_Mensal_{SIM_TAG}_{ANO_ANALISE}.csv"
...
for fp in [META_FP, SIM12_FP]:
    if not fp.exists():
        raise FileNotFoundError(fp)

meta = pd.read_csv(META_FP, dtype=str, encoding="utf-8-sig")
sim12 = pd.read_csv(SIM12_FP, encoding="utf-8-sig", sep=None, engine="python")

meta = clean_cols(meta)
sim12 = clean_cols(sim12)

for name, dfx, req in [
    ("META", meta, ["Hora_decimal", "Mes", "Posto"]),
    (SIM_TAG, sim12, ["Hora_decimal", "P_3f_S", "Q_3f_S"]),
]:
...
sim12["Hora_decimal"] = pd.to_numeric(sim12["Hora_decimal"], errors="coerce")
sim12["P_3f_S"] = pd.to_numeric(sim12["P_3f_S"], errors="coerce")
sim12["Q_3f_S"] = pd.to_numeric(sim12["Q_3f_S"], errors="coerce")
sim12.dropna(subset=["Hora_decimal", "P_3f_S", "Q_3f_S"], inplace=True)

meta["k"] = make_k(meta["Hora_decimal"])
sim12["k"] = make_k(sim12["Hora_decimal"])

meta = meta[["k", "Mes", "Posto"]].drop_duplicates(subset=["k"], keep="first").copy()
sim12 = sim12[["k", "P_3f_S", "Q_3f_S"]].drop_duplicates(subset=["k"], keep="first").copy()

df12 = meta.merge(sim12, on="k", how="left").rename(
    columns={"P_3f_S": "P_SIM12", "Q_3f_S": "Q_SIM12"}
)

if df12["P_SIM12"].isna().any():
    faltas12 = int(df12["P_SIM12"].isna().sum())
    raise ValueError(f"Falha de alinhamento por Hora_decimal/k: SIM12 faltas={faltas12}")

df12["E_P_SIM12_kWh"] = df12["P_SIM12"] * DT_H
df12["E_Q_SIM12_kVArh"] = df12["Q_SIM12"] * DT_H

group_cols = ["Mes", "Posto"]

agg12 = df12.groupby(group_cols, dropna=False).agg(
    N_pontos_SIM12=("P_SIM12", lambda s: int(s.notna().sum())),
    Consumo_P_SIM12_kWh=("E_P_SIM12_kWh", "sum"),
    Consumo_Q_SIM12_kVArh=("E_Q_SIM12_kVArh", "sum"),
    DemandaMax_P_SIM12_kW=("P_SIM12", "max"),
    DemandaMax_Q_SIM12_kVAr=("Q_SIM12", "max"),
).reset_index()

meta_counts = meta.groupby(group_cols, dropna=False).agg(N_pontos_META=("Posto", "size")).reset_index()
agg = agg12.merge(meta_counts, on=group_cols, how="left")
...
print("SIM12 usado:", SIM12_FP)
print(agg[["Mes", "Posto", "N_pontos_META", "N_pontos_SIM12", "DemandaMax_P_SIM12_kW"]])

# %% Celula 011 - SIMULACAO 012: tabela de custos (SIM12 - ANO 2039, 4 BESS)
# =============================================================================
# Entrada:
#   Simulacao 012 segmentos_ano_2039/01_Integralizacao_Mensal_SIM12_2039.csv
#
# Saida:
#   Simulacao 012 segmentos_ano_2039/Tabela_demanda_consumo_por_posto_SIM12_2039.csv
#
# Demandas contratadas do ano 2039:
#   DCFP = 5000 kW * (1 + 14 * 0,5%) = 5350 kW
#   DCP sem BESS = 3000 kW * (1 + 14 * 0,5%) = 3210 kW
#   DCP com 4 BESS = 3210 - (727 + 767 + 787 + 800) = 129 kW
#
# Observacao:
...
SIM12_DIR = WORK_DIR / f"Simulacao 012 segmentos_ano_{ANO_ANALISE}"
SIM12_DIR.mkdir(parents=True, exist_ok=True)

IN_FP = SIM12_DIR / f"01_Integralizacao_Mensal_{SIM_TAG}_{ANO_ANALISE}.csv"
OUT_SIM12 = SIM12_DIR / f"Tabela_demanda_consumo_por_posto_{SIM_TAG}_{ANO_ANALISE}.csv"

# -------------------------
# Demandas contratadas
# -------------------------
# Premissa:
# - 2039 e o ano 12 da analise.
# - Dados de carga sao de 2025.
# - Crescimento linear anual = 0,5% sobre a demanda-base de 2025.
# - Em 2039 aplica-se crescimento acumulado de 7,0%.
# - A demanda contratada de ponta com 4 BESS considera abatimento das potencias remanescentes dos bancos: 727, 767, 787 e 800 kW.
DCP_SEM_BESS = 3210.0
DCFP_COM_BESS = 5350.0

# Potencias remanescentes no ano de 2039:
# BESS1 instalado no inicio, BESS2 em 2034, BESS3 em 2037 e BESS4 em 2039.
BESS1_P_KW = 727.0
BESS2_P_KW = 767.0
BESS3_P_KW = 787.0
BESS4_P_KW = 800.0

P_BESS_TOTAL_KW = BESS1_P_KW + BESS2_P_KW + BESS3_P_KW + BESS4_P_KW  # 3081 kW
DCP_COM_BESS = max(0.0, DCP_SEM_BESS - P_BESS_TOTAL_KW)              # 129 kW

CONTR_SIM12 = {"DCFP": DCFP_COM_BESS, "DCP": DCP_COM_BESS}
...
tab12 = tabela_custos(
    df,
    SIM_TAG,
    DCFP=CONTR_SIM12["DCFP"],
    DCP=CONTR_SIM12["DCP"],
)

tab12.to_csv(OUT_SIM12, index=False, encoding="utf-8")

print("DONE. Saved (Simulacao 012 - 4 BESS - 2039):")
print(" -", OUT_SIM12)
print("\nDemandas contratadas usadas em 2039:")
print(f" - DCFP = {DCFP_COM_BESS:.0f} kW")
print(f" - DCP sem BESS = {DCP_SEM_BESS:.0f} kW")
print(f" - DCP com 4 BESS = {DCP_COM_BESS:.0f} kW")
print("\nResumo anual:")
print(tab12[tab12["MES"].isin(["TOTAIS", "CTOTAL", "R$TOTAL", "DCFP", "DCP"])])
