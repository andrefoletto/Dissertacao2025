# -*- coding: utf-8 -*-
# =============================================================================
# 021_Script_BESS_Simulacao_020_6bus_0700_2220_2380_1250_1630_1950_duas_simulacoes_conjugado_5000_ano_2047.py
#
# Regras deste script:
...
#     Simulacao 020 segmentos_pass1_sem_hvac_int_ano_2047
#   PASSO 2 (simulacao completa final):
#     Simulacao 020 segmentos_ano_2047
# =============================================================================
...
DIR_PASS1 = WORK_DIR / "Simulacao 020 segmentos_pass1_sem_hvac_int_ano_2047"
...
def build_circuit_pass1(
    ...
    dss.text(
        ...
        f"kWrated=673 kWhrated=2735 Effcurve=Eff %EffCharge=95 %EffDischarge=95 "
...
DIR_PASS2 = WORK_DIR / "Simulacao 020 segmentos_ano_2047"
...
# =============================================================================
# Entrada:
#   Simulacao 020 segmentos_ano_2047/UFSM_Mon_<KEY>_s*.csv
# Saida:
#   Simulacao 020 segmentos_ano_2047/Monitor_<KEY>_anual.csv
# Remove:
...
# =============================================================================
# Entrada:
#   Simulacao 020 segmentos_ano_2047/EXP_MTR_GERAL_*.csv
# Saida:
#   Simulacao 020 segmentos_ano_2047/EXP_MTR_GERAL_anual.csv
# Remove:
#   EXP_MTR_GERAL_*.csv
# =============================================================================
...
#   Simulacao 020 segmentos_ano_2047/Monitor_<KEY>_anual.csv
# Saidas:
#   Simulacao 020 segmentos_ano_2047/Curva_de_Carga_<KEY>_sim_20.csv
# Remove:
...
#   Simulacao 020 segmentos_ano_2047/EXP_MTR_GERAL_anual.csv
# Saida:
#   Simulacao 020 segmentos_ano_2047/Resumo_EnergyMeter_Geral_Sim20.csv
# =============================================================================
...
# %% Celula 010 - ENERGIA e DEMANDA MAX por mes e posto (SIM20 - ANO 2047, 6 BESS 1 MW / 3,25 MWh cada, degradados)
# =============================================================================
# Entradas:
#   META: 00_dados_de_entrada_ano_letivo_2025_15min.csv
#         Usado apenas como mapa anual de Hora_decimal, Mes e Posto.
#   SIM20: curva GERAL com 6 BESS degradados em operacao conjugada para o ano 2047
# Saida:
#   Simulacao 020 segmentos_ano_2047/01_Integralizacao_Mensal_SIM20_2047.csv
#
# Observacao:
#   Esta celula nao compara com SIM2/RGE nem com outra simulacao.
#   Ela apenas integraliza a simulacao conjugada com 6 BESS degradados do ano 2047 por mes e posto.
# =============================================================================
...
# %% Celula 011 - SIMULACAO 020: tabela de custos (SIM20 - ANO 2047, 6 BESS 1 MW / 3,25 MWh cada, degradados)
# =============================================================================
# Entrada:
#   Simulacao 020 segmentos_ano_2047/01_Integralizacao_Mensal_SIM20_2047.csv
#
# Saida:
#   Simulacao 020 segmentos_ano_2047/Tabela_demanda_consumo_por_posto_SIM20_2047.csv
#
# Demandas contratadas do ano 2047:
#   DCFP = 5000 kW * (1 + 22 * 0,5%) = 5550 kW
#   DCP sem BESS = 3000 kW * (1 + 22 * 0,5%) = 3330 kW
#   DCP com 6 BESS = max(0, 3330 - (6 x 673)) = 0 kW
#
# Configuracao energetica dos BESS:
#   6 bancos originalmente de 1 MW / 3,25 MWh.
#   Operacao a 80% da potencia nominal e degradacao para 84,1666667%.
#   Potencia operacional degradada por banco = 673 kW.
#   Energia degradada por banco = 2735 kWh.
#   Potencia total = 6 x 673 = 4038 kW.
#   Energia total degradada = 6 x 2735 = 16410 kWh.
#   Percentual de reserva para ponta = 92,3%.
#
# Observacao:
...
# - Em 2047 aplica-se crescimento acumulado de 11%.
# - Os 6 BESS foram definidos originalmente como bancos de 1 MW / 3,25 MWh.
# - Cada banco opera a 80% da potencia nominal e, em 2047, esta degradado para 84,1666667%.
# - Assim, cada banco contribui com 673 kW e 2735 kWh.
# - O conjunto reduz a DCP em 6 x 673 = 4038 kW e totaliza 16410 kWh.
...
BESS_E_UNITARIA_KWH = 2735.0

BESS_P_TOTAL_KW = NUM_BESS * BESS_P_UNITARIA_KW       # 4038 kW
BESS_E_TOTAL_KWH = NUM_BESS * BESS_E_UNITARIA_KWH     # 16410 kWh

# Separacao operacional da energia:
# - 92,3% da energia degradada reservada para load-shift no horario de ponta;
# - restante disponivel para peak-shave fora-ponta.
BESS_RESERVA_PONTA_PCT = 92.3
BESS_E_RESERVA_PONTA_KWH = round(BESS_E_TOTAL_KWH * BESS_RESERVA_PONTA_PCT / 100.0, 0)
...
DCP_COM_BESS = max(0.0, DCP_SEM_BESS - BESS_P_TOTAL_KW)       # 0 kW em 2047
...
print("DONE. Saved (Simulacao 020 - 6 BESS degradados - 2047 - conjugado):")
print(" -", OUT_SIM20)
