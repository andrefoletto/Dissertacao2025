# -*- coding: utf-8 -*-
# =============================================================================
# 021_Script_BESS_Simulacao_020_6bus_0700_2220_2380_1250_1630_1950_duas_simulacoes_conjugado.py
#
# Regras deste script:
...
#     Simulacao 020 segmentos_pass1_sem_hvac_int
#   PASSO 2 (simulacao completa final):
#     Simulacao 020 segmentos
# =============================================================================
...
DIR_PASS1 = WORK_DIR / "Simulacao 020 segmentos_pass1_sem_hvac_int"
...
def build_circuit_pass1(
    ...
    dss.text(
        ...
        f"kWrated=800 kWhrated=3170 Effcurve=Eff %EffCharge=95 %EffDischarge=95 "
...
DIR_PASS2 = WORK_DIR / "Simulacao 020 segmentos"
...
# =============================================================================
# Entrada:
#   Simulacao 020 segmentos/UFSM_Mon_<KEY>_s*.csv
# Saida:
#   Simulacao 020 segmentos/Monitor_<KEY>_anual.csv
# Remove:
...
# =============================================================================
# Entrada:
#   Simulacao 020 segmentos/EXP_MTR_GERAL_*.csv
# Saida:
#   Simulacao 020 segmentos/EXP_MTR_GERAL_anual.csv
# Remove:
#   EXP_MTR_GERAL_*.csv
# =============================================================================
...
#   Simulacao 020 segmentos/Monitor_<KEY>_anual.csv
# Saidas:
#   Simulacao 020 segmentos/Curva_de_Carga_<KEY>_sim_20.csv
# Remove:
...
#   Simulacao 020 segmentos/EXP_MTR_GERAL_anual.csv
# Saida:
#   Simulacao 020 segmentos/Resumo_EnergyMeter_Geral_Sim20.csv
# =============================================================================
...
# %% Celula 010 - ENERGIA e DEMANDA MAX por mes e posto (SIM2 vs SIM20)
# =============================================================================
# Entradas:
...
#   SIM20: Simulacao 020 segmentos/Curva_de_Carga_GERAL_sim_20.csv (Hora_decimal, P_3f_S, Q_3f_S)
# Saida:
#   Simulacao 020 segmentos/01_Integralizacao_Mensal_SIM2_x_SIM20_2025.csv
# =============================================================================
...
SIM20_DIR = WORK_DIR / "Simulacao 020 segmentos"
...
OUT_FP = SIM20_DIR / "01_Integralizacao_Mensal_SIM2_x_SIM20_2025.csv"
...
for fp in [META_FP, SIM2_FP, SIM20_FP]:
    if not fp.exists():
...
for name, dfx, req in [
    ...
    ("SIM20", sim20, ["Hora_decimal", "P_3f_S", "Q_3f_S"]),
]:
...
df20 = meta.merge(sim20, on="k", how="left").rename(columns={"P_3f_S": "P_SIM20", "Q_3f_S": "Q_SIM20"})
...
agg = agg2.merge(agg20, on=group_cols, how="outer")
...
# %% Celula 011 - SIMULACAO 020: Tabela de custos (SIM2 vs SIM20 - 2025) com demandas contratadas diferentes
# =============================================================================
# Entrada:
#   Simulacao 020 segmentos/01_Integralizacao_Mensal_SIM2_x_SIM20_2025.csv
#
# Saidas (todas em Simulacao 020 segmentos):
#   Tabela_demanda_consumo_por_posto_SIM2_2025.csv
#   Tabela_demanda_consumo_por_posto_SIM20_2025.csv
#   Tabela_demanda_consumo_por_posto_SIM2_vs_SIM20_2025.csv   (economia)
# =============================================================================
...
IN_FP = SIM20_DIR / "01_Integralizacao_Mensal_SIM2_x_SIM20_2025.csv"
OUT_SIM2 = SIM20_DIR / "Tabela_demanda_consumo_por_posto_SIM2_2025.csv"
OUT_SIM20 = SIM20_DIR / "Tabela_demanda_consumo_por_posto_SIM20_2025.csv"
OUT_CMP = SIM20_DIR / "Tabela_demanda_consumo_por_posto_SIM2_vs_SIM20_2025.csv"

CONTR_SIM2 = {"DCFP": 5000.0, "DCP": 3000.0}
CONTR_SIM20 = {"DCFP": 5000.0, "DCP":   0.0}  # 3000 - 4*000
...
tab20 = tabela_custos(df, "SIM20", DCFP=CONTR_SIM20["DCFP"], DCP=CONTR_SIM20["DCP"])
...
print("DONE. Saved (Simulacao 020 segmentos):")
print(" -", OUT_SIM2)
print(" -", OUT_SIM20)
print(" -", OUT_CMP)
print("\nDemandas contratadas usadas:")
print(" - SIM2: DCFP=5000, DCP=3000")
print(" - SIM20: DCFP=5000, DCP=0")
print("\nResumo anual:")
print(cmp.tail(1))
