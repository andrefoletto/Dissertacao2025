# -*- coding: utf-8 -*-
# =============================================================================
# 024_Script_BESS_Simulacao_023_4bus_0700_2220_2380_1250_duas_simulacoes_conjugado_4500_ano_2028_2031_2033.py
#
# Regras deste script:
...
#     Simulacao 023 segmentos_pass1_sem_hvac_int_ano_2033
#   PASSO 2 (simulacao completa final):
#     Simulacao 023 segmentos_ano_2033
# =============================================================================
...
DIR_PASS1 = WORK_DIR / "Simulacao 023 segmentos_pass1_sem_hvac_int_ano_2033"
...
if LIMPAR_SAIDAS_ANTERIORES_PASS1:
    padroes_limpeza_pass1 = [
        ...
        "Curva_de_Carga_BESSM*_IN_sim_23.csv",
        "SOC_por_segmento.csv",
    ]
    removidos = 0
...
def build_circuit_pass1(
    ...
    dss.text('Redirect "UFSM_cargas_medidas_ano_2033.dss"')
    dss.text('Redirect "UFSM_cargas_especiais_ano_2033.dss"')
    dss.text('Redirect "UFSM_cargas_estimadas_com_residuo_ano_2033.dss"')
    ...
    dss.text(
        ...
        f"kWrated=767 kWhrated=5270.833 Effcurve=Eff %EffCharge=95 %EffDischarge=95 "
...
# %% Celula 003 - PASSO 1: monitores -> Curva_de_Carga_BESSM1_IN_sim_23.csv, BESSM2_IN, BESSM3_IN e BESSM4_IN (SHIFT DIREITA)
# =============================================================================
# Entradas:
#   PASSO1/Monitor_BESSM1_IN_anual.csv
#   PASSO1/Monitor_BESSM2_IN_anual.csv
...
#   PASSO1/Monitor_BESSM4_IN_anual.csv
# Saidas:
#   PASSO1/Curva_de_Carga_BESSM1_IN_sim_23.csv
#   PASSO1/Curva_de_Carga_BESSM2_IN_sim_23.csv
#   PASSO1/Curva_de_Carga_BESSM3_IN_sim_23.csv
#   PASSO1/Curva_de_Carga_BESSM4_IN_sim_23.csv
# Remove:
#   PASSO1/Monitor_BESSM1_IN_anual.csv
#   PASSO1/Monitor_BESSM2_IN_anual.csv
#   PASSO1/Monitor_BESSM3_IN_anual.csv
...
for mon_base in ["BESSM1_IN", "BESSM2_IN", "BESSM3_IN", "BESSM4_IN"]:
    ...
    out_fp = DIR_PASS1 / f"Curva_de_Carga_{mon_base}_sim_23.csv"
...
for mon_base, prefix in [("BESSM1_IN", "BESSM1"), ("BESSM2_IN", "BESSM2"), ("BESSM3_IN", "BESSM3"), ("BESSM4_IN", "BESSM4")]:
    in_fp = DIR_PASS1 / f"Curva_de_Carga_{mon_base}_sim_23.csv"
...
#     SOC_15min_BESS_SIM23_2033.csv          (SOC dos quatro bancos a cada 15 min)
#
# Requisitos:
...
BESS1_E_2033_KWH = 5270.833
BESS2_E_2033_KWH = 5270.833
BESS3_E_2033_KWH = 2950.0
BESS4_E_2033_KWH = 3000.0
BESS_TOTAL_E_2033_KWH = BESS1_E_2033_KWH + BESS2_E_2033_KWH + BESS3_E_2033_KWH + BESS4_E_2033_KWH
...
DIR_PASS2 = WORK_DIR / "Simulacao 023 segmentos_ano_2033"
...
if LIMPAR_SAIDAS_ANTERIORES_PASS2:
    padroes_limpeza = [
        "UFSM_Mon_*_s*.csv",
        "Monitor_*_anual.csv",
        "Curva_de_Carga_*_sim_23.csv",
        "EXP_MTR_GERAL*.csv",
        "Resumo_EnergyMeter_Geral_Sim23.csv",
        "SOC_por_segmento.csv",
        "SOC_15min_BESS_SIM23_2033.csv",
        "01_Integralizacao_Mensal_SIM23_2033.csv",
        "Tabela_demanda_consumo_por_posto_SIM23_2033.csv",
    ]
    removidos = 0
...
def _run_segment_pass2(
    ...
    for step_idx in range(int(n_steps)):
        ...
        soc_m1_kwh = soc_m1_pct / 100.0 * BESS1_E_2033_KWH
        soc_m2_kwh = soc_m2_pct / 100.0 * BESS2_E_2033_KWH
        soc_m3_kwh = soc_m3_pct / 100.0 * BESS3_E_2033_KWH
        soc_m4_kwh = soc_m4_pct / 100.0 * BESS4_E_2033_KWH
        soc_total_kwh = soc_m1_kwh + soc_m2_kwh + soc_m3_kwh + soc_m4_kwh
        soc_total_pct_pond = 100.0 * soc_total_kwh / BESS_TOTAL_E_2033_KWH
...
soc_15min_fp = DIR_PASS2 / "SOC_15min_BESS_SIM23_2033.csv"
...
# =============================================================================
# Entrada:
#   Simulacao 023 segmentos_ano_2033/UFSM_Mon_<KEY>_s*.csv
# Saida:
#   Simulacao 023 segmentos_ano_2033/Monitor_<KEY>_anual.csv
# Remove:
...
# =============================================================================
# Entrada:
#   Simulacao 023 segmentos_ano_2033/EXP_MTR_GERAL_*.csv
# Saida:
#   Simulacao 023 segmentos_ano_2033/EXP_MTR_GERAL_anual.csv
# Remove:
#   EXP_MTR_GERAL_*.csv
# =============================================================================
...
#   Simulacao 023 segmentos_ano_2033/Monitor_<KEY>_anual.csv
# Saidas:
#   Simulacao 023 segmentos_ano_2033/Curva_de_Carga_<KEY>_sim_23.csv
# Remove:
...
for in_fp in files:
    ...
    out_fp = DIR_PASS2 / f"Curva_de_Carga_{mon_key}_sim_23.csv"
...
#   Simulacao 023 segmentos_ano_2033/EXP_MTR_GERAL_anual.csv
# Saida:
#   Simulacao 023 segmentos_ano_2033/Resumo_EnergyMeter_Geral_Sim23.csv
# =============================================================================
...
OUT_FP = DIR_PASS2 / "Resumo_EnergyMeter_Geral_Sim23.csv"
...
# %% Celula 010 - ENERGIA e DEMANDA MAX por mes e posto (SIM23 - ANO 2033, BESS modular autofinanciado)
# =============================================================================
# Entradas:
#   META: 00_dados_de_entrada_ano_letivo_2025_15min.csv
#         Usado apenas como mapa anual de Hora_decimal, Mes e Posto.
#   SIM23: curva GERAL com BESS modular autofinanciado em operacao conjugada
#          para o ano 2033.
# Saida:
#   Simulacao 023 segmentos_ano_2033/01_Integralizacao_Mensal_SIM23_2033.csv
#
# Observacao:
#   Esta celula nao compara com SIM2/RGE nem com outra simulacao.
#   Ela apenas integraliza a simulacao conjugada do ano 2033 por mes e posto.
#   Configuracao do despacho:
#     BESS1: instalado em 2028, 1 MW / 5,5 MWh, operando a 80% da potencia,
#            degradado em 2033 por fator 0,958333333 para 767 kW / 5270,8 kWh.
...
#            degradado em 2033 por fator 0,983333333 para 787 kW / 2950 kWh.
#     BESS4: instalado em 2033, 1 MW / 3,0 MWh, operando a 80% da potencia,
#            sem degradacao: 800 kW / 3000 kWh.
#            sem degradacao: 800 kW / 3000 kWh.
#     Alvo fora-ponta = 4680 kW; permissao de recarga fora-ponta = 3640 kW.
#     Em fora-ponta, BESS1 e BESS2 usam reserva de 54,5455% e BESS3/BESS4 usam reserva de 100%.
#     Em ponta, os quatro bancos usam reserva minima tecnica de 2%.
#   A curva e lida diretamente pelo sufixo sim_23, sem fallback.
# =============================================================================
...
SIM23_DIR = WORK_DIR / f"Simulacao 023 segmentos_ano_{ANO_ANALISE}"

META_FP = WORK_DIR / "00_dados_de_entrada_ano_letivo_2025_15min.csv"
SIM23_FP = SIM23_DIR / "Curva_de_Carga_GERAL_sim_23.csv"
OUT_FP = SIM23_DIR / f"01_Integralizacao_Mensal_{SIM_TAG}_{ANO_ANALISE}.csv"
...
def gerar_curva_geral_sim23_se_necessario(sim23_fp: Path, dir_pass2: Path) -> None:
    """
    Garante a existencia de Curva_de_Carga_GERAL_sim_23.csv.
...
gerar_curva_geral_sim23_se_necessario(SIM23_FP, SIM23_DIR)
if not SIM23_FP.exists():
    raise FileNotFoundError(SIM23_FP)

meta = pd.read_csv(META_FP, dtype=str, encoding="utf-8-sig")
sim23 = pd.read_csv(SIM23_FP, encoding="utf-8-sig", sep=None, engine="python")
...
if len(sim23) != TOTAL_NPTS:
    raise ValueError(f"SIM23 gerou {len(sim23)} amostras anuais de 15 min; esperado {TOTAL_NPTS}.")

df23 = meta.merge(sim23, on="k", how="left").rename(
    columns={"P_3f_S": "P_SIM23", "Q_3f_S": "Q_SIM23"}
)

faltas_p = int(df23["P_SIM23"].isna().sum())
faltas_q = int(df23["Q_SIM23"].isna().sum())
if faltas_p or faltas_q:
    raise ValueError(
        f"Falha de alinhamento por Hora_decimal/k: "
        f"P_SIM23 faltas={faltas_p}, Q_SIM23 faltas={faltas_q}"
    )

df23["E_P_SIM23_kWh"] = df23["P_SIM23"] * DT_H
df23["E_Q_SIM23_kVArh"] = df23["Q_SIM23"] * DT_H

group_cols = ["Mes", "Posto"]

agg24 = df23.groupby(group_cols, dropna=False).agg(
    N_pontos_SIM23=("P_SIM23", lambda s: int(s.notna().sum())),
    Consumo_P_SIM23_kWh=("E_P_SIM23_kWh", "sum"),
    Consumo_Q_SIM23_kVArh=("E_Q_SIM23_kVArh", "sum"),
    DemandaMax_P_SIM23_kW=("P_SIM23", "max"),
    DemandaMax_Q_SIM23_kVAr=("Q_SIM23", "max"),
).reset_index()
...
print("SIM23 usado:", SIM23_FP)
print(agg[["Mes", "Posto", "N_pontos_META", "N_pontos_SIM23", "DemandaMax_P_SIM23_kW"]])

# %% Celula 011 - SIMULACAO 023: tabela de custos (SIM23 - ANO 2033, BESS modular autofinanciado)
# =============================================================================
# Entrada:
#   Simulacao 023 segmentos_ano_2033/01_Integralizacao_Mensal_SIM23_2033.csv
#
# Saida:
#   Simulacao 023 segmentos_ano_2033/Tabela_demanda_consumo_por_posto_SIM23_2033.csv
#
# Demandas contratadas do ano 2033:
#   DCFP contratada com BESS = 4500 * 1,04 = 4680 kW
#   Alvo operacional fora-ponta do despacho = 4680 kW
#   Permissao para recarga fora-ponta = 3500 * 1,04 = 3640 kW
#   DCP sem BESS = 3000 * 1,04 = 3120 kW
#   DCP com BESS = max(0, 3120 - (767 + 767 + 787 + 800)) = 0 kW
#
...
#     Em 2033: fator 0,958333333 -> 767 kW / 5270,8 kWh.
...
SIM23_DIR = WORK_DIR / f"Simulacao 023 segmentos_ano_{ANO_ANALISE}"
SIM23_DIR.mkdir(parents=True, exist_ok=True)

IN_FP = SIM23_DIR / f"01_Integralizacao_Mensal_{SIM_TAG}_{ANO_ANALISE}.csv"
OUT_SIM23 = SIM23_DIR / f"Tabela_demanda_consumo_por_posto_{SIM_TAG}_{ANO_ANALISE}.csv"

# -------------------------
# Demandas contratadas e alvos do despacho
# -------------------------
# Fora-ponta: 4500 * 1,04 = 4680 kW
# Ponta sem BESS: 3000 * 1,04 = 3120 kW
# Ponta com BESS: max(0, 3120 - (767 + 767 + 787 + 800)) = 0 kW
DCP_SEM_BESS = 3120.0
DCFP_COM_BESS = 4680.0

KW_TARGET_FP = 4680.0
KW_TARGET_LOW_FP = 3640.0
...
BESS1_FATOR_DEGRADACAO_2033 = 0.9583333333333333
BESS1_P_2033_KW = round(BESS1_P_OPERACIONAL_2028_KW * BESS1_FATOR_DEGRADACAO_2033)
BESS1_E_2033_KWH = BESS1_E_NOMINAL_KWH * BESS1_FATOR_DEGRADACAO_2033
BESS1_E_EXCEDENTE_FP_2033_KWH = BESS1_E_EXCEDENTE_FP_NOMINAL_KWH * BESS1_FATOR_DEGRADACAO_2033
...
BESS2_FATOR_DEGRADACAO_2033 = 0.9583333333333333
BESS2_P_2033_KW = round(BESS2_P_OPERACIONAL_2028_KW * BESS2_FATOR_DEGRADACAO_2033)
BESS2_E_2033_KWH = BESS2_E_NOMINAL_KWH * BESS2_FATOR_DEGRADACAO_2033
BESS2_E_EXCEDENTE_FP_2033_KWH = BESS2_E_EXCEDENTE_FP_NOMINAL_KWH * BESS2_FATOR_DEGRADACAO_2033

BESS3_ANO_INSTALACAO = 2031
BESS3_P_NOMINAL_KW = 1000.0
BESS3_P_OPERACIONAL_2031_KW = 800.0
BESS3_E_NOMINAL_KWH = 3000.0
BESS3_FATOR_DEGRADACAO_2033 = 0.9833333333333333
BESS3_P_2033_KW = round(BESS3_P_OPERACIONAL_2031_KW * BESS3_FATOR_DEGRADACAO_2033)
BESS3_E_2033_KWH = BESS3_E_NOMINAL_KWH * BESS3_FATOR_DEGRADACAO_2033

BESS4_ANO_INSTALACAO = 2033
BESS4_P_NOMINAL_KW = 1000.0
BESS4_P_OPERACIONAL_2033_KW = 800.0
BESS4_E_NOMINAL_KWH = 3000.0
BESS4_P_2033_KW = BESS4_P_OPERACIONAL_2033_KW
BESS4_E_2033_KWH = BESS4_E_NOMINAL_KWH

BESS_P_TOTAL_KW = BESS1_P_2033_KW + BESS2_P_2033_KW + BESS3_P_2033_KW + BESS4_P_2033_KW
BESS_E_TOTAL_KWH = BESS1_E_2033_KWH + BESS2_E_2033_KWH + BESS3_E_2033_KWH + BESS4_E_2033_KWH
...
#   BESS3 e BESS4 reservam 100% para a ponta e nao participam do peak-shave fora-ponta.
# PONTA:
...
BESS1_E_RESERVA_PONTA_2033_KWH = BESS1_E_2033_KWH * BESS1_RESERVA_PONTA_PCT / 100.0
BESS2_E_RESERVA_PONTA_2033_KWH = BESS2_E_2033_KWH * BESS2_RESERVA_PONTA_PCT / 100.0
BESS3_E_RESERVA_PONTA_2033_KWH = BESS3_E_2033_KWH * BESS3_RESERVA_PONTA_PCT / 100.0
BESS4_E_RESERVA_PONTA_2033_KWH = BESS4_E_2033_KWH * BESS4_RESERVA_PONTA_PCT / 100.0
BESS_E_RESERVA_PONTA_KWH = (
    BESS1_E_RESERVA_PONTA_2033_KWH
    + BESS2_E_RESERVA_PONTA_2033_KWH
    + BESS3_E_RESERVA_PONTA_2033_KWH
    + BESS4_E_RESERVA_PONTA_2033_KWH
)
BESS_E_DISPONIVEL_FP_KWH = (BESS1_E_2033_KWH - BESS1_E_RESERVA_PONTA_2033_KWH) + (BESS2_E_2033_KWH - BESS2_E_RESERVA_PONTA_2033_KWH)

DCP_COM_BESS = max(0.0, DCP_SEM_BESS - BESS_P_TOTAL_KW)

CONTR_SIM23 = {"DCFP": DCFP_COM_BESS, "DCP": DCP_COM_BESS}
...
if not IN_FP.exists():
    curva_fp = SIM23_DIR / "Curva_de_Carga_GERAL_sim_23.csv"
    raise FileNotFoundError(
...
tab24 = tabela_custos(
    df,
    SIM_TAG,
    DCFP=CONTR_SIM23["DCFP"],
    DCP=CONTR_SIM23["DCP"],
)

tab24.to_csv(OUT_SIM23, index=False, encoding="utf-8")

print("DONE. Saved (Simulacao 023 - BESS modular autofinanciado - 2033 - conjugado):")
print(" -", OUT_SIM23)
print("\nDemandas contratadas e alvos usados em 2033:")
...
print(f" - Potencia BESS1 2033 = {BESS1_P_2033_KW:.0f} kW")
print(f" - Potencia BESS2 2033 = {BESS2_P_2033_KW:.0f} kW")
print(f" - Potencia BESS3 2033 = {BESS3_P_2033_KW:.0f} kW")
print(f" - Potencia BESS4 2033 = {BESS4_P_2033_KW:.0f} kW")
print(f" - Energia nominal total instalada = {BESS_E_NOMINAL_TOTAL_KWH:.0f} kWh")
print(f" - Energia disponivel em 2033 = {BESS_E_TOTAL_KWH:.0f} kWh")
print(f" - Energia BESS1 2033 = {BESS1_E_2033_KWH:.0f} kWh")
print(f" - Energia BESS2 2033 = {BESS2_E_2033_KWH:.0f} kWh")
print(f" - Energia BESS3 2033 = {BESS3_E_2033_KWH:.0f} kWh")
print(f" - Energia BESS4 2033 = {BESS4_E_2033_KWH:.0f} kWh")
...
print(f" - Excedente degradado BESS1 para fora-ponta = {BESS1_E_EXCEDENTE_FP_2033_KWH:.0f} kWh")
print(f" - Excedente degradado BESS2 para fora-ponta = {BESS2_E_EXCEDENTE_FP_2033_KWH:.0f} kWh")
print(f" - Energia reservada para ponta BESS1 = {BESS1_E_RESERVA_PONTA_2033_KWH:.0f} kWh")
print(f" - Energia reservada para ponta BESS2 = {BESS2_E_RESERVA_PONTA_2033_KWH:.0f} kWh")
print(f" - Energia reservada para ponta BESS3 = {BESS3_E_RESERVA_PONTA_2033_KWH:.0f} kWh")
print(f" - Energia reservada para ponta BESS4 = {BESS4_E_RESERVA_PONTA_2033_KWH:.0f} kWh")
