# -*- coding: utf-8 -*-
# =============================================================================
# 026_Script_BESS_Simulacao_025_6bus_0700_2220_2380_1250_1630_1950_duas_simulacoes_conjugado_4500_ano_2028.py
#
# Regras deste script:
...
#     Simulacao 025 segmentos_pass1_sem_hvac_int_ano_2028
#   PASSO 2 (simulacao completa final):
#     Simulacao 025 segmentos_ano_2028
# =============================================================================
...
DIR_PASS1 = WORK_DIR / "Simulacao 025 segmentos_pass1_sem_hvac_int_ano_2028"
...
def build_circuit_pass1(
    ...
    dss.text(
        ...
        f"kWrated=800 kWhrated=3833 Effcurve=Eff %EffCharge=95 %EffDischarge=95 "
...
# %% Celula 003 - PASSO 1: monitores -> Curva_de_Carga_BESSM1_IN_sim_25.csv, BESSM2_IN, BESSM3_IN, BESSM4_IN, BESSM5_IN e BESSM6_IN (SHIFT DIREITA)
# =============================================================================
# Entradas:
#   PASSO1/Monitor_BESSM1_IN_anual.csv
#   PASSO1/Monitor_BESSM2_IN_anual.csv
...
#   PASSO1/Monitor_BESSM6_IN_anual.csv
# Saidas:
#   PASSO1/Curva_de_Carga_BESSM1_IN_sim_25.csv
#   PASSO1/Curva_de_Carga_BESSM2_IN_sim_25.csv
#   PASSO1/Curva_de_Carga_BESSM3_IN_sim_25.csv
#   PASSO1/Curva_de_Carga_BESSM4_IN_sim_25.csv
#   PASSO1/Curva_de_Carga_BESSM5_IN_sim_25.csv
#   PASSO1/Curva_de_Carga_BESSM6_IN_sim_25.csv
# Remove:
#   PASSO1/Monitor_BESSM1_IN_anual.csv
#   PASSO1/Monitor_BESSM2_IN_anual.csv
#   PASSO1/Monitor_BESSM3_IN_anual.csv
...
for mon_base in ["BESSM1_IN", "BESSM2_IN", "BESSM3_IN", "BESSM4_IN", "BESSM5_IN", "BESSM6_IN"]:
    ...
    out_fp = DIR_PASS1 / f"Curva_de_Carga_{mon_base}_sim_25.csv"
...
for mon_base, prefix in [("BESSM1_IN", "BESSM1"), ("BESSM2_IN", "BESSM2"), ("BESSM3_IN", "BESSM3"), ("BESSM4_IN", "BESSM4"), ("BESSM5_IN", "BESSM5"), ("BESSM6_IN", "BESSM6")]:
    in_fp = DIR_PASS1 / f"Curva_de_Carga_{mon_base}_sim_25.csv"
...
DIR_PASS2 = WORK_DIR / "Simulacao 025 segmentos_ano_2028"
...
# =============================================================================
# Entrada:
#   Simulacao 025 segmentos_ano_2028/UFSM_Mon_<KEY>_s*.csv
# Saida:
#   Simulacao 025 segmentos_ano_2028/Monitor_<KEY>_anual.csv
# Remove:
...
# =============================================================================
# Entrada:
#   Simulacao 025 segmentos_ano_2028/EXP_MTR_GERAL_*.csv
# Saida:
#   Simulacao 025 segmentos_ano_2028/EXP_MTR_GERAL_anual.csv
# Remove:
#   EXP_MTR_GERAL_*.csv
# =============================================================================
...
#   Simulacao 025 segmentos_ano_2028/Monitor_<KEY>_anual.csv
# Saidas:
#   Simulacao 025 segmentos_ano_2028/Curva_de_Carga_<KEY>_sim_25.csv
# Remove:
...
for in_fp in files:
    ...
    out_fp = DIR_PASS2 / f"Curva_de_Carga_{mon_key}_sim_25.csv"
...
#   Simulacao 025 segmentos_ano_2028/EXP_MTR_GERAL_anual.csv
# Saida:
#   Simulacao 025 segmentos_ano_2028/Resumo_EnergyMeter_Geral_Sim25.csv
# =============================================================================
...
OUT_FP = DIR_PASS2 / "Resumo_EnergyMeter_Geral_Sim25.csv"
...
# %% Celula 010 - ENERGIA e DEMANDA MAX por mes e posto (SIM25 - ANO 2028, 6 BESS 800 kW / 3,833 MWh cada)
# =============================================================================
# Entradas:
#   META: 00_dados_de_entrada_ano_letivo_2025_15min.csv
#         Usado apenas como mapa anual de Hora_decimal, Mes e Posto.
#   SIM25: curva GERAL com 6 BESS em operacao conjugada para o ano 2028
# Saida:
#   Simulacao 025 segmentos_ano_2028/01_Integralizacao_Mensal_SIM25_2028.csv
#
# Observacao:
...
#   Nao ha fallback: a curva GERAL deve estar em Curva_de_Carga_GERAL_sim_25.csv.
# =============================================================================
...
SIM25_DIR = WORK_DIR / f"Simulacao 025 segmentos_ano_{ANO_ANALISE}"

META_FP = WORK_DIR / "00_dados_de_entrada_ano_letivo_2025_15min.csv"
SIM25_FP = SIM25_DIR / "Curva_de_Carga_GERAL_sim_25.csv"
OUT_FP = SIM25_DIR / f"01_Integralizacao_Mensal_{SIM_TAG}_{ANO_ANALISE}.csv"
...
for fp in [META_FP, SIM25_FP]:
    if not fp.exists():
        raise FileNotFoundError(fp)

meta = pd.read_csv(META_FP, dtype=str, encoding="utf-8-sig")
sim24 = pd.read_csv(SIM25_FP, encoding="utf-8-sig", sep=None, engine="python")
...
df24 = meta.merge(sim24, on="k", how="left").rename(
    columns={"P_3f_S": "P_SIM25", "Q_3f_S": "Q_SIM25"}
)

faltas_p = int(df24["P_SIM25"].isna().sum())
faltas_q = int(df24["Q_SIM25"].isna().sum())
if faltas_p or faltas_q:
    raise ValueError(
        f"Falha de alinhamento por Hora_decimal/k: "
        f"P_SIM25 faltas={faltas_p}, Q_SIM25 faltas={faltas_q}"
    )

df24["E_P_SIM25_kWh"] = df24["P_SIM25"] * DT_H
df24["E_Q_SIM25_kVArh"] = df24["Q_SIM25"] * DT_H

group_cols = ["Mes", "Posto"]

agg24 = df24.groupby(group_cols, dropna=False).agg(
    N_pontos_SIM25=("P_SIM25", lambda s: int(s.notna().sum())),
    Consumo_P_SIM25_kWh=("E_P_SIM25_kWh", "sum"),
    Consumo_Q_SIM25_kVArh=("E_Q_SIM25_kVArh", "sum"),
    DemandaMax_P_SIM25_kW=("P_SIM25", "max"),
    DemandaMax_Q_SIM25_kVAr=("Q_SIM25", "max"),
).reset_index()
...
print("SIM25 usado:", SIM25_FP)
print(agg[["Mes", "Posto", "N_pontos_META", "N_pontos_SIM25", "DemandaMax_P_SIM25_kW"]])

# %% Celula 011 - SIMULACAO 025: tabela de custos (SIM25 - ANO 2028, 6 BESS 800 kW / 3,833 MWh cada)
# =============================================================================
# Entrada:
#   Simulacao 025 segmentos_ano_2028/01_Integralizacao_Mensal_SIM25_2028.csv
#
# Saida:
#   Simulacao 025 segmentos_ano_2028/Tabela_demanda_consumo_por_posto_SIM25_2028.csv
#
# Demandas contratadas do ano 2028:
#   DCFP com BESS = 4568 kW  (corte fora-ponta agressivo: 4500 kW * 1,015)
#   DCP sem BESS = 3045 kW
#   DCP com 6 BESS = max(0, 3045 - (6 * 800)) = 0 kW
#
# Banco de energia:
#   6 BESS de 1 MW / 3,833 MWh, operando a 80% da potencia nominal.
#   Potencia operacional total = 4800 kW.
#   Energia total = 22998 kWh.
#   Reserva de ponta = 78,3%.
#   Energia reservada para ponta = 18007 kWh.
#   Energia disponivel para peak-shave fora-ponta = 4991 kWh.
...
SIM25_DIR = WORK_DIR / f"Simulacao 025 segmentos_ano_{ANO_ANALISE}"
SIM25_DIR.mkdir(parents=True, exist_ok=True)

IN_FP = SIM25_DIR / f"01_Integralizacao_Mensal_{SIM_TAG}_{ANO_ANALISE}.csv"
OUT_SIM25 = SIM25_DIR / f"Tabela_demanda_consumo_por_posto_{SIM_TAG}_{ANO_ANALISE}.csv"
...
# - Cada banco possui 1 MW / 3,833 MWh nominal, mas opera com 800 kW de potencia.
...
BESS_E_UNITARIA_KWH = 3833.0
BESS_RESERVA_PONTA_PCT = 78.3

BESS_P_TOTAL_KW = NUM_BESS * BESS_P_UNITARIA_KW       # 4800 kW
BESS_E_TOTAL_KWH = NUM_BESS * BESS_E_UNITARIA_KWH     # 22998 kWh
...
CONTR_SIM25 = {"DCFP": DCFP_COM_BESS, "DCP": DCP_COM_BESS}
...
tab25 = tabela_custos(
    df,
    SIM_TAG,
    DCFP=CONTR_SIM25["DCFP"],
    DCP=CONTR_SIM25["DCP"],
)

tab25.to_csv(OUT_SIM25, index=False, encoding="utf-8")

print("DONE. Saved (Simulacao 025 - 6 BESS - 2028 - conjugado 4500):")
print(" -", OUT_SIM25)
...
print("\nResumo anual:")
print(tab25[tab25["MES"].isin(["TOTAIS", "CTOTAL", "R$TOTAL", "DCFP", "DCP"])])
