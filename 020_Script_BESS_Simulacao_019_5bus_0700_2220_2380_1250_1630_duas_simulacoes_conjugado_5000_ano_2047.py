# -*- coding: utf-8 -*-
# =============================================================================
# 020_Script_BESS_Simulacao_019_5bus_0700_2220_2380_1250_1630_duas_simulacoes_conjugado_5000_ano_2047.py
#
# Regras deste script:
...
#     Simulacao 019 segmentos_pass1_sem_hvac_int_ano_2047
#   PASSO 2 (simulacao completa final):
#     Simulacao 019 segmentos_ano_2047
# =============================================================================
...
DIR_PASS1 = WORK_DIR / "Simulacao 019 segmentos_pass1_sem_hvac_int_ano_2047"
...
def build_circuit_pass1(
    ...
    dss.text(
        ...
        f"kWrated=673 kWhrated=2778 Effcurve=Eff %EffCharge=95 %EffDischarge=95 "
...
DIR_PASS2 = WORK_DIR / "Simulacao 019 segmentos_ano_2047"
...
# =============================================================================
# Entrada:
#   Simulacao 019 segmentos_ano_2047/UFSM_Mon_<KEY>_s*.csv
# Saida:
#   Simulacao 019 segmentos_ano_2047/Monitor_<KEY>_anual.csv
# Remove:
...
# =============================================================================
# Entrada:
#   Simulacao 019 segmentos_ano_2047/EXP_MTR_GERAL_*.csv
# Saida:
#   Simulacao 019 segmentos_ano_2047/EXP_MTR_GERAL_anual.csv
# Remove:
#   EXP_MTR_GERAL_*.csv
# =============================================================================
...
#   Simulacao 019 segmentos_ano_2047/Monitor_<KEY>_anual.csv
# Saidas:
#   Simulacao 019 segmentos_ano_2047/Curva_de_Carga_<KEY>_sim_19.csv
# Remove:
...
#   Simulacao 019 segmentos_ano_2047/EXP_MTR_GERAL_anual.csv
# Saida:
#   Simulacao 019 segmentos_ano_2047/Resumo_EnergyMeter_Geral_Sim19.csv
# =============================================================================
...
# %% Celula 010 - ENERGIA e DEMANDA MAX por mes e posto (SIM19 - ANO 2047, 5 BESS 0,673 MW / 2,778 MWh cada, degradados)
# =============================================================================
# Entradas:
#   META: 00_dados_de_entrada_ano_letivo_2025_15min.csv
#         Usado apenas como mapa anual de Hora_decimal, Mes e Posto.
#   SIM19: curva GERAL com 5 BESS em operacao conjugada para o ano 2047
# Saida:
#   Simulacao 019 segmentos_ano_2047/01_Integralizacao_Mensal_SIM19_2047.csv
#
# Observacao:
#   Esta celula nao compara com SIM2/RGE nem com outra simulacao.
#   Ela apenas integraliza a simulacao conjugada com 5 BESS do ano 2047 por mes e posto.
# =============================================================================
...
# %% Celula 011 - SIMULACAO 019: tabela de custos (SIM19 - ANO 2047, 5 BESS 0,673 MW / 2,778 MWh cada, degradados)
# =============================================================================
# Entrada:
#   Simulacao 019 segmentos_ano_2047/01_Integralizacao_Mensal_SIM19_2047.csv
#
# Saida:
#   Simulacao 019 segmentos_ano_2047/Tabela_demanda_consumo_por_posto_SIM19_2047.csv
#
# Demandas contratadas do ano 2047:
#   DCFP = 5000 kW * (1 + 22 * 0,5%) = 5550 kW
#   DCP sem BESS = 3000 kW * (1 + 22 * 0,5%) = 3330 kW
#   DCP com 5 BESS = max(0, 3330 - (5 x 673)) = 0 kW
#
# Configuracao energetica dos BESS:
#   5 bancos originalmente de 1 MW / 3,3 MWh.
#   Em 2047, cada banco e representado degradado para 0,673 MW / 2,778 MWh.
#   Potencia total = 5 x 673 = 3365 kW.
#   Energia total = 5 x 2778 = 13890 kWh.
#   Percentual de reserva para ponta = 90,9%.
#
# Observacao:
...
# -------------------------
# Premissa:
# - 2047 e o ano 22 em relacao aos dados-base de 2025.
# - Crescimento linear anual = 0,5% sobre a demanda-base de 2025.
# - Em 2047 aplica-se crescimento acumulado de 11%.
# - Os 5 BESS sao considerados degradados no ano de analise.
# - Cada banco possui transformador de 1000 kVA, mas opera degradado com kWrated=673 kW e 2778 kWh.
# - Assim, o conjunto reduz a DCP em 5 x 673 = 3365 kW e totaliza 13890 kWh.
...
BESS_E_UNITARIA_KWH = 2778.0

BESS_P_TOTAL_KW = NUM_BESS * BESS_P_UNITARIA_KW       # 3365 kW
BESS_E_TOTAL_KWH = NUM_BESS * BESS_E_UNITARIA_KWH     # 13890 kWh

# Separacao operacional da energia:
# - 90,9% da energia permanece reservada para load-shift no horario de ponta;
# - o restante fica disponivel para peak-shave fora-ponta.
BESS_RESERVA_PONTA_PCT = 90.9
...
print("DONE. Saved (Simulacao 019 - 5 BESS - 2047 - conjugado):")
print(" -", OUT_SIM19)
...
print(f" - Energia total dos BESS = {BESS_E_TOTAL_KWH:.0f} kWh")
