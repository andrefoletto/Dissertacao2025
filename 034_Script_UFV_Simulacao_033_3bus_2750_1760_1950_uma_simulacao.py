# -*- coding: utf-8 -*-
# =============================================================================
# 034_Script_UFV_Simulacao_033_3bus_2750_1760_1950_uma_simulacao.py
#
# Regras deste script:
...
DIR_PASS2 = WORK_DIR / "Simulacao 033 segmentos"
...
# =============================================================================
# Entrada:
#   Simulacao 033 segmentos/UFSM_Mon_<KEY>_s*.csv
# Saida:
#   Simulacao 033 segmentos/Monitor_<KEY>_anual.csv
# Remove:
...
# =============================================================================
# Entrada:
#   Simulacao 033 segmentos/EXP_MTR_GERAL_*.csv
# Saida:
#   Simulacao 033 segmentos/EXP_MTR_GERAL_anual.csv
# Remove:
#   EXP_MTR_GERAL_*.csv
# =============================================================================
...
# Saidas:
#   Simulacao 033 segmentos/Curva_de_Carga_<KEY>_sim_33.csv
# Remove:
...
for in_fp in files:
    ...
    out_fp = DIR_PASS2 / f"Curva_de_Carga_{mon_key}_sim_33.csv"
...
#   Simulacao 033 segmentos/EXP_MTR_GERAL_anual.csv
# Saida:
#   Simulacao 033 segmentos/Resumo_EnergyMeter_Geral_Sim33.csv
# =============================================================================
...
OUT_FP = DIR_PASS2 / "Resumo_EnergyMeter_Geral_Sim33.csv"
...
# %% Celula 006 - ENERGIA e DEMANDA MAX por mes e posto (SIM2 vs SIM33)
# =============================================================================
# Entradas:
...
#   SIM33: Simulacao 033 segmentos/Curva_de_Carga_GERAL_sim_33.csv
#         (Hora_decimal, P_3f_S, Q_3f_S)
#
# Saida:
#   Simulacao 033 segmentos/01_Integralizacao_Mensal_SIM2_x_SIM33_2025.csv
#
# OBSERVACAO:
#   - P > 0  -> consumo da rede
#   - P < 0  -> injecao na rede
...
SIM33_DIR = WORK_DIR / "Simulacao 033 segmentos"

META_FP = WORK_DIR / "00_dados_de_entrada_ano_letivo_2025_15min.csv"
SIM2_FP = SIM2_DIR / "Curva_de_Carga_GERAL_sim_2.csv"
SIM33_FP = SIM33_DIR / "Curva_de_Carga_GERAL_sim_33.csv"
OUT_FP  = SIM33_DIR / "01_Integralizacao_Mensal_SIM2_x_SIM33_2025.csv"
...
for fp in [META_FP, SIM2_FP, SIM33_FP]:
    if not fp.exists():
...
df33 = meta.merge(sim33, on="k", how="left").rename(columns={"P_3f_S": "P_SIM33", "Q_3f_S": "Q_SIM33"})

# -----------------------------------------------------------------------------
# Separacao entre consumo da rede (P > 0) e injecao na rede (P < 0)
# -----------------------------------------------------------------------------
for dfx, tag in [(df2, "SIM2"), (df33, "SIM33")]:
    pcol = f"P_{tag}"
...
agg33 = df33.groupby(group_cols, dropna=False).agg(
...
agg = agg2.merge(agg33, on=group_cols, how="outer")
...
# %% Celula 007 - SIMULACAO 033: Tabela de custos (SIM2 vs SIM33 - 2025)
# =============================================================================
# Entrada:
#   Simulacao 033 segmentos/01_Integralizacao_Mensal_SIM2_x_SIM33_2025.csv
#
# Saidas (todas em Simulacao 033 segmentos):
#   Tabela_demanda_consumo_por_posto_SIM2_2025.csv
#   Tabela_demanda_consumo_por_posto_SIM33_2025.csv
#   Tabela_demanda_consumo_por_posto_SIM2_vs_SIM33_2025.csv
#
# HIPOTESE ADOTADA:
...
IN_FP     = SIM33_DIR / "01_Integralizacao_Mensal_SIM2_x_SIM33_2025.csv"
OUT_SIM2  = SIM33_DIR / "Tabela_demanda_consumo_por_posto_SIM2_2025.csv"
OUT_SIM33 = SIM33_DIR / "Tabela_demanda_consumo_por_posto_SIM33_2025.csv"
OUT_CMP   = SIM33_DIR / "Tabela_demanda_consumo_por_posto_SIM2_vs_SIM33_2025.csv"

# -------------------------
# DEMANDAS CONTRATADAS POR SIMULACAO
# -------------------------
CONTR_SIM2  = {"DCFP": 4500.0, "DCP": 3000.0}
CONTR_SIM33 = {"DCFP": 4500.0, "DCP": 3000.0}
...
tab33 = tabela_custos(df, "SIM33", DCFP=CONTR_SIM33["DCFP"], DCP=CONTR_SIM33["DCP"])
...
t33 = tab33[tab33["MES"].isin(MESES_ORD)][["MES", "R$TOTAL_MES"]].rename(columns={"R$TOTAL_MES": "R$SIM33"})

cmp = t2.merge(t33, on="MES", how="inner")
cmp["MES"] = pd.Categorical(cmp["MES"], categories=MESES_ORD, ordered=True)
cmp = cmp.sort_values("MES").reset_index(drop=True)

cmp["Economia_R$"] = (cmp["R$SIM2"] - cmp["R$SIM33"]).round(2)
...
print("DONE. Saved (Simulacao 033 segmentos):")
print(" -", OUT_SIM2)
print(" -", OUT_SIM33)
print(" -", OUT_CMP)

print("\nDemandas contratadas usadas:")
print(" - SIM2: DCFP=4500, DCP=3000")
print(" - SIM33: DCFP=4500, DCP=3000")
