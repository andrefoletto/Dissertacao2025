# -*- coding: utf-8 -*-
# =============================================================================
# 033_Script_UFV_Simulacao_032_2bus_2750_1760_uma_simulacao_ano_2028_2033_2047.py
#
# Regras deste script:
...
#   - Uma simulacao com duas UFVs, sem comparativo com SIM2.
...
# ======================================================================================
# Saidas:
#   Simulacao 032 segmentos_ano_2028_2033_2047/
...
DIR_PASS2 = WORK_DIR / "Simulacao 032 segmentos_ano_2028_2033_2047"
...
if LIMPAR_SAIDAS_ANTERIORES:
    padroes_limpeza = [
        "UFSM_Mon_*_s*.csv",
        "Monitor_*_anual.csv",
        "Curva_de_Carga_*_sim_32_2033_2047.csv",
        "Curva_de_Carga_*_sim_32.csv",
        "EXP_MTR_GERAL*.csv",
        "Resumo_EnergyMeter_Geral_Sim32_2033_2047.csv",
        "01_Integralizacao_Mensal_SIM32_2033_2047.csv",
        "Tabela_demanda_consumo_por_posto_SIM32_2033_2047.csv",
    ]
    removidos = 0
    for padrao in padroes_limpeza:
        for fp in DIR_PASS2.glob(padrao):
            if fp.is_file():
                fp.unlink()
                removidos += 1
    print(f"[CLEAN] SIM32 2033_2047: {removidos} arquivo(s) antigo(s) removido(s) em {DIR_PASS2}")
...
def _build_circuit_pass2(dss: DSS, posto: str, seg_tag: str) -> dict[str, str]:
    ...
    # Degradacao DC das placas por 20 anos: 0.995^20.
    FATOR_DEGRADACAO_UFV1_20_ANOS = 0.995 ** 20
    PMPP_INV_ANO_BASE = 99.0
    PMPP_INV_UFV1_2047 = PMPP_INV_ANO_BASE * FATOR_DEGRADACAO_UFV1_20_ANOS
    ...
    dss.text("New line.1760_U011 phases=3 bus1=1760 bus2=U021 length=0.0200 units=km Geometry=UG_trefoil_25")
    ...
    # Degradacao DC das placas por 15 anos: 0.995^15.
    FATOR_DEGRADACAO_UFV2_15_ANOS = 0.995 ** 15
    PMPP_INV_UFV2_2047 = PMPP_INV_ANO_BASE * FATOR_DEGRADACAO_UFV2_15_ANOS
...
# =============================================================================
# Entrada:
#   Simulacao 032 segmentos_ano_2028_2033_2047/UFSM_Mon_<KEY>_s*.csv
# Saida:
#   Simulacao 032 segmentos_ano_2028_2033_2047/Monitor_<KEY>_anual.csv
# Remove:
...
# =============================================================================
# Entrada:
#   Simulacao 032 segmentos_ano_2028_2033_2047/EXP_MTR_GERAL_*.csv
# Saida:
#   Simulacao 032 segmentos_ano_2028_2033_2047/EXP_MTR_GERAL_anual.csv
# Remove:
#   EXP_MTR_GERAL_*.csv
# =============================================================================
...
# Saidas:
#   Simulacao 032 segmentos_ano_2028_2033_2047/Curva_de_Carga_<KEY>_sim_32_2033_2047.csv
# Remove:
...
for in_fp in files:
    ...
    out_fp = DIR_PASS2 / f"Curva_de_Carga_{mon_key}_sim_32_2033_2047.csv"
...
#   Simulacao 032 segmentos_ano_2028_2033_2047/EXP_MTR_GERAL_anual.csv
# Saida:
#   Simulacao 032 segmentos_ano_2028_2033_2047/Resumo_EnergyMeter_Geral_Sim32_2033_2047.csv
# =============================================================================
...
OUT_FP = DIR_PASS2 / "Resumo_EnergyMeter_Geral_Sim32_2033_2047.csv"
...
# %% Celula 006 - ENERGIA e DEMANDA MAX por mes e posto (SIM32 - 2033_2047)
# =============================================================================
# Entradas:
#   META: 00_dados_de_entrada_ano_letivo_2025_15min.csv
#         (Hora_decimal, Mes, Posto)
#   SIM32: Simulacao 032 segmentos_ano_2028_2033_2047/Curva_de_Carga_GERAL_sim_32_2033_2047.csv
#         (Hora_decimal, P_3f_S, Q_3f_S)
#
# Saida:
#   Simulacao 032 segmentos_ano_2028_2033_2047/01_Integralizacao_Mensal_SIM32_2033_2047.csv
#
# OBSERVACAO:
#   - P > 0  -> consumo da rede
#   - P < 0  -> injecao na rede
...
WORK_DIR = Path(r"C:\Users\afole\OneDrive\Dissertacao2025")
SIM32_DIR = WORK_DIR / "Simulacao 032 segmentos_ano_2028_2033_2047"

META_FP = WORK_DIR / "00_dados_de_entrada_ano_letivo_2025_15min.csv"
SIM32_FP = SIM32_DIR / "Curva_de_Carga_GERAL_sim_32_2033_2047.csv"
OUT_FP = SIM32_DIR / "01_Integralizacao_Mensal_SIM32_2033_2047.csv"
...
for fp in [META_FP, SIM32_FP]:
    if not fp.exists():
        raise FileNotFoundError(fp)

meta = pd.read_csv(META_FP, dtype=str, encoding="utf-8-sig")
sim32 = pd.read_csv(SIM32_FP, encoding="utf-8-sig", sep=None, engine="python")

for name, dfx, req in [
    ("META", meta, ["Hora_decimal", "Mes", "Posto"]),
    ("SIM32", sim32, ["Hora_decimal", "P_3f_S", "Q_3f_S"]),
]:
...
sim32["Hora_decimal"] = pd.to_numeric(sim32["Hora_decimal"], errors="coerce")
sim32["P_3f_S"] = pd.to_numeric(sim32["P_3f_S"], errors="coerce")
sim32["Q_3f_S"] = pd.to_numeric(sim32["Q_3f_S"], errors="coerce")
sim32.dropna(subset=["Hora_decimal", "P_3f_S", "Q_3f_S"], inplace=True)

meta["k"] = make_k(meta["Hora_decimal"])
sim32["k"] = make_k(sim32["Hora_decimal"])

meta = meta[["k", "Mes", "Posto"]].drop_duplicates(subset=["k"], keep="first").copy()
sim32 = sim32[["k", "P_3f_S", "Q_3f_S"]].drop_duplicates(subset=["k"], keep="first").copy()

df32 = meta.merge(sim32, on="k", how="left").rename(
    columns={"P_3f_S": "P_SIM32", "Q_3f_S": "Q_SIM32"}
)

# -----------------------------------------------------------------------------
# Separacao entre consumo da rede (P > 0) e injecao na rede (P < 0)
# -----------------------------------------------------------------------------
df32["P_cons_SIM32_kW"] = df32["P_SIM32"].clip(lower=0)
df32["P_inj_SIM32_kW"] = (-df32["P_SIM32"]).clip(lower=0)

df32["E_cons_SIM32_kWh"] = df32["P_cons_SIM32_kW"] * DT_H
df32["E_inj_SIM32_kWh"] = df32["P_inj_SIM32_kW"] * DT_H
df32["E_Q_SIM32_kVArh"] = df32["Q_SIM32"] * DT_H

group_cols = ["Mes", "Posto"]

agg = df32.groupby(group_cols, dropna=False).agg(
    N_pontos_SIM32=("P_SIM32", lambda s: int(s.notna().sum())),
    Consumo_P_SIM32_kWh=("E_cons_SIM32_kWh", "sum"),
    Injecao_P_SIM32_kWh=("E_inj_SIM32_kWh", "sum"),
    Consumo_Q_SIM32_kVArh=("E_Q_SIM32_kVArh", "sum"),
    DemandaMax_P_SIM32_kW=("P_cons_SIM32_kW", "max"),
    DemandaMax_Inj_SIM32_kW=("P_inj_SIM32_kW", "max"),
).reset_index()
...
# %% Celula 007 - SIMULACAO 032: Tabela de custos (SIM32 - 2033_2047)
# =============================================================================
# Entrada:
#   Simulacao 032 segmentos_ano_2028_2033_2047/01_Integralizacao_Mensal_SIM32_2033_2047.csv
#
# Saida:
#   Simulacao 032 segmentos_ano_2028_2033_2047/Tabela_demanda_consumo_por_posto_SIM32_2033_2047.csv
#
# HIPOTESE ADOTADA:
...
WORK_DIR = Path(r"C:\Users\afole\OneDrive\Dissertacao2025")
SIM32_DIR = WORK_DIR / "Simulacao 032 segmentos_ano_2028_2033_2047"
SIM32_DIR.mkdir(parents=True, exist_ok=True)

IN_FP = SIM32_DIR / "01_Integralizacao_Mensal_SIM32_2033_2047.csv"
OUT_SIM32 = SIM32_DIR / "Tabela_demanda_consumo_por_posto_SIM32_2033_2047.csv"

# -------------------------
# DEMANDAS CONTRATADAS
# -------------------------
CONTR_SIM32 = {"DCFP": 5550.0, "DCP": 3330.0}
...
tab32 = tabela_custos(
    df,
    "SIM32",
    DCFP=CONTR_SIM32["DCFP"],
    DCP=CONTR_SIM32["DCP"],
)

tab32.to_csv(OUT_SIM32, index=False, encoding="utf-8")

print("DONE. Saved (Simulacao 032 segmentos_ano_2028_2033_2047):")
print(" -", OUT_SIM32)

print("\nDemandas contratadas usadas:")
print(" - SIM32: DCFP=5550 kW, DCP=3330 kW")
