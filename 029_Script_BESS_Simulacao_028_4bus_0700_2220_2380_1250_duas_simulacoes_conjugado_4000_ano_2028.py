# -*- coding: utf-8 -*-
# =============================================================================
# 029_Script_BESS_Simulacao_028_4bus_0700_2220_2380_1250_duas_simulacoes_conjugado_4000_ano_2028.py
#
# Regras deste script:
...
#     Simulacao 028 segmentos_pass1_sem_hvac_int_ano_2028
#   PASSO 2 (simulacao completa final):
#     Simulacao 028 segmentos_ano_2028
# =============================================================================
...
TARGETS = {
    ...
    "FORA_PONTA": {"kWtarget": 4060, "kWtargetLow": 3553},
}

DIR_PASS1 = WORK_DIR / "Simulacao 028 segmentos_pass1_sem_hvac_int_ano_2028"
...
def build_circuit_pass1(
    ...
    dss.text(
        ...
        f"kWrated=800 kWhrated=6000 Effcurve=Eff %EffCharge=95 %EffDischarge=95 "
...
DIR_PASS2 = WORK_DIR / "Simulacao 028 segmentos_ano_2028"
...
# =============================================================================
# Entrada:
#   Simulacao 028 segmentos_ano_2028/UFSM_Mon_<KEY>_s*.csv
# Saida:
#   Simulacao 028 segmentos_ano_2028/Monitor_<KEY>_anual.csv
# Remove:
...
# =============================================================================
# Entrada:
#   Simulacao 028 segmentos_ano_2028/EXP_MTR_GERAL_*.csv
# Saida:
#   Simulacao 028 segmentos_ano_2028/EXP_MTR_GERAL_anual.csv
# Remove:
#   EXP_MTR_GERAL_*.csv
# =============================================================================
...
#   Simulacao 028 segmentos_ano_2028/Monitor_<KEY>_anual.csv
# Saidas:
#   Simulacao 028 segmentos_ano_2028/Curva_de_Carga_<KEY>_sim_28.csv
# Remove:
...
#   Simulacao 028 segmentos_ano_2028/EXP_MTR_GERAL_anual.csv
# Saida:
#   Simulacao 028 segmentos_ano_2028/Resumo_EnergyMeter_Geral_Sim28.csv
# =============================================================================
...
# %% Celula 010 - ENERGIA e DEMANDA MAX por mes e posto (SIM28 - ANO 2028, 4 BESS 800 kW / 6,0 MWh cada)
# =============================================================================
# Entradas:
#   META: 00_dados_de_entrada_ano_letivo_2025_15min.csv
#         Usado apenas como mapa anual de Hora_decimal, Mes e Posto.
#   SIM28: curva GERAL com 4 BESS em operacao conjugada para o ano 2028
# Saida:
#   Simulacao 028 segmentos_ano_2028/01_Integralizacao_Mensal_SIM28_2028.csv
#
# Observacao:
...
#   Todas as celulas deste arquivo foram padronizadas para SIM28.
# =============================================================================
...
sim28 = clean_cols(sim28)

for name, dfx, req in [
    ("META", meta, ["Hora_decimal", "Mes", "Posto"]),
    (SIM_TAG, sim28, ["Hora_decimal", "P_3f_S", "Q_3f_S"]),
]:
...
sim28["Hora_decimal"] = pd.to_numeric(sim28["Hora_decimal"], errors="coerce")
sim28["P_3f_S"] = pd.to_numeric(sim28["P_3f_S"], errors="coerce")
sim28["Q_3f_S"] = pd.to_numeric(sim28["Q_3f_S"], errors="coerce")
sim28.dropna(subset=["Hora_decimal", "P_3f_S", "Q_3f_S"], inplace=True)
...
df28 = meta.merge(sim28, on="k", how="left").rename(
    columns={"P_3f_S": "P_SIM28", "Q_3f_S": "Q_SIM28"}
)

faltas_p = int(df28["P_SIM28"].isna().sum())
faltas_q = int(df28["Q_SIM28"].isna().sum())
...
agg = agg28.merge(meta_counts, on=group_cols, how="left")
...
# %% Celula 011 - SIMULACAO 028: tabela de custos (SIM28 - ANO 2028, 4 BESS 800 kW / 6,0 MWh cada)
# =============================================================================
# Entrada:
#   Simulacao 028 segmentos_ano_2028/01_Integralizacao_Mensal_SIM28_2028.csv
#
# Saida:
#   Simulacao 028 segmentos_ano_2028/Tabela_demanda_consumo_por_posto_SIM28_2028.csv
#
# Demandas contratadas do ano 2028:
#   DCFP com BESS = 4060 kW  (4000 kW + 1,5%)
#   DCP sem BESS = 3045 kW
#   DCP com 4 BESS = 3045 - (4 * 800) = 0 kW
#
# Banco de energia:
#   4 BESS de 1 MW / 6,0 MWh, operando a 80% da potencia nominal.
#   Potencia operacional total = 3200 kW.
#   Energia total = 24000 kWh.
#   Reserva de ponta = 50,0%.
...
# - A demanda contratada fora-ponta foi definida em 4060 kW.
# - A permissao de recarga fora-ponta foi definida em 3553 kW.
# - Os 4 BESS sao instalados no proprio ano de analise.
# - Cada banco possui 1 MW / 6,0 MWh nominal, mas opera com 800 kW de potencia.
...
BESS_E_UNITARIA_KWH = 6000.0
BESS_RESERVA_PONTA_PCT = 50.0

BESS_P_TOTAL_KW = NUM_BESS * BESS_P_UNITARIA_KW       # 3200 kW
BESS_E_TOTAL_KWH = NUM_BESS * BESS_E_UNITARIA_KWH     # 24000 kWh
...
DCFP_COM_BESS = 4060.0                                  # corte fora-ponta: 4000 kW + 1,5%
KW_TARGET_LOW_FP = 3553.0                               # permissao de recarga fora-ponta
...
print("DONE. Saved (Simulacao 028 - 4 BESS - 2028 - conjugado 4000):")
print(" -", OUT_SIM28)
...
print("\nResumo anual:")
print(tab28[tab28["MES"].isin(["TOTAIS", "CTOTAL", "R$TOTAL", "DCFP", "KW_TARGET_LOW_FP", "DCP"])])
