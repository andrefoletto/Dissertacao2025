# -*- coding: utf-8 -*-
# =============================================================================
# 030_Script_BESS_Simulacao_029_5bus_0700_2220_2380_1250_1630_duas_simulacoes_conjugado_4000_ano_2028_2030_2031.py
#
# Regras deste script:
...
#     Simulacao 029 segmentos_pass1_sem_hvac_int_ano_2031
#   PASSO 2 (simulacao completa final):
#     Simulacao 029 segmentos_ano_2031
# =============================================================================
...
TARGETS = {
    ...
    "FORA_PONTA": {"kWtarget": 4120, "kWtargetLow": 3605},
}

# Reservas operacionais por banco:
# - FORA_PONTA: BESSM1, BESSM2 e BESSM3 usam somente o excedente degradado para peak-shave FP;
#               BESSM4 e BESSM5 ficam integralmente reservados para a ponta.
# - PONTA: todos ficam liberados ate a reserva minima tecnica.
BESS1_RESERVA_PONTA_PCT = 100.0 * 3000.0 / 7000.0  # 42.8571429%
BESS2_RESERVA_PONTA_PCT = 100.0 * 3000.0 / 7000.0  # 42.8571429%
BESS3_RESERVA_PONTA_PCT = 100.0 * 3000.0 / 7000.0  # 42.8571429%
BESS4_RESERVA_PONTA_PCT = 100.0
BESS5_RESERVA_PONTA_PCT = 100.0
BESS_RESERVA_MIN_PONTA_PCT = 2.0

DIR_PASS1 = WORK_DIR / "Simulacao 029 segmentos_pass1_sem_hvac_int_ano_2031"
...
if LIMPAR_SAIDAS_ANTERIORES_PASS1:
    padroes_limpeza_pass1 = [
        ...
        "Curva_de_Carga_BESSM*_IN_sim_29.csv",
        "SOC_por_segmento.csv",
    ]
    removidos = 0
...
def build_circuit_pass1(
    ...
    if posto == "FORA_PONTA":
        ...
        reserve_m4_pct = BESS4_RESERVA_PONTA_PCT
        reserve_m5_pct = BESS5_RESERVA_PONTA_PCT
    elif posto == "PONTA":
        ...
        reserve_m4_pct = BESS_RESERVA_MIN_PONTA_PCT
        reserve_m5_pct = BESS_RESERVA_MIN_PONTA_PCT
    else:
    ...
    dss.text('Redirect "UFSM_cargas_medidas_ano_2031.dss"')
    dss.text('Redirect "UFSM_cargas_especiais_ano_2031.dss"')
    dss.text('Redirect "UFSM_cargas_estimadas_com_residuo_ano_2031.dss"')
    ...
    # HVAC externo (obrigatorio) para os cinco bancos
    ...
    dss.text(
        ...
        f"kWrated=786.667 kWhrated=6883.333 Effcurve=Eff %EffCharge=95 %EffDischarge=95 "
    ...
    dss.text(
        ...
        f"kWrated=793.333 kWhrated=2975 Effcurve=Eff %EffCharge=95 %EffDischarge=95 "
        f"%reserve={reserve_m4_pct} dispmode=external"
    )
    dss.text(
        "New Storage.BESSM5 phases=3 bus1=B053 kv=0.38 "
        f"kWrated=800 kWhrated=3000 Effcurve=Eff %EffCharge=95 %EffDischarge=95 "
        f"%reserve={reserve_m5_pct} dispmode=external"
    )
    ...
    dss.text(f"Storage.BESSM4.%reserve={reserve_m4_pct}")
    dss.text(f"Storage.BESSM5.%reserve={reserve_m5_pct}")
...
# %% Celula 003 - PASSO 1: monitores -> Curva_de_Carga_BESSM1_IN_sim_29.csv, BESSM2_IN, BESSM3_IN, BESSM4_IN e BESSM5_IN (SHIFT DIREITA)
# =============================================================================
# Entradas:
#   PASSO1/Monitor_BESSM1_IN_anual.csv
#   PASSO1/Monitor_BESSM2_IN_anual.csv
...
#   PASSO1/Monitor_BESSM4_IN_anual.csv
# Saidas:
#   PASSO1/Curva_de_Carga_BESSM1_IN_sim_29.csv
#   PASSO1/Curva_de_Carga_BESSM2_IN_sim_29.csv
#   PASSO1/Curva_de_Carga_BESSM3_IN_sim_29.csv
#   PASSO1/Curva_de_Carga_BESSM4_IN_sim_29.csv
#   PASSO1/Curva_de_Carga_BESSM5_IN_sim_29.csv
# Remove:
#   PASSO1/Monitor_BESSM1_IN_anual.csv
#   PASSO1/Monitor_BESSM2_IN_anual.csv
#   PASSO1/Monitor_BESSM3_IN_anual.csv
...
for mon_base in ["BESSM1_IN", "BESSM2_IN", "BESSM3_IN", "BESSM4_IN", "BESSM5_IN"]:
    ...
    out_fp = DIR_PASS1 / f"Curva_de_Carga_{mon_base}_sim_29.csv"
...
# %% Celula 004 - PASSO 1: gera LoadShapes do HVAC interno para BESSM1, BESSM2, BESSM3 e BESSM4
# =============================================================================
# Entradas:
#   PASSO1/Curva_de_Carga_BESSM1_IN_sim_29.csv
#   PASSO1/Curva_de_Carga_BESSM2_IN_sim_29.csv
...
for mon_base, prefix in [("BESSM1_IN", "BESSM1"), ("BESSM2_IN", "BESSM2"), ("BESSM3_IN", "BESSM3"), ("BESSM4_IN", "BESSM4"), ("BESSM5_IN", "BESSM5")]:
    in_fp = DIR_PASS1 / f"Curva_de_Carga_{mon_base}_sim_29.csv"
...
#     SOC_15min_BESS_SIM29_2031.csv          (SOC dos cinco bancos a cada 15 min)
#
# Requisitos:
...
BESS1_E_2031_KWH = 6883.333
BESS2_E_2031_KWH = 6883.333
BESS3_E_2031_KWH = 6883.333
BESS4_E_2031_KWH = 2975.0
BESS5_E_2031_KWH = 3000.0
BESS_TOTAL_E_2031_KWH = BESS1_E_2031_KWH + BESS2_E_2031_KWH + BESS3_E_2031_KWH + BESS4_E_2031_KWH + BESS5_E_2031_KWH
...
MON_SPECS = {
    ...
    "BESSM4_IN":    {"element": "line.B041_B042", "terminal": 1},
    "BESSM5_IN":    {"element": "line.B051_B052", "terminal": 1},
...
DIR_PASS2 = WORK_DIR / "Simulacao 029 segmentos_ano_2031"
...
if LIMPAR_SAIDAS_ANTERIORES_PASS2:
    padroes_limpeza = [
        "UFSM_Mon_*_s*.csv",
        "Monitor_*_anual.csv",
        "Curva_de_Carga_*_sim_29.csv",
        "EXP_MTR_GERAL*.csv",
        "Resumo_EnergyMeter_Geral_Sim29.csv",
        "SOC_por_segmento.csv",
        "SOC_15min_BESS_SIM29_2031.csv",
        "01_Integralizacao_Mensal_SIM29_2031.csv",
        "Tabela_demanda_consumo_por_posto_SIM29_2031.csv",
    ]
    removidos = 0
...
def _run_segment_pass2(
    ...
    _build_circuit_pass2(
        ...
        soc_init_m4_pct=soc_ini_m4,
        soc_init_m5_pct=soc_ini_m5,
        posto=posto,
    ...
    for step_idx in range(int(n_steps)):
        ...
        soc_m4_pct = _get_soc_pct(dss_soc, "BESSM4")
        soc_m5_pct = _get_soc_pct(dss_soc, "BESSM5")

        soc_m1_kwh = soc_m1_pct / 100.0 * BESS1_E_2031_KWH
        soc_m2_kwh = soc_m2_pct / 100.0 * BESS2_E_2031_KWH
        soc_m3_kwh = soc_m3_pct / 100.0 * BESS3_E_2031_KWH
        soc_m4_kwh = soc_m4_pct / 100.0 * BESS4_E_2031_KWH
        soc_m5_kwh = soc_m5_pct / 100.0 * BESS5_E_2031_KWH
        soc_total_kwh = soc_m1_kwh + soc_m2_kwh + soc_m3_kwh + soc_m4_kwh + soc_m5_kwh
        soc_total_pct_pond = 100.0 * soc_total_kwh / BESS_TOTAL_E_2031_KWH
        ...
        if posto == "FORA_PONTA":
            ...
            reserve_m4_pct_log = BESS4_RESERVA_PONTA_PCT
            reserve_m5_pct_log = BESS5_RESERVA_PONTA_PCT
        elif posto == "PONTA":
            ...
            reserve_m4_pct_log = BESS_RESERVA_MIN_PONTA_PCT
            reserve_m5_pct_log = BESS_RESERVA_MIN_PONTA_PCT
        else:
            raise ValueError(f"Posto invalido para reserve: {posto}")
        ...
        soc_rows.append(
            {
                ...
                "reserve_BESSM4_pct": float(reserve_m4_pct_log),
                "reserve_BESSM5_pct": float(reserve_m5_pct_log),
                "SOC_BESSM1_pct": float(soc_m1_pct),
                "SOC_BESSM2_pct": float(soc_m2_pct),
                "SOC_BESSM3_pct": float(soc_m3_pct),
                "SOC_BESSM4_pct": float(soc_m4_pct),
                "SOC_BESSM5_pct": float(soc_m5_pct),
                "SOC_BESSM1_kWh": float(soc_m1_kwh),
                "SOC_BESSM2_kWh": float(soc_m2_kwh),
                "SOC_BESSM3_kWh": float(soc_m3_kwh),
                "SOC_BESSM4_kWh": float(soc_m4_kwh),
                "SOC_BESSM5_kWh": float(soc_m5_kwh),
...
for i, row in segments.iterrows():
    ...
    if soc_15min_seg.empty:
        ...
        soc_final_m4 = float(soc_15min_seg["SOC_BESSM4_pct"].iloc[-1])
        soc_final_m5 = float(soc_15min_seg["SOC_BESSM5_pct"].iloc[-1])
...
soc_15min_fp = DIR_PASS2 / "SOC_15min_BESS_SIM29_2031.csv"
...
# =============================================================================
# Entrada:
#   Simulacao 029 segmentos_ano_2031/UFSM_Mon_<KEY>_s*.csv
# Saida:
#   Simulacao 029 segmentos_ano_2031/Monitor_<KEY>_anual.csv
# Remove:
...
# =============================================================================
# Entrada:
#   Simulacao 029 segmentos_ano_2031/EXP_MTR_GERAL_*.csv
# Saida:
#   Simulacao 029 segmentos_ano_2031/EXP_MTR_GERAL_anual.csv
# Remove:
#   EXP_MTR_GERAL_*.csv
# =============================================================================
...
#   Simulacao 029 segmentos_ano_2031/Monitor_<KEY>_anual.csv
# Saidas:
#   Simulacao 029 segmentos_ano_2031/Curva_de_Carga_<KEY>_sim_29.csv
# Remove:
...
for in_fp in files:
    ...
    out_fp = DIR_PASS2 / f"Curva_de_Carga_{mon_key}_sim_29.csv"
...
#   Simulacao 029 segmentos_ano_2031/EXP_MTR_GERAL_anual.csv
# Saida:
#   Simulacao 029 segmentos_ano_2031/Resumo_EnergyMeter_Geral_Sim29.csv
# =============================================================================
...
OUT_FP = DIR_PASS2 / "Resumo_EnergyMeter_Geral_Sim29.csv"
...
# %% Celula 010 - ENERGIA e DEMANDA MAX por mes e posto (SIM29 - ANO 2031, BESS modular autofinanciado)
# =============================================================================
# Entradas:
#   META: 00_dados_de_entrada_ano_letivo_2025_15min.csv
#         Usado apenas como mapa anual de Hora_decimal, Mes e Posto.
#   SIM29: curva GERAL com BESS modular autofinanciado em operacao conjugada
#          para o ano 2031.
# Saida:
#   Simulacao 029 segmentos_ano_2031/01_Integralizacao_Mensal_SIM29_2031.csv
#
# Observacao:
#   Esta celula nao compara com SIM2/RGE nem com outra simulacao.
#   Ela apenas integraliza a simulacao conjugada do ano 2031 por mes e posto.
#   Configuracao do despacho:
#     BESS1: instalado em 2028, 1 MW / 7,0 MWh, operando a 80% da potencia,
#            degradado em 2031 por fator 0,983333333 para 786,667 kW / 6883,3 kWh.
#     BESS2: instalado em 2028, 1 MW / 7,0 MWh, operando a 80% da potencia,
#            degradado em 2031 por fator 0,983333333 para 786,667 kW / 6883,3 kWh.
#     BESS3: instalado em 2028, 1 MW / 7,0 MWh, operando a 80% da potencia,
#            degradado em 2031 por fator 0,983333333 para 786,667 kW / 6883,3 kWh.
#     BESS4: instalado em 2030, 1 MW / 3,0 MWh, operando a 80% da potencia,
#            fator 0,991666667: 793,333 kW / 2975 kWh.
#     BESS5: instalado em 2031, 1 MW / 3,0 MWh, operando a 80% da potencia,
#            sem degradacao: 800 kW / 3000 kWh.
#     Alvo fora-ponta = 4120 kW; permissao de recarga fora-ponta = 3605 kW.
#     Em fora-ponta, BESS1, BESS2 e BESS3 usam reserva de 42,8571%; BESS4 e BESS5 usam reserva de 100%.
#     Em ponta, os cinco bancos usam reserva minima tecnica de 2%.
#   A curva e lida diretamente pelo sufixo sim_29, sem fallback.
# =============================================================================
...
# Pasta fixa do cenario 2030_2031.
# Nao usar f"Simulacao 029 segmentos_ano_{ANO_ANALISE}", pois ANO_ANALISE=2031
# criaria indevidamente a pasta "Simulacao 029 segmentos_ano_2031".
SIM29_DIR = WORK_DIR / "Simulacao 029 segmentos_ano_2031"

META_FP = WORK_DIR / "00_dados_de_entrada_ano_letivo_2025_15min.csv"
SIM29_FP = SIM29_DIR / "Curva_de_Carga_GERAL_sim_29.csv"
OUT_FP = SIM29_DIR / f"01_Integralizacao_Mensal_{SIM_TAG}_{ANO_ANALISE}.csv"
...
def gerar_curva_geral_sim23_se_necessario(sim23_fp: Path, dir_pass2: Path) -> None:
    """
    Garante a existencia de Curva_de_Carga_GERAL_sim_29.csv.
    ...
    def _validar_e_gravar_curva(mon: pd.DataFrame, origem: str) -> None:
        ...
        df_out["P_3f_S"] = df_out["P1 (kW)"] + df_out["P2 (kW)"] + df_out["P3 (kW)"]
        df_out["Q_3f_S"] = df_out["Q1 (kvar)"] + df_out["Q2 (kvar)"] + df_out["Q3 (kvar)"]
        df_out[["Hora_decimal", "P_3f_S", "Q_3f_S"]].to_csv(sim23_fp, index=False, encoding="utf-8-sig")
        print("[AUTO] Curva GERAL gerada em:", sim23_fp)

    if sim23_fp.exists():
        curva = pd.read_csv(sim23_fp, encoding="utf-8-sig", sep=None, engine="python")
        if len(curva) == TOTAL_NPTS:
            return
        print(f"[AUTO] Curva GERAL existente tem {len(curva)} linhas; removendo arquivo invalido: {sim23_fp.name}")
        sim23_fp.unlink()
    ...
    if not seg_files:
        raise FileNotFoundError(
            f"Nao encontrei {sim23_fp}. Tambem nao encontrei Monitor_GERAL_anual.csv nem UFSM_Mon_GERAL_s*.csv em {dir_pass2}. "
...
gerar_curva_geral_sim23_se_necessario(SIM29_FP, SIM29_DIR)
if not SIM29_FP.exists():
    raise FileNotFoundError(SIM29_FP)

meta = pd.read_csv(META_FP, dtype=str, encoding="utf-8-sig")
sim23 = pd.read_csv(SIM29_FP, encoding="utf-8-sig", sep=None, engine="python")

meta = clean_cols(meta)
sim23 = clean_cols(sim23)

for name, dfx, req in [
    ("META", meta, ["Hora_decimal", "Mes", "Posto"]),
    (SIM_TAG, sim23, ["Hora_decimal", "P_3f_S", "Q_3f_S"]),
]:
...
sim23["Hora_decimal"] = pd.to_numeric(sim23["Hora_decimal"], errors="coerce")
sim23["P_3f_S"] = pd.to_numeric(sim23["P_3f_S"], errors="coerce")
sim23["Q_3f_S"] = pd.to_numeric(sim23["Q_3f_S"], errors="coerce")
sim23.dropna(subset=["Hora_decimal", "P_3f_S", "Q_3f_S"], inplace=True)

meta["k"] = make_k(meta["Hora_decimal"])
sim23["k"] = make_k(sim23["Hora_decimal"])

meta = meta[(meta["k"] >= 0) & (meta["k"] < TOTAL_NPTS)].copy()
sim23 = sim23[(sim23["k"] >= 0) & (sim23["k"] < TOTAL_NPTS)].copy()

meta = meta[["k", "Mes", "Posto"]].drop_duplicates(subset=["k"], keep="first").copy()
sim23 = sim23[["k", "P_3f_S", "Q_3f_S"]].drop_duplicates(subset=["k"], keep="first").copy()

if len(meta) != TOTAL_NPTS:
    raise ValueError(f"META gerou {len(meta)} amostras anuais de 15 min; esperado {TOTAL_NPTS}.")
if len(sim23) != TOTAL_NPTS:
    raise ValueError(f"SIM29 gerou {len(sim23)} amostras anuais de 15 min; esperado {TOTAL_NPTS}.")

df23 = meta.merge(sim23, on="k", how="left").rename(
    columns={"P_3f_S": "P_SIM29", "Q_3f_S": "Q_SIM29"}
)

faltas_p = int(df23["P_SIM29"].isna().sum())
faltas_q = int(df23["Q_SIM29"].isna().sum())
if faltas_p or faltas_q:
    raise ValueError(
        f"Falha de alinhamento por Hora_decimal/k: "
        f"P_SIM29 faltas={faltas_p}, Q_SIM29 faltas={faltas_q}"
    )

df23["E_P_SIM29_kWh"] = df23["P_SIM29"] * DT_H
df23["E_Q_SIM29_kVArh"] = df23["Q_SIM29"] * DT_H

group_cols = ["Mes", "Posto"]

agg24 = df23.groupby(group_cols, dropna=False).agg(
    N_pontos_SIM29=("P_SIM29", lambda s: int(s.notna().sum())),
    Consumo_P_SIM29_kWh=("E_P_SIM29_kWh", "sum"),
    Consumo_Q_SIM29_kVArh=("E_Q_SIM29_kVArh", "sum"),
    DemandaMax_P_SIM29_kW=("P_SIM29", "max"),
    DemandaMax_Q_SIM29_kVAr=("Q_SIM29", "max"),
).reset_index()

meta_counts = meta.groupby(group_cols, dropna=False).agg(N_pontos_META=("Posto", "size")).reset_index()
agg = agg24.merge(meta_counts, on=group_cols, how="left")
...
print("SIM29 usado:", SIM29_FP)
print(agg[["Mes", "Posto", "N_pontos_META", "N_pontos_SIM29", "DemandaMax_P_SIM29_kW"]])

# %% Celula 011 - SIMULACAO 029: tabela de custos (SIM29 - ANO 2031, BESS modular autofinanciado)
# =============================================================================
# Entrada:
#   Simulacao 029 segmentos_ano_2031/01_Integralizacao_Mensal_SIM29_2031.csv
#
# Saida:
#   Simulacao 029 segmentos_ano_2031/Tabela_demanda_consumo_por_posto_SIM29_2031.csv
#
# Demandas contratadas do ano 2031:
#   DCFP contratada com BESS = 4000 * 1,03 = 4120 kW
#   Alvo operacional fora-ponta do despacho = 4120 kW
#   Permissao para recarga fora-ponta = 3500 * 1,03 = 3605 kW
#   DCP sem BESS = 3000 * 1,03 = 3090 kW
#   DCP com BESS = max(0, 3090 - (786,667 + 786,667 + 786,667 + 793,333 + 800)) = 0 kW
#
# Banco de energia modular autofinanciado:
#   BESS1 instalado em 2028:
#     1 MW / 7,0 MWh nominal, operando a 80% da potencia nominal em 2028.
#     Em 2031: fator 0,983333333 -> 786,667 kW / 6883,3 kWh.
#     Possui excedente nominal de 4000 kWh para suprir energia de peak-shave fora-ponta.
#   BESS2 instalado em 2028:
#     1 MW / 7,0 MWh nominal, operando a 80% da potencia nominal em 2028.
#     Em 2031: fator 0,983333333 -> 786,667 kW / 6883,3 kWh.
#     Possui excedente nominal de 4000 kWh para suprir energia de peak-shave fora-ponta.
#   BESS3 instalado em 2028:
#     1 MW / 7,0 MWh nominal, operando a 80% da potencia nominal em 2028.
#     Em 2031: fator 0,983333333 -> 786,667 kW / 6883,3 kWh.
#     Possui excedente nominal de 4000 kWh para suprir energia de peak-shave fora-ponta.
#   BESS4 instalado em 2030:
#     1 MW / 3,0 MWh nominal, operando a 80% da potencia nominal.
#     Em 2031: fator 0,991666667 -> 793,333 kW / 2975 kWh.
#   BESS5 instalado em 2031:
#     1 MW / 3,0 MWh nominal, operando a 80% da potencia nominal, sem degradacao: 800 kW / 3000 kWh.
#   Os bancos possuem energias efetivas diferentes em 2031.
#   BESS1, BESS2 e BESS3 foram dimensionados com excedente para suprir os picos fora-ponta.
#   BESS4 e BESS5 possuem somente os 3 MWh nominais necessarios para a ponta.
#   Em fora-ponta, BESS1, BESS2 e BESS3 usam reserva de 42,8571%; BESS4 e BESS5 usam reserva de 100%.
#   Em ponta, os cinco bancos usam reserva minima tecnica de 2%.
#
# Observacao:
...
# criaria indevidamente a pasta "Simulacao 029 segmentos_ano_2031".
SIM29_DIR = WORK_DIR / "Simulacao 029 segmentos_ano_2031"
SIM29_DIR.mkdir(parents=True, exist_ok=True)

IN_FP = SIM29_DIR / f"01_Integralizacao_Mensal_{SIM_TAG}_{ANO_ANALISE}.csv"
OUT_SIM29 = SIM29_DIR / f"Tabela_demanda_consumo_por_posto_{SIM_TAG}_{ANO_ANALISE}.csv"

# -------------------------
# Demandas contratadas e alvos do despacho
# -------------------------
# Fora-ponta: 4000 * 1,03 = 4120 kW
# Ponta sem BESS: 3000 * 1,03 = 3090 kW
# Ponta com BESS: max(0, 3090 - (786,667 + 786,667 + 786,667 + 793,333 + 800)) = 0 kW
DCP_SEM_BESS = 3090.0
DCFP_COM_BESS = 4120.0

KW_TARGET_FP = 4120.0
KW_TARGET_LOW_FP = 3605.0
...
BESS1_E_NOMINAL_KWH = 7000.0
BESS1_E_BASE_PONTA_NOMINAL_KWH = 3000.0
BESS1_E_EXCEDENTE_FP_NOMINAL_KWH = 4000.0
BESS1_FATOR_DEGRADACAO_2031 = 0.9833333333333333
BESS1_P_2031_KW = BESS1_P_OPERACIONAL_2028_KW * BESS1_FATOR_DEGRADACAO_2031
BESS1_E_2031_KWH = BESS1_E_NOMINAL_KWH * BESS1_FATOR_DEGRADACAO_2031
BESS1_E_EXCEDENTE_FP_2031_KWH = BESS1_E_EXCEDENTE_FP_NOMINAL_KWH * BESS1_FATOR_DEGRADACAO_2031

BESS2_ANO_INSTALACAO = 2028
BESS2_P_NOMINAL_KW = 1000.0
BESS2_P_OPERACIONAL_2028_KW = 800.0
BESS2_E_NOMINAL_KWH = 7000.0
BESS2_E_BASE_PONTA_NOMINAL_KWH = 3000.0
BESS2_E_EXCEDENTE_FP_NOMINAL_KWH = 4000.0
BESS2_FATOR_DEGRADACAO_2031 = 0.9833333333333333
BESS2_P_2031_KW = BESS2_P_OPERACIONAL_2028_KW * BESS2_FATOR_DEGRADACAO_2031
BESS2_E_2031_KWH = BESS2_E_NOMINAL_KWH * BESS2_FATOR_DEGRADACAO_2031
BESS2_E_EXCEDENTE_FP_2031_KWH = BESS2_E_EXCEDENTE_FP_NOMINAL_KWH * BESS2_FATOR_DEGRADACAO_2031

BESS3_ANO_INSTALACAO = 2028
BESS3_P_NOMINAL_KW = 1000.0
BESS3_P_OPERACIONAL_2028_KW = 800.0
BESS3_E_NOMINAL_KWH = 7000.0
BESS3_E_BASE_PONTA_NOMINAL_KWH = 3000.0
BESS3_E_EXCEDENTE_FP_NOMINAL_KWH = 4000.0
BESS3_FATOR_DEGRADACAO_2031 = 0.9833333333333333
BESS3_P_2031_KW = BESS3_P_OPERACIONAL_2028_KW * BESS3_FATOR_DEGRADACAO_2031
BESS3_E_2031_KWH = BESS3_E_NOMINAL_KWH * BESS3_FATOR_DEGRADACAO_2031
BESS3_E_EXCEDENTE_FP_2031_KWH = BESS3_E_EXCEDENTE_FP_NOMINAL_KWH * BESS3_FATOR_DEGRADACAO_2031

BESS4_ANO_INSTALACAO = 2030
BESS4_P_NOMINAL_KW = 1000.0
BESS4_P_OPERACIONAL_2030_KW = 800.0
BESS4_E_NOMINAL_KWH = 3000.0
BESS4_FATOR_DEGRADACAO_2031 = 0.9916666666666667
BESS4_P_2031_KW = BESS4_P_OPERACIONAL_2030_KW * BESS4_FATOR_DEGRADACAO_2031
BESS4_E_2031_KWH = BESS4_E_NOMINAL_KWH * BESS4_FATOR_DEGRADACAO_2031

BESS5_ANO_INSTALACAO = 2031
BESS5_P_NOMINAL_KW = 1000.0
BESS5_P_OPERACIONAL_2031_KW = 800.0
BESS5_E_NOMINAL_KWH = 3000.0
BESS5_FATOR_DEGRADACAO_2031 = 1.0
BESS5_P_2031_KW = BESS5_P_OPERACIONAL_2031_KW * BESS5_FATOR_DEGRADACAO_2031
BESS5_E_2031_KWH = BESS5_E_NOMINAL_KWH * BESS5_FATOR_DEGRADACAO_2031

BESS_P_TOTAL_KW = BESS1_P_2031_KW + BESS2_P_2031_KW + BESS3_P_2031_KW + BESS4_P_2031_KW + BESS5_P_2031_KW
BESS_E_TOTAL_KWH = BESS1_E_2031_KWH + BESS2_E_2031_KWH + BESS3_E_2031_KWH + BESS4_E_2031_KWH + BESS5_E_2031_KWH
BESS_E_NOMINAL_TOTAL_KWH = BESS1_E_NOMINAL_KWH + BESS2_E_NOMINAL_KWH + BESS3_E_NOMINAL_KWH + BESS4_E_NOMINAL_KWH + BESS5_E_NOMINAL_KWH

# Separacao operacional coerente com o despacho OpenDSS corrigido.
# FORA_PONTA:
#   BESS1, BESS2 e BESS3 reservam 3000/7000 = 42,8571% para a ponta e usam somente o excedente.
#   BESS4 e BESS5 reservam 100% para a ponta e nao participam do peak-shave fora-ponta.
# PONTA:
#   BESS1, BESS2, BESS3, BESS4 e BESS5 ficam liberados ate a reserva minima tecnica de 2%.
BESS1_RESERVA_PONTA_PCT = 100.0 * BESS1_E_BASE_PONTA_NOMINAL_KWH / BESS1_E_NOMINAL_KWH
BESS2_RESERVA_PONTA_PCT = 100.0 * BESS2_E_BASE_PONTA_NOMINAL_KWH / BESS2_E_NOMINAL_KWH
BESS3_RESERVA_PONTA_PCT = 100.0 * BESS3_E_BASE_PONTA_NOMINAL_KWH / BESS3_E_NOMINAL_KWH
BESS4_RESERVA_PONTA_PCT = 100.0
BESS5_RESERVA_PONTA_PCT = 100.0
BESS_RESERVA_MIN_PONTA_PCT = 2.0

BESS1_E_RESERVA_PONTA_2031_KWH = BESS1_E_2031_KWH * BESS1_RESERVA_PONTA_PCT / 100.0
BESS2_E_RESERVA_PONTA_2031_KWH = BESS2_E_2031_KWH * BESS2_RESERVA_PONTA_PCT / 100.0
BESS3_E_RESERVA_PONTA_2031_KWH = BESS3_E_2031_KWH * BESS3_RESERVA_PONTA_PCT / 100.0
BESS4_E_RESERVA_PONTA_2031_KWH = BESS4_E_2031_KWH * BESS4_RESERVA_PONTA_PCT / 100.0
BESS5_E_RESERVA_PONTA_2031_KWH = BESS5_E_2031_KWH * BESS5_RESERVA_PONTA_PCT / 100.0
BESS_E_RESERVA_PONTA_KWH = (
    BESS1_E_RESERVA_PONTA_2031_KWH
    + BESS2_E_RESERVA_PONTA_2031_KWH
    + BESS3_E_RESERVA_PONTA_2031_KWH
    + BESS4_E_RESERVA_PONTA_2031_KWH
    + BESS5_E_RESERVA_PONTA_2031_KWH
)
BESS_E_DISPONIVEL_FP_KWH = (
    (BESS1_E_2031_KWH - BESS1_E_RESERVA_PONTA_2031_KWH)
    + (BESS2_E_2031_KWH - BESS2_E_RESERVA_PONTA_2031_KWH)
    + (BESS3_E_2031_KWH - BESS3_E_RESERVA_PONTA_2031_KWH)
)

DCP_COM_BESS = max(0.0, DCP_SEM_BESS - BESS_P_TOTAL_KW)

CONTR_SIM29 = {"DCFP": DCFP_COM_BESS, "DCP": DCP_COM_BESS}
...
if not IN_FP.exists():
    curva_fp = SIM29_DIR / "Curva_de_Carga_GERAL_sim_29.csv"
    raise FileNotFoundError(
...
tab29 = tabela_custos(
    df,
    SIM_TAG,
    DCFP=CONTR_SIM29["DCFP"],
    DCP=CONTR_SIM29["DCP"],
)

tab29.to_csv(OUT_SIM29, index=False, encoding="utf-8")

print("DONE. Saved (Simulacao 029 - BESS modular autofinanciado - 2031 - conjugado 4000):")
print(" -", OUT_SIM29)
print("\nDemandas contratadas e alvos usados em 2031:")
...
print(f" - Potencia BESS1 2031 = {BESS1_P_2031_KW:.0f} kW")
print(f" - Potencia BESS2 2031 = {BESS2_P_2031_KW:.0f} kW")
print(f" - Potencia BESS3 2031 = {BESS3_P_2031_KW:.0f} kW")
print(f" - Potencia BESS4 2031 = {BESS4_P_2031_KW:.0f} kW")
print(f" - Potencia BESS5 2031 = {BESS5_P_2031_KW:.0f} kW")
print(f" - Energia nominal total instalada = {BESS_E_NOMINAL_TOTAL_KWH:.0f} kWh")
print(f" - Energia disponivel em 2031 = {BESS_E_TOTAL_KWH:.0f} kWh")
print(f" - Energia BESS1 2031 = {BESS1_E_2031_KWH:.0f} kWh")
print(f" - Energia BESS2 2031 = {BESS2_E_2031_KWH:.0f} kWh")
print(f" - Energia BESS3 2031 = {BESS3_E_2031_KWH:.0f} kWh")
print(f" - Energia BESS4 2031 = {BESS4_E_2031_KWH:.0f} kWh")
print(f" - Energia BESS5 2031 = {BESS5_E_2031_KWH:.0f} kWh")
...
print(f" - Reserva BESS4 em fora-ponta = {BESS4_RESERVA_PONTA_PCT:.4f}%")
print(f" - Reserva BESS5 em fora-ponta = {BESS5_RESERVA_PONTA_PCT:.4f}%")
print(f" - Reserva minima tecnica em ponta = {BESS_RESERVA_MIN_PONTA_PCT:.1f}%")
print(f" - Excedente nominal BESS1/BESS2/BESS3 para fora-ponta = {BESS1_E_EXCEDENTE_FP_NOMINAL_KWH:.0f} kWh")
print(f" - Excedente degradado BESS1 para fora-ponta = {BESS1_E_EXCEDENTE_FP_2031_KWH:.0f} kWh")
print(f" - Excedente degradado BESS2 para fora-ponta = {BESS2_E_EXCEDENTE_FP_2031_KWH:.0f} kWh")
print(f" - Excedente degradado BESS3 para fora-ponta = {BESS3_E_EXCEDENTE_FP_2031_KWH:.0f} kWh")
print(f" - Energia reservada para ponta BESS1 = {BESS1_E_RESERVA_PONTA_2031_KWH:.0f} kWh")
print(f" - Energia reservada para ponta BESS2 = {BESS2_E_RESERVA_PONTA_2031_KWH:.0f} kWh")
print(f" - Energia reservada para ponta BESS3 = {BESS3_E_RESERVA_PONTA_2031_KWH:.0f} kWh")
print(f" - Energia reservada para ponta BESS4 = {BESS4_E_RESERVA_PONTA_2031_KWH:.0f} kWh")
print(f" - Energia reservada para ponta BESS5 = {BESS5_E_RESERVA_PONTA_2031_KWH:.0f} kWh")
...
print("\nResumo anual:")
print(tab29[tab29["MES"].isin(["TOTAIS", "CTOTAL", "R$TOTAL", "DCFP", "DCP"])])
