# -*- coding: utf-8 -*-
# =============================================================================
# 034_Script_UFV_Simulacao_033_3bus_2750_1760_1950_uma_simulacao_ano_2028_2033_2036.py
#
# Regras deste script:
...
#   - Curvas de carga da rede do ano de 2036.
#   - Uma simulacao com tres UFVs, sem comparativo com SIM2.
#   - UFV1 instalada em 2028, com degradacao DC das placas no ano de 2036.
#   - UFV2 instalada em 2033, com degradacao DC das placas no ano de 2036.
#   - UFV3 instalada em 2036, sem degradacao no ano de entrada.
#   - Demandas contratadas: DCFP=5275 kW e DCP=3165 kW.
# =============================================================================

# %% Celula 001 - PASSO UNICO: simulacao segmentada da rede base (monitores + EnergyMeter)
# ======================================================================================
# Saidas:
#   Simulacao 033 segmentos_ano_2028_2033_2036/
...
DIR_PASS2 = WORK_DIR / "Simulacao 033 segmentos_ano_2028_2033_2036"
...
if LIMPAR_SAIDAS_ANTERIORES:
    padroes_limpeza = [
        "UFSM_Mon_*_s*.csv",
        "Monitor_*_anual.csv",
        "Curva_de_Carga_*_sim_33_2036.csv",
        "Curva_de_Carga_*_sim_33.csv",
        "EXP_MTR_GERAL*.csv",
        "Resumo_EnergyMeter_Geral_Sim33_2036.csv",
        "01_Integralizacao_Mensal_SIM33_2036.csv",
        "Tabela_demanda_consumo_por_posto_SIM33_2036.csv",
    ]
    removidos = 0
    for padrao in padroes_limpeza:
        for fp in DIR_PASS2.glob(padrao):
            if fp.is_file():
                fp.unlink()
                removidos += 1
    print(f"[CLEAN] SIM33 2036: {removidos} arquivo(s) antigo(s) removido(s) em {DIR_PASS2}")
...
def _build_circuit_pass2(dss: DSS, posto: str, seg_tag: str) -> dict[str, str]:
    ...
    # UFV1 instalada em 2028 e simulada em 2036.
    # Degradacao DC das placas por 8 anos: 0.995^8.
    PMPP_INV_ANO_BASE = 99.0
    FATOR_DEGRADACAO_UFV1_8_ANOS = 0.995 ** 8
    PMPP_INV_UFV1_2036 = PMPP_INV_ANO_BASE * FATOR_DEGRADACAO_UFV1_8_ANOS

    for i in range(1, 9):
        dss.text(
            f"New PVSystem.UFV8001_INV{i} phases=3 bus1=U013 kv=0.38 "
            f"pmpp={PMPP_INV_UFV1_2036:.3f} kVA=100 pf=1.0 conn=wye "
    ...
    # 88 strings x 20 modulos
    # 11 strings por inversor -> 220 modulos/inversor -> 99.0 kWpdc por inversor
    # UFV2 instalada em 2033 e simulada em 2036.
    # Degradacao DC das placas por 3 anos: 0.995^3.
    FATOR_DEGRADACAO_UFV2_3_ANOS = 0.995 ** 3
    PMPP_INV_UFV2_2036 = PMPP_INV_ANO_BASE * FATOR_DEGRADACAO_UFV2_3_ANOS

    for i in range(1, 9):
        dss.text(
            f"New PVSystem.UFV8002_INV{i} phases=3 bus1=U023 kv=0.38 "
            f"pmpp={PMPP_INV_UFV2_2036:.3f} kVA=100 pf=1.0 conn=wye "
            f"%cutin=0.1 %cutout=0.1 "
            f"effcurve=Eff_Huawei100k P-TCurve=P_T_Trina450 "
    ...
    # 11 strings por inversor -> 220 modulos/inversor -> 99.0 kWpdc por inversor
    # UFV3 instalada em 2036; sem degradacao no ano de entrada.
    PMPP_INV_UFV3_2036 = PMPP_INV_ANO_BASE

    for i in range(1, 9):
        dss.text(
            f"New PVSystem.UFV8003_INV{i} phases=3 bus1=U033 kv=0.38 "
            f"pmpp={PMPP_INV_UFV3_2036:.3f} kVA=100 pf=1.0 conn=wye "
            f"%cutin=0.1 %cutout=0.1 "
            f"effcurve=Eff_Huawei100k P-TCurve=P_T_Trina450 "
...
# =============================================================================
# Entrada:
#   Simulacao 033 segmentos_ano_2028_2033_2036/UFSM_Mon_<KEY>_s*.csv
# Saida:
#   Simulacao 033 segmentos_ano_2028_2033_2036/Monitor_<KEY>_anual.csv
# Remove:
...
# =============================================================================
# Entrada:
#   Simulacao 033 segmentos_ano_2028_2033_2036/EXP_MTR_GERAL_*.csv
# Saida:
#   Simulacao 033 segmentos_ano_2028_2033_2036/EXP_MTR_GERAL_anual.csv
# Remove:
#   EXP_MTR_GERAL_*.csv
# =============================================================================
...
# Saidas:
#   Simulacao 033 segmentos_ano_2028_2033_2036/Curva_de_Carga_<KEY>_sim_33_2036.csv
# Remove:
...
for in_fp in files:
    ...
    out_fp = DIR_PASS2 / f"Curva_de_Carga_{mon_key}_sim_33_2036.csv"
...
#   Simulacao 033 segmentos_ano_2028_2033_2036/EXP_MTR_GERAL_anual.csv
# Saida:
#   Simulacao 033 segmentos_ano_2028_2033_2036/Resumo_EnergyMeter_Geral_Sim33_2036.csv
# =============================================================================
...
OUT_FP = DIR_PASS2 / "Resumo_EnergyMeter_Geral_Sim33_2036.csv"
...
# %% Celula 006 - ENERGIA e DEMANDA MAX por mes e posto (SIM33 - 2036)
# =============================================================================
# Entradas:
#   META: 00_dados_de_entrada_ano_letivo_2025_15min.csv
#         (Hora_decimal, Mes, Posto)
#   SIM33: Simulacao 033 segmentos_ano_2028_2033_2036/Curva_de_Carga_GERAL_sim_33_2036.csv
#         (Hora_decimal, P_3f_S, Q_3f_S)
#
# Saida:
#   Simulacao 033 segmentos_ano_2028_2033_2036/01_Integralizacao_Mensal_SIM33_2036.csv
#
# OBSERVACAO:
#   - P > 0  -> consumo da rede
#   - P < 0  -> injecao na rede
...
WORK_DIR = Path(r"C:\Users\afole\OneDrive\Dissertacao2025")
SIM33_DIR = WORK_DIR / "Simulacao 033 segmentos_ano_2028_2033_2036"

META_FP = WORK_DIR / "00_dados_de_entrada_ano_letivo_2025_15min.csv"
SIM33_FP = SIM33_DIR / "Curva_de_Carga_GERAL_sim_33_2036.csv"
OUT_FP = SIM33_DIR / "01_Integralizacao_Mensal_SIM33_2036.csv"
...
# %% Celula 007 - SIMULACAO 033: Tabela de custos (SIM33 - 2036)
# =============================================================================
# Entrada:
#   Simulacao 033 segmentos_ano_2028_2033_2036/01_Integralizacao_Mensal_SIM33_2036.csv
#
# Saida:
#   Simulacao 033 segmentos_ano_2028_2033_2036/Tabela_demanda_consumo_por_posto_SIM33_2036.csv
#
# HIPOTESE ADOTADA:
...
IN_FP = SIM33_DIR / "01_Integralizacao_Mensal_SIM33_2036.csv"
OUT_SIM33 = SIM33_DIR / "Tabela_demanda_consumo_por_posto_SIM33_2036.csv"

# -------------------------
# DEMANDAS CONTRATADAS
# -------------------------
CONTR_SIM33 = {"DCFP": 5275.0, "DCP": 3165.0}
...
print("DONE. Saved (Simulacao 033 segmentos_ano_2028_2033_2036):")
print(" -", OUT_SIM33)

print("\nDemandas contratadas usadas:")
print(" - SIM33: DCFP=5275 kW, DCP=3165 kW")
