# -*- coding: utf-8 -*-
# =============================================================================
# 020_Script_BESS_Simulacao_019_5bus_0700_2220_2380_1250_1630_duas_simulacoes_conjugado_5000_ano_2028_2033_2036_2038_2039.py
#
# Regras deste script:
...
#     Simulacao 019 segmentos_pass1_sem_hvac_int_ano_2039
#   PASSO 2 (simulacao completa final):
#     Simulacao 019 segmentos_ano_2039
# =============================================================================
...
TARGETS = {
    ...
    "FORA_PONTA": {"kWtarget": 5350, "kWtargetLow": 4280},
}

# Reservas operacionais por banco:
# - FORA_PONTA: BESSM1 usa somente o excedente degradado para peak-shave FP;
#               BESSM2, BESSM3, BESSM4 e BESSM5 ficam integralmente reservados para a ponta.
# - PONTA: todos ficam liberados ate a reserva minima tecnica.
BESS1_RESERVA_PONTA_PCT = 100.0 * 3000.0 / 4500.0  # 66.6666667%
BESS2_RESERVA_PONTA_PCT = 100.0
BESS3_RESERVA_PONTA_PCT = 100.0
BESS4_RESERVA_PONTA_PCT = 100.0
BESS5_RESERVA_PONTA_PCT = 100.0
BESS_RESERVA_MIN_PONTA_PCT = 2.0

DIR_PASS1 = WORK_DIR / "Simulacao 019 segmentos_pass1_sem_hvac_int_ano_2039"
...
if LIMPAR_SAIDAS_ANTERIORES_PASS1:
    padroes_limpeza_pass1 = [
        ...
        "Curva_de_Carga_BESSM*_IN_sim_19.csv",
        "SOC_por_segmento.csv",
    ]
    removidos = 0
...
def build_circuit_pass1(
    ...
    dss.text('Redirect "UFSM_cargas_medidas_ano_2039.dss"')
    dss.text('Redirect "UFSM_cargas_especiais_ano_2039.dss"')
    dss.text('Redirect "UFSM_cargas_estimadas_com_residuo_ano_2039.dss"')
    ...
    dss.text(
        ...
        f"kWrated=727 kWhrated=4088 Effcurve=Eff %EffCharge=95 %EffDischarge=95 "
        f"%reserve={reserve_m1_pct} dispmode=external"
    )
    dss.text(
        "New Storage.BESSM2 phases=3 bus1=B023 kv=0.38 "
        f"kWrated=760 kWhrated=2850 Effcurve=Eff %EffCharge=95 %EffDischarge=95 "
        f"%reserve={reserve_m2_pct} dispmode=external"
    )
    dss.text(
        "New Storage.BESSM3 phases=3 bus1=B033 kv=0.38 "
        f"kWrated=780 kWhrated=2925 Effcurve=Eff %EffCharge=95 %EffDischarge=95 "
        f"%reserve={reserve_m3_pct} dispmode=external"
    )
    dss.text(
        "New Storage.BESSM4 phases=3 bus1=B043 kv=0.38 "
        f"kWrated=793 kWhrated=2975 Effcurve=Eff %EffCharge=95 %EffDischarge=95 "
...
def run_one_segment_pass1(
    ...
    print(
        ...
        f"SOC_ini_M3={float(soc_ini_m3):.3f}% | SOC_ini_M4={float(soc_ini_m4):.3f}% | SOC_ini_M5={float(soc_ini_m5):.3f}%"
    )
...
# %% Celula 002 - PASSO 1: emenda monitores BESSM1_IN, BESSM2_IN, BESSM3_IN e BESSM4_IN
# =============================================================================
# Entrada:
...
# %% Celula 003 - PASSO 1: monitores -> Curva_de_Carga_BESSM1_IN_sim_19.csv, BESSM2_IN, BESSM3_IN e BESSM4_IN (SHIFT DIREITA)
# =============================================================================
# Entradas:
#   PASSO1/Monitor_BESSM1_IN_anual.csv
#   PASSO1/Monitor_BESSM2_IN_anual.csv
...
#   PASSO1/Monitor_BESSM4_IN_anual.csv
# Saidas:
#   PASSO1/Curva_de_Carga_BESSM1_IN_sim_19.csv
#   PASSO1/Curva_de_Carga_BESSM2_IN_sim_19.csv
#   PASSO1/Curva_de_Carga_BESSM3_IN_sim_19.csv
#   PASSO1/Curva_de_Carga_BESSM4_IN_sim_19.csv
# Remove:
#   PASSO1/Monitor_BESSM1_IN_anual.csv
#   PASSO1/Monitor_BESSM2_IN_anual.csv
#   PASSO1/Monitor_BESSM3_IN_anual.csv
...
for mon_base in ["BESSM1_IN", "BESSM2_IN", "BESSM3_IN", "BESSM4_IN", "BESSM5_IN"]:
    ...
    out_fp = DIR_PASS1 / f"Curva_de_Carga_{mon_base}_sim_19.csv"
...
for mon_base, prefix in [("BESSM1_IN", "BESSM1"), ("BESSM2_IN", "BESSM2"), ("BESSM3_IN", "BESSM3"), ("BESSM4_IN", "BESSM4"), ("BESSM5_IN", "BESSM5")]:
    in_fp = DIR_PASS1 / f"Curva_de_Carga_{mon_base}_sim_19.csv"
...
#     SOC_15min_BESS_SIM19_2039.csv          (SOC dos cinco bancos a cada 15 min)
#
# Requisitos:
...
BESS1_E_2039_KWH = 4088.0
BESS2_E_2039_KWH = 2850.0
BESS3_E_2039_KWH = 2925.0
BESS4_E_2039_KWH = 2975.0
BESS5_E_2039_KWH = 3000.0
BESS_TOTAL_E_2039_KWH = BESS1_E_2039_KWH + BESS2_E_2039_KWH + BESS3_E_2039_KWH + BESS4_E_2039_KWH + BESS5_E_2039_KWH
...
DIR_PASS2 = WORK_DIR / "Simulacao 019 segmentos_ano_2039"
...
if LIMPAR_SAIDAS_ANTERIORES_PASS2:
    padroes_limpeza = [
        "UFSM_Mon_*_s*.csv",
        "Monitor_*_anual.csv",
        "Curva_de_Carga_*_sim_19.csv",
        "EXP_MTR_GERAL*.csv",
        "Resumo_EnergyMeter_Geral_Sim19.csv",
        "SOC_por_segmento.csv",
        "SOC_15min_BESS_SIM19_2039.csv",
        "01_Integralizacao_Mensal_SIM19_2039.csv",
        "Tabela_demanda_consumo_por_posto_SIM19_2039.csv",
    ]
    removidos = 0
...
def _run_segment_pass2(
    ...
    for step_idx in range(int(n_steps)):
        ...
        soc_m1_kwh = soc_m1_pct / 100.0 * BESS1_E_2039_KWH
        soc_m2_kwh = soc_m2_pct / 100.0 * BESS2_E_2039_KWH
        soc_m3_kwh = soc_m3_pct / 100.0 * BESS3_E_2039_KWH
        soc_m4_kwh = soc_m4_pct / 100.0 * BESS4_E_2039_KWH
        soc_m5_kwh = soc_m5_pct / 100.0 * BESS5_E_2039_KWH
        soc_total_kwh = soc_m1_kwh + soc_m2_kwh + soc_m3_kwh + soc_m4_kwh + soc_m5_kwh
        soc_total_pct_pond = 100.0 * soc_total_kwh / BESS_TOTAL_E_2039_KWH
...
soc_15min_fp = DIR_PASS2 / "SOC_15min_BESS_SIM19_2039.csv"
...
# =============================================================================
# Entrada:
#   Simulacao 019 segmentos_ano_2039/UFSM_Mon_<KEY>_s*.csv
# Saida:
#   Simulacao 019 segmentos_ano_2039/Monitor_<KEY>_anual.csv
# Remove:
...
# =============================================================================
# Entrada:
#   Simulacao 019 segmentos_ano_2039/EXP_MTR_GERAL_*.csv
# Saida:
#   Simulacao 019 segmentos_ano_2039/EXP_MTR_GERAL_anual.csv
# Remove:
#   EXP_MTR_GERAL_*.csv
# =============================================================================
...
#   Simulacao 019 segmentos_ano_2039/Monitor_<KEY>_anual.csv
# Saidas:
#   Simulacao 019 segmentos_ano_2039/Curva_de_Carga_<KEY>_sim_19.csv
# Remove:
...
for in_fp in files:
    ...
    out_fp = DIR_PASS2 / f"Curva_de_Carga_{mon_key}_sim_19.csv"
...
#   Simulacao 019 segmentos_ano_2039/EXP_MTR_GERAL_anual.csv
# Saida:
#   Simulacao 019 segmentos_ano_2039/Resumo_EnergyMeter_Geral_Sim19.csv
# =============================================================================
...
OUT_FP = DIR_PASS2 / "Resumo_EnergyMeter_Geral_Sim19.csv"
...
# %% Celula 010 - ENERGIA e DEMANDA MAX por mes e posto (SIM19 - ANO 2039, BESS modular autofinanciado)
# =============================================================================
# Entradas:
#   META: 00_dados_de_entrada_ano_letivo_2025_15min.csv
#         Usado apenas como mapa anual de Hora_decimal, Mes e Posto.
#   SIM19: curva GERAL com BESS modular autofinanciado em operacao conjugada
#          para o ano 2039.
# Saida:
#   Simulacao 019 segmentos_ano_2039/01_Integralizacao_Mensal_SIM19_2039.csv
#
# Observacao:
#   Esta celula nao compara com SIM2/RGE nem com outra simulacao.
#   Ela apenas integraliza a simulacao conjugada do ano 2039 por mes e posto.
#   Configuracao do despacho:
#     BESS1: instalado em 2028, 1 MW / 4,5 MWh, operando a 80% da potencia,
#            degradado em 2039 por fator 0,908333333 para 727 kW / 4088 kWh.
#     BESS2: instalado em 2033, 1 MW / 3,0 MWh, operando a 80% da potencia,
#            degradado em 2039 por fator 0,950000000 para 760 kW / 2850 kWh.
#     BESS3: instalado em 2036, 1 MW / 3,0 MWh, operando a 80% da potencia,
#            degradado em 2039 por fator 0,975000000 para 780 kW / 2925 kWh.
#     BESS4: instalado em 2038, 1 MW / 3,0 MWh, operando a 80% da potencia,
#            degradado em 2039 por fator 0,991666667 para 793 kW / 2975 kWh.
#     BESS5: instalado em 2039, 1 MW / 3,0 MWh, operando a 80% da potencia,
#            sem degradacao: 800 kW / 3000 kWh.
#     Alvo fora-ponta = 5350 kW; permissao de recarga fora-ponta = 4280 kW.
#     Em fora-ponta, BESS1 usa reserva de 66,6667% e BESS2/BESS3/BESS4/BESS5 usam reserva de 100%.
#     Em ponta, os cinco bancos usam reserva minima tecnica de 2%.
#   A curva e lida diretamente pelo sufixo sim_19, sem fallback.
# =============================================================================
...
SIM19_DIR = WORK_DIR / f"Simulacao 019 segmentos_ano_{ANO_ANALISE}"

META_FP = WORK_DIR / "00_dados_de_entrada_ano_letivo_2025_15min.csv"
SIM19_FP = SIM19_DIR / "Curva_de_Carga_GERAL_sim_19.csv"
OUT_FP = SIM19_DIR / f"01_Integralizacao_Mensal_{SIM_TAG}_{ANO_ANALISE}.csv"
...
def gerar_curva_geral_sim19_se_necessario(sim19_fp: Path, dir_pass2: Path) -> None:
    """
    Garante a existencia de Curva_de_Carga_GERAL_sim_19.csv.
    ...
    def _validar_e_gravar_curva(mon: pd.DataFrame, origem: str) -> None:
        ...
        df_out["P_3f_S"] = df_out["P1 (kW)"] + df_out["P2 (kW)"] + df_out["P3 (kW)"]
        df_out["Q_3f_S"] = df_out["Q1 (kvar)"] + df_out["Q2 (kvar)"] + df_out["Q3 (kvar)"]
        df_out[["Hora_decimal", "P_3f_S", "Q_3f_S"]].to_csv(sim19_fp, index=False, encoding="utf-8-sig")
        print("[AUTO] Curva GERAL gerada em:", sim19_fp)

    if sim19_fp.exists():
        curva = pd.read_csv(sim19_fp, encoding="utf-8-sig", sep=None, engine="python")
        if len(curva) == TOTAL_NPTS:
            return
        print(f"[AUTO] Curva GERAL existente tem {len(curva)} linhas; removendo arquivo invalido: {sim19_fp.name}")
        sim19_fp.unlink()
    ...
    if not seg_files:
        raise FileNotFoundError(
            f"Nao encontrei {sim19_fp}. Tambem nao encontrei Monitor_GERAL_anual.csv nem UFSM_Mon_GERAL_s*.csv em {dir_pass2}. "
...
gerar_curva_geral_sim19_se_necessario(SIM19_FP, SIM19_DIR)
if not SIM19_FP.exists():
    raise FileNotFoundError(SIM19_FP)

meta = pd.read_csv(META_FP, dtype=str, encoding="utf-8-sig")
sim19 = pd.read_csv(SIM19_FP, encoding="utf-8-sig", sep=None, engine="python")

meta = clean_cols(meta)
sim19 = clean_cols(sim19)

for name, dfx, req in [
    ("META", meta, ["Hora_decimal", "Mes", "Posto"]),
    (SIM_TAG, sim19, ["Hora_decimal", "P_3f_S", "Q_3f_S"]),
]:
...
sim19["Hora_decimal"] = pd.to_numeric(sim19["Hora_decimal"], errors="coerce")
sim19["P_3f_S"] = pd.to_numeric(sim19["P_3f_S"], errors="coerce")
sim19["Q_3f_S"] = pd.to_numeric(sim19["Q_3f_S"], errors="coerce")
sim19.dropna(subset=["Hora_decimal", "P_3f_S", "Q_3f_S"], inplace=True)

meta["k"] = make_k(meta["Hora_decimal"])
sim19["k"] = make_k(sim19["Hora_decimal"])

meta = meta[(meta["k"] >= 0) & (meta["k"] < TOTAL_NPTS)].copy()
sim19 = sim19[(sim19["k"] >= 0) & (sim19["k"] < TOTAL_NPTS)].copy()

meta = meta[["k", "Mes", "Posto"]].drop_duplicates(subset=["k"], keep="first").copy()
sim19 = sim19[["k", "P_3f_S", "Q_3f_S"]].drop_duplicates(subset=["k"], keep="first").copy()

if len(meta) != TOTAL_NPTS:
    raise ValueError(f"META gerou {len(meta)} amostras anuais de 15 min; esperado {TOTAL_NPTS}.")
if len(sim19) != TOTAL_NPTS:
    raise ValueError(f"SIM19 gerou {len(sim19)} amostras anuais de 15 min; esperado {TOTAL_NPTS}.")

df19 = meta.merge(sim19, on="k", how="left").rename(
    columns={"P_3f_S": "P_SIM19", "Q_3f_S": "Q_SIM19"}
)

faltas_p = int(df19["P_SIM19"].isna().sum())
faltas_q = int(df19["Q_SIM19"].isna().sum())
if faltas_p or faltas_q:
    raise ValueError(
        f"Falha de alinhamento por Hora_decimal/k: "
        f"P_SIM19 faltas={faltas_p}, Q_SIM19 faltas={faltas_q}"
    )

df19["E_P_SIM19_kWh"] = df19["P_SIM19"] * DT_H
df19["E_Q_SIM19_kVArh"] = df19["Q_SIM19"] * DT_H

group_cols = ["Mes", "Posto"]

agg19 = df19.groupby(group_cols, dropna=False).agg(
    N_pontos_SIM19=("P_SIM19", lambda s: int(s.notna().sum())),
    Consumo_P_SIM19_kWh=("E_P_SIM19_kWh", "sum"),
    Consumo_Q_SIM19_kVArh=("E_Q_SIM19_kVArh", "sum"),
    DemandaMax_P_SIM19_kW=("P_SIM19", "max"),
    DemandaMax_Q_SIM19_kVAr=("Q_SIM19", "max"),
).reset_index()

meta_counts = meta.groupby(group_cols, dropna=False).agg(N_pontos_META=("Posto", "size")).reset_index()
agg = agg19.merge(meta_counts, on=group_cols, how="left")
...
print("SIM19 usado:", SIM19_FP)
print(agg[["Mes", "Posto", "N_pontos_META", "N_pontos_SIM19", "DemandaMax_P_SIM19_kW"]])

# %% Celula 011 - SIMULACAO 019: tabela de custos (SIM19 - ANO 2039, BESS modular autofinanciado)
# =============================================================================
# Entrada:
#   Simulacao 019 segmentos_ano_2039/01_Integralizacao_Mensal_SIM19_2039.csv
#
# Saida:
#   Simulacao 019 segmentos_ano_2039/Tabela_demanda_consumo_por_posto_SIM19_2039.csv
#
# Demandas contratadas do ano 2039:
#   DCFP contratada com BESS = 5000 * 1,070 = 5350 kW
#   Alvo operacional fora-ponta do despacho = 5350 kW
#   Permissao para recarga fora-ponta = 0,80 * 5350 = 4280 kW
#   DCP sem BESS = 3000 * 1,070 = 3210 kW
#   DCP com BESS = max(0, 3210 - (727 + 760 + 780 + 793 + 800)) = 0 kW
#
# Banco de energia modular autofinanciado:
#   BESS1 instalado em 2028:
#     1 MW / 4,5 MWh nominal, operando a 80% da potencia nominal em 2028.
#     Em 2039: fator 0,908333333 -> 727 kW / 4088 kWh.
#     Possui excedente nominal de 1500 kWh para suprir energia de peak-shave fora-ponta.
#   BESS2 instalado em 2033:
#     1 MW / 3,0 MWh nominal, operando a 80% da potencia nominal em 2033.
#     Em 2039: fator 0,950000000 -> 760 kW / 2850 kWh.
#   BESS3 instalado em 2036:
#     1 MW / 3,0 MWh nominal, operando a 80% da potencia nominal em 2036.
#     Em 2039: fator 0,975000000 -> 780 kW / 2925 kWh.
#   BESS4 instalado em 2038:
#     1 MW / 3,0 MWh nominal, operando a 80% da potencia nominal em 2038.
#     Em 2039: fator 0,991666667 -> 793 kW / 2975 kWh.
#   BESS5 instalado em 2039:
#     1 MW / 3,0 MWh nominal, operando a 80% da potencia nominal: 800 kW / 3000 kWh.
#   Os bancos possuem energias efetivas diferentes em 2039.
#   O BESS1 foi dimensionado com excedente para suprir os picos fora-ponta ate 2047.
#   Os bancos instalados posteriormente possuem somente os 3 MWh nominais necessarios para a ponta.
#   Em fora-ponta, BESS1 usa reserva de 66,6667% e BESS2/BESS3/BESS4/BESS5 usam reserva de 100%.
#   Em ponta, os cinco bancos usam reserva minima tecnica de 2%.
#
# Observacao:
...
SIM19_DIR = WORK_DIR / f"Simulacao 019 segmentos_ano_{ANO_ANALISE}"
SIM19_DIR.mkdir(parents=True, exist_ok=True)

IN_FP = SIM19_DIR / f"01_Integralizacao_Mensal_{SIM_TAG}_{ANO_ANALISE}.csv"
OUT_SIM19 = SIM19_DIR / f"Tabela_demanda_consumo_por_posto_{SIM_TAG}_{ANO_ANALISE}.csv"

# -------------------------
# Demandas contratadas e alvos do despacho
# -------------------------
# Fora-ponta: 5000 * 1,070 = 5350 kW
# Ponta sem BESS: 3000 * 1,070 = 3210 kW
# Ponta com BESS: max(0, 3210 - (727 + 760 + 780 + 793 + 800)) = 0 kW
DCP_SEM_BESS = 3210.0
DCFP_COM_BESS = 5350.0

KW_TARGET_FP = 5350.0
KW_TARGET_LOW_FP = 4280.0
...
BESS1_E_NOMINAL_KWH = 4500.0
BESS1_E_BASE_PONTA_NOMINAL_KWH = 3000.0
BESS1_E_EXCEDENTE_FP_NOMINAL_KWH = 1500.0
BESS1_FATOR_DEGRADACAO_2039 = 0.9083333333333333
BESS1_P_2039_KW = round(BESS1_P_OPERACIONAL_2028_KW * BESS1_FATOR_DEGRADACAO_2039)
BESS1_E_2039_KWH = round(BESS1_E_NOMINAL_KWH * BESS1_FATOR_DEGRADACAO_2039)
BESS1_E_EXCEDENTE_FP_2039_KWH = BESS1_E_EXCEDENTE_FP_NOMINAL_KWH * BESS1_FATOR_DEGRADACAO_2039

BESS2_ANO_INSTALACAO = 2033
BESS2_P_NOMINAL_KW = 1000.0
BESS2_P_OPERACIONAL_2033_KW = 800.0
BESS2_E_NOMINAL_KWH = 3000.0
BESS2_FATOR_DEGRADACAO_2039 = 0.950000000
BESS2_P_2039_KW = round(BESS2_P_OPERACIONAL_2033_KW * BESS2_FATOR_DEGRADACAO_2039)
BESS2_E_2039_KWH = round(BESS2_E_NOMINAL_KWH * BESS2_FATOR_DEGRADACAO_2039)

BESS3_ANO_INSTALACAO = 2036
BESS3_P_NOMINAL_KW = 1000.0
BESS3_P_OPERACIONAL_2036_KW = 800.0
BESS3_E_NOMINAL_KWH = 3000.0
BESS3_FATOR_DEGRADACAO_2039 = 0.975000000
BESS3_P_2039_KW = round(BESS3_P_OPERACIONAL_2036_KW * BESS3_FATOR_DEGRADACAO_2039)
BESS3_E_2039_KWH = round(BESS3_E_NOMINAL_KWH * BESS3_FATOR_DEGRADACAO_2039)

BESS4_ANO_INSTALACAO = 2038
BESS4_P_NOMINAL_KW = 1000.0
BESS4_P_OPERACIONAL_2038_KW = 800.0
BESS4_E_NOMINAL_KWH = 3000.0
BESS4_FATOR_DEGRADACAO_2039 = 0.9916666666666667
BESS4_P_2039_KW = round(BESS4_P_OPERACIONAL_2038_KW * BESS4_FATOR_DEGRADACAO_2039)
BESS4_E_2039_KWH = round(BESS4_E_NOMINAL_KWH * BESS4_FATOR_DEGRADACAO_2039)

BESS5_ANO_INSTALACAO = 2039
BESS5_P_NOMINAL_KW = 1000.0
BESS5_P_OPERACIONAL_2039_KW = 800.0
BESS5_E_NOMINAL_KWH = 3000.0
BESS5_P_2039_KW = BESS5_P_OPERACIONAL_2039_KW
BESS5_E_2039_KWH = BESS5_E_NOMINAL_KWH

BESS_P_TOTAL_KW = BESS1_P_2039_KW + BESS2_P_2039_KW + BESS3_P_2039_KW + BESS4_P_2039_KW + BESS5_P_2039_KW
BESS_E_TOTAL_KWH = BESS1_E_2039_KWH + BESS2_E_2039_KWH + BESS3_E_2039_KWH + BESS4_E_2039_KWH + BESS5_E_2039_KWH
...
# FORA_PONTA:
#   BESS1 reserva 3000/4500 = 66,6667% para a ponta e usa somente o excedente.
#   BESS2, BESS3, BESS4 e BESS5 reservam 100% para a ponta e nao participam do peak-shave fora-ponta.
# PONTA:
...
BESS1_E_RESERVA_PONTA_2039_KWH = BESS1_E_2039_KWH * BESS1_RESERVA_PONTA_PCT / 100.0
BESS2_E_RESERVA_PONTA_2039_KWH = BESS2_E_2039_KWH * BESS2_RESERVA_PONTA_PCT / 100.0
BESS3_E_RESERVA_PONTA_2039_KWH = BESS3_E_2039_KWH * BESS3_RESERVA_PONTA_PCT / 100.0
BESS4_E_RESERVA_PONTA_2039_KWH = BESS4_E_2039_KWH * BESS4_RESERVA_PONTA_PCT / 100.0
BESS5_E_RESERVA_PONTA_2039_KWH = BESS5_E_2039_KWH * BESS5_RESERVA_PONTA_PCT / 100.0
BESS_E_RESERVA_PONTA_KWH = (
    BESS1_E_RESERVA_PONTA_2039_KWH
    + BESS2_E_RESERVA_PONTA_2039_KWH
    + BESS3_E_RESERVA_PONTA_2039_KWH
    + BESS4_E_RESERVA_PONTA_2039_KWH
    + BESS5_E_RESERVA_PONTA_2039_KWH
)
BESS_E_DISPONIVEL_FP_KWH = BESS1_E_2039_KWH - BESS1_E_RESERVA_PONTA_2039_KWH

DCP_COM_BESS = max(0.0, DCP_SEM_BESS - BESS_P_TOTAL_KW)

CONTR_SIM19 = {"DCFP": DCFP_COM_BESS, "DCP": DCP_COM_BESS}
...
if not IN_FP.exists():
    curva_fp = SIM19_DIR / "Curva_de_Carga_GERAL_sim_19.csv"
    raise FileNotFoundError(
...
tab19 = tabela_custos(
    df,
    SIM_TAG,
    DCFP=CONTR_SIM19["DCFP"],
    DCP=CONTR_SIM19["DCP"],
)

tab19.to_csv(OUT_SIM19, index=False, encoding="utf-8")

print("DONE. Saved (Simulacao 019 - BESS modular autofinanciado - 2039 - conjugado):")
print(" -", OUT_SIM19)
print("\nDemandas contratadas e alvos usados em 2039:")
...
print(f" - Potencia BESS1 2039 = {BESS1_P_2039_KW:.0f} kW")
print(f" - Potencia BESS2 2039 = {BESS2_P_2039_KW:.0f} kW")
print(f" - Potencia BESS3 2039 = {BESS3_P_2039_KW:.0f} kW")
print(f" - Potencia BESS4 2039 = {BESS4_P_2039_KW:.0f} kW")
print(f" - Potencia BESS5 2039 = {BESS5_P_2039_KW:.0f} kW")
print(f" - Energia nominal total instalada = {BESS_E_NOMINAL_TOTAL_KWH:.0f} kWh")
print(f" - Energia disponivel em 2039 = {BESS_E_TOTAL_KWH:.0f} kWh")
print(f" - Energia BESS1 2039 = {BESS1_E_2039_KWH:.0f} kWh")
print(f" - Energia BESS2 2039 = {BESS2_E_2039_KWH:.0f} kWh")
print(f" - Energia BESS3 2039 = {BESS3_E_2039_KWH:.0f} kWh")
print(f" - Energia BESS4 2039 = {BESS4_E_2039_KWH:.0f} kWh")
print(f" - Energia BESS5 2039 = {BESS5_E_2039_KWH:.0f} kWh")
...
print(f" - Excedente nominal BESS1 para fora-ponta = {BESS1_E_EXCEDENTE_FP_NOMINAL_KWH:.0f} kWh")
print(f" - Excedente degradado BESS1 para fora-ponta = {BESS1_E_EXCEDENTE_FP_2039_KWH:.0f} kWh")
print(f" - Energia reservada para ponta BESS1 = {BESS1_E_RESERVA_PONTA_2039_KWH:.0f} kWh")
print(f" - Energia reservada para ponta BESS2 = {BESS2_E_RESERVA_PONTA_2039_KWH:.0f} kWh")
print(f" - Energia reservada para ponta BESS3 = {BESS3_E_RESERVA_PONTA_2039_KWH:.0f} kWh")
print(f" - Energia reservada para ponta BESS4 = {BESS4_E_RESERVA_PONTA_2039_KWH:.0f} kWh")
print(f" - Energia reservada para ponta BESS5 = {BESS5_E_RESERVA_PONTA_2039_KWH:.0f} kWh")
...
print("\nResumo anual:")
print(tab19[tab19["MES"].isin(["TOTAIS", "CTOTAL", "R$TOTAL", "DCFP", "DCP"])])
