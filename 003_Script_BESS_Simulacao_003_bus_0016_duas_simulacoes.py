# -*- coding: utf-8 -*-
# =============================================================================
# Script_BESS_Simulacao_003_bus_0016_duas_simulacoes.py
#
# Regras deste script:
...
#     Simulacao 003 segmentos_pass1_sem_hvac_int
#   PASSO 2 (simulacao completa final):
#     Simulacao 003 segmentos
# =============================================================================
...
DIR_PASS1 = WORK_DIR / "Simulacao 003 segmentos_pass1_sem_hvac_int"
...
def build_circuit_pass1(dss: DSS, soc_init_pct: float, posto: str, seg_tag: str) -> str:
    ...
    # BESS em 0016
    dss.text("New line.0016_B011 phases=3 bus1=0016 bus2=B011 length=0.0150 units=km Geometry=UG_trefoil_25")
...
# %% Celula 003 - PASSO 1: monitor -> Curva_de_Carga_BESS_IN_sim_3.csv (SHIFT DIREITA)
# =============================================================================
# Entradas:
#   PASSO1/Monitor_BESS_IN_anual.csv
# Saida:
#   PASSO1/Curva_de_Carga_BESS_IN_sim_3.csv (Hora_decimal, P_3f_S, Q_3f_S)
# Remove:
#   PASSO1/Monitor_BESS_IN_anual.csv
# =============================================================================
...
OUT_FP = DIR_PASS1 / "Curva_de_Carga_BESS_IN_sim_3.csv"
...
# =============================================================================
# Entrada:
#   PASSO1/Curva_de_Carga_BESS_IN_sim_3.csv
# Saidas (WORK_DIR):
...
IN_FP = DIR_PASS1 / "Curva_de_Carga_BESS_IN_sim_3.csv"
...
DIR_PASS2 = WORK_DIR / "Simulacao 003 segmentos"
...
# =============================================================================
# Entrada:
#   Simulacao 003 segmentos/UFSM_Mon_<KEY>_s*.csv
# Saida:
#   Simulacao 003 segmentos/Monitor_<KEY>_anual.csv
# Remove:
...
# =============================================================================
# Entrada:
#   Simulacao 003 segmentos/EXP_MTR_GERAL_*.csv
# Saida:
#   Simulacao 003 segmentos/EXP_MTR_GERAL_anual.csv
# Remove:
#   EXP_MTR_GERAL_*.csv
# =============================================================================
...
#   Simulacao 003 segmentos/Monitor_<KEY>_anual.csv
# Saidas:
#   Simulacao 003 segmentos/Curva_de_Carga_<KEY>_sim_3.csv
# Remove:
...
for in_fp in files:
    ...
    out_fp = DIR_PASS2 / f"Curva_de_Carga_{mon_key}_sim_3.csv"
...
#   Simulacao 003 segmentos/EXP_MTR_GERAL_anual.csv
# Saida:
#   Simulacao 003 segmentos/Resumo_EnergyMeter_Geral_Sim3.csv
# =============================================================================
...
OUT_FP = DIR_PASS2 / "Resumo_EnergyMeter_Geral_Sim3.csv"
...
# %% Celula 010 - ENERGIA e DEMANDA MAX por mes e posto (SIM2 vs SIM3)
# =============================================================================
# Entradas:
...
#   SIM3: Simulacao 003 segmentos/Curva_de_Carga_GERAL_sim_3.csv (Hora_decimal, P_3f_S, Q_3f_S)
# Saida:
#   Simulacao 003 segmentos/01_Integralizacao_Mensal_SIM2_x_SIM3_2025.csv
# =============================================================================
...
SIM3_DIR = WORK_DIR / "Simulacao 003 segmentos"

META_FP = WORK_DIR / "00_dados_de_entrada_ano_letivo_2025_15min.csv"
SIM2_FP = SIM2_DIR / "Curva_de_Carga_GERAL_sim_2.csv"
SIM3_FP = SIM3_DIR / "Curva_de_Carga_GERAL_sim_3.csv"
OUT_FP  = SIM3_DIR / "01_Integralizacao_Mensal_SIM2_x_SIM3_2025.csv"
...
for fp in [META_FP, SIM2_FP, SIM3_FP]:
    if not fp.exists():
...
sim3 = pd.read_csv(SIM3_FP, encoding="utf-8-sig", sep=None, engine="python")

for name, dfx, req in [
    ("META", meta, ["Hora_decimal","Mes","Posto"]),
    ("SIM2", sim2, ["Hora_decimal","P_3f_S","Q_3f_S"]),
    ("SIM3", sim3, ["Hora_decimal","P_3f_S","Q_3f_S"]),
]:
...
sim3["k"] = make_k(sim3["Hora_decimal"])
...
sim3 = sim3[["k","P_3f_S","Q_3f_S"]].drop_duplicates(subset=["k"], keep="first").copy()

df2 = meta.merge(sim2, on="k", how="left").rename(columns={"P_3f_S":"P_SIM2","Q_3f_S":"Q_SIM2"})
df3 = meta.merge(sim3, on="k", how="left").rename(columns={"P_3f_S":"P_SIM3","Q_3f_S":"Q_SIM3"})

df2["E_P_SIM2_kWh"]   = df2["P_SIM2"] * DT_H
df2["E_Q_SIM2_kVArh"] = df2["Q_SIM2"] * DT_H
df3["E_P_SIM3_kWh"]   = df3["P_SIM3"] * DT_H
df3["E_Q_SIM3_kVArh"] = df3["Q_SIM3"] * DT_H
...
agg3 = df3.groupby(group_cols, dropna=False).agg(
    N_pontos_SIM3=("P_SIM3", lambda s: int(s.notna().sum())),
    Consumo_P_SIM3_kWh=("E_P_SIM3_kWh","sum"),
    Consumo_Q_SIM3_kVArh=("E_Q_SIM3_kVArh","sum"),
    DemandaMax_P_SIM3_kW=("P_SIM3","max"),
    DemandaMax_Q_SIM3_kVAr=("Q_SIM3","max"),
).reset_index()

agg = agg2.merge(agg3, on=group_cols, how="outer")
...
# %% Celula 011 - SIMULACAO 003: Tabela de custos (SIM2 vs SIM3 - 2025) com demandas contratadas diferentes
# =============================================================================
# Entrada:
#   Simulacao 003 segmentos/01_Integralizacao_Mensal_SIM2_x_SIM3_2025.csv
#
# Saidas (todas em Simulacao 003 segmentos):
#   Tabela_demanda_consumo_por_posto_SIM2_2025.csv
#   Tabela_demanda_consumo_por_posto_SIM3_2025.csv
#   Tabela_demanda_consumo_por_posto_SIM2_vs_SIM3_2025.csv   (economia)
# =============================================================================
...
IN_FP    = SIM3_DIR / "01_Integralizacao_Mensal_SIM2_x_SIM3_2025.csv"
OUT_SIM2 = SIM3_DIR / "Tabela_demanda_consumo_por_posto_SIM2_2025.csv"
OUT_SIM3 = SIM3_DIR / "Tabela_demanda_consumo_por_posto_SIM3_2025.csv"
OUT_CMP  = SIM3_DIR / "Tabela_demanda_consumo_por_posto_SIM2_vs_SIM3_2025.csv"

# -------------------------
# DEMANDAS CONTRATADAS POR SIMULACAO
# -------------------------
CONTR_SIM2 = {"DCFP": 5000.0, "DCP": 3000.0}
CONTR_SIM3 = {"DCFP": 5000.0, "DCP": 2200.0}  # 3000 - 800
...
print("DONE. Saved (Simulacao 003 segmentos):")
print(" -", OUT_SIM2)
print(" -", OUT_SIM3)
print(" -", OUT_CMP)
print("\nDemandas contratadas usadas:")
print(" - SIM2: DCFP=5000, DCP=3000")
print(" - SIM3: DCFP=5000, DCP=2200")
print("\nResumo anual:")
print(cmp.tail(1))
