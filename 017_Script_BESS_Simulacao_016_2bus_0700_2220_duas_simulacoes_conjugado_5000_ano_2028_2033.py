# -*- coding: utf-8 -*-
# =============================================================================
# 017_Script_BESS_Simulacao_016_2bus_0700_2220_duas_simulacoes_conjugado_5000_ano_2028_2033.py
#
# Regras deste script:
...
#     Simulacao 016 segmentos_pass1_sem_hvac_int_ano_2033
#   PASSO 2 (simulacao completa final):
#     Simulacao 016 segmentos_ano_2033
# =============================================================================
...
TARGETS = {
    ...
    "FORA_PONTA": {"kWtarget": 5200, "kWtargetLow": 4160},
}

# Reservas operacionais por banco:
# - FORA_PONTA: BESSM1 usa somente o excedente degradado para peak-shave FP;
#               BESSM2 fica integralmente reservado para a ponta.
# - PONTA: ambos ficam liberados ate a reserva minima tecnica.
BESS1_RESERVA_PONTA_PCT = 100.0 * 3000.0 / 4500.0  # 66.6666667%
BESS2_RESERVA_PONTA_PCT = 100.0
BESS_RESERVA_MIN_PONTA_PCT = 2.0

DIR_PASS1 = WORK_DIR / "Simulacao 016 segmentos_pass1_sem_hvac_int_ano_2033"
...
def build_circuit_pass1(
    ...
    dss.text(
        ...
        f"kWrated=767 kWhrated=4313 Effcurve=Eff %EffCharge=95 %EffDischarge=95 "
...
#     SOC_15min_BESS_SIM16_2033.csv          (SOC das duas baterias a cada 15 min)
#
# Requisitos:
...
BESS1_E_2033_KWH = 4313.0
BESS2_E_2033_KWH = 3000.0
BESS_TOTAL_E_2033_KWH = BESS1_E_2033_KWH + BESS2_E_2033_KWH
...
DIR_PASS2 = WORK_DIR / "Simulacao 016 segmentos_ano_2033"
...
if LIMPAR_SAIDAS_ANTERIORES_PASS2:
    padroes_limpeza = [
        ...
        "SOC_por_segmento.csv",
        "SOC_15min_BESS_SIM16_2033.csv",
        "01_Integralizacao_Mensal_SIM16_2033.csv",
        "Tabela_demanda_consumo_por_posto_SIM16_2033.csv",
    ]
    removidos = 0
...
soc_15min_fp = DIR_PASS2 / "SOC_15min_BESS_SIM16_2033.csv"
...
# =============================================================================
# Entrada:
#   Simulacao 016 segmentos_ano_2033/UFSM_Mon_<KEY>_s*.csv
# Saida:
#   Simulacao 016 segmentos_ano_2033/Monitor_<KEY>_anual.csv
# Remove:
...
# =============================================================================
# Entrada:
#   Simulacao 016 segmentos_ano_2033/EXP_MTR_GERAL_*.csv
# Saida:
#   Simulacao 016 segmentos_ano_2033/EXP_MTR_GERAL_anual.csv
# Remove:
#   EXP_MTR_GERAL_*.csv
# =============================================================================
...
#   Simulacao 016 segmentos_ano_2033/Monitor_<KEY>_anual.csv
# Saidas:
#   Simulacao 016 segmentos_ano_2033/Curva_de_Carga_<KEY>_sim_16.csv
# Remove:
...
#   Simulacao 016 segmentos_ano_2033/EXP_MTR_GERAL_anual.csv
# Saida:
#   Simulacao 016 segmentos_ano_2033/Resumo_EnergyMeter_Geral_Sim16.csv
# =============================================================================
...
# %% Celula 010 - ENERGIA e DEMANDA MAX por mes e posto (SIM16 - ANO 2033, BESS modular autofinanciado)
# =============================================================================
# Entradas:
#   META: 00_dados_de_entrada_ano_letivo_2025_15min.csv
#         Usado apenas como mapa anual de Hora_decimal, Mes e Posto.
#   SIM16: curva GERAL com BESS modular autofinanciado em operacao conjugada
#          para o ano 2033.
# Saida:
#   Simulacao 016 segmentos_ano_2033/01_Integralizacao_Mensal_SIM16_2033.csv
#
# Observacao:
...
#     BESS1: instalado em 2028, 1 MW / 4,5 MWh, degradado em 2033 para 767 kW / 4313 kWh.
#     BESS2: instalado em 2033, 1 MW / 3,0 MWh, operando a 800 kW / 3000 kWh.
#     Alvo fora-ponta = 5200 kW; permissao de recarga fora-ponta = 4160 kW.
#     Em fora-ponta, BESS1 usa reserva de 66,6667% e BESS2 usa reserva de 100%.
#     Em ponta, ambos usam reserva minima tecnica de 2%.
...
SIM16_DIR = WORK_DIR / f"Simulacao 016 segmentos_ano_{ANO_ANALISE}"
...
# %% Celula 011 - SIMULACAO 016: tabela de custos (SIM16 - ANO 2033, BESS modular autofinanciado)
# =============================================================================
# Entrada:
#   Simulacao 016 segmentos_ano_2033/01_Integralizacao_Mensal_SIM16_2033.csv
#
# Saida:
#   Simulacao 016 segmentos_ano_2033/Tabela_demanda_consumo_por_posto_SIM16_2033.csv
#
# Demandas contratadas do ano 2033:
#   DCFP contratada com BESS = 5000 + 4% = 5200 kW
#   Alvo operacional fora-ponta do despacho = 5200 kW
#   Permissao para recarga fora-ponta = 4160 kW
#   DCP sem BESS = 3000 + 4% = 3120 kW
#   DCP com BESS = 3120 - (767 + 800) = 1553 kW
#
...
# -------------------------
# Fora-ponta: 5000 + 4% = 5200 kW
# Ponta sem BESS: 3000 + 4% = 3120 kW
# Ponta com BESS: 3120 - (767 + 800) = 1553 kW
DCP_SEM_BESS = 3120.0
DCFP_COM_BESS = 5200.0

KW_TARGET_FP = 5200.0
KW_TARGET_LOW_FP = 4160.0
...
BESS1_P_2033_KW = 767.0
BESS1_E_2033_KWH = 4313.0
BESS1_FATOR_DEGRADACAO_E_2033 = BESS1_E_2033_KWH / BESS1_E_NOMINAL_KWH
BESS1_E_EXCEDENTE_FP_2033_KWH = BESS1_E_EXCEDENTE_FP_NOMINAL_KWH * BESS1_FATOR_DEGRADACAO_E_2033
...
BESS_P_TOTAL_KW = BESS1_P_2033_KW + BESS2_P_OPERACIONAL_2033_KW
BESS_E_TOTAL_KWH = BESS1_E_2033_KWH + BESS2_E_2033_KWH
...
#   BESS2 reserva 100% para a ponta e nao participa do peak-shave fora-ponta.
# PONTA:
#   BESS1 e BESS2 ficam liberados ate a reserva minima tecnica de 2%.
...
BESS_E_RESERVA_PONTA_KWH = BESS1_E_RESERVA_PONTA_2033_KWH + BESS2_E_RESERVA_PONTA_2033_KWH
BESS_E_DISPONIVEL_FP_KWH = BESS1_E_2033_KWH - BESS1_E_RESERVA_PONTA_2033_KWH
...
tab16 = tabela_custos(
    df,
    SIM_TAG,
    DCFP=CONTR_SIM16["DCFP"],
    DCP=CONTR_SIM16["DCP"],
)

tab16.to_csv(OUT_SIM16, index=False, encoding="utf-8")

print("DONE. Saved (Simulacao 016 - BESS modular autofinanciado - 2033 - conjugado):")
print(" -", OUT_SIM16)
...
print(f" - Potencia BESS2 2033 = {BESS2_P_OPERACIONAL_2033_KW:.0f} kW")
