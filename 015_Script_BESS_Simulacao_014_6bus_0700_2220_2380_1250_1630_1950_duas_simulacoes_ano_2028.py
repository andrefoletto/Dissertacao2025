# -*- coding: utf-8 -*-
# =============================================================================
# 015_Script_BESS_Simulacao_014_6bus_0700_2220_2380_1250_1630_1950_duas_simulacoes_2028.py
#
# Regras deste script:
...
#     Simulacao 014 segmentos_pass1_sem_hvac_int_ano_2028
#   PASSO 2 (simulacao completa final):
#     Simulacao 014 segmentos_ano_2028
# =============================================================================
...
DIR_PASS1 = WORK_DIR / "Simulacao 014 segmentos_pass1_sem_hvac_int_ano_2028"
...
# %% Celula 003 - PASSO 1: monitores -> Curva_de_Carga_BESSM1_IN_sim_14.csv, BESSM2_IN, BESSM3_IN, BESSM4_IN, BESSM5_IN e BESSM6_IN (SHIFT DIREITA)
# =============================================================================
# Entradas:
#   PASSO1/Monitor_BESSM1_IN_anual.csv
#   PASSO1/Monitor_BESSM2_IN_anual.csv
...
#   PASSO1/Monitor_BESSM6_IN_anual.csv
# Saidas:
#   PASSO1/Curva_de_Carga_BESSM1_IN_sim_14.csv
#   PASSO1/Curva_de_Carga_BESSM2_IN_sim_14.csv
#   PASSO1/Curva_de_Carga_BESSM3_IN_sim_14.csv
#   PASSO1/Curva_de_Carga_BESSM4_IN_sim_14.csv
#   PASSO1/Curva_de_Carga_BESSM5_IN_sim_14.csv
#   PASSO1/Curva_de_Carga_BESSM6_IN_sim_14.csv
# Remove:
#   PASSO1/Monitor_BESSM1_IN_anual.csv
#   PASSO1/Monitor_BESSM2_IN_anual.csv
#   PASSO1/Monitor_BESSM3_IN_anual.csv
...
for mon_base in ["BESSM1_IN", "BESSM2_IN", "BESSM3_IN", "BESSM4_IN", "BESSM5_IN", "BESSM6_IN"]:
    ...
    out_fp = DIR_PASS1 / f"Curva_de_Carga_{mon_base}_sim_14.csv"
...
for mon_base, prefix in [("BESSM1_IN", "BESSM1"), ("BESSM2_IN", "BESSM2"), ("BESSM3_IN", "BESSM3"), ("BESSM4_IN", "BESSM4"), ("BESSM5_IN", "BESSM5"), ("BESSM6_IN", "BESSM6")]:
    in_fp = DIR_PASS1 / f"Curva_de_Carga_{mon_base}_sim_14.csv"
...
DIR_PASS2 = WORK_DIR / "Simulacao 014 segmentos_ano_2028"
...
# =============================================================================
# Entrada:
#   Simulacao 014 segmentos_ano_2028/UFSM_Mon_<KEY>_s*.csv
# Saida:
#   Simulacao 014 segmentos_ano_2028/Monitor_<KEY>_anual.csv
# Remove:
...
# =============================================================================
# Entrada:
#   Simulacao 014 segmentos_ano_2028/EXP_MTR_GERAL_*.csv
# Saida:
#   Simulacao 014 segmentos_ano_2028/EXP_MTR_GERAL_anual.csv
# Remove:
#   EXP_MTR_GERAL_*.csv
# =============================================================================
...
#   Simulacao 014 segmentos_ano_2028/Monitor_<KEY>_anual.csv
# Saidas:
#   Simulacao 014 segmentos_ano_2028/Curva_de_Carga_<KEY>_sim_14.csv
# Remove:
...
for in_fp in files:
    ...
    out_fp = DIR_PASS2 / f"Curva_de_Carga_{mon_key}_sim_14.csv"
...
#   Simulacao 014 segmentos_ano_2028/EXP_MTR_GERAL_anual.csv
# Saida:
#   Simulacao 014 segmentos_ano_2028/Resumo_EnergyMeter_Geral_Sim14.csv
# =============================================================================
...
OUT_FP = DIR_PASS2 / "Resumo_EnergyMeter_Geral_Sim14.csv"
...
# %% Celula 010 - ENERGIA e DEMANDA MAX por mes e posto (SIM14 - ANO 2028, 6 BESS)
# =============================================================================
# Entradas:
#   META: 00_dados_de_entrada_ano_letivo_2025_15min.csv
#         Usado apenas como mapa anual de Hora_decimal, Mes e Posto.
#   SIM14: curva GERAL com 6 BESS para o ano 2028
# Saida:
#   Simulacao 014 segmentos_ano_2028/01_Integralizacao_Mensal_SIM14_2028.csv
#
# Observacao:
#   Esta celula nao compara com SIM2/RGE nem com outra simulacao.
#   Ela apenas integraliza a simulacao com 6 BESS do ano 2028 por mes e posto.
# =============================================================================
...
SIM14_DIR = WORK_DIR / f"Simulacao 014 segmentos_ano_{ANO_ANALISE}"

META_FP = WORK_DIR / "00_dados_de_entrada_ano_letivo_2025_15min.csv"
SIM14_FP = SIM14_DIR / "Curva_de_Carga_GERAL_sim_14.csv"
OUT_FP = SIM14_DIR / f"01_Integralizacao_Mensal_{SIM_TAG}_{ANO_ANALISE}.csv"
...
for fp in [META_FP, SIM14_FP]:
    if not fp.exists():
        raise FileNotFoundError(fp)

meta = pd.read_csv(META_FP, dtype=str, encoding="utf-8-sig")
sim14 = pd.read_csv(SIM14_FP, encoding="utf-8-sig", sep=None, engine="python")

meta = clean_cols(meta)
sim14 = clean_cols(sim14)

for name, dfx, req in [
    ("META", meta, ["Hora_decimal", "Mes", "Posto"]),
    (SIM_TAG, sim14, ["Hora_decimal", "P_3f_S", "Q_3f_S"]),
]:
...
sim14["Hora_decimal"] = pd.to_numeric(sim14["Hora_decimal"], errors="coerce")
sim14["P_3f_S"] = pd.to_numeric(sim14["P_3f_S"], errors="coerce")
sim14["Q_3f_S"] = pd.to_numeric(sim14["Q_3f_S"], errors="coerce")
sim14.dropna(subset=["Hora_decimal", "P_3f_S", "Q_3f_S"], inplace=True)

meta["k"] = make_k(meta["Hora_decimal"])
sim14["k"] = make_k(sim14["Hora_decimal"])

meta = meta[["k", "Mes", "Posto"]].drop_duplicates(subset=["k"], keep="first").copy()
sim14 = sim14[["k", "P_3f_S", "Q_3f_S"]].drop_duplicates(subset=["k"], keep="first").copy()

df14 = meta.merge(sim14, on="k", how="left").rename(
    columns={"P_3f_S": "P_SIM14", "Q_3f_S": "Q_SIM14"}
)

if df14["P_SIM14"].isna().any():
    faltas14 = int(df14["P_SIM14"].isna().sum())
    raise ValueError(f"Falha de alinhamento por Hora_decimal/k: SIM14 faltas={faltas14}")

df14["E_P_SIM14_kWh"] = df14["P_SIM14"] * DT_H
df14["E_Q_SIM14_kVArh"] = df14["Q_SIM14"] * DT_H

group_cols = ["Mes", "Posto"]

agg14 = df14.groupby(group_cols, dropna=False).agg(
    N_pontos_SIM14=("P_SIM14", lambda s: int(s.notna().sum())),
    Consumo_P_SIM14_kWh=("E_P_SIM14_kWh", "sum"),
    Consumo_Q_SIM14_kVArh=("E_Q_SIM14_kVArh", "sum"),
    DemandaMax_P_SIM14_kW=("P_SIM14", "max"),
    DemandaMax_Q_SIM14_kVAr=("Q_SIM14", "max"),
).reset_index()

meta_counts = meta.groupby(group_cols, dropna=False).agg(N_pontos_META=("Posto", "size")).reset_index()
agg = agg14.merge(meta_counts, on=group_cols, how="left")
...
print("SIM14 usado:", SIM14_FP)
print(agg[["Mes", "Posto", "N_pontos_META", "N_pontos_SIM14", "DemandaMax_P_SIM14_kW"]])

# %% Celula 011 - SIMULACAO 014: tabela de custos (SIM14 - ANO 2028, 6 BESS)
# =============================================================================
# Entrada:
#   Simulacao 014 segmentos_ano_2028/01_Integralizacao_Mensal_SIM14_2028.csv
#
# Saida:
#   Simulacao 014 segmentos_ano_2028/Tabela_demanda_consumo_por_posto_SIM14_2028.csv
#
...
SIM14_DIR = WORK_DIR / f"Simulacao 014 segmentos_ano_{ANO_ANALISE}"
SIM14_DIR.mkdir(parents=True, exist_ok=True)

IN_FP = SIM14_DIR / f"01_Integralizacao_Mensal_{SIM_TAG}_{ANO_ANALISE}.csv"
OUT_SIM14 = SIM14_DIR / f"Tabela_demanda_consumo_por_posto_{SIM_TAG}_{ANO_ANALISE}.csv"
...
# - A demanda contratada de ponta com 6 BESS considera abatimento de 6 x 800 kW,
#   limitado a zero para evitar demanda contratada negativa.
DCP_BASE_2025 = 3000.0
DCFP_BASE_2025 = 5000.0
TAXA_CRESCIMENTO_LINEAR = 0.005
ANOS_CRESCIMENTO = ANO_ANALISE - ANO_BASE_DADOS
FATOR_CRESCIMENTO = 1.0 + ANOS_CRESCIMENTO * TAXA_CRESCIMENTO_LINEAR

NUM_BESS = 6
BESS_P_KW = 800.0

DCP_SEM_BESS = DCP_BASE_2025 * FATOR_CRESCIMENTO       # 3045 kW
DCFP_COM_BESS = DCFP_BASE_2025 * FATOR_CRESCIMENTO     # 5075 kW
DCP_COM_BESS = max(0.0, DCP_SEM_BESS - NUM_BESS * BESS_P_KW)

CONTR_SIM14 = {
    "DCFP": float(round(DCFP_COM_BESS, 0)),
    "DCP": float(round(DCP_COM_BESS, 0)),
}
...
tab14 = tabela_custos(df, SIM_TAG, DCFP=CONTR_SIM14["DCFP"], DCP=CONTR_SIM14["DCP"])
tab14.to_csv(OUT_SIM14, index=False, encoding="utf-8")

print(f"DONE. Saved ({SIM_TAG} - {ANO_ANALISE}):")
print(" -", OUT_SIM14)
print("\nDemandas contratadas usadas:")
print(f" - {SIM_TAG}: DCFP={CONTR_SIM14['DCFP']:.0f}, DCP={CONTR_SIM14['DCP']:.0f}")
print("\nResumo anual:")
print(tab14.tail(1))
