# -*- coding: utf-8 -*-
# =============================================================================
# 019_Script_BESS_Simulacao_018_4bus_0700_2220_2380_1250_duas_simulacoes_conjugado_5000_ano_2028_2033_2036_2038.py
#
# Regras deste script:
...
#     Simulacao 018 segmentos_pass1_sem_hvac_int_ano_2038
#   PASSO 2 (simulacao completa final):
#     Simulacao 018 segmentos_ano_2038
# =============================================================================
...
TARGETS = {
    ...
    "FORA_PONTA": {"kWtarget": 5325, "kWtargetLow": 4260},
}

# Reservas operacionais por banco:
# - FORA_PONTA: BESSM1 usa somente o excedente degradado para peak-shave FP;
#               BESSM2, BESSM3 e BESSM4 ficam integralmente reservados para a ponta.
...
DIR_PASS1 = WORK_DIR / "Simulacao 018 segmentos_pass1_sem_hvac_int_ano_2038"
...
if LIMPAR_SAIDAS_ANTERIORES_PASS1:
    padroes_limpeza_pass1 = [
        ...
        "Curva_de_Carga_BESSM*_IN_sim_18.csv",
        "SOC_por_segmento.csv",
    ]
    removidos = 0
...
def build_circuit_pass1(
    ...
    dss.text('Redirect "UFSM_cargas_medidas_ano_2038.dss"')
    dss.text('Redirect "UFSM_cargas_especiais_ano_2038.dss"')
    dss.text('Redirect "UFSM_cargas_estimadas_com_residuo_ano_2038.dss"')
    ...
    dss.text(
        ...
        f"kWrated=733 kWhrated=4125 Effcurve=Eff %EffCharge=95 %EffDischarge=95 "
        f"%reserve={reserve_m1_pct} dispmode=external"
    )
    dss.text(
        "New Storage.BESSM2 phases=3 bus1=B023 kv=0.38 "
        f"kWrated=767 kWhrated=2875 Effcurve=Eff %EffCharge=95 %EffDischarge=95 "
        f"%reserve={reserve_m2_pct} dispmode=external"
    )
    dss.text(
        "New Storage.BESSM3 phases=3 bus1=B033 kv=0.38 "
        f"kWrated=787 kWhrated=2950 Effcurve=Eff %EffCharge=95 %EffDischarge=95 "
...
def run_one_segment_pass1(
    ...
    print(
        ...
        f"SOC_ini_M3={float(soc_ini_m3):.3f}% | SOC_ini_M4={float(soc_ini_m4):.3f}%"
    )
...
# %% Celula 003 - PASSO 1: monitores -> Curva_de_Carga_BESSM1_IN_sim_18.csv, BESSM2_IN, BESSM3_IN e BESSM4_IN (SHIFT DIREITA)
# =============================================================================
# Entradas:
#   PASSO1/Monitor_BESSM1_IN_anual.csv
#   PASSO1/Monitor_BESSM2_IN_anual.csv
...
#   PASSO1/Monitor_BESSM4_IN_anual.csv
# Saidas:
#   PASSO1/Curva_de_Carga_BESSM1_IN_sim_18.csv
#   PASSO1/Curva_de_Carga_BESSM2_IN_sim_18.csv
#   PASSO1/Curva_de_Carga_BESSM3_IN_sim_18.csv
#   PASSO1/Curva_de_Carga_BESSM4_IN_sim_18.csv
# Remove:
#   PASSO1/Monitor_BESSM1_IN_anual.csv
#   PASSO1/Monitor_BESSM2_IN_anual.csv
#   PASSO1/Monitor_BESSM3_IN_anual.csv
...
for mon_base in ["BESSM1_IN", "BESSM2_IN", "BESSM3_IN", "BESSM4_IN"]:
    ...
    out_fp = DIR_PASS1 / f"Curva_de_Carga_{mon_base}_sim_18.csv"
...
for mon_base, prefix in [("BESSM1_IN", "BESSM1"), ("BESSM2_IN", "BESSM2"), ("BESSM3_IN", "BESSM3"), ("BESSM4_IN", "BESSM4")]:
    in_fp = DIR_PASS1 / f"Curva_de_Carga_{mon_base}_sim_18.csv"
...
#     SOC_15min_BESS_SIM18_2038.csv          (SOC dos quatro bancos a cada 15 min)
#
# Requisitos:
...
BESS1_E_2038_KWH = 4125.0
BESS2_E_2038_KWH = 2875.0
BESS3_E_2038_KWH = 2950.0
BESS4_E_2038_KWH = 3000.0
BESS_TOTAL_E_2038_KWH = BESS1_E_2038_KWH + BESS2_E_2038_KWH + BESS3_E_2038_KWH + BESS4_E_2038_KWH
...
DIR_PASS2 = WORK_DIR / "Simulacao 018 segmentos_ano_2038"
...
if LIMPAR_SAIDAS_ANTERIORES_PASS2:
    padroes_limpeza = [
        "UFSM_Mon_*_s*.csv",
        "Monitor_*_anual.csv",
        "Curva_de_Carga_*_sim_18.csv",
        "EXP_MTR_GERAL*.csv",
        "Resumo_EnergyMeter_Geral_Sim17.csv",
        "SOC_por_segmento.csv",
        "SOC_15min_BESS_SIM18_2038.csv",
        "01_Integralizacao_Mensal_SIM18_2038.csv",
        "Tabela_demanda_consumo_por_posto_SIM18_2038.csv",
    ]
    removidos = 0
...
def _run_segment_pass2(
    ...
    for step_idx in range(int(n_steps)):
        ...
        soc_m1_kwh = soc_m1_pct / 100.0 * BESS1_E_2038_KWH
        soc_m2_kwh = soc_m2_pct / 100.0 * BESS2_E_2038_KWH
        soc_m3_kwh = soc_m3_pct / 100.0 * BESS3_E_2038_KWH
        soc_m4_kwh = soc_m4_pct / 100.0 * BESS4_E_2038_KWH
        soc_total_kwh = soc_m1_kwh + soc_m2_kwh + soc_m3_kwh + soc_m4_kwh
        soc_total_pct_pond = 100.0 * soc_total_kwh / BESS_TOTAL_E_2038_KWH
...
soc_15min_fp = DIR_PASS2 / "SOC_15min_BESS_SIM18_2038.csv"
...
# =============================================================================
# Entrada:
#   Simulacao 018 segmentos_ano_2038/UFSM_Mon_<KEY>_s*.csv
# Saida:
#   Simulacao 018 segmentos_ano_2038/Monitor_<KEY>_anual.csv
# Remove:
...
# =============================================================================
# Entrada:
#   Simulacao 018 segmentos_ano_2038/EXP_MTR_GERAL_*.csv
# Saida:
#   Simulacao 018 segmentos_ano_2038/EXP_MTR_GERAL_anual.csv
# Remove:
#   EXP_MTR_GERAL_*.csv
# =============================================================================
...
#   Simulacao 018 segmentos_ano_2038/Monitor_<KEY>_anual.csv
# Saidas:
#   Simulacao 018 segmentos_ano_2038/Curva_de_Carga_<KEY>_sim_18.csv
# Remove:
...
for in_fp in files:
    ...
    out_fp = DIR_PASS2 / f"Curva_de_Carga_{mon_key}_sim_18.csv"
...
#   Simulacao 018 segmentos_ano_2038/EXP_MTR_GERAL_anual.csv
# Saida:
#   Simulacao 018 segmentos_ano_2038/Resumo_EnergyMeter_Geral_Sim17.csv
# =============================================================================
...
OUT_FP = DIR_PASS2 / "Resumo_EnergyMeter_Geral_Sim17.csv"
...
# %% Celula 010 - ENERGIA e DEMANDA MAX por mes e posto (SIM18 - ANO 2038, BESS modular autofinanciado)
# =============================================================================
# Entradas:
#   META: 00_dados_de_entrada_ano_letivo_2025_15min.csv
#         Usado apenas como mapa anual de Hora_decimal, Mes e Posto.
#   SIM18: curva GERAL com BESS modular autofinanciado em operacao conjugada
#          para o ano 2038.
# Saida:
#   Simulacao 018 segmentos_ano_2038/01_Integralizacao_Mensal_SIM18_2038.csv
#
# Observacao:
#   Esta celula nao compara com SIM2/RGE nem com outra simulacao.
#   Ela apenas integraliza a simulacao conjugada do ano 2038 por mes e posto.
#   Configuracao do despacho:
#     BESS1: instalado em 2028, 1 MW / 4,5 MWh, operando a 80% da potencia,
#            degradado em 2038 por fator 0,916666667 para 733 kW / 4125 kWh.
#     BESS2: instalado em 2033, 1 MW / 3,0 MWh, operando a 80% da potencia,
#            degradado em 2038 por fator 0,958333333 para 767 kW / 2875 kWh.
#     BESS3: instalado em 2036, 1 MW / 3,0 MWh, operando a 80% da potencia,
#            degradado em 2038 por fator 0,983333333 para 787 kW / 2950 kWh.
#     BESS4: instalado em 2038, 1 MW / 3,0 MWh, operando a 80% da potencia,
#            sem degradacao: 800 kW / 3000 kWh.
#     Alvo fora-ponta = 5325 kW; permissao de recarga fora-ponta = 4260 kW.
#     Em fora-ponta, BESS1 usa reserva de 66,6667% e BESS2/BESS3/BESS4 usam reserva de 100%.
#     Em ponta, os quatro bancos usam reserva minima tecnica de 2%.
#   A curva e lida diretamente pelo sufixo sim_18, sem fallback.
# =============================================================================
...
SIM18_DIR = WORK_DIR / f"Simulacao 018 segmentos_ano_{ANO_ANALISE}"

META_FP = WORK_DIR / "00_dados_de_entrada_ano_letivo_2025_15min.csv"
SIM18_FP = SIM18_DIR / "Curva_de_Carga_GERAL_sim_18.csv"
OUT_FP = SIM18_DIR / f"01_Integralizacao_Mensal_{SIM_TAG}_{ANO_ANALISE}.csv"
...
def gerar_curva_geral_sim18_se_necessario(sim18_fp: Path, dir_pass2: Path) -> None:
    """
    Garante a existencia de Curva_de_Carga_GERAL_sim_18.csv.
    ...
    def _validar_e_gravar_curva(mon: pd.DataFrame, origem: str) -> None:
        ...
        df_out["P_3f_S"] = df_out["P1 (kW)"] + df_out["P2 (kW)"] + df_out["P3 (kW)"]
        df_out["Q_3f_S"] = df_out["Q1 (kvar)"] + df_out["Q2 (kvar)"] + df_out["Q3 (kvar)"]
        df_out[["Hora_decimal", "P_3f_S", "Q_3f_S"]].to_csv(sim18_fp, index=False, encoding="utf-8-sig")
        print("[AUTO] Curva GERAL gerada em:", sim18_fp)

    if sim18_fp.exists():
        curva = pd.read_csv(sim18_fp, encoding="utf-8-sig", sep=None, engine="python")
        if len(curva) == TOTAL_NPTS:
            return
        print(f"[AUTO] Curva GERAL existente tem {len(curva)} linhas; removendo arquivo invalido: {sim18_fp.name}")
        sim18_fp.unlink()
    ...
    if not seg_files:
        raise FileNotFoundError(
            f"Nao encontrei {sim18_fp}. Tambem nao encontrei Monitor_GERAL_anual.csv nem UFSM_Mon_GERAL_s*.csv em {dir_pass2}. "
...
gerar_curva_geral_sim18_se_necessario(SIM18_FP, SIM18_DIR)
if not SIM18_FP.exists():
    raise FileNotFoundError(SIM18_FP)

meta = pd.read_csv(META_FP, dtype=str, encoding="utf-8-sig")
sim18 = pd.read_csv(SIM18_FP, encoding="utf-8-sig", sep=None, engine="python")

meta = clean_cols(meta)
sim18 = clean_cols(sim18)

for name, dfx, req in [
    ("META", meta, ["Hora_decimal", "Mes", "Posto"]),
    (SIM_TAG, sim18, ["Hora_decimal", "P_3f_S", "Q_3f_S"]),
]:
...
sim18["Hora_decimal"] = pd.to_numeric(sim18["Hora_decimal"], errors="coerce")
sim18["P_3f_S"] = pd.to_numeric(sim18["P_3f_S"], errors="coerce")
sim18["Q_3f_S"] = pd.to_numeric(sim18["Q_3f_S"], errors="coerce")
sim18.dropna(subset=["Hora_decimal", "P_3f_S", "Q_3f_S"], inplace=True)

meta["k"] = make_k(meta["Hora_decimal"])
sim18["k"] = make_k(sim18["Hora_decimal"])

meta = meta[(meta["k"] >= 0) & (meta["k"] < TOTAL_NPTS)].copy()
sim18 = sim18[(sim18["k"] >= 0) & (sim18["k"] < TOTAL_NPTS)].copy()

meta = meta[["k", "Mes", "Posto"]].drop_duplicates(subset=["k"], keep="first").copy()
sim18 = sim18[["k", "P_3f_S", "Q_3f_S"]].drop_duplicates(subset=["k"], keep="first").copy()

if len(meta) != TOTAL_NPTS:
    raise ValueError(f"META gerou {len(meta)} amostras anuais de 15 min; esperado {TOTAL_NPTS}.")
if len(sim18) != TOTAL_NPTS:
    raise ValueError(f"SIM18 gerou {len(sim18)} amostras anuais de 15 min; esperado {TOTAL_NPTS}.")

df18 = meta.merge(sim18, on="k", how="left").rename(
    columns={"P_3f_S": "P_SIM18", "Q_3f_S": "Q_SIM18"}
)

faltas_p = int(df18["P_SIM18"].isna().sum())
faltas_q = int(df18["Q_SIM18"].isna().sum())
if faltas_p or faltas_q:
    raise ValueError(
        f"Falha de alinhamento por Hora_decimal/k: "
        f"P_SIM18 faltas={faltas_p}, Q_SIM18 faltas={faltas_q}"
    )

df18["E_P_SIM18_kWh"] = df18["P_SIM18"] * DT_H
df18["E_Q_SIM18_kVArh"] = df18["Q_SIM18"] * DT_H

group_cols = ["Mes", "Posto"]

agg18 = df18.groupby(group_cols, dropna=False).agg(
    N_pontos_SIM18=("P_SIM18", lambda s: int(s.notna().sum())),
    Consumo_P_SIM18_kWh=("E_P_SIM18_kWh", "sum"),
    Consumo_Q_SIM18_kVArh=("E_Q_SIM18_kVArh", "sum"),
    DemandaMax_P_SIM18_kW=("P_SIM18", "max"),
    DemandaMax_Q_SIM18_kVAr=("Q_SIM18", "max"),
).reset_index()

meta_counts = meta.groupby(group_cols, dropna=False).agg(N_pontos_META=("Posto", "size")).reset_index()
agg = agg18.merge(meta_counts, on=group_cols, how="left")
...
print("SIM18 usado:", SIM18_FP)
print(agg[["Mes", "Posto", "N_pontos_META", "N_pontos_SIM18", "DemandaMax_P_SIM18_kW"]])

# %% Celula 011 - SIMULACAO 018: tabela de custos (SIM18 - ANO 2038, BESS modular autofinanciado)
# =============================================================================
# Entrada:
#   Simulacao 018 segmentos_ano_2038/01_Integralizacao_Mensal_SIM18_2038.csv
#
# Saida:
#   Simulacao 018 segmentos_ano_2038/Tabela_demanda_consumo_por_posto_SIM18_2038.csv
#
# Demandas contratadas do ano 2038:
#   DCFP contratada com BESS = 5000 * 1,065 = 5325 kW
#   Alvo operacional fora-ponta do despacho = 5325 kW
#   Permissao para recarga fora-ponta = 0,80 * 5325 = 4260 kW
#   DCP sem BESS = 3000 * 1,065 = 3195 kW
#   DCP com BESS = 3195 - (733 + 767 + 787 + 800) = 108 kW
#
...
#     Em 2038: fator 0,916666667 -> 733 kW / 4125 kWh.
...
SIM18_DIR = WORK_DIR / f"Simulacao 018 segmentos_ano_{ANO_ANALISE}"
SIM18_DIR.mkdir(parents=True, exist_ok=True)

IN_FP = SIM18_DIR / f"01_Integralizacao_Mensal_{SIM_TAG}_{ANO_ANALISE}.csv"
OUT_SIM18 = SIM18_DIR / f"Tabela_demanda_consumo_por_posto_{SIM_TAG}_{ANO_ANALISE}.csv"

# -------------------------
# Demandas contratadas e alvos do despacho
# -------------------------
# Fora-ponta: 5000 * 1,065 = 5325 kW
# Ponta sem BESS: 3000 * 1,065 = 3195 kW
# Ponta com BESS: 3195 - (733 + 767 + 787 + 800) = 108 kW
DCP_SEM_BESS = 3195.0
DCFP_COM_BESS = 5325.0

KW_TARGET_FP = 5325.0
KW_TARGET_LOW_FP = 4260.0
...
BESS1_FATOR_DEGRADACAO_2038 = 0.916666667
BESS1_P_2038_KW = round(BESS1_P_OPERACIONAL_2028_KW * BESS1_FATOR_DEGRADACAO_2038)
BESS1_E_2038_KWH = round(BESS1_E_NOMINAL_KWH * BESS1_FATOR_DEGRADACAO_2038)
BESS1_E_EXCEDENTE_FP_2038_KWH = BESS1_E_EXCEDENTE_FP_NOMINAL_KWH * BESS1_FATOR_DEGRADACAO_2038

BESS2_ANO_INSTALACAO = 2033
BESS2_P_NOMINAL_KW = 1000.0
BESS2_P_OPERACIONAL_2033_KW = 800.0
BESS2_E_NOMINAL_KWH = 3000.0
BESS2_FATOR_DEGRADACAO_2038 = 0.958333333
BESS2_P_2038_KW = round(BESS2_P_OPERACIONAL_2033_KW * BESS2_FATOR_DEGRADACAO_2038)
BESS2_E_2038_KWH = round(BESS2_E_NOMINAL_KWH * BESS2_FATOR_DEGRADACAO_2038)

BESS3_ANO_INSTALACAO = 2036
BESS3_P_NOMINAL_KW = 1000.0
BESS3_P_OPERACIONAL_2036_KW = 800.0
BESS3_E_NOMINAL_KWH = 3000.0
BESS3_FATOR_DEGRADACAO_2038 = 0.983333333
BESS3_P_2038_KW = round(BESS3_P_OPERACIONAL_2036_KW * BESS3_FATOR_DEGRADACAO_2038)
BESS3_E_2038_KWH = round(BESS3_E_NOMINAL_KWH * BESS3_FATOR_DEGRADACAO_2038)

BESS4_ANO_INSTALACAO = 2038
BESS4_P_NOMINAL_KW = 1000.0
BESS4_P_OPERACIONAL_2038_KW = 800.0
BESS4_E_NOMINAL_KWH = 3000.0
BESS4_P_2038_KW = BESS4_P_OPERACIONAL_2038_KW
BESS4_E_2038_KWH = BESS4_E_NOMINAL_KWH

BESS_P_TOTAL_KW = BESS1_P_2038_KW + BESS2_P_2038_KW + BESS3_P_2038_KW + BESS4_P_2038_KW
BESS_E_TOTAL_KWH = BESS1_E_2038_KWH + BESS2_E_2038_KWH + BESS3_E_2038_KWH + BESS4_E_2038_KWH
...
#   BESS2, BESS3 e BESS4 reservam 100% para a ponta e nao participam do peak-shave fora-ponta.
# PONTA:
...
BESS1_E_RESERVA_PONTA_2038_KWH = BESS1_E_2038_KWH * BESS1_RESERVA_PONTA_PCT / 100.0
BESS2_E_RESERVA_PONTA_2038_KWH = BESS2_E_2038_KWH * BESS2_RESERVA_PONTA_PCT / 100.0
BESS3_E_RESERVA_PONTA_2038_KWH = BESS3_E_2038_KWH * BESS3_RESERVA_PONTA_PCT / 100.0
BESS4_E_RESERVA_PONTA_2038_KWH = BESS4_E_2038_KWH * BESS4_RESERVA_PONTA_PCT / 100.0
BESS_E_RESERVA_PONTA_KWH = (
    BESS1_E_RESERVA_PONTA_2038_KWH
    + BESS2_E_RESERVA_PONTA_2038_KWH
    + BESS3_E_RESERVA_PONTA_2038_KWH
    + BESS4_E_RESERVA_PONTA_2038_KWH
)
BESS_E_DISPONIVEL_FP_KWH = BESS1_E_2038_KWH - BESS1_E_RESERVA_PONTA_2038_KWH

DCP_COM_BESS = max(0.0, DCP_SEM_BESS - BESS_P_TOTAL_KW)

CONTR_SIM18 = {"DCFP": DCFP_COM_BESS, "DCP": DCP_COM_BESS}
...
if not IN_FP.exists():
    curva_fp = SIM18_DIR / "Curva_de_Carga_GERAL_sim_18.csv"
    raise FileNotFoundError(
...
tab18 = tabela_custos(
    df,
    SIM_TAG,
    DCFP=CONTR_SIM18["DCFP"],
    DCP=CONTR_SIM18["DCP"],
)

tab18.to_csv(OUT_SIM18, index=False, encoding="utf-8")

print("DONE. Saved (Simulacao 018 - BESS modular autofinanciado - 2038 - conjugado):")
print(" -", OUT_SIM18)
print("\nDemandas contratadas e alvos usados em 2038:")
...
print(f" - Potencia BESS1 2038 = {BESS1_P_2038_KW:.0f} kW")
print(f" - Potencia BESS2 2038 = {BESS2_P_2038_KW:.0f} kW")
print(f" - Potencia BESS3 2038 = {BESS3_P_2038_KW:.0f} kW")
print(f" - Potencia BESS4 2038 = {BESS4_P_2038_KW:.0f} kW")
print(f" - Energia nominal total instalada = {BESS_E_NOMINAL_TOTAL_KWH:.0f} kWh")
print(f" - Energia disponivel em 2038 = {BESS_E_TOTAL_KWH:.0f} kWh")
print(f" - Energia BESS1 2038 = {BESS1_E_2038_KWH:.0f} kWh")
print(f" - Energia BESS2 2038 = {BESS2_E_2038_KWH:.0f} kWh")
print(f" - Energia BESS3 2038 = {BESS3_E_2038_KWH:.0f} kWh")
print(f" - Energia BESS4 2038 = {BESS4_E_2038_KWH:.0f} kWh")
...
print(f" - Excedente degradado BESS1 para fora-ponta = {BESS1_E_EXCEDENTE_FP_2038_KWH:.0f} kWh")
print(f" - Energia reservada para ponta BESS1 = {BESS1_E_RESERVA_PONTA_2038_KWH:.0f} kWh")
print(f" - Energia reservada para ponta BESS2 = {BESS2_E_RESERVA_PONTA_2038_KWH:.0f} kWh")
print(f" - Energia reservada para ponta BESS3 = {BESS3_E_RESERVA_PONTA_2038_KWH:.0f} kWh")
print(f" - Energia reservada para ponta BESS4 = {BESS4_E_RESERVA_PONTA_2038_KWH:.0f} kWh")
...
print("\nResumo anual:")
print(tab18[tab18["MES"].isin(["TOTAIS", "CTOTAL", "R$TOTAL", "DCFP", "DCP"])])
