# -*- coding: utf-8 -*-
# =============================================================================
# 036_Script_UFV_Simulacao_035_5bus_2750_1760_1950_1960_3520_uma_simulacao.py
#
# Regras deste script:
...
DIR_PASS2 = WORK_DIR / "Simulacao 035 segmentos"
...
# =============================================================================
# Entrada:
#   Simulacao 035 segmentos/UFSM_Mon_<KEY>_s*.csv
# Saida:
#   Simulacao 035 segmentos/Monitor_<KEY>_anual.csv
# Remove:
...
# =============================================================================
# Entrada:
#   Simulacao 035 segmentos/EXP_MTR_GERAL_*.csv
# Saida:
#   Simulacao 035 segmentos/EXP_MTR_GERAL_anual.csv
# Remove:
#   EXP_MTR_GERAL_*.csv
# =============================================================================
...
# Saidas:
#   Simulacao 035 segmentos/Curva_de_Carga_<KEY>_sim_35.csv
# Remove:
...
for in_fp in files:
    ...
    out_fp = DIR_PASS2 / f"Curva_de_Carga_{mon_key}_sim_35.csv"
...
#   Simulacao 035 segmentos/EXP_MTR_GERAL_anual.csv
# Saida:
#   Simulacao 035 segmentos/Resumo_EnergyMeter_Geral_Sim35.csv
# =============================================================================
...
OUT_FP = DIR_PASS2 / "Resumo_EnergyMeter_Geral_Sim35.csv"
...
# %% Celula 006 - ENERGIA e DEMANDA MAX por mes e posto (SIM2 vs SIM35)
# =============================================================================
# Entradas:
...
#   SIM35: Simulacao 035 segmentos/Curva_de_Carga_GERAL_sim_35.csv
#         (Hora_decimal, P_3f_S, Q_3f_S)
#
# Saida:
#   Simulacao 035 segmentos/01_Integralizacao_Mensal_SIM2_x_SIM35_2025.csv
#
# OBSERVACAO:
#   - P > 0  -> consumo da rede
#   - P < 0  -> injecao na rede
...
SIM35_DIR = WORK_DIR / "Simulacao 035 segmentos"

META_FP = WORK_DIR / "00_dados_de_entrada_ano_letivo_2025_15min.csv"
SIM2_FP = SIM2_DIR / "Curva_de_Carga_GERAL_sim_2.csv"
SIM35_FP = SIM35_DIR / "Curva_de_Carga_GERAL_sim_35.csv"
OUT_FP  = SIM35_DIR / "01_Integralizacao_Mensal_SIM2_x_SIM35_2025.csv"
...
for fp in [META_FP, SIM2_FP, SIM35_FP]:
    if not fp.exists():
...
sim35 = pd.read_csv(SIM35_FP, encoding="utf-8-sig", sep=None, engine="python")

for name, dfx, req in [
    ("META", meta, ["Hora_decimal", "Mes", "Posto"]),
    ("SIM2", sim2, ["Hora_decimal", "P_3f_S", "Q_3f_S"]),
    ("SIM35", sim35, ["Hora_decimal", "P_3f_S", "Q_3f_S"]),
]:
...
sim35["k"] = make_k(sim35["Hora_decimal"])
...
sim35 = sim35[["k", "P_3f_S", "Q_3f_S"]].drop_duplicates(subset=["k"], keep="first").copy()

df2 = meta.merge(sim2, on="k", how="left").rename(columns={"P_3f_S": "P_SIM2", "Q_3f_S": "Q_SIM2"})
df35 = meta.merge(sim35, on="k", how="left").rename(columns={"P_3f_S": "P_SIM35", "Q_3f_S": "Q_SIM35"})

# -----------------------------------------------------------------------------
# Separacao entre consumo da rede (P > 0) e injecao na rede (P < 0)
# -----------------------------------------------------------------------------
for dfx, tag in [(df2, "SIM2"), (df35, "SIM35")]:
    pcol = f"P_{tag}"
...
agg35 = df35.groupby(group_cols, dropna=False).agg(
    N_pontos_SIM35=("P_SIM35", lambda s: int(s.notna().sum())),
    Consumo_P_SIM35_kWh=("E_cons_SIM35_kWh", "sum"),
    Injecao_P_SIM35_kWh=("E_inj_SIM35_kWh", "sum"),
    Consumo_Q_SIM35_kVArh=("E_Q_SIM35_kVArh", "sum"),
    DemandaMax_P_SIM35_kW=("P_cons_SIM35_kW", "max"),
    DemandaMax_Inj_SIM35_kW=("P_inj_SIM35_kW", "max"),
).reset_index()

agg = agg2.merge(agg35, on=group_cols, how="outer")
...
# %% Celula 007 - SIMULACAO 035: Tabela de custos (SIM2 vs SIM35 - 2025)
# =============================================================================
# Entrada:
#   Simulacao 035 segmentos/01_Integralizacao_Mensal_SIM2_x_SIM35_2025.csv
#
# Saidas (todas em Simulacao 035 segmentos):
#   Tabela_demanda_consumo_por_posto_SIM2_2025.csv
#   Tabela_demanda_consumo_por_posto_SIM35_2025.csv
#   Tabela_demanda_consumo_por_posto_SIM2_vs_SIM35_2025.csv
#
# HIPOTESE ADOTADA:
...
WORK_DIR = Path(r"C:\Users\afole\OneDrive\Dissertacao2025")
SIM35_DIR = WORK_DIR / "Simulacao 035 segmentos"
SIM35_DIR.mkdir(parents=True, exist_ok=True)

IN_FP     = SIM35_DIR / "01_Integralizacao_Mensal_SIM2_x_SIM35_2025.csv"
OUT_SIM2  = SIM35_DIR / "Tabela_demanda_consumo_por_posto_SIM2_2025.csv"
OUT_SIM35 = SIM35_DIR / "Tabela_demanda_consumo_por_posto_SIM35_2025.csv"
OUT_CMP   = SIM35_DIR / "Tabela_demanda_consumo_por_posto_SIM2_vs_SIM35_2025.csv"

# -------------------------
# DEMANDAS CONTRATADAS POR SIMULACAO
# -------------------------
CONTR_SIM2  = {"DCFP": 5000.0, "DCP": 3000.0}
CONTR_SIM35 = {"DCFP": 5000.0, "DCP": 3000.0}
...
tab35 = tabela_custos(df, "SIM35", DCFP=CONTR_SIM35["DCFP"], DCP=CONTR_SIM35["DCP"])

tab2.to_csv(OUT_SIM2, index=False, encoding="utf-8")
tab35.to_csv(OUT_SIM35, index=False, encoding="utf-8")

# -------------------------
# Comparativo (economia)
# -------------------------
t2 = tab2[tab2["MES"].isin(MESES_ORD)][["MES", "R$TOTAL_MES"]].rename(columns={"R$TOTAL_MES": "R$SIM2"})
t35 = tab35[tab35["MES"].isin(MESES_ORD)][["MES", "R$TOTAL_MES"]].rename(columns={"R$TOTAL_MES": "R$SIM35"})

cmp = t2.merge(t35, on="MES", how="inner")
cmp["MES"] = pd.Categorical(cmp["MES"], categories=MESES_ORD, ordered=True)
cmp = cmp.sort_values("MES").reset_index(drop=True)

cmp["Economia_R$"] = (cmp["R$SIM2"] - cmp["R$SIM35"]).round(2)
...
print("DONE. Saved (Simulacao 035 segmentos):")
print(" -", OUT_SIM2)
print(" -", OUT_SIM35)
print(" -", OUT_CMP)
...
print(" - SIM35: DCFP=5000, DCP=3000")
