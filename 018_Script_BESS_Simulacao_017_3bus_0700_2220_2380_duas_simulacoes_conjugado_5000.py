# -*- coding: utf-8 -*-
# =============================================================================
# 018_Script_BESS_Simulacao_017_3bus_0700_2220_2380_duas_simulacoes_conjugado.py
#
# Regras deste script:
...
#     Simulacao 017 segmentos_pass1_sem_hvac_int
#   PASSO 2 (simulacao completa final):
#     Simulacao 017 segmentos
# =============================================================================
...
DIR_PASS1 = WORK_DIR / "Simulacao 017 segmentos_pass1_sem_hvac_int"
...
def build_circuit_pass1(
    ...
    dss.text(
        ...
        f"kWrated=800 kWhrated=3350 Effcurve=Eff %EffCharge=95 %EffDischarge=95 "
...
DIR_PASS2 = WORK_DIR / "Simulacao 017 segmentos"
...
# =============================================================================
# Entrada:
#   Simulacao 017 segmentos/UFSM_Mon_<KEY>_s*.csv
# Saida:
#   Simulacao 017 segmentos/Monitor_<KEY>_anual.csv
# Remove:
...
# =============================================================================
# Entrada:
#   Simulacao 017 segmentos/EXP_MTR_GERAL_*.csv
# Saida:
#   Simulacao 017 segmentos/EXP_MTR_GERAL_anual.csv
# Remove:
#   EXP_MTR_GERAL_*.csv
# =============================================================================
...
#   Simulacao 017 segmentos/Monitor_<KEY>_anual.csv
# Saidas:
#   Simulacao 017 segmentos/Curva_de_Carga_<KEY>_sim_17.csv
# Remove:
...
#   Simulacao 017 segmentos/EXP_MTR_GERAL_anual.csv
# Saida:
#   Simulacao 017 segmentos/Resumo_EnergyMeter_Geral_Sim17.csv
# =============================================================================
...
# %% Celula 010 - ENERGIA e DEMANDA MAX por mes e posto (SIM2 vs SIM17)
# =============================================================================
# Entradas:
...
#   SIM17: Simulacao 017 segmentos/Curva_de_Carga_GERAL_sim_17.csv (Hora_decimal, P_3f_S, Q_3f_S)
# Saida:
#   Simulacao 017 segmentos/01_Integralizacao_Mensal_SIM2_x_SIM17_2025.csv
# =============================================================================
...
SIM17_DIR = WORK_DIR / "Simulacao 017 segmentos"
...
OUT_FP = SIM17_DIR / "01_Integralizacao_Mensal_SIM2_x_SIM17_2025.csv"
...
for fp in [META_FP, SIM2_FP, SIM17_FP]:
    if not fp.exists():
...
for name, dfx, req in [
    ...
    ("SIM17", sim17, ["Hora_decimal", "P_3f_S", "Q_3f_S"]),
]:
...
df17 = meta.merge(sim17, on="k", how="left").rename(columns={"P_3f_S": "P_SIM17", "Q_3f_S": "Q_SIM17"})
...
agg = agg2.merge(agg17, on=group_cols, how="outer")
...
# %% Celula 011 - SIMULACAO 017: Tabela de custos (SIM2 vs SIM17 - 2025) com demandas contratadas diferentes
# =============================================================================
# Entrada:
#   Simulacao 017 segmentos/01_Integralizacao_Mensal_SIM2_x_SIM17_2025.csv
#
# Saidas (todas em Simulacao 017 segmentos):
#   Tabela_demanda_consumo_por_posto_SIM2_2025.csv
#   Tabela_demanda_consumo_por_posto_SIM17_2025.csv
#   Tabela_demanda_consumo_por_posto_SIM2_vs_SIM17_2025.csv   (economia)
# =============================================================================
...
IN_FP = SIM17_DIR / "01_Integralizacao_Mensal_SIM2_x_SIM17_2025.csv"
OUT_SIM2 = SIM17_DIR / "Tabela_demanda_consumo_por_posto_SIM2_2025.csv"
OUT_SIM17 = SIM17_DIR / "Tabela_demanda_consumo_por_posto_SIM17_2025.csv"
OUT_CMP = SIM17_DIR / "Tabela_demanda_consumo_por_posto_SIM2_vs_SIM17_2025.csv"

CONTR_SIM2 = {"DCFP": 5000.0, "DCP": 3000.0}
CONTR_SIM17 = {"DCFP": 5000.0, "DCP": 600.0}  # 3000 - 2*400
...
tab17 = tabela_custos(df, "SIM17", DCFP=CONTR_SIM17["DCFP"], DCP=CONTR_SIM17["DCP"])
...
print("DONE. Saved (Simulacao 017 segmentos):")
print(" -", OUT_SIM2)
print(" -", OUT_SIM17)
print(" -", OUT_CMP)
print("\nDemandas contratadas usadas:")
print(" - SIM2: DCFP=5000, DCP=3000")
print(" - SIM17: DCFP=5000, DCP=600")
print("\nResumo anual:")
print(cmp.tail(1))
