# -*- coding: utf-8 -*-
# =============================================================================
# 023_Script_BESS_Simulacao_022_3bus_0700_2220_2380_duas_simulacoes_conjugado_4500_ano_2028.py
#
# Regras deste script:
...
#     Simulacao 022 segmentos_pass1_sem_hvac_int_ano_2028
#   PASSO 2 (simulacao completa final):
#     Simulacao 022 segmentos_ano_2028
# =============================================================================
...
DIR_PASS1 = WORK_DIR / "Simulacao 022 segmentos_pass1_sem_hvac_int_ano_2028"
...
# %% Celula 003 - PASSO 1: monitores -> Curva_de_Carga_BESSM1_IN_sim_22.csv, BESSM2_IN e BESSM3_IN (SHIFT DIREITA)
# =============================================================================
# Entradas:
#   PASSO1/Monitor_BESSM1_IN_anual.csv
#   PASSO1/Monitor_BESSM2_IN_anual.csv
...
#   PASSO1/Curva_de_Carga_BESSM3_IN_sim_22.csv
# Remove:
#   PASSO1/Monitor_BESSM1_IN_anual.csv
#   PASSO1/Monitor_BESSM2_IN_anual.csv
#   PASSO1/Monitor_BESSM3_IN_anual.csv
...
DIR_PASS2 = WORK_DIR / "Simulacao 022 segmentos_ano_2028"
...
# =============================================================================
# Entrada:
#   Simulacao 022 segmentos_ano_2028/UFSM_Mon_<KEY>_s*.csv
# Saida:
#   Simulacao 022 segmentos_ano_2028/Monitor_<KEY>_anual.csv
# Remove:
...
# =============================================================================
# Entrada:
#   Simulacao 022 segmentos_ano_2028/EXP_MTR_GERAL_*.csv
# Saida:
#   Simulacao 022 segmentos_ano_2028/EXP_MTR_GERAL_anual.csv
# Remove:
#   EXP_MTR_GERAL_*.csv
# =============================================================================
...
#   Simulacao 022 segmentos_ano_2028/Monitor_<KEY>_anual.csv
# Saidas:
#   Simulacao 022 segmentos_ano_2028/Curva_de_Carga_<KEY>_sim_22.csv
# Remove:
...
#   Simulacao 022 segmentos_ano_2028/EXP_MTR_GERAL_anual.csv
# Saida:
#   Simulacao 022 segmentos_ano_2028/Resumo_EnergyMeter_Geral_Sim22.csv
# =============================================================================
...
# %% Celula 010 - ENERGIA e DEMANDA MAX por mes e posto (SIM22 - ANO 2028, 3 BESS 800 kW / 4,667 MWh cada)
# =============================================================================
# Entradas:
#   META: 00_dados_de_entrada_ano_letivo_2025_15min.csv
#         Usado apenas como mapa anual de Hora_decimal, Mes e Posto.
#   SIM22: curva GERAL com 3 BESS em operacao conjugada para o ano 2028
# Saida:
#   Simulacao 022 segmentos_ano_2028/01_Integralizacao_Mensal_SIM22_2028.csv
#
# Observacao:
...
#   A curva e procurada com sufixo sim_22 ou sim_022.
#   Todas as celulas deste arquivo foram padronizadas para SIM22.
# =============================================================================
...
CURVAS_CANDIDATAS = [
    SIM22_DIR / "Curva_de_Carga_GERAL_sim_22.csv",
    SIM22_DIR / "Curva_de_Carga_GERAL_sim_022.csv",
]
SIM22_FP = next((fp for fp in CURVAS_CANDIDATAS if fp.exists()), None)
...
df22 = meta.merge(sim22, on="k", how="left").rename(
    columns={"P_3f_S": "P_SIM22", "Q_3f_S": "Q_SIM22"}
)

faltas_p = int(df22["P_SIM22"].isna().sum())
faltas_q = int(df22["Q_SIM22"].isna().sum())
...
df22["E_P_SIM22_kWh"] = df22["P_SIM22"] * DT_H
df22["E_Q_SIM22_kVArh"] = df22["Q_SIM22"] * DT_H

group_cols = ["Mes", "Posto"]

agg22 = df22.groupby(group_cols, dropna=False).agg(
...
agg = agg22.merge(meta_counts, on=group_cols, how="left")
...
# %% Celula 011 - SIMULACAO 022: tabela de custos (SIM22 - ANO 2028, 3 BESS 800 kW / 4,667 MWh cada)
# =============================================================================
# Entrada:
#   Simulacao 022 segmentos_ano_2028/01_Integralizacao_Mensal_SIM22_2028.csv
#
# Saida:
#   Simulacao 022 segmentos_ano_2028/Tabela_demanda_consumo_por_posto_SIM22_2028.csv
#
...
#
# Banco de energia:
#   3 BESS de 1 MW / 4,667 MWh, operando a 80% da potencia nominal.
#   Potencia operacional total = 2400 kW.
#   Energia total = 14001 kWh.
#   Reserva de ponta = 64,3%.
...
# - Os 3 BESS sao instalados no proprio ano de analise.
# - Cada banco possui 1 MW / 4,667 MWh nominal, mas opera com 800 kW de potencia.
...
BESS_E_UNITARIA_KWH = 4667.0
BESS_RESERVA_PONTA_PCT = 64.3

BESS_P_TOTAL_KW = NUM_BESS * BESS_P_UNITARIA_KW       # 2400 kW
BESS_E_TOTAL_KWH = NUM_BESS * BESS_E_UNITARIA_KWH     # 14001 kWh
...
print("DONE. Saved (Simulacao 022 - 3 BESS - 2028 - conjugado 4500):")
print(" -", OUT_SIM22)
