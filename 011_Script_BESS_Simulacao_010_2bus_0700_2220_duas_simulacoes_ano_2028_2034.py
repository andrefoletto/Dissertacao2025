# -*- coding: utf-8 -*-
# =============================================================================
# 011_Script_BESS_Simulacao_010_2bus_0700_2220_duas_simulacoes_ano_2028_2034.py
#
# Regras deste script:
...
#     Simulacao 010 segmentos_pass1_sem_hvac_int_ano_2034
#   PASSO 2 (simulacao completa final):
#     Simulacao 010 segmentos_ano_2034
# =============================================================================
...
TARGETS = {
    ...
    "FORA_PONTA": {"kWtarget": 9999, "kWtargetLow": 5225},
}

DIR_PASS1 = WORK_DIR / "Simulacao 010 segmentos_pass1_sem_hvac_int_ano_2034"
...
# %% Celula 003 - PASSO 1: monitores -> Curva_de_Carga_BESSM1_IN_sim_10.csv e BESSM2_IN (SHIFT DIREITA)
# =============================================================================
# Entradas:
#   PASSO1/Monitor_BESSM1_IN_anual.csv
#   PASSO1/Monitor_BESSM2_IN_anual.csv
# Saidas:
#   PASSO1/Curva_de_Carga_BESSM1_IN_sim_10.csv
#   PASSO1/Curva_de_Carga_BESSM2_IN_sim_10.csv
# Remove:
#   PASSO1/Monitor_BESSM1_IN_anual.csv
#   PASSO1/Monitor_BESSM2_IN_anual.csv
# =============================================================================
...
for mon_base in ["BESSM1_IN", "BESSM2_IN"]:
    ...
    out_fp = DIR_PASS1 / f"Curva_de_Carga_{mon_base}_sim_10.csv"
...
for mon_base, prefix in [("BESSM1_IN", "BESSM1"), ("BESSM2_IN", "BESSM2")]:
    in_fp = DIR_PASS1 / f"Curva_de_Carga_{mon_base}_sim_10.csv"
...
DIR_PASS2 = WORK_DIR / "Simulacao 010 segmentos_ano_2034"
...
# =============================================================================
# Entrada:
#   Simulacao 010 segmentos_ano_2034/UFSM_Mon_<KEY>_s*.csv
# Saida:
#   Simulacao 010 segmentos_ano_2034/Monitor_<KEY>_anual.csv
# Remove:
...
# =============================================================================
# Entrada:
#   Simulacao 010 segmentos_ano_2034/EXP_MTR_GERAL_*.csv
# Saida:
#   Simulacao 010 segmentos_ano_2034/EXP_MTR_GERAL_anual.csv
# Remove:
#   EXP_MTR_GERAL_*.csv
# =============================================================================
...
#   Simulacao 010 segmentos_ano_2034/Monitor_<KEY>_anual.csv
# Saidas:
#   Simulacao 010 segmentos_ano_2034/Curva_de_Carga_<KEY>_sim_10.csv
# Remove:
...
for in_fp in files:
    ...
    out_fp = DIR_PASS2 / f"Curva_de_Carga_{mon_key}_sim_10.csv"
...
#   Simulacao 010 segmentos_ano_2034/EXP_MTR_GERAL_anual.csv
# Saida:
#   Simulacao 010 segmentos_ano_2034/Resumo_EnergyMeter_Geral_Sim10.csv
# =============================================================================
...
OUT_FP = DIR_PASS2 / "Resumo_EnergyMeter_Geral_Sim10.csv"
...
# %% Celula 010 - ENERGIA e DEMANDA MAX por mes e posto (SIM10 - ANO 2034, 2 BESS)
# =============================================================================
# Entradas:
#   META: 00_dados_de_entrada_ano_letivo_2025_15min.csv
#         Usado apenas como mapa anual de Hora_decimal, Mes e Posto.
#   SIM10: curva GERAL com 2 BESS para o ano 2034
# Saida:
#   Simulacao 010 segmentos_ano_2034/01_Integralizacao_Mensal_SIM10_2034.csv
# =============================================================================
...
SIM10_DIR = WORK_DIR / f"Simulacao 010 segmentos_ano_{ANO_ANALISE}"

META_FP = WORK_DIR / "00_dados_de_entrada_ano_letivo_2025_15min.csv"
SIM10_FP = SIM10_DIR / "Curva_de_Carga_GERAL_sim_10.csv"
OUT_FP = SIM10_DIR / f"01_Integralizacao_Mensal_{SIM_TAG}_{ANO_ANALISE}.csv"
...
for fp in [META_FP, SIM10_FP]:
    if not fp.exists():
        raise FileNotFoundError(fp)

meta = pd.read_csv(META_FP, dtype=str, encoding="utf-8-sig")
sim10 = pd.read_csv(SIM10_FP, encoding="utf-8-sig", sep=None, engine="python")

for name, dfx, req in [
    ("META", meta, ["Hora_decimal", "Mes", "Posto"]),
    (SIM_TAG, sim10, ["Hora_decimal", "P_3f_S", "Q_3f_S"]),
]:
...
sim10["Hora_decimal"] = pd.to_numeric(sim10["Hora_decimal"], errors="coerce")
sim10["P_3f_S"] = pd.to_numeric(sim10["P_3f_S"], errors="coerce")
sim10["Q_3f_S"] = pd.to_numeric(sim10["Q_3f_S"], errors="coerce")
sim10.dropna(subset=["Hora_decimal", "P_3f_S", "Q_3f_S"], inplace=True)

meta["k"] = make_k(meta["Hora_decimal"])
sim10["k"] = make_k(sim10["Hora_decimal"])

meta = meta[["k", "Mes", "Posto"]].drop_duplicates(subset=["k"], keep="first").copy()
sim10 = sim10[["k", "P_3f_S", "Q_3f_S"]].drop_duplicates(subset=["k"], keep="first").copy()

df10 = meta.merge(sim10, on="k", how="left").rename(
    columns={"P_3f_S": "P_SIM10", "Q_3f_S": "Q_SIM10"}
)

if df10["P_SIM10"].isna().any():
    faltas10 = int(df10["P_SIM10"].isna().sum())
    raise ValueError(f"Falha de alinhamento por Hora_decimal/k: SIM10 faltas={faltas10}")

df10["E_P_SIM10_kWh"] = df10["P_SIM10"] * DT_H
df10["E_Q_SIM10_kVArh"] = df10["Q_SIM10"] * DT_H

group_cols = ["Mes", "Posto"]

agg10 = df10.groupby(group_cols, dropna=False).agg(
    N_pontos_SIM10=("P_SIM10", lambda s: int(s.notna().sum())),
    Consumo_P_SIM10_kWh=("E_P_SIM10_kWh", "sum"),
    Consumo_Q_SIM10_kVArh=("E_Q_SIM10_kVArh", "sum"),
    DemandaMax_P_SIM10_kW=("P_SIM10", "max"),
    DemandaMax_Q_SIM10_kVAr=("Q_SIM10", "max"),
).reset_index()

meta_counts = meta.groupby(group_cols, dropna=False).agg(N_pontos_META=("Posto", "size")).reset_index()
agg = agg10.merge(meta_counts, on=group_cols, how="left")
...
print("SIM10 usado:", SIM10_FP)

# %% Celula 011 - SIMULACAO 010: tabela de custos (SIM10 - ANO 2034, 2 BESS)
# =============================================================================
# Entrada:
#   Simulacao 010 segmentos_ano_2034/01_Integralizacao_Mensal_SIM10_2034.csv
#
# Saida:
#   Simulacao 010 segmentos_ano_2034/Tabela_demanda_consumo_por_posto_SIM10_2034.csv
# =============================================================================
...
SIM10_DIR = WORK_DIR / f"Simulacao 010 segmentos_ano_{ANO_ANALISE}"
SIM10_DIR.mkdir(parents=True, exist_ok=True)

IN_FP = SIM10_DIR / f"01_Integralizacao_Mensal_{SIM_TAG}_{ANO_ANALISE}.csv"
OUT_SIM10 = SIM10_DIR / f"Tabela_demanda_consumo_por_posto_{SIM_TAG}_{ANO_ANALISE}.csv"
...
# - Em 2034 aplica-se crescimento acumulado de 4,5% (9 anos desde 2025).
# - BESS1 instalado no inicio do horizonte, com potencia remanescente de 760 kW em 2034.
# - BESS2 instalado em 2034, ainda com potencia nominal de 800 kW.
# - Demanda de ponta com BESS = 3135 - (760 + 800) = 1575 kW.
DCP_BASE_2025 = 3000.0
DCFP_BASE_2025 = 5000.0
CRESC_LINEAR_ANUAL = 0.005
BESS1_P_KW_2034 = 760.0
BESS2_P_KW_2034 = 800.0

anos_decorridos = ANO_ANALISE - ANO_BASE_DADOS
fator_linear = 1.0 + CRESC_LINEAR_ANUAL * anos_decorridos

DCP_SEM_BESS = round(DCP_BASE_2025 * fator_linear, 0)       # 3135 kW em 2034
DCFP_COM_BESS = round(DCFP_BASE_2025 * fator_linear, 0)     # 5225 kW em 2034
DCP_COM_BESS = max(0.0, DCP_SEM_BESS - (BESS1_P_KW_2034 + BESS2_P_KW_2034))  # 1575 kW em 2034

CONTR_SIM10 = {"DCFP": DCFP_COM_BESS, "DCP": DCP_COM_BESS}
...
tab10 = tabela_custos(df, SIM_TAG, DCFP=CONTR_SIM10["DCFP"], DCP=CONTR_SIM10["DCP"])
tab10.to_csv(OUT_SIM10, index=False, encoding="utf-8")

print(f"DONE. Saved (Simulacao 010 segmentos_ano_{ANO_ANALISE}):")
print(" -", OUT_SIM10)
print("\nDemandas contratadas usadas:")
print(f" - {SIM_TAG}: DCFP={CONTR_SIM10['DCFP']:.0f}, DCP={CONTR_SIM10['DCP']:.0f}")
print("\nResumo anual:")
print(tab10.tail(1))
