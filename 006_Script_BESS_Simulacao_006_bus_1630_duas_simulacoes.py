# -*- coding: utf-8 -*-
# =============================================================================
# Script_BESS_Simulacao_006_bus_1630_duas_simulacoes.py
#
# Regras deste script:
...
#     Simulacao 006 segmentos_pass1_sem_hvac_int
#   PASSO 2 (simulacao completa final):
#     Simulacao 006 segmentos
# =============================================================================
...
TARGETS = {
    ...
    "FORA_PONTA": {"kWtarget": 9999, "kWtargetLow": 5000},
}

DIR_PASS1 = WORK_DIR / "Simulacao 006 segmentos_pass1_sem_hvac_int"
...
def build_circuit_pass1(dss: DSS, soc_init_pct: float, posto: str, seg_tag: str) -> str:
    ...
    # BESS em 1630
    dss.text("New line.1630_B011 phases=3 bus1=1630 bus2=B011 length=0.0150 units=km Geometry=UG_trefoil_25")
...
# %% Celula 003 - PASSO 1: monitor -> Curva_de_Carga_BESS_IN_sim_6.csv (SHIFT DIREITA)
# =============================================================================
# Entradas:
#   PASSO1/Monitor_BESS_IN_anual.csv
# Saida:
#   PASSO1/Curva_de_Carga_BESS_IN_sim_6.csv (Hora_decimal, P_3f_S, Q_3f_S)
# Remove:
#   PASSO1/Monitor_BESS_IN_anual.csv
# =============================================================================
...
OUT_FP = DIR_PASS1 / "Curva_de_Carga_BESS_IN_sim_6.csv"
...
# =============================================================================
# Entrada:
#   PASSO1/Curva_de_Carga_BESS_IN_sim_6.csv
# Saidas (WORK_DIR):
...
IN_FP = DIR_PASS1 / "Curva_de_Carga_BESS_IN_sim_6.csv"
...
DIR_PASS2 = WORK_DIR / "Simulacao 006 segmentos"
...
# =============================================================================
# Entrada:
#   Simulacao 006 segmentos/UFSM_Mon_<KEY>_s*.csv
# Saida:
#   Simulacao 006 segmentos/Monitor_<KEY>_anual.csv
# Remove:
...
# =============================================================================
# Entrada:
#   Simulacao 006 segmentos/EXP_MTR_GERAL_*.csv
# Saida:
#   Simulacao 006 segmentos/EXP_MTR_GERAL_anual.csv
# Remove:
#   EXP_MTR_GERAL_*.csv
# =============================================================================
...
#   Simulacao 006 segmentos/Monitor_<KEY>_anual.csv
# Saidas:
#   Simulacao 006 segmentos/Curva_de_Carga_<KEY>_sim_6.csv
# Remove:
...
for in_fp in files:
    ...
    out_fp = DIR_PASS2 / f"Curva_de_Carga_{mon_key}_sim_6.csv"
...
#   Simulacao 006 segmentos/EXP_MTR_GERAL_anual.csv
# Saida:
#   Simulacao 006 segmentos/Resumo_EnergyMeter_Geral_Sim6.csv
# =============================================================================
...
OUT_FP = DIR_PASS2 / "Resumo_EnergyMeter_Geral_Sim6.csv"
...
# %% Celula 010 - ENERGIA e DEMANDA MAX por mes e posto (SIM2 vs SIM6)
# =============================================================================
# Entradas:
...
#   SIM6: Simulacao 006 segmentos/Curva_de_Carga_GERAL_sim_6.csv (Hora_decimal, P_3f_S, Q_3f_S)
# Saida:
#   Simulacao 006 segmentos/01_Integralizacao_Mensal_SIM2_x_SIM6_2025.csv
# =============================================================================
...
SIM6_DIR = WORK_DIR / "Simulacao 006 segmentos"

META_FP = WORK_DIR / "00_dados_de_entrada_ano_letivo_2025_15min.csv"
SIM2_FP = SIM2_DIR / "Curva_de_Carga_GERAL_sim_2.csv"
SIM6_FP = SIM6_DIR / "Curva_de_Carga_GERAL_sim_6.csv"
OUT_FP  = SIM6_DIR / "01_Integralizacao_Mensal_SIM2_x_SIM6_2025.csv"
...
for fp in [META_FP, SIM2_FP, SIM6_FP]:
    if not fp.exists():
...
sim6 = pd.read_csv(SIM6_FP, encoding="utf-8-sig", sep=None, engine="python")

for name, dfx, req in [
    ("META", meta, ["Hora_decimal","Mes","Posto"]),
    ("SIM2", sim2, ["Hora_decimal","P_3f_S","Q_3f_S"]),
    ("SIM6", sim6, ["Hora_decimal","P_3f_S","Q_3f_S"]),
]:
...
sim6["k"] = make_k(sim6["Hora_decimal"])
...
sim6 = sim6[["k","P_3f_S","Q_3f_S"]].drop_duplicates(subset=["k"], keep="first").copy()

df2 = meta.merge(sim2, on="k", how="left").rename(columns={"P_3f_S":"P_SIM2","Q_3f_S":"Q_SIM2"})
df6 = meta.merge(sim6, on="k", how="left").rename(columns={"P_3f_S":"P_SIM6","Q_3f_S":"Q_SIM6"})

df2["E_P_SIM2_kWh"]   = df2["P_SIM2"] * DT_H
df2["E_Q_SIM2_kVArh"] = df2["Q_SIM2"] * DT_H
df6["E_P_SIM6_kWh"]   = df6["P_SIM6"] * DT_H
df6["E_Q_SIM6_kVArh"] = df6["Q_SIM6"] * DT_H
...
agg6 = df6.groupby(group_cols, dropna=False).agg(
    N_pontos_SIM6=("P_SIM6", lambda s: int(s.notna().sum())),
    Consumo_P_SIM6_kWh=("E_P_SIM6_kWh","sum"),
    Consumo_Q_SIM6_kVArh=("E_Q_SIM6_kVArh","sum"),
    DemandaMax_P_SIM6_kW=("P_SIM6","max"),
    DemandaMax_Q_SIM6_kVAr=("Q_SIM6","max"),
).reset_index()

agg = agg2.merge(agg6, on=group_cols, how="outer")
...
# %% Celula 011 - SIMULACAO 006: Tabela de custos (SIM2 vs SIM6 - 2025) com demandas contratadas diferentes
# =============================================================================
# Entrada:
#   Simulacao 006 segmentos/01_Integralizacao_Mensal_SIM2_x_SIM6_2025.csv
#
# Saidas (todas em Simulacao 006 segmentos):
#   Tabela_demanda_consumo_por_posto_SIM2_2025.csv
#   Tabela_demanda_consumo_por_posto_SIM6_2025.csv
#   Tabela_demanda_consumo_por_posto_SIM2_vs_SIM6_2025.csv   (economia)
# =============================================================================
...
WORK_DIR = Path(r"C:\Users\afole\OneDrive\Dissertacao2025")
SIM6_DIR = WORK_DIR / "Simulacao 006 segmentos"
SIM6_DIR.mkdir(parents=True, exist_ok=True)

IN_FP    = SIM6_DIR / "01_Integralizacao_Mensal_SIM2_x_SIM6_2025.csv"
OUT_SIM2 = SIM6_DIR / "Tabela_demanda_consumo_por_posto_SIM2_2025.csv"
OUT_SIM6 = SIM6_DIR / "Tabela_demanda_consumo_por_posto_SIM6_2025.csv"
OUT_CMP  = SIM6_DIR / "Tabela_demanda_consumo_por_posto_SIM2_vs_SIM6_2025.csv"

# -------------------------
# DEMANDAS CONTRATADAS POR SIMULACAO
# -------------------------
CONTR_SIM2 = {"DCFP": 5000.0, "DCP": 3000.0}
CONTR_SIM6 = {"DCFP": 5000.0, "DCP": 2200.0}  # 3000 - 800
...
def tabela_custos(df_in: pd.DataFrame, fonte: str, DCFP: float, DCP: float) -> pd.DataFrame:
    ...
    # Linha de totais (max de demanda, soma de energia e soma de R$)
    totais = {
        "MES": "TOTAIS",
...
print("DONE. Saved (Simulacao 006 segmentos):")
print(" -", OUT_SIM2)
print(" -", OUT_SIM6)
print(" -", OUT_CMP)
print("\nDemandas contratadas usadas:")
print(" - SIM2: DCFP=5000, DCP=3000")
print(" - SIM6: DCFP=5000, DCP=2200")
print("\nResumo anual:")
print(cmp.tail(1))
