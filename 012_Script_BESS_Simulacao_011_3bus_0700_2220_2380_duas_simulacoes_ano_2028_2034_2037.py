# -*- coding: utf-8 -*-
# =============================================================================
# 012_Script_BESS_Simulacao_011_3bus_0700_2220_2380_duas_simulacoes_ano_2028_2034_2037.py
#
# Regras deste script:
...
#     Simulacao 011 segmentos_pass1_sem_hvac_int_ano_2037
#   PASSO 2 (simulacao completa final):
#     Simulacao 011 segmentos_ano_2037
# =============================================================================
...
TARGETS = {
    ...
    "FORA_PONTA": {"kWtarget": 9999, "kWtargetLow": 5300},
}

DIR_PASS1 = WORK_DIR / "Simulacao 011 segmentos_pass1_sem_hvac_int_ano_2037"
...
def build_circuit_pass1(
    ...
    dss.text('Redirect "UFSM_cargas_medidas_ano_2037.dss"')
    dss.text('Redirect "UFSM_cargas_especiais_ano_2037.dss"')
    dss.text('Redirect "UFSM_cargas_estimadas_com_residuo_ano_2037.dss"')
    ...
    dss.text(
        ...
        "kWrated=740 kWhrated=2775 Effcurve=Eff %EffCharge=95 %EffDischarge=95 "
        "%reserve=2.5 dispmode=external"
    )
    dss.text(
        "New Storage.BESSM2 phases=3 bus1=B023 kv=0.38 "
        "kWrated=780 kWhrated=2925 Effcurve=Eff %EffCharge=95 %EffDischarge=95 "
        "%reserve=2.5 dispmode=external"
    )
    dss.text(
...
# %% Celula 003 - PASSO 1: monitores -> Curva_de_Carga_BESSM1_IN_sim_11.csv, BESSM2_IN e BESSM3_IN (SHIFT DIREITA)
# =============================================================================
# Entradas:
#   PASSO1/Monitor_BESSM1_IN_anual.csv
#   PASSO1/Monitor_BESSM2_IN_anual.csv
#   PASSO1/Monitor_BESSM3_IN_anual.csv
# Saidas:
#   PASSO1/Curva_de_Carga_BESSM1_IN_sim_11.csv
#   PASSO1/Curva_de_Carga_BESSM2_IN_sim_11.csv
#   PASSO1/Curva_de_Carga_BESSM3_IN_sim_11.csv
# Remove:
#   PASSO1/Monitor_BESSM1_IN_anual.csv
#   PASSO1/Monitor_BESSM2_IN_anual.csv
#   PASSO1/Monitor_BESSM3_IN_anual.csv
...
for mon_base in ["BESSM1_IN", "BESSM2_IN", "BESSM3_IN"]:
    ...
    out_fp = DIR_PASS1 / f"Curva_de_Carga_{mon_base}_sim_11.csv"
...
for mon_base, prefix in [("BESSM1_IN", "BESSM1"), ("BESSM2_IN", "BESSM2"), ("BESSM3_IN", "BESSM3")]:
    in_fp = DIR_PASS1 / f"Curva_de_Carga_{mon_base}_sim_11.csv"
...
DIR_PASS2 = WORK_DIR / "Simulacao 011 segmentos_ano_2037"
...
# =============================================================================
# Entrada:
#   Simulacao 011 segmentos_ano_2037/UFSM_Mon_<KEY>_s*.csv
# Saida:
#   Simulacao 011 segmentos_ano_2037/Monitor_<KEY>_anual.csv
# Remove:
...
# =============================================================================
# Entrada:
#   Simulacao 011 segmentos_ano_2037/EXP_MTR_GERAL_*.csv
# Saida:
#   Simulacao 011 segmentos_ano_2037/EXP_MTR_GERAL_anual.csv
# Remove:
#   EXP_MTR_GERAL_*.csv
# =============================================================================
...
#   Simulacao 011 segmentos_ano_2037/Monitor_<KEY>_anual.csv
# Saidas:
#   Simulacao 011 segmentos_ano_2037/Curva_de_Carga_<KEY>_sim_11.csv
# Remove:
...
for in_fp in files:
    ...
    out_fp = DIR_PASS2 / f"Curva_de_Carga_{mon_key}_sim_11.csv"
...
#   Simulacao 011 segmentos_ano_2037/EXP_MTR_GERAL_anual.csv
# Saida:
#   Simulacao 011 segmentos_ano_2037/Resumo_EnergyMeter_Geral_Sim11.csv
# =============================================================================
...
OUT_FP = DIR_PASS2 / "Resumo_EnergyMeter_Geral_Sim11.csv"
...
# %% Celula 010 - ENERGIA e DEMANDA MAX por mes e posto (SIM11 - ANO 2037, 3 BESS)
# =============================================================================
# Entradas:
#   META: 00_dados_de_entrada_ano_letivo_2025_15min.csv
#         Usado apenas como mapa anual de Hora_decimal, Mes e Posto.
#   SIM11: curva GERAL com 3 BESS para o ano 2037
# Saida:
#   Simulacao 011 segmentos_ano_2037/01_Integralizacao_Mensal_SIM11_2037.csv
#
# Observacao:
#   Esta celula nao compara com SIM2/RGE nem com outra simulacao.
#   Ela apenas integraliza a simulacao com 3 BESS do ano 2037 por mes e posto.
# =============================================================================
...
SIM11_DIR = WORK_DIR / f"Simulacao 011 segmentos_ano_{ANO_ANALISE}"

META_FP = WORK_DIR / "00_dados_de_entrada_ano_letivo_2025_15min.csv"
SIM11_FP = SIM11_DIR / "Curva_de_Carga_GERAL_sim_11.csv"
OUT_FP = SIM11_DIR / f"01_Integralizacao_Mensal_{SIM_TAG}_{ANO_ANALISE}.csv"
...
for fp in [META_FP, SIM11_FP]:
    if not fp.exists():
        raise FileNotFoundError(fp)

meta = pd.read_csv(META_FP, dtype=str, encoding="utf-8-sig")
sim11 = pd.read_csv(SIM11_FP, encoding="utf-8-sig", sep=None, engine="python")

meta = clean_cols(meta)
sim11 = clean_cols(sim11)

for name, dfx, req in [
    ("META", meta, ["Hora_decimal", "Mes", "Posto"]),
    (SIM_TAG, sim11, ["Hora_decimal", "P_3f_S", "Q_3f_S"]),
]:
...
sim11["Hora_decimal"] = pd.to_numeric(sim11["Hora_decimal"], errors="coerce")
sim11["P_3f_S"] = pd.to_numeric(sim11["P_3f_S"], errors="coerce")
sim11["Q_3f_S"] = pd.to_numeric(sim11["Q_3f_S"], errors="coerce")
sim11.dropna(subset=["Hora_decimal", "P_3f_S", "Q_3f_S"], inplace=True)

meta["k"] = make_k(meta["Hora_decimal"])
sim11["k"] = make_k(sim11["Hora_decimal"])

meta = meta[["k", "Mes", "Posto"]].drop_duplicates(subset=["k"], keep="first").copy()
sim11 = sim11[["k", "P_3f_S", "Q_3f_S"]].drop_duplicates(subset=["k"], keep="first").copy()

df11 = meta.merge(sim11, on="k", how="left").rename(
    columns={"P_3f_S": "P_SIM11", "Q_3f_S": "Q_SIM11"}
)

if df11["P_SIM11"].isna().any():
    faltas11 = int(df11["P_SIM11"].isna().sum())
    raise ValueError(f"Falha de alinhamento por Hora_decimal/k: SIM11 faltas={faltas11}")

df11["E_P_SIM11_kWh"] = df11["P_SIM11"] * DT_H
df11["E_Q_SIM11_kVArh"] = df11["Q_SIM11"] * DT_H

group_cols = ["Mes", "Posto"]

agg11 = df11.groupby(group_cols, dropna=False).agg(
    N_pontos_SIM11=("P_SIM11", lambda s: int(s.notna().sum())),
    Consumo_P_SIM11_kWh=("E_P_SIM11_kWh", "sum"),
    Consumo_Q_SIM11_kVArh=("E_Q_SIM11_kVArh", "sum"),
    DemandaMax_P_SIM11_kW=("P_SIM11", "max"),
    DemandaMax_Q_SIM11_kVAr=("Q_SIM11", "max"),
).reset_index()

meta_counts = meta.groupby(group_cols, dropna=False).agg(N_pontos_META=("Posto", "size")).reset_index()
agg = agg11.merge(meta_counts, on=group_cols, how="left")
...
print("SIM11 usado:", SIM11_FP)
print(agg[["Mes", "Posto", "N_pontos_META", "N_pontos_SIM11", "DemandaMax_P_SIM11_kW"]])

# %% Celula 011 - SIMULACAO 011: tabela de custos (SIM11 - ANO 2037, 3 BESS)
# =============================================================================
# Entrada:
#   Simulacao 011 segmentos_ano_2037/01_Integralizacao_Mensal_SIM11_2037.csv
#
# Saida:
#   Simulacao 011 segmentos_ano_2037/Tabela_demanda_consumo_por_posto_SIM11_2037.csv
#
# Demandas contratadas do ano 2037:
#   DCFP = 5000 kW * (1 + 12 * 0,5%) = 5300 kW
#   DCP sem BESS = 3000 kW * (1 + 12 * 0,5%) = 3180 kW
#   DCP com 3 BESS = 3180 - (740 + 780 + 800) = 860 kW
#   BESS1 instalado no inicio, BESS2 em 2034 e BESS3 em 2037.
#
# Observacao:
...
SIM11_DIR = WORK_DIR / f"Simulacao 011 segmentos_ano_{ANO_ANALISE}"
SIM11_DIR.mkdir(parents=True, exist_ok=True)

IN_FP = SIM11_DIR / f"01_Integralizacao_Mensal_{SIM_TAG}_{ANO_ANALISE}.csv"
OUT_SIM11 = SIM11_DIR / f"Tabela_demanda_consumo_por_posto_{SIM_TAG}_{ANO_ANALISE}.csv"

# -------------------------
# Demandas contratadas
# -------------------------
# Premissa:
# - 2037 e o ano da insercao do BESS3.
# - Dados de carga sao de 2025.
# - Crescimento linear anual = 0,5% sobre a demanda-base de 2025.
# - Em 2037 aplica-se crescimento acumulado de 6,0%.
# - O BESS1 ja apresenta potencia remanescente de 740 kW.
# - O BESS2 ja apresenta potencia remanescente de 780 kW.
# - O BESS3 e novo no ano de 2037, com potencia de 800 kW.
...
DCP_SEM_BESS = round(DCP_BASE_2025 * fator_linear, 0)       # 3180 kW em 2037
DCFP_COM_BESS = round(DCFP_BASE_2025 * fator_linear, 0)     # 5300 kW em 2037
P_BESS_TOTAL_KW = BESS1_P_KW + BESS2_P_KW + BESS3_P_KW      # 2320 kW
DCP_COM_BESS = max(0.0, DCP_SEM_BESS - P_BESS_TOTAL_KW)     # 860 kW em 2037

CONTR_SIM11 = {"DCFP": DCFP_COM_BESS, "DCP": DCP_COM_BESS}
...
tab11 = tabela_custos(df, SIM_TAG, DCFP=CONTR_SIM11["DCFP"], DCP=CONTR_SIM11["DCP"])
tab11.to_csv(OUT_SIM11, index=False, encoding="utf-8")

print(f"DONE. Saved (Simulacao 011 segmentos_ano_{ANO_ANALISE}):")
print(" -", OUT_SIM11)
print("\nDemandas contratadas usadas:")
print(f" - {SIM_TAG}: DCFP={CONTR_SIM11['DCFP']:.0f}, DCP={CONTR_SIM11['DCP']:.0f}")
print("\nResumo anual:")
print(tab11.tail(1))
