# -*- coding: utf-8 -*-
# =============================================================================
# 026_Script_BESS_Simulacao_025_6bus_0700_2220_2380_1250_1630_1950_duas_simulacoes_conjugado_4500.py
#
# Regras deste script:
...
#     Simulacao 025 segmentos_pass1_sem_hvac_int
#   PASSO 2 (simulacao completa final):
#     Simulacao 025 segmentos
# =============================================================================
...
DIR_PASS1 = WORK_DIR / "Simulacao 025 segmentos_pass1_sem_hvac_int"
...
def build_circuit_pass1(
    ...
    dss.text(
        ...
        f"kWrated=800 kWhrated=3670 Effcurve=Eff %EffCharge=95 %EffDischarge=95 "
...
DIR_PASS2 = WORK_DIR / "Simulacao 025 segmentos"
...
# =============================================================================
# Entrada:
#   Simulacao 025 segmentos/UFSM_Mon_<KEY>_s*.csv
# Saida:
#   Simulacao 025 segmentos/Monitor_<KEY>_anual.csv
# Remove:
...
# =============================================================================
# Entrada:
#   Simulacao 025 segmentos/EXP_MTR_GERAL_*.csv
# Saida:
#   Simulacao 025 segmentos/EXP_MTR_GERAL_anual.csv
# Remove:
#   EXP_MTR_GERAL_*.csv
# =============================================================================
...
#   Simulacao 025 segmentos/Monitor_<KEY>_anual.csv
# Saidas:
#   Simulacao 025 segmentos/Curva_de_Carga_<KEY>_sim_25.csv
# Remove:
...
#   Simulacao 025 segmentos/EXP_MTR_GERAL_anual.csv
# Saida:
#   Simulacao 025 segmentos/Resumo_EnergyMeter_Geral_Sim25.csv
# =============================================================================
...
# %% Celula 010 - ENERGIA e DEMANDA MAX por mes e posto (SIM2 vs SIM25)
# =============================================================================
# Entradas:
...
#   SIM25: Simulacao 025 segmentos/Curva_de_Carga_GERAL_sim_25.csv (Hora_decimal, P_3f_S, Q_3f_S)
# Saida:
#   Simulacao 025 segmentos/01_Integralizacao_Mensal_SIM2_x_SIM25_2025.csv
# =============================================================================
...
SIM25_DIR = WORK_DIR / "Simulacao 025 segmentos"
...
OUT_FP = SIM25_DIR / "01_Integralizacao_Mensal_SIM2_x_SIM25_2025.csv"
...
for fp in [META_FP, SIM2_FP, SIM25_FP]:
    if not fp.exists():
...
for name, dfx, req in [
    ...
    ("SIM25", sim25, ["Hora_decimal", "P_3f_S", "Q_3f_S"]),
]:
...
df25 = meta.merge(sim25, on="k", how="left").rename(columns={"P_3f_S": "P_SIM25", "Q_3f_S": "Q_SIM25"})
...
agg = agg2.merge(agg25, on=group_cols, how="outer")
...
# %% Celula 011 - SIMULACAO 025: Tabela de custos (SIM2 vs SIM25 - 2025) com demandas contratadas diferentes
# =============================================================================
# Entrada:
#   Simulacao 025 segmentos/01_Integralizacao_Mensal_SIM2_x_SIM25_2025.csv
#
# Saidas (todas em Simulacao 025 segmentos):
#   Tabela_demanda_consumo_por_posto_SIM2_2025.csv
#   Tabela_demanda_consumo_por_posto_SIM25_2025.csv
#   Tabela_demanda_consumo_por_posto_SIM2_vs_SIM25_2025.csv   (economia)
# =============================================================================
...
IN_FP = SIM25_DIR / "01_Integralizacao_Mensal_SIM2_x_SIM25_2025.csv"
OUT_SIM2 = SIM25_DIR / "Tabela_demanda_consumo_por_posto_SIM2_2025.csv"
OUT_SIM25 = SIM25_DIR / "Tabela_demanda_consumo_por_posto_SIM25_2025.csv"
OUT_CMP = SIM25_DIR / "Tabela_demanda_consumo_por_posto_SIM2_vs_SIM25_2025.csv"

CONTR_SIM2 = {"DCFP": 5000.0, "DCP": 3000.0}
CONTR_SIM25 = {"DCFP": 4500.0, "DCP":   0.0}  # 3000 - 4*000
...
tab25 = tabela_custos(df, "SIM25", DCFP=CONTR_SIM25["DCFP"], DCP=CONTR_SIM25["DCP"])
...
print("DONE. Saved (Simulacao 025 segmentos):")
print(" -", OUT_SIM2)
print(" -", OUT_SIM25)
print(" -", OUT_CMP)
print("\nDemandas contratadas usadas:")
print(" - SIM2: DCFP=5000, DCP=3000")
print(" - SIM25: DCFP=4500, DCP=0")
print("\nResumo anual:")
print(cmp.tail(1))
