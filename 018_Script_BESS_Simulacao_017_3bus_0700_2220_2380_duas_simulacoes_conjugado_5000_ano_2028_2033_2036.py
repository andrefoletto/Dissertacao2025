# -*- coding: utf-8 -*-
# =============================================================================
# 018_Script_BESS_Simulacao_017_3bus_0700_2220_2380_duas_simulacoes_conjugado_5000_ano_2028_2033_2036.py
#
# Regras deste script:
...
#     Simulacao 017 segmentos_pass1_sem_hvac_int_ano_2036
#   PASSO 2 (simulacao completa final):
#     Simulacao 017 segmentos_ano_2036
# =============================================================================
...
DIR_PASS1 = WORK_DIR / "Simulacao 017 segmentos_pass1_sem_hvac_int_ano_2036"
...
if LIMPAR_SAIDAS_ANTERIORES_PASS1:
    padroes_limpeza_pass1 = [
        ...
        "Curva_de_Carga_BESSM*_IN_sim_17.csv",
        "SOC_por_segmento.csv",
    ]
    removidos = 0
...
def build_circuit_pass1(
    ...
    dss.text('Redirect "UFSM_cargas_medidas_ano_2036.dss"')
    dss.text('Redirect "UFSM_cargas_especiais_ano_2036.dss"')
    dss.text('Redirect "UFSM_cargas_estimadas_com_residuo_ano_2036.dss"')
    ...
    dss.text(
        ...
        f"kWrated=747 kWhrated=4200 Effcurve=Eff %EffCharge=95 %EffDischarge=95 "
...
# %% Celula 003 - PASSO 1: monitores -> Curva_de_Carga_BESSM1_IN_sim_17.csv e BESSM2_IN (SHIFT DIREITA)
# =============================================================================
# Entradas:
#   PASSO1/Monitor_BESSM1_IN_anual.csv
#   PASSO1/Monitor_BESSM2_IN_anual.csv
# Saidas:
#   PASSO1/Curva_de_Carga_BESSM1_IN_sim_17.csv
#   PASSO1/Curva_de_Carga_BESSM2_IN_sim_17.csv
# Remove:
#   PASSO1/Monitor_BESSM1_IN_anual.csv
#   PASSO1/Monitor_BESSM2_IN_anual.csv
# =============================================================================
...
for mon_base in ["BESSM1_IN", "BESSM2_IN", "BESSM3_IN"]:
    ...
    out_fp = DIR_PASS1 / f"Curva_de_Carga_{mon_base}_sim_17.csv"
...
for mon_base, prefix in [("BESSM1_IN", "BESSM1"), ("BESSM2_IN", "BESSM2"), ("BESSM3_IN", "BESSM3")]:
    in_fp = DIR_PASS1 / f"Curva_de_Carga_{mon_base}_sim_17.csv"
...
#     SOC_15min_BESS_SIM17_2036.csv          (SOC dos tres bancos a cada 15 min)
#
# Requisitos:
...
BESS1_E_2036_KWH = 4200.0
BESS2_E_2036_KWH = 2925.0
BESS3_E_2036_KWH = 3000.0
BESS_TOTAL_E_2036_KWH = BESS1_E_2036_KWH + BESS2_E_2036_KWH + BESS3_E_2036_KWH
...
DIR_PASS2 = WORK_DIR / "Simulacao 017 segmentos_ano_2036"
...
if LIMPAR_SAIDAS_ANTERIORES_PASS2:
    padroes_limpeza = [
        "UFSM_Mon_*_s*.csv",
        "Monitor_*_anual.csv",
        "Curva_de_Carga_*_sim_17.csv",
        "EXP_MTR_GERAL*.csv",
        "Resumo_EnergyMeter_Geral_Sim17.csv",
        "SOC_por_segmento.csv",
        "SOC_15min_BESS_SIM17_2036.csv",
        "01_Integralizacao_Mensal_SIM17_2036.csv",
        "Tabela_demanda_consumo_por_posto_SIM17_2036.csv",
    ]
    removidos = 0
...
def _run_segment_pass2(
    ...
    for step_idx in range(int(n_steps)):
        ...
        soc_m1_kwh = soc_m1_pct / 100.0 * BESS1_E_2036_KWH
        soc_m2_kwh = soc_m2_pct / 100.0 * BESS2_E_2036_KWH
        soc_m3_kwh = soc_m3_pct / 100.0 * BESS3_E_2036_KWH
        soc_total_kwh = soc_m1_kwh + soc_m2_kwh + soc_m3_kwh
        soc_total_pct_pond = 100.0 * soc_total_kwh / BESS_TOTAL_E_2036_KWH
...
soc_15min_fp = DIR_PASS2 / "SOC_15min_BESS_SIM17_2036.csv"
...
# =============================================================================
# Entrada:
#   Simulacao 017 segmentos_ano_2036/UFSM_Mon_<KEY>_s*.csv
# Saida:
#   Simulacao 017 segmentos_ano_2036/Monitor_<KEY>_anual.csv
# Remove:
...
# =============================================================================
# Entrada:
#   Simulacao 017 segmentos_ano_2036/EXP_MTR_GERAL_*.csv
# Saida:
#   Simulacao 017 segmentos_ano_2036/EXP_MTR_GERAL_anual.csv
# Remove:
#   EXP_MTR_GERAL_*.csv
# =============================================================================
...
#   Simulacao 017 segmentos_ano_2036/Monitor_<KEY>_anual.csv
# Saidas:
#   Simulacao 017 segmentos_ano_2036/Curva_de_Carga_<KEY>_sim_17.csv
# Remove:
...
for in_fp in files:
    ...
    out_fp = DIR_PASS2 / f"Curva_de_Carga_{mon_key}_sim_17.csv"
...
#   Simulacao 017 segmentos_ano_2036/EXP_MTR_GERAL_anual.csv
# Saida:
#   Simulacao 017 segmentos_ano_2036/Resumo_EnergyMeter_Geral_Sim17.csv
# =============================================================================
...
# %% Celula 010 - ENERGIA e DEMANDA MAX por mes e posto (SIM17 - ANO 2036, BESS modular autofinanciado)
# =============================================================================
# Entradas:
#   META: 00_dados_de_entrada_ano_letivo_2025_15min.csv
#         Usado apenas como mapa anual de Hora_decimal, Mes e Posto.
#   SIM17: curva GERAL com BESS modular autofinanciado em operacao conjugada
#          para o ano 2036.
# Saida:
#   Simulacao 017 segmentos_ano_2036/01_Integralizacao_Mensal_SIM17_2036.csv
#
# Observacao:
#   Esta celula nao compara com SIM2/RGE nem com outra simulacao.
#   Ela apenas integraliza a simulacao conjugada do ano 2036 por mes e posto.
#   Configuracao do despacho:
#     BESS1: instalado em 2028, 1 MW / 4,5 MWh, operando a 80% da potencia,
#            degradado em 2036 por fator 0,933333333 para 747 kW / 4200 kWh.
#     BESS2: instalado em 2033, 1 MW / 3,0 MWh, operando a 80% da potencia,
#            degradado em 2036 por fator 0,975 para 780 kW / 2925 kWh.
#     BESS3: instalado em 2036, 1 MW / 3,0 MWh, operando a 80% da potencia,
#            sem degradacao: 800 kW / 3000 kWh.
#     Alvo fora-ponta = 5275 kW; permissao de recarga fora-ponta = 4220 kW.
#     Em fora-ponta, BESS1 usa reserva de 66,6667% e BESS2/BESS3 usam reserva de 100%.
#     Em ponta, os tres bancos usam reserva minima tecnica de 2%.
#   A curva e lida diretamente pelo sufixo sim_17, sem fallback.
# =============================================================================
...
SIM17_DIR = WORK_DIR / f"Simulacao 017 segmentos_ano_{ANO_ANALISE}"

META_FP = WORK_DIR / "00_dados_de_entrada_ano_letivo_2025_15min.csv"
SIM17_FP = SIM17_DIR / "Curva_de_Carga_GERAL_sim_17.csv"
OUT_FP = SIM17_DIR / f"01_Integralizacao_Mensal_{SIM_TAG}_{ANO_ANALISE}.csv"
...
def gerar_curva_geral_sim17_se_necessario(sim17_fp: Path, dir_pass2: Path) -> None:
    """
    Garante a existencia de Curva_de_Carga_GERAL_sim_17.csv.
    ...
    def _validar_e_gravar_curva(mon: pd.DataFrame, origem: str) -> None:
        ...
        df_out["P_3f_S"] = df_out["P1 (kW)"] + df_out["P2 (kW)"] + df_out["P3 (kW)"]
        df_out["Q_3f_S"] = df_out["Q1 (kvar)"] + df_out["Q2 (kvar)"] + df_out["Q3 (kvar)"]
        df_out[["Hora_decimal", "P_3f_S", "Q_3f_S"]].to_csv(sim17_fp, index=False, encoding="utf-8-sig")
        print("[AUTO] Curva GERAL gerada em:", sim17_fp)

    if sim17_fp.exists():
        curva = pd.read_csv(sim17_fp, encoding="utf-8-sig", sep=None, engine="python")
        if len(curva) == TOTAL_NPTS:
            return
        print(f"[AUTO] Curva GERAL existente tem {len(curva)} linhas; removendo arquivo invalido: {sim17_fp.name}")
        sim17_fp.unlink()
    ...
    if not seg_files:
        raise FileNotFoundError(
            f"Nao encontrei {sim17_fp}. Tambem nao encontrei Monitor_GERAL_anual.csv nem UFSM_Mon_GERAL_s*.csv em {dir_pass2}. "
...
gerar_curva_geral_sim17_se_necessario(SIM17_FP, SIM17_DIR)
if not SIM17_FP.exists():
    raise FileNotFoundError(SIM17_FP)

meta = pd.read_csv(META_FP, dtype=str, encoding="utf-8-sig")
sim17 = pd.read_csv(SIM17_FP, encoding="utf-8-sig", sep=None, engine="python")

meta = clean_cols(meta)
sim17 = clean_cols(sim17)

for name, dfx, req in [
    ("META", meta, ["Hora_decimal", "Mes", "Posto"]),
    (SIM_TAG, sim17, ["Hora_decimal", "P_3f_S", "Q_3f_S"]),
]:
...
sim17["Hora_decimal"] = pd.to_numeric(sim17["Hora_decimal"], errors="coerce")
sim17["P_3f_S"] = pd.to_numeric(sim17["P_3f_S"], errors="coerce")
sim17["Q_3f_S"] = pd.to_numeric(sim17["Q_3f_S"], errors="coerce")
sim17.dropna(subset=["Hora_decimal", "P_3f_S", "Q_3f_S"], inplace=True)

meta["k"] = make_k(meta["Hora_decimal"])
sim17["k"] = make_k(sim17["Hora_decimal"])

meta = meta[(meta["k"] >= 0) & (meta["k"] < TOTAL_NPTS)].copy()
sim17 = sim17[(sim17["k"] >= 0) & (sim17["k"] < TOTAL_NPTS)].copy()

meta = meta[["k", "Mes", "Posto"]].drop_duplicates(subset=["k"], keep="first").copy()
sim17 = sim17[["k", "P_3f_S", "Q_3f_S"]].drop_duplicates(subset=["k"], keep="first").copy()

if len(meta) != TOTAL_NPTS:
    raise ValueError(f"META gerou {len(meta)} amostras anuais de 15 min; esperado {TOTAL_NPTS}.")
if len(sim17) != TOTAL_NPTS:
    raise ValueError(f"SIM17 gerou {len(sim17)} amostras anuais de 15 min; esperado {TOTAL_NPTS}.")

df16 = meta.merge(sim17, on="k", how="left").rename(
    columns={"P_3f_S": "P_SIM17", "Q_3f_S": "Q_SIM17"}
)

faltas_p = int(df16["P_SIM17"].isna().sum())
faltas_q = int(df16["Q_SIM17"].isna().sum())
if faltas_p or faltas_q:
    raise ValueError(
        f"Falha de alinhamento por Hora_decimal/k: "
        f"P_SIM17 faltas={faltas_p}, Q_SIM17 faltas={faltas_q}"
    )

df16["E_P_SIM17_kWh"] = df16["P_SIM17"] * DT_H
df16["E_Q_SIM17_kVArh"] = df16["Q_SIM17"] * DT_H

group_cols = ["Mes", "Posto"]

agg16 = df16.groupby(group_cols, dropna=False).agg(
    N_pontos_SIM17=("P_SIM17", lambda s: int(s.notna().sum())),
    Consumo_P_SIM17_kWh=("E_P_SIM17_kWh", "sum"),
    Consumo_Q_SIM17_kVArh=("E_Q_SIM17_kVArh", "sum"),
    DemandaMax_P_SIM17_kW=("P_SIM17", "max"),
    DemandaMax_Q_SIM17_kVAr=("Q_SIM17", "max"),
).reset_index()
...
print("SIM17 usado:", SIM17_FP)
print(agg[["Mes", "Posto", "N_pontos_META", "N_pontos_SIM17", "DemandaMax_P_SIM17_kW"]])

# %% Celula 011 - SIMULACAO 017: tabela de custos (SIM17 - ANO 2036, BESS modular autofinanciado)
# =============================================================================
# Entrada:
#   Simulacao 017 segmentos_ano_2036/01_Integralizacao_Mensal_SIM17_2036.csv
#
# Saida:
#   Simulacao 017 segmentos_ano_2036/Tabela_demanda_consumo_por_posto_SIM17_2036.csv
#
# Demandas contratadas do ano 2036:
#   DCFP contratada com BESS = 5000 * 1,055 = 5275 kW
#   Alvo operacional fora-ponta do despacho = 5275 kW
#   Permissao para recarga fora-ponta = 0,80 * 5275 = 4220 kW
#   DCP sem BESS = 3000 * 1,055 = 3165 kW
#   DCP com BESS = 3165 - (747 + 780 + 800) = 838 kW
#
...
#     Em 2036: fator 0,933333333 -> 747 kW / 4200 kWh.
...
SIM17_DIR = WORK_DIR / f"Simulacao 017 segmentos_ano_{ANO_ANALISE}"
SIM17_DIR.mkdir(parents=True, exist_ok=True)

IN_FP = SIM17_DIR / f"01_Integralizacao_Mensal_{SIM_TAG}_{ANO_ANALISE}.csv"
OUT_SIM17 = SIM17_DIR / f"Tabela_demanda_consumo_por_posto_{SIM_TAG}_{ANO_ANALISE}.csv"

# -------------------------
# Demandas contratadas e alvos do despacho
# -------------------------
# Fora-ponta: 5000 * 1,055 = 5275 kW
# Ponta sem BESS: 3000 * 1,055 = 3165 kW
# Ponta com BESS: 3165 - (747 + 780 + 800) = 838 kW
DCP_SEM_BESS = 3165.0
DCFP_COM_BESS = 5275.0

KW_TARGET_FP = 5275.0
KW_TARGET_LOW_FP = 4220.0
...
BESS1_FATOR_DEGRADACAO_2036 = 0.933333333
BESS1_P_2036_KW = round(BESS1_P_OPERACIONAL_2028_KW * BESS1_FATOR_DEGRADACAO_2036)
BESS1_E_2036_KWH = round(BESS1_E_NOMINAL_KWH * BESS1_FATOR_DEGRADACAO_2036)
BESS1_E_EXCEDENTE_FP_2036_KWH = BESS1_E_EXCEDENTE_FP_NOMINAL_KWH * BESS1_FATOR_DEGRADACAO_2036

BESS2_ANO_INSTALACAO = 2033
BESS2_P_NOMINAL_KW = 1000.0
BESS2_P_OPERACIONAL_2033_KW = 800.0
BESS2_E_NOMINAL_KWH = 3000.0
BESS2_FATOR_DEGRADACAO_2036 = 0.975
BESS2_P_2036_KW = round(BESS2_P_OPERACIONAL_2033_KW * BESS2_FATOR_DEGRADACAO_2036)
BESS2_E_2036_KWH = round(BESS2_E_NOMINAL_KWH * BESS2_FATOR_DEGRADACAO_2036)
...
BESS_P_TOTAL_KW = BESS1_P_2036_KW + BESS2_P_2036_KW + BESS3_P_OPERACIONAL_2036_KW
BESS_E_TOTAL_KWH = BESS1_E_2036_KWH + BESS2_E_2036_KWH + BESS3_E_2036_KWH
...
#   BESS2 e BESS3 reservam 100% para a ponta e nao participam do peak-shave fora-ponta.
# PONTA:
...
BESS1_E_RESERVA_PONTA_2036_KWH = BESS1_E_2036_KWH * BESS1_RESERVA_PONTA_PCT / 100.0
BESS2_E_RESERVA_PONTA_2036_KWH = BESS2_E_2036_KWH * BESS2_RESERVA_PONTA_PCT / 100.0
BESS3_E_RESERVA_PONTA_2036_KWH = BESS3_E_2036_KWH * BESS3_RESERVA_PONTA_PCT / 100.0
BESS_E_RESERVA_PONTA_KWH = BESS1_E_RESERVA_PONTA_2036_KWH + BESS2_E_RESERVA_PONTA_2036_KWH + BESS3_E_RESERVA_PONTA_2036_KWH
BESS_E_DISPONIVEL_FP_KWH = BESS1_E_2036_KWH - BESS1_E_RESERVA_PONTA_2036_KWH

DCP_COM_BESS = max(0.0, DCP_SEM_BESS - BESS_P_TOTAL_KW)

CONTR_SIM17 = {"DCFP": DCFP_COM_BESS, "DCP": DCP_COM_BESS}
...
if not IN_FP.exists():
    curva_fp = SIM17_DIR / "Curva_de_Carga_GERAL_sim_17.csv"
    raise FileNotFoundError(
...
tab17 = tabela_custos(
    df,
    SIM_TAG,
    DCFP=CONTR_SIM17["DCFP"],
    DCP=CONTR_SIM17["DCP"],
)

tab17.to_csv(OUT_SIM17, index=False, encoding="utf-8")

print("DONE. Saved (Simulacao 017 - BESS modular autofinanciado - 2036 - conjugado):")
print(" -", OUT_SIM17)
print("\nDemandas contratadas e alvos usados em 2036:")
...
print(f" - Potencia BESS1 2036 = {BESS1_P_2036_KW:.0f} kW")
print(f" - Potencia BESS2 2036 = {BESS2_P_2036_KW:.0f} kW")
print(f" - Potencia BESS3 2036 = {BESS3_P_OPERACIONAL_2036_KW:.0f} kW")
print(f" - Energia nominal total instalada = {BESS_E_NOMINAL_TOTAL_KWH:.0f} kWh")
print(f" - Energia disponivel em 2036 = {BESS_E_TOTAL_KWH:.0f} kWh")
print(f" - Energia BESS1 2036 = {BESS1_E_2036_KWH:.0f} kWh")
print(f" - Energia BESS2 2036 = {BESS2_E_2036_KWH:.0f} kWh")
print(f" - Energia BESS3 2036 = {BESS3_E_2036_KWH:.0f} kWh")
...
print(f" - Excedente degradado BESS1 para fora-ponta = {BESS1_E_EXCEDENTE_FP_2036_KWH:.0f} kWh")
print(f" - Energia reservada para ponta BESS1 = {BESS1_E_RESERVA_PONTA_2036_KWH:.0f} kWh")
print(f" - Energia reservada para ponta BESS2 = {BESS2_E_RESERVA_PONTA_2036_KWH:.0f} kWh")
print(f" - Energia reservada para ponta BESS3 = {BESS3_E_RESERVA_PONTA_2036_KWH:.0f} kWh")
...
print("\nResumo anual:")
print(tab17[tab17["MES"].isin(["TOTAIS", "CTOTAL", "R$TOTAL", "DCFP", "DCP"])])
