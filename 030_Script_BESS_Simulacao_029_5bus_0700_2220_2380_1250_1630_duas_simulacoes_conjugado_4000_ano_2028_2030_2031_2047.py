# -*- coding: utf-8 -*-
# =============================================================================
# 030_Script_BESS_Simulacao_029_5bus_0700_2220_2380_1250_1630_duas_simulacoes_conjugado_4000_ano_2028_2030_2031_2047.py
#
# Regras deste script:
...
#     Simulacao 029 segmentos_pass1_sem_hvac_int_ano_2031_2047
#   PASSO 2 (simulacao completa final):
#     Simulacao 029 segmentos_ano_2031_2047
# =============================================================================
...
DIR_PASS1 = WORK_DIR / "Simulacao 029 segmentos_pass1_sem_hvac_int_ano_2031_2047"
...
def build_circuit_pass1(
    ...
    dss.text(
        ...
        f"kWrated=673.333 kWhrated=5891.667 Effcurve=Eff %EffCharge=95 %EffDischarge=95 "
    ...
    dss.text(
        ...
        f"kWrated=686.667 kWhrated=2575 Effcurve=Eff %EffCharge=95 %EffDischarge=95 "
        f"%reserve={reserve_m4_pct} dispmode=external"
    )
    dss.text(
        "New Storage.BESSM5 phases=3 bus1=B053 kv=0.38 "
        f"kWrated=693.333 kWhrated=2600 Effcurve=Eff %EffCharge=95 %EffDischarge=95 "
...
#     SOC_15min_BESS_SIM29_2047.csv          (SOC dos cinco bancos a cada 15 min)
#
# Requisitos:
...
BESS1_E_2047_KWH = 5891.667
BESS2_E_2047_KWH = 5891.667
BESS3_E_2047_KWH = 5891.667
BESS4_E_2047_KWH = 2575.0
BESS5_E_2047_KWH = 2600.0
BESS_TOTAL_E_2047_KWH = BESS1_E_2047_KWH + BESS2_E_2047_KWH + BESS3_E_2047_KWH + BESS4_E_2047_KWH + BESS5_E_2047_KWH
...
DIR_PASS2 = WORK_DIR / "Simulacao 029 segmentos_ano_2031_2047"
...
if LIMPAR_SAIDAS_ANTERIORES_PASS2:
    padroes_limpeza = [
        ...
        "SOC_por_segmento.csv",
        "SOC_15min_BESS_SIM29_2047.csv",
        "01_Integralizacao_Mensal_SIM29_2047.csv",
        "Tabela_demanda_consumo_por_posto_SIM29_2047.csv",
    ]
    removidos = 0
...
def _run_segment_pass2(
    ...
    for step_idx in range(int(n_steps)):
        ...
        soc_m4_kwh = soc_m4_pct / 100.0 * BESS4_E_2047_KWH
        soc_m5_kwh = soc_m5_pct / 100.0 * BESS5_E_2047_KWH
...
soc_15min_fp = DIR_PASS2 / "SOC_15min_BESS_SIM29_2047.csv"
...
# =============================================================================
# Entrada:
#   Simulacao 029 segmentos_ano_2031_2047/UFSM_Mon_<KEY>_s*.csv
# Saida:
#   Simulacao 029 segmentos_ano_2031_2047/Monitor_<KEY>_anual.csv
# Remove:
...
# =============================================================================
# Entrada:
#   Simulacao 029 segmentos_ano_2031_2047/EXP_MTR_GERAL_*.csv
# Saida:
#   Simulacao 029 segmentos_ano_2031_2047/EXP_MTR_GERAL_anual.csv
# Remove:
#   EXP_MTR_GERAL_*.csv
# =============================================================================
...
#   Simulacao 029 segmentos_ano_2031_2047/Monitor_<KEY>_anual.csv
# Saidas:
#   Simulacao 029 segmentos_ano_2031_2047/Curva_de_Carga_<KEY>_sim_29.csv
# Remove:
...
#   Simulacao 029 segmentos_ano_2031_2047/EXP_MTR_GERAL_anual.csv
# Saida:
#   Simulacao 029 segmentos_ano_2031_2047/Resumo_EnergyMeter_Geral_Sim29.csv
# =============================================================================
...
#          para o ano 2031.
# Saida:
#   Simulacao 029 segmentos_ano_2031_2047/01_Integralizacao_Mensal_SIM29_2047.csv
#
# Observacao:
...
#            degradado em 2047 por fator 0,841666667 para 673,333 kW / 5891,7 kWh.
...
#            degradado em 2047 por fator 0,858333333 para 686,667 kW / 2575 kWh.
#     BESS5: instalado em 2031, 1 MW / 3,0 MWh, operando a 80% da potencia,
#            em 2047: fator 0,866666667 -> 693,333 kW / 2600 kWh.
#     Alvo fora-ponta = 4440 kW; permissao de recarga fora-ponta = 3885 kW.
...
# Pasta fixa do cenario 2031_2047.
# Nao usar f"Simulacao 029 segmentos_ano_{ANO_ANALISE}", pois ANO_ANALISE=2047
# criaria indevidamente a pasta "Simulacao 029 segmentos_ano_2047".
SIM29_DIR = WORK_DIR / "Simulacao 029 segmentos_ano_2031_2047"
...
#
# Saida:
#   Simulacao 029 segmentos_ano_2031_2047/Tabela_demanda_consumo_por_posto_SIM29_2047.csv
#
# Demandas contratadas do ano 2047:
#   DCFP contratada com BESS = 4000 * 1,11 = 4440 kW
#   Alvo operacional fora-ponta do despacho = 4440 kW
#   Permissao para recarga fora-ponta = 3500 * 1,11 = 3885 kW
#   DCP sem BESS = 3000 * 1,11 = 3330 kW
#   DCP com BESS = max(0, 3330 - (673,333 + 673,333 + 673,333 + 686,667 + 693,333)) = 0 kW
#
...
#     Em 2047: fator 0,841666667 -> 673,333 kW / 5891,7 kWh.
...
#     Em 2047: fator 0,858333333 -> 686,667 kW / 2575 kWh.
#   BESS5 instalado em 2031:
#     1 MW / 3,0 MWh nominal, operando a 80% da potencia nominal. Em 2047: fator 0,866666667 -> 693,333 kW / 2600 kWh.
...
# -------------------------
# Fora-ponta: 4000 * 1,11 = 4440 kW
# Ponta sem BESS: 3000 * 1,11 = 3330 kW
# Ponta com BESS: max(0, 3330 - (673,333 + 673,333 + 673,333 + 686,667 + 693,333)) = 0 kW
DCP_SEM_BESS = 3330.0
...
BESS1_FATOR_DEGRADACAO_2047 = 0.8416666666666667
BESS1_P_2047_KW = BESS1_P_OPERACIONAL_2028_KW * BESS1_FATOR_DEGRADACAO_2047
...
BESS2_FATOR_DEGRADACAO_2047 = 0.8416666666666667
BESS2_P_2047_KW = BESS2_P_OPERACIONAL_2028_KW * BESS2_FATOR_DEGRADACAO_2047
BESS2_E_2047_KWH = BESS2_E_NOMINAL_KWH * BESS2_FATOR_DEGRADACAO_2047
BESS2_E_EXCEDENTE_FP_2047_KWH = BESS2_E_EXCEDENTE_FP_NOMINAL_KWH * BESS2_FATOR_DEGRADACAO_2047
...
BESS3_FATOR_DEGRADACAO_2047 = 0.8416666666666667
BESS3_P_2047_KW = BESS3_P_OPERACIONAL_2028_KW * BESS3_FATOR_DEGRADACAO_2047
BESS3_E_2047_KWH = BESS3_E_NOMINAL_KWH * BESS3_FATOR_DEGRADACAO_2047
BESS3_E_EXCEDENTE_FP_2047_KWH = BESS3_E_EXCEDENTE_FP_NOMINAL_KWH * BESS3_FATOR_DEGRADACAO_2047

BESS4_ANO_INSTALACAO = 2030
BESS4_P_NOMINAL_KW = 1000.0
BESS4_P_OPERACIONAL_2030_KW = 800.0
BESS4_E_NOMINAL_KWH = 3000.0
BESS4_FATOR_DEGRADACAO_2047 = 0.8583333333333333
BESS4_P_2047_KW = BESS4_P_OPERACIONAL_2030_KW * BESS4_FATOR_DEGRADACAO_2047
BESS4_E_2047_KWH = BESS4_E_NOMINAL_KWH * BESS4_FATOR_DEGRADACAO_2047

BESS5_ANO_INSTALACAO = 2031
BESS5_P_NOMINAL_KW = 1000.0
BESS5_P_OPERACIONAL_2031_KW = 800.0
BESS5_E_NOMINAL_KWH = 3000.0
BESS5_FATOR_DEGRADACAO_2047 = 0.8666666666666667
BESS5_P_2047_KW = BESS5_P_OPERACIONAL_2031_KW * BESS5_FATOR_DEGRADACAO_2047
BESS5_E_2047_KWH = BESS5_E_NOMINAL_KWH * BESS5_FATOR_DEGRADACAO_2047

BESS_P_TOTAL_KW = BESS1_P_2047_KW + BESS2_P_2047_KW + BESS3_P_2047_KW + BESS4_P_2047_KW + BESS5_P_2047_KW
BESS_E_TOTAL_KWH = BESS1_E_2047_KWH + BESS2_E_2047_KWH + BESS3_E_2047_KWH + BESS4_E_2047_KWH + BESS5_E_2047_KWH
...
BESS4_E_RESERVA_PONTA_2047_KWH = BESS4_E_2047_KWH * BESS4_RESERVA_PONTA_PCT / 100.0
BESS5_E_RESERVA_PONTA_2047_KWH = BESS5_E_2047_KWH * BESS5_RESERVA_PONTA_PCT / 100.0
BESS_E_RESERVA_PONTA_KWH = (
    BESS1_E_RESERVA_PONTA_2047_KWH
    + BESS2_E_RESERVA_PONTA_2047_KWH
    + BESS3_E_RESERVA_PONTA_2047_KWH
    + BESS4_E_RESERVA_PONTA_2047_KWH
    + BESS5_E_RESERVA_PONTA_2047_KWH
)
BESS_E_DISPONIVEL_FP_KWH = (
    (BESS1_E_2047_KWH - BESS1_E_RESERVA_PONTA_2047_KWH)
    + (BESS2_E_2047_KWH - BESS2_E_RESERVA_PONTA_2047_KWH)
    + (BESS3_E_2047_KWH - BESS3_E_RESERVA_PONTA_2047_KWH)
)
...
print(f" - Potencia BESS4 2047 = {BESS4_P_2047_KW:.0f} kW")
print(f" - Potencia BESS5 2047 = {BESS5_P_2047_KW:.0f} kW")
...
print(f" - Energia BESS4 2047 = {BESS4_E_2047_KWH:.0f} kWh")
print(f" - Energia BESS5 2047 = {BESS5_E_2047_KWH:.0f} kWh")
...
print(f" - Excedente degradado BESS1 para fora-ponta = {BESS1_E_EXCEDENTE_FP_2047_KWH:.0f} kWh")
print(f" - Excedente degradado BESS2 para fora-ponta = {BESS2_E_EXCEDENTE_FP_2047_KWH:.0f} kWh")
print(f" - Excedente degradado BESS3 para fora-ponta = {BESS3_E_EXCEDENTE_FP_2047_KWH:.0f} kWh")
...
print(f" - Energia reservada para ponta BESS4 = {BESS4_E_RESERVA_PONTA_2047_KWH:.0f} kWh")
print(f" - Energia reservada para ponta BESS5 = {BESS5_E_RESERVA_PONTA_2047_KWH:.0f} kWh")
