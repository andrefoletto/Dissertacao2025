# -*- coding: utf-8 -*-
# =============================================================================
# 020_Script_BESS_Simulacao_019_5bus_0700_2220_2380_1250_1630_duas_simulacoes_conjugado_5000_ano_2028_2033_2036_2038_2039_2047.py
#
# Regras deste script:
...
#     Simulacao 019 segmentos_pass1_sem_hvac_int_ano_2039_2047
#   PASSO 2 (simulacao completa final):
#     Simulacao 019 segmentos_ano_2039_2047
# =============================================================================
...
TARGETS = {
    ...
    "FORA_PONTA": {"kWtarget": 5550, "kWtargetLow": 4440},
}
...
DIR_PASS1 = WORK_DIR / "Simulacao 019 segmentos_pass1_sem_hvac_int_ano_2039_2047"
...
#     SOC_15min_BESS_SIM19_2047.csv          (SOC dos cinco bancos a cada 15 min)
#
# Requisitos:
...
BESS1_E_2047_KWH = 3788.0
BESS2_E_2047_KWH = 2650.0
BESS3_E_2047_KWH = 2725.0
BESS4_E_2047_KWH = 2775.0
BESS5_E_2047_KWH = 2800.0
...
DIR_PASS2 = WORK_DIR / "Simulacao 019 segmentos_ano_2039_2047"
...
if LIMPAR_SAIDAS_ANTERIORES_PASS2:
    padroes_limpeza = [
        ...
        "SOC_por_segmento.csv",
        "SOC_15min_BESS_SIM19_2047.csv",
        "01_Integralizacao_Mensal_SIM19_2047.csv",
        "Tabela_demanda_consumo_por_posto_SIM19_2047.csv",
    ]
    removidos = 0
...
soc_15min_fp = DIR_PASS2 / "SOC_15min_BESS_SIM19_2047.csv"
...
# =============================================================================
# Entrada:
#   Simulacao 019 segmentos_ano_2039_2047/UFSM_Mon_<KEY>_s*.csv
# Saida:
#   Simulacao 019 segmentos_ano_2039_2047/Monitor_<KEY>_anual.csv
# Remove:
...
# =============================================================================
# Entrada:
#   Simulacao 019 segmentos_ano_2039_2047/EXP_MTR_GERAL_*.csv
# Saida:
#   Simulacao 019 segmentos_ano_2039_2047/EXP_MTR_GERAL_anual.csv
# Remove:
#   EXP_MTR_GERAL_*.csv
# =============================================================================
...
#   Simulacao 019 segmentos_ano_2039_2047/Monitor_<KEY>_anual.csv
# Saidas:
#   Simulacao 019 segmentos_ano_2039_2047/Curva_de_Carga_<KEY>_sim_19.csv
# Remove:
...
#   Simulacao 019 segmentos_ano_2039_2047/EXP_MTR_GERAL_anual.csv
# Saida:
#   Simulacao 019 segmentos_ano_2039_2047/Resumo_EnergyMeter_Geral_Sim19.csv
# =============================================================================
...
# %% Celula 010 - ENERGIA e DEMANDA MAX por mes e posto (SIM19 - ANO 2047, BESS modular autofinanciado)
# =============================================================================
# Entradas:
...
#          para o ano 2047.
# Saida:
#   Simulacao 019 segmentos_ano_2039_2047/01_Integralizacao_Mensal_SIM19_2047.csv
#
# Observacao:
...
#            degradado em 2047 por fator 0,841666667 para 673 kW / 3788 kWh.
#     BESS2: instalado em 2033, 1 MW / 3,0 MWh, operando a 80% da potencia,
#            degradado em 2047 por fator 0,883333333 para 707 kW / 2650 kWh.
#     BESS3: instalado em 2036, 1 MW / 3,0 MWh, operando a 80% da potencia,
#            degradado em 2047 por fator 0,908333333 para 727 kW / 2725 kWh.
#     BESS4: instalado em 2038, 1 MW / 3,0 MWh, operando a 80% da potencia,
#            degradado em 2047 por fator 0,925000000 para 740 kW / 2775 kWh.
#     BESS5: instalado em 2039, 1 MW / 3,0 MWh, operando a 80% da potencia,
#            degradado em 2047 por fator 0,933333333 para 747 kW / 2800 kWh.
#     Alvo fora-ponta = 5550 kW; permissao de recarga fora-ponta = 4440 kW.
...
SIM19_DIR = WORK_DIR / "Simulacao 019 segmentos_ano_2039_2047"
...
# %% Celula 011 - SIMULACAO 019: tabela de custos (SIM19 - ANO 2047, BESS modular autofinanciado)
# =============================================================================
# Entrada:
#   Simulacao 019 segmentos_ano_2039_2047/01_Integralizacao_Mensal_SIM19_2047.csv
#
# Saida:
#   Simulacao 019 segmentos_ano_2039_2047/Tabela_demanda_consumo_por_posto_SIM19_2047.csv
#
# Demandas contratadas do ano 2047:
#   DCFP contratada com BESS = 5000 * 1,110 = 5550 kW
#   Alvo operacional fora-ponta do despacho = 5550 kW
#   Permissao para recarga fora-ponta = 0,80 * 5550 = 4440 kW
#   DCP sem BESS = 3000 * 1,110 = 3330 kW
#   DCP com BESS = max(0, 3330 - (673 + 707 + 727 + 740 + 747)) = 0 kW
#
...
#     Em 2047: fator 0,841666667 -> 673 kW / 3788 kWh.
...
#     Em 2047: fator 0,883333333 -> 707 kW / 2650 kWh.
#   BESS3 instalado em 2036:
#     1 MW / 3,0 MWh nominal, operando a 80% da potencia nominal em 2036.
#     Em 2047: fator 0,908333333 -> 727 kW / 2725 kWh.
#   BESS4 instalado em 2038:
#     1 MW / 3,0 MWh nominal, operando a 80% da potencia nominal em 2038.
#     Em 2047: fator 0,925000000 -> 740 kW / 2775 kWh.
#   BESS5 instalado em 2039:
#     1 MW / 3,0 MWh nominal, operando a 80% da potencia nominal em 2039.
#     Em 2047: fator 0,933333333 -> 747 kW / 2800 kWh.
...
# -------------------------
# Fora-ponta: 5000 * 1,110 = 5550 kW
# Ponta sem BESS: 3000 * 1,110 = 3330 kW
# Ponta com BESS: max(0, 3330 - (673 + 707 + 727 + 740 + 747)) = 0 kW
DCP_SEM_BESS = 3330.0
DCFP_COM_BESS = 5550.0

KW_TARGET_FP = 5550.0
KW_TARGET_LOW_FP = 4440.0
...
BESS1_E_2047_KWH = round(BESS1_E_NOMINAL_KWH * BESS1_FATOR_DEGRADACAO_2047)
...
BESS2_FATOR_DEGRADACAO_2047 = 0.8833333333333333
BESS2_P_2047_KW = round(BESS2_P_OPERACIONAL_2033_KW * BESS2_FATOR_DEGRADACAO_2047)
BESS2_E_2047_KWH = round(BESS2_E_NOMINAL_KWH * BESS2_FATOR_DEGRADACAO_2047)

BESS3_ANO_INSTALACAO = 2036
BESS3_P_NOMINAL_KW = 1000.0
BESS3_P_OPERACIONAL_2036_KW = 800.0
BESS3_E_NOMINAL_KWH = 3000.0
BESS3_FATOR_DEGRADACAO_2047 = 0.9083333333333333
BESS3_P_2047_KW = round(BESS3_P_OPERACIONAL_2036_KW * BESS3_FATOR_DEGRADACAO_2047)
BESS3_E_2047_KWH = round(BESS3_E_NOMINAL_KWH * BESS3_FATOR_DEGRADACAO_2047)

BESS4_ANO_INSTALACAO = 2038
BESS4_P_NOMINAL_KW = 1000.0
BESS4_P_OPERACIONAL_2038_KW = 800.0
BESS4_E_NOMINAL_KWH = 3000.0
BESS4_FATOR_DEGRADACAO_2047 = 0.9250000000000000
BESS4_P_2047_KW = round(BESS4_P_OPERACIONAL_2038_KW * BESS4_FATOR_DEGRADACAO_2047)
BESS4_E_2047_KWH = round(BESS4_E_NOMINAL_KWH * BESS4_FATOR_DEGRADACAO_2047)

BESS5_ANO_INSTALACAO = 2039
BESS5_P_NOMINAL_KW = 1000.0
BESS5_P_OPERACIONAL_2039_KW = 800.0
BESS5_E_NOMINAL_KWH = 3000.0
BESS5_FATOR_DEGRADACAO_2047 = 0.9333333333333333
BESS5_P_2047_KW = round(BESS5_P_OPERACIONAL_2039_KW * BESS5_FATOR_DEGRADACAO_2047)
BESS5_E_2047_KWH = round(BESS5_E_NOMINAL_KWH * BESS5_FATOR_DEGRADACAO_2047)
...
BESS_E_DISPONIVEL_FP_KWH = BESS1_E_2047_KWH - BESS1_E_RESERVA_PONTA_2047_KWH
...
print("DONE. Saved (Simulacao 019 - BESS modular autofinanciado - 2047 - conjugado):")
print(" -", OUT_SIM19)
