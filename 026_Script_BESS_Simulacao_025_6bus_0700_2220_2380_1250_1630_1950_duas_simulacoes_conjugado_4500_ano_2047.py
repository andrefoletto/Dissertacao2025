# -*- coding: utf-8 -*-
# =============================================================================
# 026_Script_BESS_Simulacao_025_6bus_0700_2220_2380_1250_1630_1950_duas_simulacoes_conjugado_4500_ano_2047.py
#
# Regras deste script:
...
#     Simulacao 025 segmentos_pass1_sem_hvac_int_ano_2047
#   PASSO 2 (simulacao completa final):
#     Simulacao 025 segmentos_ano_2047
# =============================================================================
...
DIR_PASS1 = WORK_DIR / "Simulacao 025 segmentos_pass1_sem_hvac_int_ano_2047"
...
def build_circuit_pass1(
    ...
    dss.text(
        ...
        f"kWrated=673 kWhrated=3226 Effcurve=Eff %EffCharge=95 %EffDischarge=95 "
...
DIR_PASS2 = WORK_DIR / "Simulacao 025 segmentos_ano_2047"
...
# =============================================================================
# Entrada:
#   Simulacao 025 segmentos_ano_2047/UFSM_Mon_<KEY>_s*.csv
# Saida:
#   Simulacao 025 segmentos_ano_2047/Monitor_<KEY>_anual.csv
# Remove:
...
# =============================================================================
# Entrada:
#   Simulacao 025 segmentos_ano_2047/EXP_MTR_GERAL_*.csv
# Saida:
#   Simulacao 025 segmentos_ano_2047/EXP_MTR_GERAL_anual.csv
# Remove:
#   EXP_MTR_GERAL_*.csv
# =============================================================================
...
#   Simulacao 025 segmentos_ano_2047/Monitor_<KEY>_anual.csv
# Saidas:
#   Simulacao 025 segmentos_ano_2047/Curva_de_Carga_<KEY>_sim_25.csv
# Remove:
...
#   Simulacao 025 segmentos_ano_2047/EXP_MTR_GERAL_anual.csv
# Saida:
#   Simulacao 025 segmentos_ano_2047/Resumo_EnergyMeter_Geral_Sim25.csv
# =============================================================================
...
# %% Celula 010 - ENERGIA e DEMANDA MAX por mes e posto (SIM25 - ANO 2047, 6 BESS degradados)
# =============================================================================
# Entradas:
#   META: 00_dados_de_entrada_ano_letivo_2025_15min.csv
#         Usado apenas como mapa anual de Hora_decimal, Mes e Posto.
#   SIM25: curva GERAL com 6 BESS em operacao conjugada para o ano 2047
# Saida:
#   Simulacao 025 segmentos_ano_2047/01_Integralizacao_Mensal_SIM25_2047.csv
#
# Observacao:
#   Esta celula nao compara com SIM2/RGE nem com outra simulacao.
#   Ela apenas integraliza a simulacao conjugada com 6 BESS do ano 2047 por mes e posto.
...
sim25 = pd.read_csv(SIM25_FP, encoding="utf-8-sig", sep=None, engine="python")

meta = clean_cols(meta)
sim25 = clean_cols(sim25)

for name, dfx, req in [
    ("META", meta, ["Hora_decimal", "Mes", "Posto"]),
    (SIM_TAG, sim25, ["Hora_decimal", "P_3f_S", "Q_3f_S"]),
]:
...
sim25["Hora_decimal"] = pd.to_numeric(sim25["Hora_decimal"], errors="coerce")
sim25["P_3f_S"] = pd.to_numeric(sim25["P_3f_S"], errors="coerce")
sim25["Q_3f_S"] = pd.to_numeric(sim25["Q_3f_S"], errors="coerce")
sim25.dropna(subset=["Hora_decimal", "P_3f_S", "Q_3f_S"], inplace=True)

meta["k"] = make_k(meta["Hora_decimal"])
sim25["k"] = make_k(sim25["Hora_decimal"])

meta = meta[["k", "Mes", "Posto"]].drop_duplicates(subset=["k"], keep="first").copy()
sim25 = sim25[["k", "P_3f_S", "Q_3f_S"]].drop_duplicates(subset=["k"], keep="first").copy()

df25 = meta.merge(sim25, on="k", how="left").rename(
    columns={"P_3f_S": "P_SIM25", "Q_3f_S": "Q_SIM25"}
)

faltas_p = int(df25["P_SIM25"].isna().sum())
faltas_q = int(df25["Q_SIM25"].isna().sum())
...
df25["E_P_SIM25_kWh"] = df25["P_SIM25"] * DT_H
df25["E_Q_SIM25_kVArh"] = df25["Q_SIM25"] * DT_H

group_cols = ["Mes", "Posto"]

agg25 = df25.groupby(group_cols, dropna=False).agg(
...
agg = agg25.merge(meta_counts, on=group_cols, how="left")
...
# %% Celula 011 - SIMULACAO 025: tabela de custos (SIM25 - ANO 2047, 6 BESS degradados)
# =============================================================================
# Entrada:
#   Simulacao 025 segmentos_ano_2047/01_Integralizacao_Mensal_SIM25_2047.csv
#
# Saida:
#   Simulacao 025 segmentos_ano_2047/Tabela_demanda_consumo_por_posto_SIM25_2047.csv
#
# Demandas contratadas do ano 2047:
#   DCFP com BESS = 4995 kW  (corte fora-ponta agressivo)
#   DCP sem BESS = 3330 kW
#   DCP com 6 BESS = max(0, 3330 - (6 * 673)) = 0 kW
#
# Banco de energia:
#   6 BESS de 1 MW / 3,833 MWh instalados em 2028.
...
#   Energia degradada adotada por banco = 3226 kWh.
...
# - Cada banco possui 1 MW / 3,833 MWh nominal.
...
# - Em 2047, usa-se energia degradada de 3226 kWh por banco.
...
BESS_E_NOMINAL_UNITARIA_KWH = 3833.0
BESS_E_UNITARIA_KWH = 3226.0
BESS_RESERVA_PONTA_PCT = 78.3

BESS_P_TOTAL_KW = NUM_BESS * BESS_P_UNITARIA_KW                 # 4038 kW
BESS_E_TOTAL_KWH = NUM_BESS * BESS_E_UNITARIA_KWH               # 19356 kWh
...
print("DONE. Saved (Simulacao 025 - 6 BESS - 2047 - conjugado 4500):")
print(" -", OUT_SIM25)
