# -*- coding: utf-8 -*-
# =============================================================================
# Script_BESS_Simulacao_004_bus_0700_duas_simulacoes_ano_2028.py
#
# Regras deste script:
...
#     Simulacao 004 segmentos_pass1_sem_hvac_int_ano_2028
#   PASSO 2 (simulacao completa final):
#     Simulacao 004 segmentos_ano_2028
# =============================================================================
...
TARGETS = {
    ...
    "FORA_PONTA": {"kWtarget": 9999, "kWtargetLow": 5075},
}

DIR_PASS1 = WORK_DIR / "Simulacao 004 segmentos_pass1_sem_hvac_int_ano_2028"
...
def build_circuit_pass1(dss: DSS, soc_init_pct: float, posto: str, seg_tag: str) -> str:
    ...
    dss.text(
        ...
        "kWrated=800 kWhrated=3000 Effcurve=Eff %EffCharge=95 %EffDischarge=95 "
    ...
    # Monitor unico: entrada MT do BESS
...
# %% Celula 003 - PASSO 1: monitor -> Curva_de_Carga_BESS_IN_sim_4.csv (SHIFT DIREITA)
# =============================================================================
# Entradas:
#   PASSO1/Monitor_BESS_IN_anual.csv
# Saida:
#   PASSO1/Curva_de_Carga_BESS_IN_sim_4.csv (Hora_decimal, P_3f_S, Q_3f_S)
# Remove:
#   PASSO1/Monitor_BESS_IN_anual.csv
# =============================================================================
...
OUT_FP = DIR_PASS1 / "Curva_de_Carga_BESS_IN_sim_4.csv"
...
# =============================================================================
# Entrada:
#   PASSO1/Curva_de_Carga_BESS_IN_sim_4.csv
# Saidas (WORK_DIR):
...
IN_FP = DIR_PASS1 / "Curva_de_Carga_BESS_IN_sim_4.csv"
...
DIR_PASS2 = WORK_DIR / "Simulacao 004 segmentos_ano_2028"
...
# =============================================================================
# Entrada:
#   Simulacao 004 segmentos_ano_2028/UFSM_Mon_<KEY>_s*.csv
# Saida:
#   Simulacao 004 segmentos_ano_2028/Monitor_<KEY>_anual.csv
# Remove:
...
# =============================================================================
# Entrada:
#   Simulacao 004 segmentos_ano_2028/EXP_MTR_GERAL_*.csv
# Saida:
#   Simulacao 004 segmentos_ano_2028/EXP_MTR_GERAL_anual.csv
# Remove:
#   EXP_MTR_GERAL_*.csv
# =============================================================================
...
#   Simulacao 004 segmentos_ano_2028/Monitor_<KEY>_anual.csv
# Saidas:
#   Simulacao 004 segmentos_ano_2028/Curva_de_Carga_<KEY>_sim_4.csv
# Remove:
...
for in_fp in files:
    ...
    out_fp = DIR_PASS2 / f"Curva_de_Carga_{mon_key}_sim_4.csv"
...
#   Simulacao 004 segmentos_ano_2028/EXP_MTR_GERAL_anual.csv
# Saida:
#   Simulacao 004 segmentos_ano_2028/Resumo_EnergyMeter_Geral_Sim4.csv
# =============================================================================
...
OUT_FP = DIR_PASS2 / "Resumo_EnergyMeter_Geral_Sim4.csv"
...
# %% Celula 010 - ENERGIA e DEMANDA MAX por mes e posto (SIM4 - ANO 2028)
# =============================================================================
# Entradas:
#   META: 00_dados_de_entrada_ano_letivo_2025_15min.csv
#         Usado apenas como mapa anual de Hora_decimal, Mes e Posto.
#   SIM4: curva GERAL com BESS para o ano 2028
# Saida:
#   Simulacao 004 segmentos_ano_2028/01_Integralizacao_Mensal_SIM4_2028.csv
# =============================================================================
...
SIM4_DIR = WORK_DIR / f"Simulacao 004 segmentos_ano_{ANO_ANALISE}"

META_FP = WORK_DIR / "00_dados_de_entrada_ano_letivo_2025_15min.csv"
SIM4_FP = SIM4_DIR / "Curva_de_Carga_GERAL_sim_4.csv"
OUT_FP  = SIM4_DIR / f"01_Integralizacao_Mensal_SIM4_{ANO_ANALISE}.csv"
...
for fp in [META_FP, SIM4_FP]:
    if not fp.exists():
        raise FileNotFoundError(fp)

meta = pd.read_csv(META_FP, dtype=str, encoding="utf-8-sig")
sim4 = pd.read_csv(SIM4_FP, encoding="utf-8-sig", sep=None, engine="python")

for name, dfx, req in [
    ("META", meta, ["Hora_decimal","Mes","Posto"]),
    ("SIM4", sim4, ["Hora_decimal","P_3f_S","Q_3f_S"]),
]:
...
sim4["Hora_decimal"] = pd.to_numeric(sim4["Hora_decimal"], errors="coerce")
sim4["P_3f_S"] = pd.to_numeric(sim4["P_3f_S"], errors="coerce")
sim4["Q_3f_S"] = pd.to_numeric(sim4["Q_3f_S"], errors="coerce")
sim4.dropna(subset=["Hora_decimal", "P_3f_S", "Q_3f_S"], inplace=True)

meta["k"] = make_k(meta["Hora_decimal"])
sim4["k"] = make_k(sim4["Hora_decimal"])

meta = meta[["k","Mes","Posto"]].drop_duplicates(subset=["k"], keep="first").copy()
sim4 = sim4[["k","P_3f_S","Q_3f_S"]].drop_duplicates(subset=["k"], keep="first").copy()

df4 = meta.merge(sim4, on="k", how="left").rename(columns={"P_3f_S":"P_SIM4","Q_3f_S":"Q_SIM4"})

if df4["P_SIM4"].isna().any():
    faltas4 = int(df4["P_SIM4"].isna().sum())
    raise ValueError(f"Falha de alinhamento por Hora_decimal/k: SIM4 faltas={faltas4}")

df4["E_P_SIM4_kWh"]   = df4["P_SIM4"] * DT_H
df4["E_Q_SIM4_kVArh"] = df4["Q_SIM4"] * DT_H

group_cols = ["Mes","Posto"]

agg4 = df4.groupby(group_cols, dropna=False).agg(
    N_pontos_SIM4=("P_SIM4", lambda s: int(s.notna().sum())),
    Consumo_P_SIM4_kWh=("E_P_SIM4_kWh","sum"),
    Consumo_Q_SIM4_kVArh=("E_Q_SIM4_kVArh","sum"),
    DemandaMax_P_SIM4_kW=("P_SIM4","max"),
    DemandaMax_Q_SIM4_kVAr=("Q_SIM4","max"),
).reset_index()

meta_counts = meta.groupby(group_cols, dropna=False).agg(N_pontos_META=("Posto","size")).reset_index()
agg = agg4.merge(meta_counts, on=group_cols, how="left")
...
print("SIM4 usado:", SIM4_FP)

# %% Celula 011 - SIMULACAO 004: tabela de custos (SIM4 - ANO 2028)
# =============================================================================
# Entrada:
#   Simulacao 004 segmentos_ano_2028/01_Integralizacao_Mensal_SIM4_2028.csv
#
# Saida:
#   Simulacao 004 segmentos_ano_2028/Tabela_demanda_consumo_por_posto_SIM4_2028.csv
# =============================================================================
...
IN_FP    = SIM4_DIR / f"01_Integralizacao_Mensal_SIM4_{ANO_ANALISE}.csv"
OUT_SIM4 = SIM4_DIR / f"Tabela_demanda_consumo_por_posto_SIM4_{ANO_ANALISE}.csv"
...
# - A demanda contratada de ponta com BESS considera abatimento da potencia do banco.
...
DCP_COM_BESS = max(0.0, DCP_SEM_BESS - BESS_P_KW)           # 2245 kW em 2028

CONTR_SIM4 = {"DCFP": DCFP_COM_BESS, "DCP": DCP_COM_BESS}
...
tab4 = tabela_custos(df, "SIM4", DCFP=CONTR_SIM4["DCFP"], DCP=CONTR_SIM4["DCP"])
tab4.to_csv(OUT_SIM4, index=False, encoding="utf-8")

print(f"DONE. Saved (Simulacao 004 segmentos_ano_{ANO_ANALISE}):")
print(" -", OUT_SIM4)
print("\nDemandas contratadas usadas:")
print(f" - SIM4: DCFP={CONTR_SIM4['DCFP']:.0f}, DCP={CONTR_SIM4['DCP']:.0f}")
print("\nResumo anual:")
print(tab4.tail(1))
