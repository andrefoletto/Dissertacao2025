# -*- coding: utf-8 -*-
# =============================================================================
# 022_Script_BESS_Simulacao_021_2bus_0700_2220_duas_simulacoes_conjugado_4500_ano_2028.py
#
# Regras deste script:
...
#     Simulacao 021 segmentos_pass1_sem_hvac_int_ano_2028
#   PASSO 2 (simulacao completa final):
#     Simulacao 021 segmentos_ano_2028
# =============================================================================
...
DIR_PASS1 = WORK_DIR / "Simulacao 021 segmentos_pass1_sem_hvac_int_ano_2028"
...
# %% Celula 003 - PASSO 1: monitores -> Curva_de_Carga_BESSM1_IN_sim_21.csv e BESSM2_IN (SHIFT DIREITA)
# =============================================================================
# Entradas:
#   PASSO1/Monitor_BESSM1_IN_anual.csv
#   PASSO1/Monitor_BESSM2_IN_anual.csv
# Saidas:
#   PASSO1/Curva_de_Carga_BESSM1_IN_sim_21.csv
#   PASSO1/Curva_de_Carga_BESSM2_IN_sim_21.csv
# Remove:
#   PASSO1/Monitor_BESSM1_IN_anual.csv
#   PASSO1/Monitor_BESSM2_IN_anual.csv
# =============================================================================
...
for mon_base in ["BESSM1_IN", "BESSM2_IN"]:
    ...
    out_fp = DIR_PASS1 / f"Curva_de_Carga_{mon_base}_sim_21.csv"
...
for mon_base, prefix in [("BESSM1_IN", "BESSM1"), ("BESSM2_IN", "BESSM2")]:
    in_fp = DIR_PASS1 / f"Curva_de_Carga_{mon_base}_sim_21.csv"
...
DIR_PASS2 = WORK_DIR / "Simulacao 021 segmentos_ano_2028"
...
# =============================================================================
# Entrada:
#   Simulacao 021 segmentos_ano_2028/UFSM_Mon_<KEY>_s*.csv
# Saida:
#   Simulacao 021 segmentos_ano_2028/Monitor_<KEY>_anual.csv
# Remove:
...
# =============================================================================
# Entrada:
#   Simulacao 021 segmentos_ano_2028/EXP_MTR_GERAL_*.csv
# Saida:
#   Simulacao 021 segmentos_ano_2028/EXP_MTR_GERAL_anual.csv
# Remove:
#   EXP_MTR_GERAL_*.csv
# =============================================================================
...
#   Simulacao 021 segmentos_ano_2028/Monitor_<KEY>_anual.csv
# Saidas:
#   Simulacao 021 segmentos_ano_2028/Curva_de_Carga_<KEY>_sim_21.csv
# Remove:
...
for in_fp in files:
    ...
    out_fp = DIR_PASS2 / f"Curva_de_Carga_{mon_key}_sim_21.csv"
...
#   Simulacao 021 segmentos_ano_2028/EXP_MTR_GERAL_anual.csv
# Saida:
#   Simulacao 021 segmentos_ano_2028/Resumo_EnergyMeter_Geral_Sim21.csv
# =============================================================================
...
OUT_FP = DIR_PASS2 / "Resumo_EnergyMeter_Geral_Sim21.csv"
...
# %% Celula 010 - ENERGIA e DEMANDA MAX por mes e posto (SIM21 - ANO 2028, 2 BESS 800 kW / 5,5 MWh cada)
# =============================================================================
# Entradas:
#   META: 00_dados_de_entrada_ano_letivo_2025_15min.csv
#         Usado apenas como mapa anual de Hora_decimal, Mes e Posto.
#   SIM21: curva GERAL com 2 BESS em operacao conjugada para o ano 2028
# Saida:
#   Simulacao 021 segmentos_ano_2028/01_Integralizacao_Mensal_SIM21_2028.csv
#
# Observacao:
#   Esta celula nao compara com SIM2/RGE nem com outra simulacao.
#   Ela apenas integraliza a simulacao conjugada com 2 BESS do ano 2028 por mes e posto.
#   A curva e procurada com sufixo sim_21 ou sim_021.
#   Todas as celulas deste arquivo foram padronizadas para SIM21.
# =============================================================================
...
SIM21_DIR = WORK_DIR / f"Simulacao 021 segmentos_ano_{ANO_ANALISE}"

META_FP = WORK_DIR / "00_dados_de_entrada_ano_letivo_2025_15min.csv"
CURVAS_CANDIDATAS = [
    SIM21_DIR / "Curva_de_Carga_GERAL_sim_21.csv",
    SIM21_DIR / "Curva_de_Carga_GERAL_sim_021.csv",
]
SIM21_FP = next((fp for fp in CURVAS_CANDIDATAS if fp.exists()), None)
OUT_FP = SIM21_DIR / f"01_Integralizacao_Mensal_{SIM_TAG}_{ANO_ANALISE}.csv"
...
for fp in [META_FP, SIM21_FP]:
    if not fp.exists():
        raise FileNotFoundError(fp)

meta = pd.read_csv(META_FP, dtype=str, encoding="utf-8-sig")
sim21 = pd.read_csv(SIM21_FP, encoding="utf-8-sig", sep=None, engine="python")

meta = clean_cols(meta)
sim21 = clean_cols(sim21)

for name, dfx, req in [
    ("META", meta, ["Hora_decimal", "Mes", "Posto"]),
    (SIM_TAG, sim21, ["Hora_decimal", "P_3f_S", "Q_3f_S"]),
]:
...
sim21["Hora_decimal"] = pd.to_numeric(sim21["Hora_decimal"], errors="coerce")
sim21["P_3f_S"] = pd.to_numeric(sim21["P_3f_S"], errors="coerce")
sim21["Q_3f_S"] = pd.to_numeric(sim21["Q_3f_S"], errors="coerce")
sim21.dropna(subset=["Hora_decimal", "P_3f_S", "Q_3f_S"], inplace=True)

meta["k"] = make_k(meta["Hora_decimal"])
sim21["k"] = make_k(sim21["Hora_decimal"])

meta = meta[["k", "Mes", "Posto"]].drop_duplicates(subset=["k"], keep="first").copy()
sim21 = sim21[["k", "P_3f_S", "Q_3f_S"]].drop_duplicates(subset=["k"], keep="first").copy()

df21 = meta.merge(sim21, on="k", how="left").rename(
    columns={"P_3f_S": "P_SIM21", "Q_3f_S": "Q_SIM21"}
)

faltas_p = int(df21["P_SIM21"].isna().sum())
faltas_q = int(df21["Q_SIM21"].isna().sum())
if faltas_p or faltas_q:
    raise ValueError(
        f"Falha de alinhamento por Hora_decimal/k: "
        f"P_SIM21 faltas={faltas_p}, Q_SIM21 faltas={faltas_q}"
    )

df21["E_P_SIM21_kWh"] = df21["P_SIM21"] * DT_H
df21["E_Q_SIM21_kVArh"] = df21["Q_SIM21"] * DT_H

group_cols = ["Mes", "Posto"]

agg21 = df21.groupby(group_cols, dropna=False).agg(
    N_pontos_SIM21=("P_SIM21", lambda s: int(s.notna().sum())),
    Consumo_P_SIM21_kWh=("E_P_SIM21_kWh", "sum"),
    Consumo_Q_SIM21_kVArh=("E_Q_SIM21_kVArh", "sum"),
    DemandaMax_P_SIM21_kW=("P_SIM21", "max"),
    DemandaMax_Q_SIM21_kVAr=("Q_SIM21", "max"),
).reset_index()

meta_counts = meta.groupby(group_cols, dropna=False).agg(N_pontos_META=("Posto", "size")).reset_index()
agg = agg21.merge(meta_counts, on=group_cols, how="left")
...
print("SIM21 usado:", SIM21_FP)
print(agg[["Mes", "Posto", "N_pontos_META", "N_pontos_SIM21", "DemandaMax_P_SIM21_kW"]])

# %% Celula 011 - SIMULACAO 021: tabela de custos (SIM21 - ANO 2028, 2 BESS 800 kW / 5,5 MWh cada)
# =============================================================================
# Entrada:
#   Simulacao 021 segmentos_ano_2028/01_Integralizacao_Mensal_SIM21_2028.csv
#
# Saida:
#   Simulacao 021 segmentos_ano_2028/Tabela_demanda_consumo_por_posto_SIM21_2028.csv
#
# Demandas contratadas do ano 2028:
#   DCFP com BESS = 4568 kW  (corte fora-ponta agressivo: 4500 kW * 1,015)
#   DCP sem BESS = 3045 kW
#   DCP com 2 BESS = 3045 - (2 * 800) = 1445 kW
#
# Banco de energia:
#   2 BESS de 1 MW / 5,5 MWh, operando a 80% da potencia nominal.
#   Potencia operacional total = 1600 kW.
#   Energia total = 11000 kWh.
#   Reserva de ponta = 54,5%.
#   Como o limite fora-ponta e mais agressivo, ha mais energia disponivel para peak-shave fora-ponta.
#
# Observacao:
...
SIM21_DIR = WORK_DIR / f"Simulacao 021 segmentos_ano_{ANO_ANALISE}"
SIM21_DIR.mkdir(parents=True, exist_ok=True)

IN_FP = SIM21_DIR / f"01_Integralizacao_Mensal_{SIM_TAG}_{ANO_ANALISE}.csv"
OUT_SIM21 = SIM21_DIR / f"Tabela_demanda_consumo_por_posto_{SIM_TAG}_{ANO_ANALISE}.csv"
...
# - Os 2 BESS sao instalados no proprio ano de analise.
# - Cada banco possui 1 MW / 5,5 MWh nominal, mas opera com 800 kW de potencia.
...
BESS_E_UNITARIA_KWH = 5500.0
BESS_RESERVA_PONTA_PCT = 54.5

BESS_P_TOTAL_KW = NUM_BESS * BESS_P_UNITARIA_KW       # 1600 kW
BESS_E_TOTAL_KWH = NUM_BESS * BESS_E_UNITARIA_KWH     # 11000 kWh
...
DCP_COM_BESS = max(0.0, DCP_SEM_BESS - BESS_P_TOTAL_KW) # 1445 kW em 2028

CONTR_SIM21 = {"DCFP": DCFP_COM_BESS, "DCP": DCP_COM_BESS}
...
tab21 = tabela_custos(
    df,
    SIM_TAG,
    DCFP=CONTR_SIM21["DCFP"],
    DCP=CONTR_SIM21["DCP"],
)

tab21.to_csv(OUT_SIM21, index=False, encoding="utf-8")

print("DONE. Saved (Simulacao 021 - 2 BESS - 2028 - conjugado 4500):")
print(" -", OUT_SIM21)
print("\nDemandas contratadas usadas em 2028:")
print(f" - DCFP = {DCFP_COM_BESS:.0f} kW")
print(f" - DCP sem BESS = {DCP_SEM_BESS:.0f} kW")
print(f" - DCP com 2 BESS = {DCP_COM_BESS:.0f} kW")
...
print("\nResumo anual:")
print(tab21[tab21["MES"].isin(["TOTAIS", "CTOTAL", "R$TOTAL", "DCFP", "DCP"])])
