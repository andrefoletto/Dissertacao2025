# -*- coding: utf-8 -*-
# =============================================================================
# 037_Script_UFV_Simulacao_036_6bus_2750_1760_1950_1960_3520_3540_uma_simulacao.py
#
# Regras deste script:
...
#   - Inclui UFV2 ~ 792 kWp em U023 (BT do trafo UFV8002_1000), coerente com o arranjo
#     validado no PVsyst: 1760 modulos de 450 W, 88 strings x 20 modulos,
#     8 inversores x 100 kW, 11 strings por inversor.
#   - Inclui UFV3 ~ 792 kWp em U033 (BT do trafo UFV8003_1000), coerente com o arranjo
#     validado no PVsyst: 1760 modulos de 450 W, 88 strings x 20 modulos,
#     8 inversores x 100 kW, 11 strings por inversor.
#   - Inclui UFV4 ~ 792 kWp em U043 (BT do trafo UFV8004_1000), coerente com o arranjo
#     validado no PVsyst: 1760 modulos de 450 W, 88 strings x 20 modulos,
#     8 inversores x 100 kW, 11 strings por inversor.
#   - Inclui UFV5 ~ 792 kWp em U053 (BT do trafo UFV8005_1000), coerente com o arranjo
#     validado no PVsyst: 1760 modulos de 450 W, 88 strings x 20 modulos,
#     8 inversores x 100 kW, 11 strings por inversor.
#   - Inclui UFV6 ~ 792 kWp em U063 (BT do trafo UFV8006_1000), coerente com o arranjo
#     validado no PVsyst: 1760 modulos de 450 W, 88 strings x 20 modulos,
#     8 inversores x 100 kW, 11 strings por inversor.
# ======================================================================================
...
ARQ_POA_UFV2 = WORK_DIR / "UFV_poa_2025.csv"
ARQ_TMOD_UFV2 = WORK_DIR / "UFV_tmod_2025.csv"
ARQ_POA_UFV3 = WORK_DIR / "UFV_poa_2025.csv"
ARQ_TMOD_UFV3 = WORK_DIR / "UFV_tmod_2025.csv"
ARQ_POA_UFV4 = WORK_DIR / "UFV_poa_2025.csv"
ARQ_TMOD_UFV4 = WORK_DIR / "UFV_tmod_2025.csv"
ARQ_POA_UFV5 = WORK_DIR / "UFV_poa_2025.csv"
ARQ_TMOD_UFV5 = WORK_DIR / "UFV_tmod_2025.csv"
ARQ_POA_UFV6 = WORK_DIR / "UFV_poa_2025.csv"
ARQ_TMOD_UFV6 = WORK_DIR / "UFV_tmod_2025.csv"
...
MON_SPECS = {
    ...
    "1760_ANTES":  {"element": "line.1750_1760", "terminal": 1},
    "1760_DEPOIS": {"element": "line.1760_1770", "terminal": 1},
    "1960_ANTES":  {"element": "line.1950_1960", "terminal": 1},
    "1960_DEPOIS": {"element": "line.1960_1970", "terminal": 1},
    "3520_ANTES":  {"element": "line.3510_3520", "terminal": 1},
    "3520_DEPOIS": {"element": "line.3520_3530", "terminal": 1},
    "3540_ANTES":  {"element": "line.3530_3540", "terminal": 1},
    "3540_DEPOIS": {"element": "line.3540_3550", "terminal": 1},
    "TR_UFV1_MT":  {"element": "transformer.UFV8001_1000", "terminal": 1},
    "TR_UFV1_BT":  {"element": "transformer.UFV8001_1000", "terminal": 2},
    "TR_UFV2_MT":  {"element": "transformer.UFV8002_1000", "terminal": 1},
    "TR_UFV2_BT":  {"element": "transformer.UFV8002_1000", "terminal": 2},
    "TR_UFV3_MT":  {"element": "transformer.UFV8003_1000", "terminal": 1},
    "TR_UFV3_BT":  {"element": "transformer.UFV8003_1000", "terminal": 2},
    "TR_UFV4_MT":  {"element": "transformer.UFV8004_1000", "terminal": 1},
    "TR_UFV4_BT":  {"element": "transformer.UFV8004_1000", "terminal": 2},
    "TR_UFV5_MT":  {"element": "transformer.UFV8005_1000", "terminal": 1},
    "TR_UFV5_BT":  {"element": "transformer.UFV8005_1000", "terminal": 2},
    "TR_UFV6_MT":  {"element": "transformer.UFV8006_1000", "terminal": 1},
    "TR_UFV6_BT":  {"element": "transformer.UFV8006_1000", "terminal": 2},

}
MON_KEYS = list(MON_SPECS.keys())

DIR_PASS2 = WORK_DIR / "Simulacao 036 segmentos"
...
def _build_circuit_pass2(dss: DSS, posto: str, seg_tag: str) -> dict[str, str]:
    ...
    # UFV2 em 1760 -> trafo 13.8 / 0.38 kV -> barra BT U023
    dss.text("New line.1760_U021 phases=3 bus1=1760 bus2=U021 length=0.0200 units=km Geometry=UG_trefoil_25")
    dss.text("New line.U021_U022 phases=3 bus1=U021 bus2=U022 Switch=y enable=true")
    dss.text(
        "New transformer.UFV8002_1000 XHL=6.0 windings=2 %loadloss=1.46 %noloadloss=0.29 %imag=1.5 "
        "wdg=1 bus=U022 kv=13.8 kva=1000 conn=delta "
        "wdg=2 bus=U023 kv=0.38 kva=1000 conn=wye"
    )

    # === CURVAS DE EFICIENCIA E TEMPERATURA ===
    # Curva de eficiencia do inversor Huawei SUN2000-100KTL-M1/M2 (aproximacao CC/CA)
    # Ja definidas na UFV1

    # Curva de potencia x temperatura do modulo Trina 450 Wp (gammaP = -0.34 %/°C)
    # Ja definidas na UFV1
    
    # Curvas anuais de irradiancia POA e temperatura do modulo - 2025
    dss.text(f'New Loadshape.Irrad_UFV2 npts={TOTAL_NPTS} interval=0 csvfile="{ARQ_POA_UFV2}"')
    dss.text(f'New Tshape.Temp_UFV2     npts={TOTAL_NPTS} interval=0 csvfile="{ARQ_TMOD_UFV2}"')
    ...
    for i in range(1, 9):
        dss.text(
            f"New PVSystem.UFV8002_INV{i} phases=3 bus1=U023 kv=0.38 "
            f"pmpp=99.0 kVA=100 pf=1.0 conn=wye "
            f"%cutin=0.1 %cutout=0.1 "
            f"effcurve=Eff_Huawei100k P-TCurve=P_T_Trina450 "
            f"yearly=Irrad_UFV2 Tyearly=Temp_UFV2"
        )

    # UFV3 em 1950 -> trafo 13.8 / 0.38 kV -> barra BT U033
    dss.text("New line.1950_U031 phases=3 bus1=1950 bus2=U031 length=0.0200 units=km Geometry=UG_trefoil_25")
    dss.text("New line.U031_U032 phases=3 bus1=U031 bus2=U032 Switch=y enable=true")
    dss.text(
        "New transformer.UFV8003_1000 XHL=6.0 windings=2 %loadloss=1.46 %noloadloss=0.29 %imag=1.5 "
        "wdg=1 bus=U032 kv=13.8 kva=1000 conn=delta "
        "wdg=2 bus=U033 kv=0.38 kva=1000 conn=wye"
    )
    ...
    # Curvas anuais de irradiancia POA e temperatura do modulo - 2025
    dss.text(f'New Loadshape.Irrad_UFV3 npts={TOTAL_NPTS} interval=0 csvfile="{ARQ_POA_UFV3}"')
    dss.text(f'New Tshape.Temp_UFV3     npts={TOTAL_NPTS} interval=0 csvfile="{ARQ_TMOD_UFV3}"')
    ...
    for i in range(1, 9):
        dss.text(
            f"New PVSystem.UFV8003_INV{i} phases=3 bus1=U033 kv=0.38 "
            f"pmpp=99.0 kVA=100 pf=1.0 conn=wye "
            f"%cutin=0.1 %cutout=0.1 "
            f"effcurve=Eff_Huawei100k P-TCurve=P_T_Trina450 "
            f"yearly=Irrad_UFV3 Tyearly=Temp_UFV3"
        )

    # UFV4 em 1960 -> trafo 13.8 / 0.38 kV -> barra BT U043
    dss.text("New line.1960_U041 phases=3 bus1=1960 bus2=U041 length=0.0200 units=km Geometry=UG_trefoil_25")
    dss.text("New line.U041_U042 phases=3 bus1=U041 bus2=U042 Switch=y enable=true")
    dss.text(
        "New transformer.UFV8004_1000 XHL=6.0 windings=2 %loadloss=1.46 %noloadloss=0.29 %imag=1.5 "
        "wdg=1 bus=U042 kv=13.8 kva=1000 conn=delta "
        "wdg=2 bus=U043 kv=0.38 kva=1000 conn=wye"
    )
    ...
    # Curvas anuais de irradiancia POA e temperatura do modulo - 2025
    dss.text(f'New Loadshape.Irrad_UFV4 npts={TOTAL_NPTS} interval=0 csvfile="{ARQ_POA_UFV4}"')
    dss.text(f'New Tshape.Temp_UFV4     npts={TOTAL_NPTS} interval=0 csvfile="{ARQ_TMOD_UFV4}"')
    ...
    for i in range(1, 9):
        dss.text(
            f"New PVSystem.UFV8004_INV{i} phases=3 bus1=U043 kv=0.38 "
            f"pmpp=99.0 kVA=100 pf=1.0 conn=wye "
            f"%cutin=0.1 %cutout=0.1 "
            f"effcurve=Eff_Huawei100k P-TCurve=P_T_Trina450 "
            f"yearly=Irrad_UFV4 Tyearly=Temp_UFV4"
        )

    # UFV5 em 3520 -> trafo 13.8 / 0.38 kV -> barra BT U053
    dss.text("New line.3520_U051 phases=3 bus1=3520 bus2=U051 length=0.0200 units=km Geometry=UG_trefoil_25")
    dss.text("New line.U051_U052 phases=3 bus1=U051 bus2=U052 Switch=y enable=true")
    dss.text(
        "New transformer.UFV8005_1000 XHL=6.0 windings=2 %loadloss=1.46 %noloadloss=0.29 %imag=1.5 "
        "wdg=1 bus=U052 kv=13.8 kva=1000 conn=delta "
        "wdg=2 bus=U053 kv=0.38 kva=1000 conn=wye"
    )
    ...
    # Curvas anuais de irradiancia POA e temperatura do modulo - 2025
    dss.text(f'New Loadshape.Irrad_UFV5 npts={TOTAL_NPTS} interval=0 csvfile="{ARQ_POA_UFV5}"')
    dss.text(f'New Tshape.Temp_UFV5     npts={TOTAL_NPTS} interval=0 csvfile="{ARQ_TMOD_UFV5}"')
    ...
    for i in range(1, 9):
        dss.text(
            f"New PVSystem.UFV8005_INV{i} phases=3 bus1=U053 kv=0.38 "
            f"pmpp=99.0 kVA=100 pf=1.0 conn=wye "
            f"%cutin=0.1 %cutout=0.1 "
            f"effcurve=Eff_Huawei100k P-TCurve=P_T_Trina450 "
            f"yearly=Irrad_UFV5 Tyearly=Temp_UFV5"
        )

    # UFV5 em 3540 -> trafo 13.8 / 0.38 kV -> barra BT U063
    dss.text("New line.3540_U061 phases=3 bus1=3540 bus2=U061 length=0.0200 units=km Geometry=UG_trefoil_25")
    dss.text("New line.U061_U062 phases=3 bus1=U061 bus2=U062 Switch=y enable=true")
    dss.text(
        "New transformer.UFV8006_1000 XHL=6.0 windings=2 %loadloss=1.46 %noloadloss=0.29 %imag=1.5 "
        "wdg=1 bus=U062 kv=13.8 kva=1000 conn=delta "
        "wdg=2 bus=U063 kv=0.38 kva=1000 conn=wye"
    )
    ...
    # Curvas anuais de irradiancia POA e temperatura do modulo - 2025
    dss.text(f'New Loadshape.Irrad_UFV6 npts={TOTAL_NPTS} interval=0 csvfile="{ARQ_POA_UFV6}"')
    dss.text(f'New Tshape.Temp_UFV6     npts={TOTAL_NPTS} interval=0 csvfile="{ARQ_TMOD_UFV6}"')
    ...
    for i in range(1, 9):
        dss.text(
            f"New PVSystem.UFV8006_INV{i} phases=3 bus1=U053 kv=0.38 "
            f"pmpp=99.0 kVA=100 pf=1.0 conn=wye "
            f"%cutin=0.1 %cutout=0.1 "
            f"effcurve=Eff_Huawei100k P-TCurve=P_T_Trina450 "
            f"yearly=Irrad_UFV6 Tyearly=Temp_UFV6"
        )
...
# =============================================================================
# Entrada:
#   Simulacao 036 segmentos/UFSM_Mon_<KEY>_s*.csv
# Saida:
#   Simulacao 036 segmentos/Monitor_<KEY>_anual.csv
# Remove:
...
# =============================================================================
# Entrada:
#   Simulacao 036 segmentos/EXP_MTR_GERAL_*.csv
# Saida:
#   Simulacao 036 segmentos/EXP_MTR_GERAL_anual.csv
# Remove:
#   EXP_MTR_GERAL_*.csv
# =============================================================================
...
# Saidas:
#   Simulacao 036 segmentos/Curva_de_Carga_<KEY>_sim_36.csv
# Remove:
...
for in_fp in files:
    ...
    out_fp = DIR_PASS2 / f"Curva_de_Carga_{mon_key}_sim_36.csv"
...
#   Simulacao 036 segmentos/EXP_MTR_GERAL_anual.csv
# Saida:
#   Simulacao 036 segmentos/Resumo_EnergyMeter_Geral_Sim36.csv
# =============================================================================
...
OUT_FP = DIR_PASS2 / "Resumo_EnergyMeter_Geral_Sim36.csv"
...
# %% Celula 006 - ENERGIA e DEMANDA MAX por mes e posto (SIM2 vs SIM36)
# =============================================================================
# Entradas:
...
#   SIM36: Simulacao 036 segmentos/Curva_de_Carga_GERAL_sim_36.csv
#         (Hora_decimal, P_3f_S, Q_3f_S)
#
# Saida:
#   Simulacao 036 segmentos/01_Integralizacao_Mensal_SIM2_x_SIM36_2025.csv
#
# OBSERVACAO:
#   - P > 0  -> consumo da rede
#   - P < 0  -> injecao na rede
...
SIM36_DIR = WORK_DIR / "Simulacao 036 segmentos"

META_FP = WORK_DIR / "00_dados_de_entrada_ano_letivo_2025_15min.csv"
SIM2_FP = SIM2_DIR / "Curva_de_Carga_GERAL_sim_2.csv"
SIM36_FP = SIM36_DIR / "Curva_de_Carga_GERAL_sim_36.csv"
OUT_FP  = SIM36_DIR / "01_Integralizacao_Mensal_SIM2_x_SIM36_2025.csv"
...
for fp in [META_FP, SIM2_FP, SIM36_FP]:
    if not fp.exists():
...
sim36 = pd.read_csv(SIM36_FP, encoding="utf-8-sig", sep=None, engine="python")

for name, dfx, req in [
    ("META", meta, ["Hora_decimal", "Mes", "Posto"]),
    ("SIM2", sim2, ["Hora_decimal", "P_3f_S", "Q_3f_S"]),
    ("SIM36", sim36, ["Hora_decimal", "P_3f_S", "Q_3f_S"]),
]:
...
sim36["k"] = make_k(sim36["Hora_decimal"])
...
sim36 = sim36[["k", "P_3f_S", "Q_3f_S"]].drop_duplicates(subset=["k"], keep="first").copy()

df2 = meta.merge(sim2, on="k", how="left").rename(columns={"P_3f_S": "P_SIM2", "Q_3f_S": "Q_SIM2"})
df36 = meta.merge(sim36, on="k", how="left").rename(columns={"P_3f_S": "P_SIM36", "Q_3f_S": "Q_SIM36"})

# -----------------------------------------------------------------------------
# Separacao entre consumo da rede (P > 0) e injecao na rede (P < 0)
# -----------------------------------------------------------------------------
for dfx, tag in [(df2, "SIM2"), (df36, "SIM36")]:
    pcol = f"P_{tag}"
...
agg36 = df36.groupby(group_cols, dropna=False).agg(
    N_pontos_SIM36=("P_SIM36", lambda s: int(s.notna().sum())),
    Consumo_P_SIM36_kWh=("E_cons_SIM36_kWh", "sum"),
    Injecao_P_SIM36_kWh=("E_inj_SIM36_kWh", "sum"),
    Consumo_Q_SIM36_kVArh=("E_Q_SIM36_kVArh", "sum"),
    DemandaMax_P_SIM36_kW=("P_cons_SIM36_kW", "max"),
    DemandaMax_Inj_SIM36_kW=("P_inj_SIM36_kW", "max"),
).reset_index()

agg = agg2.merge(agg36, on=group_cols, how="outer")
...
# %% Celula 007 - SIMULACAO 036: Tabela de custos (SIM2 vs SIM36 - 2025)
# =============================================================================
# Entrada:
#   Simulacao 036 segmentos/01_Integralizacao_Mensal_SIM2_x_SIM36_2025.csv
#
# Saidas (todas em Simulacao 036 segmentos):
#   Tabela_demanda_consumo_por_posto_SIM2_2025.csv
#   Tabela_demanda_consumo_por_posto_SIM36_2025.csv
#   Tabela_demanda_consumo_por_posto_SIM2_vs_SIM36_2025.csv
#
# HIPOTESE ADOTADA:
...
WORK_DIR = Path(r"C:\Users\afole\OneDrive\Dissertacao2025")
SIM36_DIR = WORK_DIR / "Simulacao 036 segmentos"
SIM36_DIR.mkdir(parents=True, exist_ok=True)

IN_FP     = SIM36_DIR / "01_Integralizacao_Mensal_SIM2_x_SIM36_2025.csv"
OUT_SIM2  = SIM36_DIR / "Tabela_demanda_consumo_por_posto_SIM2_2025.csv"
OUT_SIM36 = SIM36_DIR / "Tabela_demanda_consumo_por_posto_SIM36_2025.csv"
OUT_CMP   = SIM36_DIR / "Tabela_demanda_consumo_por_posto_SIM2_vs_SIM36_2025.csv"

# -------------------------
# DEMANDAS CONTRATADAS POR SIMULACAO
# -------------------------
CONTR_SIM2  = {"DCFP": 5000.0, "DCP": 3000.0}
CONTR_SIM36 = {"DCFP": 5000.0, "DCP": 3000.0}
...
def tabela_custos(df_in: pd.DataFrame, fonte: str, DCFP: float, DCP: float) -> pd.DataFrame:
    ...
    Usa:
      - Consumo_P_{fonte}_kWh
      - Injecao_P_{fonte}_kWh
      - DemandaMax_P_{fonte}_kW

    Como o arquivo vem separado por Posto, a mesma coluna e lida para PONTA
    e FORA_PONTA via filtro.
    """
...
tab36 = tabela_custos(df, "SIM36", DCFP=CONTR_SIM36["DCFP"], DCP=CONTR_SIM36["DCP"])

tab2.to_csv(OUT_SIM2, index=False, encoding="utf-8")
tab36.to_csv(OUT_SIM36, index=False, encoding="utf-8")

# -------------------------
# Comparativo (economia)
# -------------------------
t2 = tab2[tab2["MES"].isin(MESES_ORD)][["MES", "R$TOTAL_MES"]].rename(columns={"R$TOTAL_MES": "R$SIM2"})
t36 = tab36[tab36["MES"].isin(MESES_ORD)][["MES", "R$TOTAL_MES"]].rename(columns={"R$TOTAL_MES": "R$SIM36"})

cmp = t2.merge(t36, on="MES", how="inner")
cmp["MES"] = pd.Categorical(cmp["MES"], categories=MESES_ORD, ordered=True)
cmp = cmp.sort_values("MES").reset_index(drop=True)

cmp["Economia_R$"] = (cmp["R$SIM2"] - cmp["R$SIM36"]).round(2)
...
print("DONE. Saved (Simulacao 036 segmentos):")
print(" -", OUT_SIM2)
print(" -", OUT_SIM36)
print(" -", OUT_CMP)
...
print(" - SIM36: DCFP=5000, DCP=3000")
