# -*- coding: utf-8 -*-
# =============================================================================
# 024_Script_BESS_Simulacao_023_4bus_0700_2220_2380_1250_duas_simulacoes_conjugado_4500.py
#
# Regras deste script:
...
#     Simulacao 023 segmentos_pass1_sem_hvac_int
#   PASSO 2 (simulacao completa final):
#     Simulacao 023 segmentos
# =============================================================================
...
TARGETS = {
    ...
    "FORA_PONTA": {"kWtarget": 4500, "kWtargetLow": 3500},
}

DIR_PASS1 = WORK_DIR / "Simulacao 023 segmentos_pass1_sem_hvac_int"
...
DIR_PASS2 = WORK_DIR / "Simulacao 023 segmentos"
...
# =============================================================================
# Entrada:
#   Simulacao 023 segmentos/UFSM_Mon_<KEY>_s*.csv
# Saida:
#   Simulacao 023 segmentos/Monitor_<KEY>_anual.csv
# Remove:
...
# =============================================================================
# Entrada:
#   Simulacao 023 segmentos/EXP_MTR_GERAL_*.csv
# Saida:
#   Simulacao 023 segmentos/EXP_MTR_GERAL_anual.csv
# Remove:
#   EXP_MTR_GERAL_*.csv
# =============================================================================
...
#   Simulacao 023 segmentos/Monitor_<KEY>_anual.csv
# Saidas:
#   Simulacao 023 segmentos/Curva_de_Carga_<KEY>_sim_23.csv
# Remove:
...
#   Simulacao 023 segmentos/EXP_MTR_GERAL_anual.csv
# Saida:
#   Simulacao 023 segmentos/Resumo_EnergyMeter_Geral_Sim23.csv
# =============================================================================
...
# %% Celula 010 - ENERGIA e DEMANDA MAX por mes e posto (SIM2 vs SIM23)
# =============================================================================
# Entradas:
...
#   SIM23: Simulacao 023 segmentos/Curva_de_Carga_GERAL_sim_23.csv (Hora_decimal, P_3f_S, Q_3f_S)
# Saida:
#   Simulacao 023 segmentos/01_Integralizacao_Mensal_SIM2_x_SIM23_2025.csv
# =============================================================================
...
SIM23_DIR = WORK_DIR / "Simulacao 023 segmentos"
...
OUT_FP = SIM23_DIR / "01_Integralizacao_Mensal_SIM2_x_SIM23_2025.csv"
...
for fp in [META_FP, SIM2_FP, SIM23_FP]:
    if not fp.exists():
...
for name, dfx, req in [
    ...
    ("SIM23", sim23, ["Hora_decimal", "P_3f_S", "Q_3f_S"]),
]:
...
df23 = meta.merge(sim23, on="k", how="left").rename(columns={"P_3f_S": "P_SIM23", "Q_3f_S": "Q_SIM23"})
...
agg = agg2.merge(agg23, on=group_cols, how="outer")
...
# %% Celula 011 - SIMULACAO 023: Tabela de custos (SIM2 vs SIM23 - 2025) com demandas contratadas diferentes
# =============================================================================
# Entrada:
#   Simulacao 023 segmentos/01_Integralizacao_Mensal_SIM2_x_SIM23_2025.csv
#
# Saidas (todas em Simulacao 023 segmentos):
#   Tabela_demanda_consumo_por_posto_SIM2_2025.csv
#   Tabela_demanda_consumo_por_posto_SIM23_2025.csv
#   Tabela_demanda_consumo_por_posto_SIM2_vs_SIM23_2025.csv   (economia)
# =============================================================================
...
IN_FP = SIM23_DIR / "01_Integralizacao_Mensal_SIM2_x_SIM23_2025.csv"
OUT_SIM2 = SIM23_DIR / "Tabela_demanda_consumo_por_posto_SIM2_2025.csv"
OUT_SIM23 = SIM23_DIR / "Tabela_demanda_consumo_por_posto_SIM23_2025.csv"
OUT_CMP = SIM23_DIR / "Tabela_demanda_consumo_por_posto_SIM2_vs_SIM23_2025.csv"

CONTR_SIM2 = {"DCFP": 5000.0, "DCP": 3000.0}
CONTR_SIM23 = {"DCFP": 4500.0, "DCP":   0.0}  # 3000 - 3*800
...
tab23 = tabela_custos(df, "SIM23", DCFP=CONTR_SIM23["DCFP"], DCP=CONTR_SIM23["DCP"])
...
print("DONE. Saved (Simulacao 023 segmentos):")
print(" -", OUT_SIM2)
print(" -", OUT_SIM23)
print(" -", OUT_CMP)
print("\nDemandas contratadas usadas:")
print(" - SIM2: DCFP=5000, DCP=3000")
print(" - SIM23: DCFP=4500, DCP=0")
print("\nResumo anual:")
print(cmp.tail(1))
