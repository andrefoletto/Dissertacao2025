# -*- coding: utf-8 -*-
# =============================================================================
# 031_Script_BESS_Simulacao_030_6bus_0700_2220_2380_1250_1630_1950_duas_simulacoes_conjugado_4000_ano_2028.py
#
# Regras deste script:
...
#     Simulacao 030 segmentos_pass1_sem_hvac_int_ano_2028
#   PASSO 2 (simulacao completa final):
#     Simulacao 030 segmentos_ano_2028
# =============================================================================
...
DIR_PASS1 = WORK_DIR / "Simulacao 030 segmentos_pass1_sem_hvac_int_ano_2028"
...
DIR_PASS2 = WORK_DIR / "Simulacao 030 segmentos_ano_2028"
...
# =============================================================================
# Entrada:
#   Simulacao 030 segmentos_ano_2028/UFSM_Mon_<KEY>_s*.csv
# Saida:
#   Simulacao 030 segmentos_ano_2028/Monitor_<KEY>_anual.csv
# Remove:
...
# =============================================================================
# Entrada:
#   Simulacao 030 segmentos_ano_2028/EXP_MTR_GERAL_*.csv
# Saida:
#   Simulacao 030 segmentos_ano_2028/EXP_MTR_GERAL_anual.csv
# Remove:
#   EXP_MTR_GERAL_*.csv
# =============================================================================
...
#   Simulacao 030 segmentos_ano_2028/Monitor_<KEY>_anual.csv
# Saidas:
#   Simulacao 030 segmentos_ano_2028/Curva_de_Carga_<KEY>_sim_30.csv
# Remove:
...
#   Simulacao 030 segmentos_ano_2028/EXP_MTR_GERAL_anual.csv
# Saida:
#   Simulacao 030 segmentos_ano_2028/Resumo_EnergyMeter_Geral_Sim30.csv
# =============================================================================
...
# %% Celula 010 - ENERGIA e DEMANDA MAX por mes e posto (SIM30 - ANO 2028, 6 BESS 800 kW / 5 MWh cada)
# =============================================================================
# Entradas:
#   META: 00_dados_de_entrada_ano_letivo_2025_15min.csv
#         Usado apenas como mapa anual de Hora_decimal, Mes e Posto.
#   SIM30: curva GERAL com 6 BESS em operacao conjugada para o ano 2028
# Saida:
#   Simulacao 030 segmentos_ano_2028/01_Integralizacao_Mensal_SIM30_2028.csv
#
# Observacao:
...
#   A curva e lida diretamente pelo sufixo sim_30, sem fallback.
#   Todas as celulas deste arquivo foram padronizadas para SIM30.
# =============================================================================
...
SIM30_DIR = WORK_DIR / f"Simulacao 030 segmentos_ano_{ANO_ANALISE}"

META_FP = WORK_DIR / "00_dados_de_entrada_ano_letivo_2025_15min.csv"
SIM30_FP = SIM30_DIR / "Curva_de_Carga_GERAL_sim_30.csv"
OUT_FP = SIM30_DIR / f"01_Integralizacao_Mensal_{SIM_TAG}_{ANO_ANALISE}.csv"
...
sim30 = clean_cols(sim30)

for name, dfx, req in [
    ("META", meta, ["Hora_decimal", "Mes", "Posto"]),
    (SIM_TAG, sim30, ["Hora_decimal", "P_3f_S", "Q_3f_S"]),
]:
...
sim30["Hora_decimal"] = pd.to_numeric(sim30["Hora_decimal"], errors="coerce")
sim30["P_3f_S"] = pd.to_numeric(sim30["P_3f_S"], errors="coerce")
sim30["Q_3f_S"] = pd.to_numeric(sim30["Q_3f_S"], errors="coerce")
sim30.dropna(subset=["Hora_decimal", "P_3f_S", "Q_3f_S"], inplace=True)
...
df30 = meta.merge(sim30, on="k", how="left").rename(
    columns={"P_3f_S": "P_SIM30", "Q_3f_S": "Q_SIM30"}
)

faltas_p = int(df30["P_SIM30"].isna().sum())
faltas_q = int(df30["Q_SIM30"].isna().sum())
if faltas_p or faltas_q:
    raise ValueError(
        f"Falha de alinhamento por Hora_decimal/k: "
        f"P_SIM30 faltas={faltas_p}, Q_SIM30 faltas={faltas_q}"
    )
...
agg = agg30.merge(meta_counts, on=group_cols, how="left")
...
print("SIM30 usado:", SIM30_FP)
print(agg[["Mes", "Posto", "N_pontos_META", "N_pontos_SIM30", "DemandaMax_P_SIM30_kW"]])


# %% Celula 011 - SIMULACAO 030: tabela de custos (SIM30 - ANO 2028, 6 BESS 800 kW / 5 MWh cada)
# =============================================================================
# Entrada:
#   Simulacao 030 segmentos_ano_2028/01_Integralizacao_Mensal_SIM30_2028.csv
#
# Saida:
#   Simulacao 030 segmentos_ano_2028/Tabela_demanda_consumo_por_posto_SIM30_2028.csv
#
...
#
# Banco de energia:
#   6 BESS de 1 MW / 5 MWh, operando a 80% da potencia nominal.
#   Potencia operacional total = 4800 kW.
#   Energia total = 30000 kWh.
#   Reserva de ponta = 60,0%.
...
IN_FP = SIM30_DIR / f"01_Integralizacao_Mensal_{SIM_TAG}_{ANO_ANALISE}.csv"
OUT_SIM30 = SIM30_DIR / f"Tabela_demanda_consumo_por_posto_{SIM_TAG}_{ANO_ANALISE}.csv"
...
# - Cada banco possui 1 MW / 5 MWh nominal, mas opera com 800 kW de potencia.
...
BESS_E_UNITARIA_KWH = 5000.0
BESS_RESERVA_PONTA_PCT = 60.0

BESS_P_TOTAL_KW = NUM_BESS * BESS_P_UNITARIA_KW       # 4800 kW
BESS_E_TOTAL_KWH = NUM_BESS * BESS_E_UNITARIA_KWH     # 30000 kWh
...
CONTR_SIM30 = {"DCFP": DCFP_COM_BESS, "DCP": DCP_COM_BESS}
...
tab30 = tabela_custos(
    df,
    SIM_TAG,
    DCFP=CONTR_SIM30["DCFP"],
    DCP=CONTR_SIM30["DCP"],
)

tab30.to_csv(OUT_SIM30, index=False, encoding="utf-8")

print("DONE. Saved (Simulacao 030 - 6 BESS - 2028 - conjugado 4000):")
print(" -", OUT_SIM30)
print("\nDemandas contratadas usadas em 2028:")
print(f" - DCFP = {DCFP_COM_BESS:.0f} kW")
print(f" - Recarga FP permitida abaixo de {KW_TARGET_LOW_FP:.0f} kW")
...
print("\nResumo anual:")
print(tab30[tab30["MES"].isin(["TOTAIS", "CTOTAL", "R$TOTAL", "DCFP", "DCP"])])
