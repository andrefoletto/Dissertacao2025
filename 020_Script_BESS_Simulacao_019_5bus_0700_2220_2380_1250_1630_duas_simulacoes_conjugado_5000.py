# -*- coding: utf-8 -*-
# =============================================================================
# 020_Script_BESS_Simulacao_019_5bus_0700_2220_2380_1250_1630_duas_simulacoes_conjugado.py
#
# Regras deste script:
...
#     Simulacao 019 segmentos_pass1_sem_hvac_int
#   PASSO 2 (simulacao completa final):
#     Simulacao 019 segmentos
# =============================================================================
...
DIR_PASS1 = WORK_DIR / "Simulacao 019 segmentos_pass1_sem_hvac_int"
...
def build_circuit_pass1(
    ...
    dss.text(
        ...
        f"kWrated=800 kWhrated=3200 Effcurve=Eff %EffCharge=95 %EffDischarge=95 "
...
DIR_PASS2 = WORK_DIR / "Simulacao 019 segmentos"
...
# =============================================================================
# Entrada:
#   Simulacao 019 segmentos/UFSM_Mon_<KEY>_s*.csv
# Saida:
#   Simulacao 019 segmentos/Monitor_<KEY>_anual.csv
# Remove:
...
# =============================================================================
# Entrada:
#   Simulacao 019 segmentos/EXP_MTR_GERAL_*.csv
# Saida:
#   Simulacao 019 segmentos/EXP_MTR_GERAL_anual.csv
# Remove:
#   EXP_MTR_GERAL_*.csv
# =============================================================================
...
#   Simulacao 019 segmentos/Monitor_<KEY>_anual.csv
# Saidas:
#   Simulacao 019 segmentos/Curva_de_Carga_<KEY>_sim_19.csv
# Remove:
...
#   Simulacao 019 segmentos/EXP_MTR_GERAL_anual.csv
# Saida:
#   Simulacao 019 segmentos/Resumo_EnergyMeter_Geral_Sim19.csv
# =============================================================================
...
# %% Celula 010 - ENERGIA e DEMANDA MAX por mes e posto (SIM2 vs SIM19)
# =============================================================================
# Entradas:
...
#   SIM19: Simulacao 019 segmentos/Curva_de_Carga_GERAL_sim_19.csv (Hora_decimal, P_3f_S, Q_3f_S)
# Saida:
#   Simulacao 019 segmentos/01_Integralizacao_Mensal_SIM2_x_SIM19_2025.csv
# =============================================================================
...
SIM19_DIR = WORK_DIR / "Simulacao 019 segmentos"
...
OUT_FP = SIM19_DIR / "01_Integralizacao_Mensal_SIM2_x_SIM19_2025.csv"
...
for fp in [META_FP, SIM2_FP, SIM19_FP]:
    if not fp.exists():
...
for name, dfx, req in [
    ...
    ("SIM19", sim19, ["Hora_decimal", "P_3f_S", "Q_3f_S"]),
]:
...
df19 = meta.merge(sim19, on="k", how="left").rename(columns={"P_3f_S": "P_SIM19", "Q_3f_S": "Q_SIM19"})
...
agg = agg2.merge(agg19, on=group_cols, how="outer")
...
# %% Celula 011 - SIMULACAO 019: Tabela de custos (SIM2 vs SIM19 - 2025) com demandas contratadas diferentes
# =============================================================================
# Entrada:
#   Simulacao 019 segmentos/01_Integralizacao_Mensal_SIM2_x_SIM19_2025.csv
#
# Saidas (todas em Simulacao 019 segmentos):
#   Tabela_demanda_consumo_por_posto_SIM2_2025.csv
#   Tabela_demanda_consumo_por_posto_SIM19_2025.csv
#   Tabela_demanda_consumo_por_posto_SIM2_vs_SIM19_2025.csv   (economia)
# =============================================================================
...
IN_FP = SIM19_DIR / "01_Integralizacao_Mensal_SIM2_x_SIM19_2025.csv"
OUT_SIM2 = SIM19_DIR / "Tabela_demanda_consumo_por_posto_SIM2_2025.csv"
OUT_SIM19 = SIM19_DIR / "Tabela_demanda_consumo_por_posto_SIM19_2025.csv"
OUT_CMP = SIM19_DIR / "Tabela_demanda_consumo_por_posto_SIM2_vs_SIM19_2025.csv"

CONTR_SIM2 = {"DCFP": 5000.0, "DCP": 3000.0}
CONTR_SIM19 = {"DCFP": 5000.0, "DCP":   0.0}  # 3000 - 4*000
...
tab19 = tabela_custos(df, "SIM19", DCFP=CONTR_SIM19["DCFP"], DCP=CONTR_SIM19["DCP"])
...
print("DONE. Saved (Simulacao 019 segmentos):")
print(" -", OUT_SIM2)
print(" -", OUT_SIM19)
print(" -", OUT_CMP)
print("\nDemandas contratadas usadas:")
print(" - SIM2: DCFP=5000, DCP=3000")
print(" - SIM19: DCFP=5000, DCP=0")
print("\nResumo anual:")
print(cmp.tail(1))
