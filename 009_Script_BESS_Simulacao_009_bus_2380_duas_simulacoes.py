# -*- coding: utf-8 -*-
# =============================================================================
# Script_BESS_Simulacao_009_bus_2380_duas_simulacoes.py
#
# Regras deste script:
...
#     Simulacao 009 segmentos_pass1_sem_hvac_int
#   PASSO 2 (simulacao completa final):
#     Simulacao 009 segmentos
# =============================================================================
...
DIR_PASS1 = WORK_DIR / "Simulacao 009 segmentos_pass1_sem_hvac_int"
...
def build_circuit_pass1(dss: DSS, soc_init_pct: float, posto: str, seg_tag: str) -> str:
    ...
    # BESS em 2380
    dss.text("New line.2380_B011 phases=3 bus1=2380 bus2=B011 length=0.0150 units=km Geometry=UG_trefoil_25")
...
# %% Celula 003 - PASSO 1: monitor -> Curva_de_Carga_BESS_IN_sim_9.csv (SHIFT DIREITA)
# =============================================================================
# Entradas:
#   PASSO1/Monitor_BESS_IN_anual.csv
# Saida:
#   PASSO1/Curva_de_Carga_BESS_IN_sim_9.csv (Hora_decimal, P_3f_S, Q_3f_S)
# Remove:
#   PASSO1/Monitor_BESS_IN_anual.csv
# =============================================================================
...
OUT_FP = DIR_PASS1 / "Curva_de_Carga_BESS_IN_sim_9.csv"
...
# =============================================================================
# Entrada:
#   PASSO1/Curva_de_Carga_BESS_IN_sim_9.csv
# Saidas (WORK_DIR):
...
IN_FP = DIR_PASS1 / "Curva_de_Carga_BESS_IN_sim_9.csv"
...
DIR_PASS2 = WORK_DIR / "Simulacao 009 segmentos"
...
# =============================================================================
# Entrada:
#   Simulacao 009 segmentos/UFSM_Mon_<KEY>_s*.csv
# Saida:
#   Simulacao 009 segmentos/Monitor_<KEY>_anual.csv
# Remove:
...
# =============================================================================
# Entrada:
#   Simulacao 009 segmentos/EXP_MTR_GERAL_*.csv
# Saida:
#   Simulacao 009 segmentos/EXP_MTR_GERAL_anual.csv
# Remove:
#   EXP_MTR_GERAL_*.csv
# =============================================================================
...
#   Simulacao 009 segmentos/Monitor_<KEY>_anual.csv
# Saidas:
#   Simulacao 009 segmentos/Curva_de_Carga_<KEY>_sim_9.csv
# Remove:
...
for in_fp in files:
    ...
    out_fp = DIR_PASS2 / f"Curva_de_Carga_{mon_key}_sim_9.csv"
...
#   Simulacao 009 segmentos/EXP_MTR_GERAL_anual.csv
# Saida:
#   Simulacao 009 segmentos/Resumo_EnergyMeter_Geral_Sim9.csv
# =============================================================================
...
OUT_FP = DIR_PASS2 / "Resumo_EnergyMeter_Geral_Sim9.csv"
...
# %% Celula 010 - ENERGIA e DEMANDA MAX por mes e posto (SIM2 vs SIM9)
# =============================================================================
# Entradas:
...
#   SIM9: Simulacao 009 segmentos/Curva_de_Carga_GERAL_sim_9.csv (Hora_decimal, P_3f_S, Q_3f_S)
# Saida:
#   Simulacao 009 segmentos/01_Integralizacao_Mensal_SIM2_x_SIM9_2025.csv
# =============================================================================
...
SIM9_DIR = WORK_DIR / "Simulacao 009 segmentos"

META_FP = WORK_DIR / "00_dados_de_entrada_ano_letivo_2025_15min.csv"
SIM2_FP = SIM2_DIR / "Curva_de_Carga_GERAL_sim_2.csv"
SIM9_FP = SIM9_DIR / "Curva_de_Carga_GERAL_sim_9.csv"
OUT_FP  = SIM9_DIR / "01_Integralizacao_Mensal_SIM2_x_SIM9_2025.csv"
...
for fp in [META_FP, SIM2_FP, SIM9_FP]:
    if not fp.exists():
...
sim9 = pd.read_csv(SIM9_FP, encoding="utf-8-sig", sep=None, engine="python")

for name, dfx, req in [
    ("META", meta, ["Hora_decimal","Mes","Posto"]),
    ("SIM2", sim2, ["Hora_decimal","P_3f_S","Q_3f_S"]),
    ("SIM9", sim9, ["Hora_decimal","P_3f_S","Q_3f_S"]),
]:
...
sim9["k"] = make_k(sim9["Hora_decimal"])
...
sim9 = sim9[["k","P_3f_S","Q_3f_S"]].drop_duplicates(subset=["k"], keep="first").copy()

df2 = meta.merge(sim2, on="k", how="left").rename(columns={"P_3f_S":"P_SIM2","Q_3f_S":"Q_SIM2"})
df9 = meta.merge(sim9, on="k", how="left").rename(columns={"P_3f_S":"P_SIM9","Q_3f_S":"Q_SIM9"})

df2["E_P_SIM2_kWh"]   = df2["P_SIM2"] * DT_H
df2["E_Q_SIM2_kVArh"] = df2["Q_SIM2"] * DT_H
df9["E_P_SIM9_kWh"]   = df9["P_SIM9"] * DT_H
df9["E_Q_SIM9_kVArh"] = df9["Q_SIM9"] * DT_H
...
agg9 = df9.groupby(group_cols, dropna=False).agg(
    N_pontos_SIM9=("P_SIM9", lambda s: int(s.notna().sum())),
    Consumo_P_SIM9_kWh=("E_P_SIM9_kWh","sum"),
    Consumo_Q_SIM9_kVArh=("E_Q_SIM9_kVArh","sum"),
    DemandaMax_P_SIM9_kW=("P_SIM9","max"),
    DemandaMax_Q_SIM9_kVAr=("Q_SIM9","max"),
).reset_index()

agg = agg2.merge(agg9, on=group_cols, how="outer")
...
# %% Celula 011 - SIMULACAO 009: Tabela de custos (SIM2 vs SIM9 - 2025) com demandas contratadas diferentes
# =============================================================================
# Entrada:
#   Simulacao 009 segmentos/01_Integralizacao_Mensal_SIM2_x_SIM9_2025.csv
#
# Saidas (todas em Simulacao 009 segmentos):
#   Tabela_demanda_consumo_por_posto_SIM2_2025.csv
#   Tabela_demanda_consumo_por_posto_SIM9_2025.csv
#   Tabela_demanda_consumo_por_posto_SIM2_vs_SIM9_2025.csv   (economia)
# =============================================================================
...
WORK_DIR = Path(r"C:\Users\afole\OneDrive\Dissertacao2025")
SIM9_DIR = WORK_DIR / "Simulacao 009 segmentos"
SIM9_DIR.mkdir(parents=True, exist_ok=True)

IN_FP    = SIM9_DIR / "01_Integralizacao_Mensal_SIM2_x_SIM9_2025.csv"
OUT_SIM2 = SIM9_DIR / "Tabela_demanda_consumo_por_posto_SIM2_2025.csv"
OUT_SIM9 = SIM9_DIR / "Tabela_demanda_consumo_por_posto_SIM9_2025.csv"
OUT_CMP  = SIM9_DIR / "Tabela_demanda_consumo_por_posto_SIM2_vs_SIM9_2025.csv"

# -------------------------
# DEMANDAS CONTRATADAS POR SIMULACAO
# -------------------------
CONTR_SIM2 = {"DCFP": 5000.0, "DCP": 3000.0}
CONTR_SIM9 = {"DCFP": 5000.0, "DCP": 2200.0}  # 3000 - 800
...
print("DONE. Saved (Simulacao 009 segmentos):")
print(" -", OUT_SIM2)
print(" -", OUT_SIM9)
print(" -", OUT_CMP)
print("\nDemandas contratadas usadas:")
print(" - SIM2: DCFP=5000, DCP=3000")
print(" - SIM9: DCFP=5000, DCP=2200")
print("\nResumo anual:")
print(cmp.tail(1))
