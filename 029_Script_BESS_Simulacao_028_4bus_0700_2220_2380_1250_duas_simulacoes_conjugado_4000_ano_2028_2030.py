# -*- coding: utf-8 -*-
# =============================================================================
# 029_Script_BESS_Simulacao_028_4bus_0700_2220_2380_1250_duas_simulacoes_conjugado_4000_ano_2028_2030.py
#
# Regras deste script:
...
#     Simulacao 028 segmentos_pass1_sem_hvac_int_ano_2030
#   PASSO 2 (simulacao completa final):
#     Simulacao 028 segmentos_ano_2030
# =============================================================================
...
DIR_PASS1 = WORK_DIR / "Simulacao 028 segmentos_pass1_sem_hvac_int_ano_2030"
...
if LIMPAR_SAIDAS_ANTERIORES_PASS1:
    padroes_limpeza_pass1 = [
        ...
        "Curva_de_Carga_BESSM*_IN_sim_28.csv",
        "SOC_por_segmento.csv",
    ]
    removidos = 0
...
def build_circuit_pass1(
    ...
    dss.text('Redirect "UFSM_cargas_medidas_ano_2030.dss"')
    dss.text('Redirect "UFSM_cargas_especiais_ano_2030.dss"')
    dss.text('Redirect "UFSM_cargas_estimadas_com_residuo_ano_2030.dss"')
    ...
    # HVAC externo (obrigatorio) para os quatro bancos
...
#     SOC_15min_BESS_SIM28_2030.csv          (SOC dos quatro bancos a cada 15 min)
#
# Requisitos:
...
BESS1_E_2030_KWH = 6883.333
BESS2_E_2030_KWH = 6883.333
BESS3_E_2030_KWH = 6883.333
BESS4_E_2030_KWH = 3000.0
BESS_TOTAL_E_2030_KWH = BESS1_E_2030_KWH + BESS2_E_2030_KWH + BESS3_E_2030_KWH + BESS4_E_2030_KWH
...
DIR_PASS2 = WORK_DIR / "Simulacao 028 segmentos_ano_2030"
...
if LIMPAR_SAIDAS_ANTERIORES_PASS2:
    padroes_limpeza = [
        "UFSM_Mon_*_s*.csv",
        "Monitor_*_anual.csv",
        "Curva_de_Carga_*_sim_28.csv",
        "EXP_MTR_GERAL*.csv",
        "Resumo_EnergyMeter_Geral_Sim28.csv",
        "SOC_por_segmento.csv",
        "SOC_15min_BESS_SIM28_2030.csv",
        "01_Integralizacao_Mensal_SIM28_2030.csv",
        "Tabela_demanda_consumo_por_posto_SIM28_2030.csv",
    ]
    removidos = 0
...
def _run_segment_pass2(
    ...
    for step_idx in range(int(n_steps)):
        ...
        soc_m1_kwh = soc_m1_pct / 100.0 * BESS1_E_2030_KWH
        soc_m2_kwh = soc_m2_pct / 100.0 * BESS2_E_2030_KWH
        soc_m3_kwh = soc_m3_pct / 100.0 * BESS3_E_2030_KWH
        soc_m4_kwh = soc_m4_pct / 100.0 * BESS4_E_2030_KWH
        soc_total_kwh = soc_m1_kwh + soc_m2_kwh + soc_m3_kwh + soc_m4_kwh
        soc_total_pct_pond = 100.0 * soc_total_kwh / BESS_TOTAL_E_2030_KWH
...
soc_15min_fp = DIR_PASS2 / "SOC_15min_BESS_SIM28_2030.csv"
...
# =============================================================================
# Entrada:
#   Simulacao 028 segmentos_ano_2030/UFSM_Mon_<KEY>_s*.csv
# Saida:
#   Simulacao 028 segmentos_ano_2030/Monitor_<KEY>_anual.csv
# Remove:
...
# =============================================================================
# Entrada:
#   Simulacao 028 segmentos_ano_2030/EXP_MTR_GERAL_*.csv
# Saida:
#   Simulacao 028 segmentos_ano_2030/EXP_MTR_GERAL_anual.csv
# Remove:
#   EXP_MTR_GERAL_*.csv
# =============================================================================
...
#   Simulacao 028 segmentos_ano_2030/Monitor_<KEY>_anual.csv
# Saidas:
#   Simulacao 028 segmentos_ano_2030/Curva_de_Carga_<KEY>_sim_28.csv
# Remove:
...
#   Simulacao 028 segmentos_ano_2030/EXP_MTR_GERAL_anual.csv
# Saida:
#   Simulacao 028 segmentos_ano_2030/Resumo_EnergyMeter_Geral_Sim28.csv
# =============================================================================
...
# %% Celula 010 - ENERGIA e DEMANDA MAX por mes e posto (SIM28 - ANO 2030, BESS modular autofinanciado)
# =============================================================================
# Entradas:
#   META: 00_dados_de_entrada_ano_letivo_2025_15min.csv
#         Usado apenas como mapa anual de Hora_decimal, Mes e Posto.
#   SIM28: curva GERAL com BESS modular autofinanciado em operacao conjugada
#          para o ano 2030.
# Saida:
#   Simulacao 028 segmentos_ano_2030/01_Integralizacao_Mensal_SIM28_2030.csv
#
# Observacao:
#   Esta celula nao compara com SIM2/RGE nem com outra simulacao.
#   Ela apenas integraliza a simulacao conjugada do ano 2030 por mes e posto.
#   Configuracao do despacho:
#     BESS1: instalado em 2028, 1 MW / 7,0 MWh, operando a 80% da potencia,
#            degradado em 2030 por fator 0,983333333 para 786,667 kW / 6883,3 kWh.
...
#     Alvo fora-ponta = 4100 kW; permissao de recarga fora-ponta = 3588 kW.
#     Em fora-ponta, BESS1, BESS2 e BESS3 usam reserva de 42,8571% e BESS4 usa reserva de 100%.
#     Em ponta, os quatro bancos usam reserva minima tecnica de 2%.
#   A curva e lida diretamente pelo sufixo sim_28, sem fallback.
# =============================================================================
...
SIM28_DIR = WORK_DIR / f"Simulacao 028 segmentos_ano_{ANO_ANALISE}"

META_FP = WORK_DIR / "00_dados_de_entrada_ano_letivo_2025_15min.csv"
SIM28_FP = SIM28_DIR / "Curva_de_Carga_GERAL_sim_28.csv"
OUT_FP = SIM28_DIR / f"01_Integralizacao_Mensal_{SIM_TAG}_{ANO_ANALISE}.csv"
...
def gerar_curva_geral_sim23_se_necessario(sim23_fp: Path, dir_pass2: Path) -> None:
    """
    Garante a existencia de Curva_de_Carga_GERAL_sim_28.csv.
...
gerar_curva_geral_sim23_se_necessario(SIM28_FP, SIM28_DIR)
if not SIM28_FP.exists():
    raise FileNotFoundError(SIM28_FP)

meta = pd.read_csv(META_FP, dtype=str, encoding="utf-8-sig")
sim23 = pd.read_csv(SIM28_FP, encoding="utf-8-sig", sep=None, engine="python")
...
if len(sim23) != TOTAL_NPTS:
    raise ValueError(f"SIM28 gerou {len(sim23)} amostras anuais de 15 min; esperado {TOTAL_NPTS}.")

df23 = meta.merge(sim23, on="k", how="left").rename(
    columns={"P_3f_S": "P_SIM28", "Q_3f_S": "Q_SIM28"}
)

faltas_p = int(df23["P_SIM28"].isna().sum())
faltas_q = int(df23["Q_SIM28"].isna().sum())
if faltas_p or faltas_q:
    raise ValueError(
        f"Falha de alinhamento por Hora_decimal/k: "
        f"P_SIM28 faltas={faltas_p}, Q_SIM28 faltas={faltas_q}"
    )

df23["E_P_SIM28_kWh"] = df23["P_SIM28"] * DT_H
df23["E_Q_SIM28_kVArh"] = df23["Q_SIM28"] * DT_H
...
print("SIM28 usado:", SIM28_FP)
print(agg[["Mes", "Posto", "N_pontos_META", "N_pontos_SIM28", "DemandaMax_P_SIM28_kW"]])

# %% Celula 011 - SIMULACAO 028: tabela de custos (SIM28 - ANO 2030, BESS modular autofinanciado)
# =============================================================================
# Entrada:
#   Simulacao 028 segmentos_ano_2030/01_Integralizacao_Mensal_SIM28_2030.csv
#
# Saida:
#   Simulacao 028 segmentos_ano_2030/Tabela_demanda_consumo_por_posto_SIM28_2030.csv
#
# Demandas contratadas do ano 2030:
#   DCFP contratada com BESS = 4000 * 1,025 = 4100 kW
#   Alvo operacional fora-ponta do despacho = 4100 kW
#   Permissao para recarga fora-ponta = 3500 * 1,025 = 3588 kW
#   DCP sem BESS = 3000 * 1,025 = 3100 kW
#   DCP com BESS = max(0, 3100 - (786,667 + 786,667 + 786,667 + 800)) = 0 kW
#
...
#     Em 2030: fator 0,983333333 -> 786,667 kW / 6883,3 kWh.
...
#   Os bancos possuem energias efetivas diferentes em 2030.
#   BESS1, BESS2 e BESS3 foram dimensionados com excedente para suprir os picos fora-ponta.
#   BESS4 possui somente os 3 MWh nominais necessarios para a ponta.
#   Em fora-ponta, BESS1, BESS2 e BESS3 usam reserva de 42,8571% e BESS4 usa reserva de 100%.
#   Em ponta, os quatro bancos usam reserva minima tecnica de 2%.
#
# Observacao:
...
IN_FP = SIM28_DIR / f"01_Integralizacao_Mensal_{SIM_TAG}_{ANO_ANALISE}.csv"
OUT_SIM28 = SIM28_DIR / f"Tabela_demanda_consumo_por_posto_{SIM_TAG}_{ANO_ANALISE}.csv"

# -------------------------
# Demandas contratadas e alvos do despacho
# -------------------------
# Fora-ponta: 4000 * 1,025 = 4100 kW
# Ponta sem BESS: 3000 * 1,025 = 3100 kW
# Ponta com BESS: max(0, 3100 - (786,667 + 786,667 + 786,667 + 800)) = 0 kW
DCP_SEM_BESS = 3100.0
DCFP_COM_BESS = 4100.0

KW_TARGET_FP = 4100.0
KW_TARGET_LOW_FP = 3588.0
...
BESS1_FATOR_DEGRADACAO_2030 = 0.9833333333333333
BESS1_P_2030_KW = BESS1_P_OPERACIONAL_2028_KW * BESS1_FATOR_DEGRADACAO_2030
BESS1_E_2030_KWH = BESS1_E_NOMINAL_KWH * BESS1_FATOR_DEGRADACAO_2030
BESS1_E_EXCEDENTE_FP_2030_KWH = BESS1_E_EXCEDENTE_FP_NOMINAL_KWH * BESS1_FATOR_DEGRADACAO_2030
...
BESS2_FATOR_DEGRADACAO_2030 = 0.9833333333333333
BESS2_P_2030_KW = BESS2_P_OPERACIONAL_2028_KW * BESS2_FATOR_DEGRADACAO_2030
BESS2_E_2030_KWH = BESS2_E_NOMINAL_KWH * BESS2_FATOR_DEGRADACAO_2030
BESS2_E_EXCEDENTE_FP_2030_KWH = BESS2_E_EXCEDENTE_FP_NOMINAL_KWH * BESS2_FATOR_DEGRADACAO_2030
...
BESS3_FATOR_DEGRADACAO_2030 = 0.9833333333333333
BESS3_P_2030_KW = BESS3_P_OPERACIONAL_2028_KW * BESS3_FATOR_DEGRADACAO_2030
BESS3_E_2030_KWH = BESS3_E_NOMINAL_KWH * BESS3_FATOR_DEGRADACAO_2030
BESS3_E_EXCEDENTE_FP_2030_KWH = BESS3_E_EXCEDENTE_FP_NOMINAL_KWH * BESS3_FATOR_DEGRADACAO_2030

BESS4_ANO_INSTALACAO = 2030
BESS4_P_NOMINAL_KW = 1000.0
BESS4_P_OPERACIONAL_2030_KW = 800.0
BESS4_E_NOMINAL_KWH = 3000.0
BESS4_P_2030_KW = BESS4_P_OPERACIONAL_2030_KW
BESS4_E_2030_KWH = BESS4_E_NOMINAL_KWH

BESS_P_TOTAL_KW = BESS1_P_2030_KW + BESS2_P_2030_KW + BESS3_P_2030_KW + BESS4_P_2030_KW
BESS_E_TOTAL_KWH = BESS1_E_2030_KWH + BESS2_E_2030_KWH + BESS3_E_2030_KWH + BESS4_E_2030_KWH
BESS_E_NOMINAL_TOTAL_KWH = BESS1_E_NOMINAL_KWH + BESS2_E_NOMINAL_KWH + BESS3_E_NOMINAL_KWH + BESS4_E_NOMINAL_KWH
...
#   BESS4 reserva 100% para a ponta e nao participa do peak-shave fora-ponta.
# PONTA:
#   BESS1, BESS2, BESS3 e BESS4 ficam liberados ate a reserva minima tecnica de 2%.
...
BESS1_E_RESERVA_PONTA_2030_KWH = BESS1_E_2030_KWH * BESS1_RESERVA_PONTA_PCT / 100.0
BESS2_E_RESERVA_PONTA_2030_KWH = BESS2_E_2030_KWH * BESS2_RESERVA_PONTA_PCT / 100.0
BESS3_E_RESERVA_PONTA_2030_KWH = BESS3_E_2030_KWH * BESS3_RESERVA_PONTA_PCT / 100.0
BESS4_E_RESERVA_PONTA_2030_KWH = BESS4_E_2030_KWH * BESS4_RESERVA_PONTA_PCT / 100.0
BESS_E_RESERVA_PONTA_KWH = (
    BESS1_E_RESERVA_PONTA_2030_KWH
    + BESS2_E_RESERVA_PONTA_2030_KWH
    + BESS3_E_RESERVA_PONTA_2030_KWH
    + BESS4_E_RESERVA_PONTA_2030_KWH
)
BESS_E_DISPONIVEL_FP_KWH = (
    (BESS1_E_2030_KWH - BESS1_E_RESERVA_PONTA_2030_KWH)
    + (BESS2_E_2030_KWH - BESS2_E_RESERVA_PONTA_2030_KWH)
    + (BESS3_E_2030_KWH - BESS3_E_RESERVA_PONTA_2030_KWH)
)

DCP_COM_BESS = max(0.0, DCP_SEM_BESS - BESS_P_TOTAL_KW)

CONTR_SIM28 = {"DCFP": DCFP_COM_BESS, "DCP": DCP_COM_BESS}
...
if not IN_FP.exists():
    curva_fp = SIM28_DIR / "Curva_de_Carga_GERAL_sim_28.csv"
    raise FileNotFoundError(
...
tab28 = tabela_custos(
    df,
    SIM_TAG,
    DCFP=CONTR_SIM28["DCFP"],
    DCP=CONTR_SIM28["DCP"],
)

tab28.to_csv(OUT_SIM28, index=False, encoding="utf-8")

print("DONE. Saved (Simulacao 028 - BESS modular autofinanciado - 2030 - conjugado 4000):")
print(" -", OUT_SIM28)
print("\nDemandas contratadas e alvos usados em 2030:")
...
print(f" - Potencia BESS1 2030 = {BESS1_P_2030_KW:.0f} kW")
print(f" - Potencia BESS2 2030 = {BESS2_P_2030_KW:.0f} kW")
print(f" - Potencia BESS3 2030 = {BESS3_P_2030_KW:.0f} kW")
print(f" - Potencia BESS4 2030 = {BESS4_P_2030_KW:.0f} kW")
print(f" - Energia nominal total instalada = {BESS_E_NOMINAL_TOTAL_KWH:.0f} kWh")
print(f" - Energia disponivel em 2030 = {BESS_E_TOTAL_KWH:.0f} kWh")
print(f" - Energia BESS1 2030 = {BESS1_E_2030_KWH:.0f} kWh")
print(f" - Energia BESS2 2030 = {BESS2_E_2030_KWH:.0f} kWh")
print(f" - Energia BESS3 2030 = {BESS3_E_2030_KWH:.0f} kWh")
print(f" - Energia BESS4 2030 = {BESS4_E_2030_KWH:.0f} kWh")
...
print(f" - Excedente degradado BESS1 para fora-ponta = {BESS1_E_EXCEDENTE_FP_2030_KWH:.0f} kWh")
print(f" - Excedente degradado BESS2 para fora-ponta = {BESS2_E_EXCEDENTE_FP_2030_KWH:.0f} kWh")
print(f" - Excedente degradado BESS3 para fora-ponta = {BESS3_E_EXCEDENTE_FP_2030_KWH:.0f} kWh")
print(f" - Energia reservada para ponta BESS1 = {BESS1_E_RESERVA_PONTA_2030_KWH:.0f} kWh")
print(f" - Energia reservada para ponta BESS2 = {BESS2_E_RESERVA_PONTA_2030_KWH:.0f} kWh")
print(f" - Energia reservada para ponta BESS3 = {BESS3_E_RESERVA_PONTA_2030_KWH:.0f} kWh")
print(f" - Energia reservada para ponta BESS4 = {BESS4_E_RESERVA_PONTA_2030_KWH:.0f} kWh")
...
print("\nResumo anual:")
print(tab28[tab28["MES"].isin(["TOTAIS", "CTOTAL", "R$TOTAL", "DCFP", "DCP"])])
