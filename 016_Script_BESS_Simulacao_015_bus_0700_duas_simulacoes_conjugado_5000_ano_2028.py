# -*- coding: utf-8 -*-
# =============================================================================
# 016_Script_BESS_Simulacao_015_bus_0700_duas_simulacoes_conjugado_5000_ano_2028.py
#
# Regras deste script:
...
#     Simulacao 015 segmentos_pass1_sem_hvac_int_ano_2028
#   PASSO 2 (simulacao completa final):
#     Simulacao 015 segmentos_ano_2028
# =============================================================================

# %% Celula 001 - PASSO 1: simulacao segmentada somente com monitor do BESS (sem EnergyMeter)
# ======================================================================================
# Saidas (PASSO 1):
#   Simulacao 015 segmentos_pass1_sem_hvac_int_ano_2028/
#     UFSM_Mon_BESS_IN_sNNNNN_<POSTO>.csv   (1 por segmento)
...
DIR_PASS1 = WORK_DIR / "Simulacao 015 segmentos_pass1_sem_hvac_int_ano_2028"
DIR_PASS1.mkdir(parents=True, exist_ok=True)
os.chdir(WORK_DIR)


def build_circuit_pass1(dss: DSS, soc_init_pct: float, posto: str, seg_tag: str) -> str:
    ...
    dss.text(
        f"New Storage.{STORAGE_NAME} phases=3 bus1=B013 kv=0.38 "
        f"kWrated=800 kWhrated=4500 Effcurve=Eff %EffCharge=95 %EffDischarge=95 "
        f"%reserve={reserve_pct} dispmode=external"
    )

    soc_init_pct = float(soc_init_pct)
    soc_init_pct = max(0.0, min(100.0, soc_init_pct))
    dss.text(f"Storage.{STORAGE_NAME}.%stored={soc_init_pct}")
    dss.text(f"Storage.{STORAGE_NAME}.%reserve={reserve_pct}")

    dss.text(
        f"New StorageController.{STORAGE_CTRL} Element=line.0015_0016 terminal=1 "
        f"ElementList=[{STORAGE_NAME}] MonPhase=AVG "
        f"ModeDischarge=PeakShave ModeCharge=PeakShaveLow "
        f"kWtarget={kWtarget} kWtargetLow={kWtargetLow}"
    )

    mname = f"BESS_IN_{seg_tag}"
    dss.text(f"New Monitor.{mname} element=line.B011_B012 terminal=1 mode=1 ppolar=no")
...
def hour_sec_from_hora_decimal(h: float, dt_h: float) -> tuple[int, int, float]:
    # quantiza para evitar lixo numerico
...
def run_one_segment_pass1(
    ...
    # >>> PRINT NO INICIO (antes do Solve) com SOC inicial do circuito
    print(
        f"[SOLVE] seg={int(seg_idx):05d} posto={posto} "
        f"hora_ini={hora_q:.2f}h -> Hour={hour_int} Sec={sec_int} "
        f"DT={DT_H:.2f}h n_steps={int(n_steps)} | SOC_ini={float(soc_ini):.3f}%"
    )
    ...
    dst = export_dir / f"UFSM_Mon_BESS_IN_s{int(seg_idx):05d}_{posto}.csv"
    if dst.exists():
        dst.unlink()
    fp.rename(dst)
...
for i, row in segments.iterrows():
    ...
    dss = DSS()
    mon_name = build_circuit_pass1(dss, soc_atual, posto, seg_tag)

    # >>> SOC "real" do circuito antes do Solve (para print e log)
    soc_ini = get_soc_pct(dss)

    mon_fp = run_one_segment_pass1(
        dss=dss,
    ...
    soc_log.append(
        {
            ...
            "n_steps": int(n_steps),
            "soc_inicial_pct": float(soc_ini),
            "soc_final_pct": float(soc_final),
            "monitor_csv": str(mon_fp),
        }
    )

    print(
        f"[OK] {seg_tag}: hora_ini={hora_ini:.2f}h steps={n_steps} | "
        f"SOC {soc_ini:.3f}% -> {soc_final:.3f}% | {mon_fp.name}"
    )
    soc_atual = soc_final
...
# %% Celula 002 - PASSO 1: emenda somente o monitor BESS_IN
# =============================================================================
# Entrada:
#   PASSO1/UFSM_Mon_BESS_IN_s*.csv
# Saida:
#   PASSO1/Monitor_BESS_IN_anual.csv
# Remove:
#   UFSM_Mon_BESS_IN_s*.csv
# =============================================================================
...
OUT_FP = DIR_PASS1 / "Monitor_BESS_IN_anual.csv"
files = sorted(DIR_PASS1.glob("UFSM_Mon_BESS_IN_s*.csv"))
if not files:
    raise FileNotFoundError(f"Nenhum UFSM_Mon_BESS_IN_s*.csv em {DIR_PASS1}")

dfs = [pd.read_csv(fp, encoding="utf-8-sig", sep=None, engine="python") for fp in files]
merged = pd.concat(dfs, ignore_index=True)

if len(merged) != 35040:
    raise ValueError(f"Emenda PASSO1 gerou {len(merged)} linhas (esperado 35040).")
...
for fp in files:
    fp.unlink()
print("[CLEAN] Segmentados removidos (BESS_IN).")

# %% Celula 003 - PASSO 1: monitor -> Curva_de_Carga_BESS_IN_sim_15.csv (SHIFT DIREITA)
# =============================================================================
# Entradas:
#   PASSO1/Monitor_BESS_IN_anual.csv
# Saida:
#   PASSO1/Curva_de_Carga_BESS_IN_sim_15.csv (Hora_decimal, P_3f_S, Q_3f_S)
# Remove:
#   PASSO1/Monitor_BESS_IN_anual.csv
# =============================================================================
...
IN_FP  = DIR_PASS1 / "Monitor_BESS_IN_anual.csv"
OUT_FP = DIR_PASS1 / "Curva_de_Carga_BESS_IN_sim_15.csv"
...
if mon[cols_vals].isna().any().any():
    raise ValueError("NaN encontrado nas colunas eletricas.")

if len(mon) != TOTAL_NPTS:
    raise ValueError(f"Monitor anual com tamanho inesperado: {len(mon)} (esperado {TOTAL_NPTS}).")
...
print("[OK] Curva trifasica salva em:", OUT_FP)
print("[CLEAN] Monitor anual removido:", IN_FP.name)
print("[INFO] SHIFT DIREITA aplicado (duplica 1a linha, remove ultima).")

# %% Celula 004 - PASSO 1: gera LoadShape do HVAC interno
# =============================================================================
# Entrada:
#   PASSO1/Curva_de_Carga_BESS_IN_sim_15.csv
# Saidas (WORK_DIR):
#   BESS_HVAC_INT_PkW_2025.csv
#   BESS_HVAC_INT_LoadShape_2025.csv
#   BESS_HVAC_INT_AUD_2025.csv
# =============================================================================
...
IN_FP = DIR_PASS1 / "Curva_de_Carga_BESS_IN_sim_15.csv"

ARQ_OUT_PKW = WORK_DIR / "BESS_HVAC_INT_PkW_2025.csv"
ARQ_OUT_PU  = WORK_DIR / "BESS_HVAC_INT_LoadShape_2025.csv"
ARQ_AUD     = WORK_DIR / "BESS_HVAC_INT_AUD_2025.csv"
...
for c in ["Hora_decimal", "P_3f_S"]:
    if c not in df.columns:
        raise ValueError(f"Coluna {c!r} ausente em {IN_FP}. Colunas: {list(df.columns)}")
...
if len(p_bess_kW) != TOTAL_NPTS or len(hora_decimal) != TOTAL_NPTS:
    raise ValueError(f"Tamanho inesperado: hora={len(hora_decimal)}, P={len(p_bess_kW)} (esperado {TOTAL_NPTS}).")
...
MON_SPECS = {
    ...
    "BESS_IN":     {"element": "line.B011_B012", "terminal": 1},
...
DIR_PASS2 = WORK_DIR / "Simulacao 015 segmentos_ano_2028"
...
def _build_circuit_pass2(dss: DSS, soc_init_pct: float, posto: str, seg_tag: str) -> dict[str, str]:
    posto = str(posto).strip().upper()
    if posto not in TARGETS:
        raise ValueError(f"Posto invalido: {posto}")
    ...
    dss.text("New LoadShape.BESS_HVAC_INT_LoadShape_2025 npts=35040 interval=0 CSVFile=BESS_HVAC_INT_LoadShape_2025.csv")
    dss.text("New load.BESSM1_AC_int phases=3 model=1 bus=B013 kv=0.38 kw=1000 conn=wye Yearly=BESS_HVAC_INT_LoadShape_2025")
...
def _get_soc_pct(dss: DSS) -> float:
    resp = dss.text(f"? Storage.{STORAGE_NAME}.%stored")
    return float(str(resp).strip())
...
for i, row in segments.iterrows():
    ...
    dss = DSS()
    mon_names = _build_circuit_pass2(dss=dss, soc_init_pct=soc_atual, posto=posto, seg_tag=seg_tag)

    # >>> SOC "real" do circuito antes do Solve (para print e log)
    soc_ini = _get_soc_pct(dss)
    ...
    print(
        f"[OK] {seg_tag}: hora_ini={hora_ini:.2f}h steps={n_steps} | "
        f"SOC {soc_ini:.3f}% -> {soc_final:.3f}% | meter={mtr_fp.name}"
    )
    soc_atual = soc_final
...
# =============================================================================
# Entrada:
#   Simulacao 015 segmentos_ano_2028/UFSM_Mon_<KEY>_s*.csv
# Saida:
#   Simulacao 015 segmentos_ano_2028/Monitor_<KEY>_anual.csv
# Remove:
...
# =============================================================================
# Entrada:
#   Simulacao 015 segmentos_ano_2028/EXP_MTR_GERAL_*.csv
# Saida:
#   Simulacao 015 segmentos_ano_2028/EXP_MTR_GERAL_anual.csv
# Remove:
#   EXP_MTR_GERAL_*.csv
# =============================================================================
...
#   Simulacao 015 segmentos_ano_2028/Monitor_<KEY>_anual.csv
# Saidas:
#   Simulacao 015 segmentos_ano_2028/Curva_de_Carga_<KEY>_sim_15.csv
# Remove:
...
for in_fp in files:
    ...
    out_fp = DIR_PASS2 / f"Curva_de_Carga_{mon_key}_sim_15.csv"
...
#   Simulacao 015 segmentos_ano_2028/EXP_MTR_GERAL_anual.csv
# Saida:
#   Simulacao 015 segmentos_ano_2028/Resumo_EnergyMeter_Geral_Sim15.csv
# =============================================================================
...
OUT_FP = DIR_PASS2 / "Resumo_EnergyMeter_Geral_Sim15.csv"
...
# %% Celula 010 - ENERGIA e DEMANDA MAX por mes e posto (SIM15 - ANO 2028, 1 BESS 1 MW / 4,5 MWh)
# =============================================================================
# Entradas:
#   META: 00_dados_de_entrada_ano_letivo_2025_15min.csv
#         Usado apenas como mapa anual de Hora_decimal, Mes e Posto.
#   SIM15: curva GERAL com 1 BESS em operacao conjugada para o ano 2028
# Saida:
#   Simulacao 015 segmentos_ano_2028/01_Integralizacao_Mensal_SIM15_2028.csv
#
# Observacao:
#   Esta celula nao compara com SIM2/RGE nem com outra simulacao.
#   Ela apenas integraliza a simulacao conjugada com 1 BESS do ano 2028 por mes e posto.
# =============================================================================
...
SIM15_DIR = WORK_DIR / f"Simulacao 015 segmentos_ano_{ANO_ANALISE}"

META_FP = WORK_DIR / "00_dados_de_entrada_ano_letivo_2025_15min.csv"
SIM15_FP = SIM15_DIR / "Curva_de_Carga_GERAL_sim_15.csv"
OUT_FP = SIM15_DIR / f"01_Integralizacao_Mensal_{SIM_TAG}_{ANO_ANALISE}.csv"
...
for fp in [META_FP, SIM15_FP]:
    if not fp.exists():
        raise FileNotFoundError(fp)

meta = pd.read_csv(META_FP, dtype=str, encoding="utf-8-sig")
sim15 = pd.read_csv(SIM15_FP, encoding="utf-8-sig", sep=None, engine="python")

meta = clean_cols(meta)
sim15 = clean_cols(sim15)

for name, dfx, req in [
    ("META", meta, ["Hora_decimal", "Mes", "Posto"]),
    (SIM_TAG, sim15, ["Hora_decimal", "P_3f_S", "Q_3f_S"]),
]:
...
sim15["Hora_decimal"] = pd.to_numeric(sim15["Hora_decimal"], errors="coerce")
sim15["P_3f_S"] = pd.to_numeric(sim15["P_3f_S"], errors="coerce")
sim15["Q_3f_S"] = pd.to_numeric(sim15["Q_3f_S"], errors="coerce")
sim15.dropna(subset=["Hora_decimal", "P_3f_S", "Q_3f_S"], inplace=True)

meta["k"] = make_k(meta["Hora_decimal"])
sim15["k"] = make_k(sim15["Hora_decimal"])

meta = meta[["k", "Mes", "Posto"]].drop_duplicates(subset=["k"], keep="first").copy()
sim15 = sim15[["k", "P_3f_S", "Q_3f_S"]].drop_duplicates(subset=["k"], keep="first").copy()

df15 = meta.merge(sim15, on="k", how="left").rename(
    columns={"P_3f_S": "P_SIM15", "Q_3f_S": "Q_SIM15"}
)

faltas_p = int(df15["P_SIM15"].isna().sum())
faltas_q = int(df15["Q_SIM15"].isna().sum())
if faltas_p or faltas_q:
    raise ValueError(
        f"Falha de alinhamento por Hora_decimal/k: "
        f"P_SIM15 faltas={faltas_p}, Q_SIM15 faltas={faltas_q}"
    )

df15["E_P_SIM15_kWh"] = df15["P_SIM15"] * DT_H
df15["E_Q_SIM15_kVArh"] = df15["Q_SIM15"] * DT_H

group_cols = ["Mes", "Posto"]

agg15 = df15.groupby(group_cols, dropna=False).agg(
    N_pontos_SIM15=("P_SIM15", lambda s: int(s.notna().sum())),
    Consumo_P_SIM15_kWh=("E_P_SIM15_kWh", "sum"),
    Consumo_Q_SIM15_kVArh=("E_Q_SIM15_kVArh", "sum"),
    DemandaMax_P_SIM15_kW=("P_SIM15", "max"),
    DemandaMax_Q_SIM15_kVAr=("Q_SIM15", "max"),
).reset_index()

meta_counts = meta.groupby(group_cols, dropna=False).agg(N_pontos_META=("Posto", "size")).reset_index()
agg = agg15.merge(meta_counts, on=group_cols, how="left")
...
print("SIM15 usado:", SIM15_FP)
print(agg[["Mes", "Posto", "N_pontos_META", "N_pontos_SIM15", "DemandaMax_P_SIM15_kW"]])

# %% Celula 011 - SIMULACAO 015: tabela de custos (SIM15 - ANO 2028, 1 BESS 1 MW / 4,5 MWh)
# =============================================================================
# Entrada:
#   Simulacao 015 segmentos_ano_2028/01_Integralizacao_Mensal_SIM15_2028.csv
#
# Saida:
#   Simulacao 015 segmentos_ano_2028/Tabela_demanda_consumo_por_posto_SIM15_2028.csv
#
# Demandas contratadas do ano 2028:
#   DCFP = 5000 kW * (1 + 3 * 0,5%) = 5075 kW
#   DCP sem BESS = 3000 kW * (1 + 3 * 0,5%) = 3045 kW
#   DCP com 1 BESS = 3045 - 800 = 2245 kW
#
# Observacao:
...
SIM15_DIR = WORK_DIR / f"Simulacao 015 segmentos_ano_{ANO_ANALISE}"
SIM15_DIR.mkdir(parents=True, exist_ok=True)

IN_FP = SIM15_DIR / f"01_Integralizacao_Mensal_{SIM_TAG}_{ANO_ANALISE}.csv"
OUT_SIM15 = SIM15_DIR / f"Tabela_demanda_consumo_por_posto_{SIM_TAG}_{ANO_ANALISE}.csv"
...
# - O BESS e instalado no proprio ano de analise, com potencia operacional de 800 kW.
# - O banco considerado e de 1 MW / 4,5 MWh, operando com limite de potencia de 80%.
DCP_BASE_2025 = 3000.0
DCFP_BASE_2025 = 5000.0
CRESC_LINEAR_ANUAL = 0.005

BESS_P_NOMINAL_KW = 1000.0
BESS_P_OPERACIONAL_KW = 800.0
BESS_E_NOMINAL_KWH = 4500.0
...
DCP_COM_BESS = max(0.0, DCP_SEM_BESS - BESS_P_OPERACIONAL_KW) # 2245 kW em 2028

CONTR_SIM15 = {"DCFP": DCFP_COM_BESS, "DCP": DCP_COM_BESS}
...
tab15 = tabela_custos(
    df,
    SIM_TAG,
    DCFP=CONTR_SIM15["DCFP"],
    DCP=CONTR_SIM15["DCP"],
)

tab15.to_csv(OUT_SIM15, index=False, encoding="utf-8")

print("DONE. Saved (Simulacao 015 - 1 BESS - 2028 - conjugado):")
print(" -", OUT_SIM15)
print("\nDemandas contratadas usadas em 2028:")
print(f" - DCFP = {DCFP_COM_BESS:.0f} kW")
print(f" - DCP sem BESS = {DCP_SEM_BESS:.0f} kW")
print(f" - DCP com 1 BESS = {DCP_COM_BESS:.0f} kW")
print(f" - Potencia operacional do BESS = {BESS_P_OPERACIONAL_KW:.0f} kW")
print(f" - Energia nominal do BESS = {BESS_E_NOMINAL_KWH:.0f} kWh")
print("\nResumo anual:")
print(tab15[tab15["MES"].isin(["TOTAIS", "CTOTAL", "R$TOTAL", "DCFP", "DCP"])])
