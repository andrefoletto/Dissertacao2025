# -*- coding: utf-8 -*-
# =============================================================================
# 017_Script_BESS_Simulacao_016_2bus_0700_2220_duas_simulacoes_conjugado_5000_ano_2028_2033_2047.py
#
# Regras deste script:
...
#     Simulacao 016 segmentos_pass1_sem_hvac_int_ano_2033_2047
#   PASSO 2 (simulacao completa final):
#     Simulacao 016 segmentos_ano_2033_2047
# =============================================================================
...
# Parametros dos BESS degradados para o ano de 2047.
BESS1_P_KW_2047 = 673.0
BESS1_E_KWH_2047 = 3788.0
BESS2_P_KW_2047 = 707.0
BESS2_E_KWH_2047 = 2650.0

# Reservas operacionais por banco:
# - BESS1 conserva 3000/4500 = 66,6667% para a ponta e usa o excedente para fora-ponta.
# - BESS2 conserva 100% para a ponta e nao deve descarregar fora-ponta.
# - Na ponta, ambos sao liberados ate a reserva minima tecnica.
BESS1_RESERVA_PONTA_PCT = 100.0 * 3000.0 / 4500.0
BESS2_RESERVA_PONTA_PCT = 100.0
BESS_RESERVA_MIN_PONTA_PCT = 2.0


def reserve_pct_por_posto(posto: str) -> tuple[float, float]:
    posto = str(posto).strip().upper()
    if posto == "FORA_PONTA":
        return BESS1_RESERVA_PONTA_PCT, BESS2_RESERVA_PONTA_PCT
    if posto == "PONTA":
        return BESS_RESERVA_MIN_PONTA_PCT, BESS_RESERVA_MIN_PONTA_PCT
...
DIR_PASS1 = WORK_DIR / "Simulacao 016 segmentos_pass1_sem_hvac_int_ano_2033_2047"
...
if LIMPAR_SAIDAS_ANTERIORES_PASS1:
    padroes_limpeza_pass1 = [
        ...
        "Curva_de_Carga_BESSM*_IN_sim_16.csv",
        "SOC_por_segmento.csv",
    ]
    removidos = 0
...
def build_circuit_pass1(
    ...
    reserve_m1_pct, reserve_m2_pct = reserve_pct_por_posto(posto)
    ...
    dss.text(
        ...
        f"kWrated={BESS1_P_KW_2047:.0f} kWhrated={BESS1_E_KWH_2047:.0f} Effcurve=Eff %EffCharge=95 %EffDischarge=95 "
        f"%reserve={reserve_m1_pct} dispmode=external"
    )
    dss.text(
        "New Storage.BESSM2 phases=3 bus1=B023 kv=0.38 "
        f"kWrated={BESS2_P_KW_2047:.0f} kWhrated={BESS2_E_KWH_2047:.0f} Effcurve=Eff %EffCharge=95 %EffDischarge=95 "
...
# Normalizacao do vetor anual de 15 min.
# Um ano nao bissexto deve ter 35040 amostras: k = 0...35039.
# Se o arquivo de posto vier com linha final em Hora_decimal = 8760, ela e descartada.
    ...
    soc_log.append(
        {
            ...
            "n_steps": int(n_steps),
            "reserve_m1_pct": float(reserve_pct_por_posto(posto)[0]),
            "reserve_m2_pct": float(reserve_pct_por_posto(posto)[1]),
...
# %% Celula 003 - PASSO 1: monitores -> Curva_de_Carga_BESSM1_IN_sim_16.csv e BESSM2_IN (SHIFT DIREITA)
# =============================================================================
# Entradas:
#   PASSO1/Monitor_BESSM1_IN_anual.csv
#   PASSO1/Monitor_BESSM2_IN_anual.csv
# Saidas:
#   PASSO1/Curva_de_Carga_BESSM1_IN_sim_16.csv
#   PASSO1/Curva_de_Carga_BESSM2_IN_sim_16.csv
# Remove:
#   PASSO1/Monitor_BESSM1_IN_anual.csv
#   PASSO1/Monitor_BESSM2_IN_anual.csv
# =============================================================================
...
for mon_base in ["BESSM1_IN", "BESSM2_IN"]:
    ...
    out_fp = DIR_PASS1 / f"Curva_de_Carga_{mon_base}_sim_16.csv"
...
for mon_base, prefix in [("BESSM1_IN", "BESSM1"), ("BESSM2_IN", "BESSM2")]:
    in_fp = DIR_PASS1 / f"Curva_de_Carga_{mon_base}_sim_16.csv"
...
DIR_PASS2 = WORK_DIR / "Simulacao 016 segmentos_ano_2033_2047"
...
if LIMPAR_SAIDAS_ANTERIORES_PASS2:
    padroes_limpeza = [
        "UFSM_Mon_*_s*.csv",
        "Monitor_*_anual.csv",
        "Curva_de_Carga_*_sim_16.csv",
        "EXP_MTR_GERAL*.csv",
        "Resumo_EnergyMeter_Geral_Sim16.csv",
        "SOC_por_segmento.csv",
        "SOC_15min_BESS_SIM16_2047.csv",
        "01_Integralizacao_Mensal_SIM16_2047.csv",
        "Tabela_demanda_consumo_por_posto_SIM16_2047.csv",
    ]
    removidos = 0
...
def _run_segment_pass2(
    ...
    soc_ini_m1: float,
    soc_ini_m2: float,
) -> tuple[dict[str, Path], Path, list[dict[str, float | int | str]]]:
    ...
    # Se a instancia DSS for compartilhada internamente pela biblioteca, o estado
    # final continua consistente, pois a passada SOC parte do mesmo SOC inicial e
    # percorre exatamente o mesmo segmento.
    ...
    for j in range(int(n_steps)):
        hora_step = round(hora_q + j * DT_H, 10)
        hour_step, sec_step, hora_step_q = hour_sec_from_hora_decimal(hora_step, DT_H)

        dss_soc.text(f"Set Mode=Yearly Hour={hour_step} Sec={sec_step} StepSize={DT_H}h Number=1")
        dss_soc.text("Solve")

        soc_m1 = _get_soc_pct(dss_soc, "BESSM1")
        soc_m2 = _get_soc_pct(dss_soc, "BESSM2")
        soc_m1_kwh = soc_m1 * BESS1_E_KWH_2047 / 100.0
        soc_m2_kwh = soc_m2 * BESS2_E_KWH_2047 / 100.0
        soc_total_kwh = soc_m1_kwh + soc_m2_kwh
        soc_total_pct = 100.0 * soc_total_kwh / (BESS1_E_KWH_2047 + BESS2_E_KWH_2047)

        soc_rows.append(
            {
                "k": int(round(hora_step_q / DT_H)),
                "Hora_decimal": float(hora_step_q),
                "Hora_decimal_fim": float(round(hora_step_q + DT_H, 10)),
                "posto": posto,
                "reserve_BESSM1_pct": float(reserve_pct_por_posto(posto)[0]),
                "reserve_BESSM2_pct": float(reserve_pct_por_posto(posto)[1]),
                "SOC_BESSM1_pct": float(soc_m1),
                "SOC_BESSM2_pct": float(soc_m2),
                "SOC_BESSM1_kWh": float(soc_m1_kwh),
                "SOC_BESSM2_kWh": float(soc_m2_kwh),
                "SOC_total_kWh": float(soc_total_kwh),
                "SOC_total_pct_ponderado": float(soc_total_pct),
            }
        )

    dss_soc.text(f'cd "{WORK_DIR}"')
    dss.text(f'cd "{WORK_DIR}"')
    return mon_paths, mtr_fp, soc_rows
...
for i, row in segments.iterrows():
    ...
    mon_paths, mtr_fp, soc_rows = _run_segment_pass2(
        dss=dss,
        hora_ini=hora_ini,
        n_steps=n_steps,
        mon_names=mon_names,
...
soc_15min_df = pd.DataFrame(soc_15min_log)
soc_15min_fp = DIR_PASS2 / "SOC_15min_BESS_SIM16_2047.csv"
...
print(f"[DONE] SOC 15 min salvo em: {soc_15min_fp}")

# %% Celula 006 - PASSO 2: emenda monitores (todos)
# =============================================================================
# Entrada:
#   Simulacao 016 segmentos_ano_2033_2047/UFSM_Mon_<KEY>_s*.csv
# Saida:
#   Simulacao 016 segmentos_ano_2033_2047/Monitor_<KEY>_anual.csv
# Remove:
...
# =============================================================================
# Entrada:
#   Simulacao 016 segmentos_ano_2033_2047/EXP_MTR_GERAL_*.csv
# Saida:
#   Simulacao 016 segmentos_ano_2033_2047/EXP_MTR_GERAL_anual.csv
# Remove:
#   EXP_MTR_GERAL_*.csv
# =============================================================================
...
#   Simulacao 016 segmentos_ano_2033_2047/Monitor_<KEY>_anual.csv
# Saidas:
#   Simulacao 016 segmentos_ano_2033_2047/Curva_de_Carga_<KEY>_sim_16.csv
# Remove:
...
for in_fp in files:
    ...
    out_fp = DIR_PASS2 / f"Curva_de_Carga_{mon_key}_sim_16.csv"
...
#   Simulacao 016 segmentos_ano_2033_2047/EXP_MTR_GERAL_anual.csv
# Saida:
#   Simulacao 016 segmentos_ano_2033_2047/Resumo_EnergyMeter_Geral_Sim16.csv
# =============================================================================
...
OUT_FP = DIR_PASS2 / "Resumo_EnergyMeter_Geral_Sim16.csv"
...
# %% Celula 010 - ENERGIA e DEMANDA MAX por mes e posto (SIM16 - ANO 2047, BESS modular autofinanciado)
# =============================================================================
# Entradas:
#   META: 00_dados_de_entrada_ano_letivo_2025_15min.csv
#         Usado apenas como mapa anual de Hora_decimal, Mes e Posto.
#   SIM16: curva GERAL com 2 BESS degradados para o ano 2047.
# Saida:
#   Simulacao 016 segmentos_ano_2033_2047/01_Integralizacao_Mensal_SIM16_2047.csv
#
# Observacao:
...
#     BESS1: instalado em 2028, degradado em 2047 para 673 kW.
#     BESS2: instalado em 2033, degradado em 2047 para 707 kW.
#     Alvo fora-ponta = 5550 kW; permissao de recarga fora-ponta = 4440 kW.
#   A curva e lida diretamente pelo sufixo sim_16, sem fallback.
# =============================================================================
...
SIM16_DIR = WORK_DIR / "Simulacao 016 segmentos_ano_2033_2047"

META_FP = WORK_DIR / "00_dados_de_entrada_ano_letivo_2025_15min.csv"
SIM16_FP = SIM16_DIR / "Curva_de_Carga_GERAL_sim_16.csv"
OUT_FP = SIM16_DIR / f"01_Integralizacao_Mensal_{SIM_TAG}_{ANO_ANALISE}.csv"
...
def gerar_curva_geral_sim16_se_necessario(sim16_fp: Path, dir_pass2: Path) -> None:
    """
    Garante a existencia de Curva_de_Carga_GERAL_sim_16.csv.
    ...
    Se encontrar apenas 507 linhas, o problema esta na Celula 005: os monitores
    foram exportados com 1 linha por segmento. Nesse caso, e necessario rerodar
    a Celula 005 desta versao corrigida, que limpa as saidas antigas e valida
    cada monitor segmentado logo apos a exportacao.
    """
    dir_pass2.mkdir(parents=True, exist_ok=True)
    cols_vals = ["P1 (kW)", "Q1 (kvar)", "P2 (kW)", "Q2 (kvar)", "P3 (kW)", "Q3 (kvar)"]
    ...
    def _validar_e_gravar_curva(mon: pd.DataFrame, origem: str) -> None:
        ...
        if len(mon) != TOTAL_NPTS:
            raise ValueError(
                ...
                "Se apareceu 507 linhas, os CSVs UFSM_Mon_GERAL_s*.csv antigos estao ruins ou a Celula 005 antiga foi usada. "
                "Rode novamente a Celula 005 desta versao corrigida, que limpa a pasta antes de simular, e depois rode 006, 007, 008, 009, 010 e 011."
            )
        ...
        df_out["P_3f_S"] = df_out["P1 (kW)"] + df_out["P2 (kW)"] + df_out["P3 (kW)"]
        df_out["Q_3f_S"] = df_out["Q1 (kvar)"] + df_out["Q2 (kvar)"] + df_out["Q3 (kvar)"]
        df_out[["Hora_decimal", "P_3f_S", "Q_3f_S"]].to_csv(sim16_fp, index=False, encoding="utf-8-sig")
        print("[AUTO] Curva GERAL gerada em:", sim16_fp)

    if sim16_fp.exists():
        curva = pd.read_csv(sim16_fp, encoding="utf-8-sig", sep=None, engine="python")
        if len(curva) == TOTAL_NPTS:
            return
        print(f"[AUTO] Curva GERAL existente tem {len(curva)} linhas; removendo arquivo invalido: {sim16_fp.name}")
        sim16_fp.unlink()
    ...
    if not seg_files:
        raise FileNotFoundError(
            f"Nao encontrei {sim16_fp}. Tambem nao encontrei Monitor_GERAL_anual.csv nem UFSM_Mon_GERAL_s*.csv em {dir_pass2}. "
    ...
    if total_linhas != TOTAL_NPTS:
        ...
        raise ValueError(
            ...
            "Rode novamente a Celula 005 desta versao corrigida; ela limpa os arquivos antigos antes de simular."
        )
...
gerar_curva_geral_sim16_se_necessario(SIM16_FP, SIM16_DIR)
if not SIM16_FP.exists():
    raise FileNotFoundError(SIM16_FP)

meta = pd.read_csv(META_FP, dtype=str, encoding="utf-8-sig")
sim16 = pd.read_csv(SIM16_FP, encoding="utf-8-sig", sep=None, engine="python")

meta = clean_cols(meta)
sim16 = clean_cols(sim16)

for name, dfx, req in [
    ("META", meta, ["Hora_decimal", "Mes", "Posto"]),
    (SIM_TAG, sim16, ["Hora_decimal", "P_3f_S", "Q_3f_S"]),
]:
...
sim16["Hora_decimal"] = pd.to_numeric(sim16["Hora_decimal"], errors="coerce")
sim16["P_3f_S"] = pd.to_numeric(sim16["P_3f_S"], errors="coerce")
sim16["Q_3f_S"] = pd.to_numeric(sim16["Q_3f_S"], errors="coerce")
sim16.dropna(subset=["Hora_decimal", "P_3f_S", "Q_3f_S"], inplace=True)

meta["k"] = make_k(meta["Hora_decimal"])
sim16["k"] = make_k(sim16["Hora_decimal"])

meta = meta[(meta["k"] >= 0) & (meta["k"] < TOTAL_NPTS)].copy()
sim16 = sim16[(sim16["k"] >= 0) & (sim16["k"] < TOTAL_NPTS)].copy()

meta = meta[["k", "Mes", "Posto"]].drop_duplicates(subset=["k"], keep="first").copy()
sim16 = sim16[["k", "P_3f_S", "Q_3f_S"]].drop_duplicates(subset=["k"], keep="first").copy()

if len(meta) != TOTAL_NPTS:
    raise ValueError(f"META gerou {len(meta)} amostras anuais de 15 min; esperado {TOTAL_NPTS}.")
if len(sim16) != TOTAL_NPTS:
    raise ValueError(f"SIM16 gerou {len(sim16)} amostras anuais de 15 min; esperado {TOTAL_NPTS}.")

df16 = meta.merge(sim16, on="k", how="left").rename(
    columns={"P_3f_S": "P_SIM16", "Q_3f_S": "Q_SIM16"}
)

faltas_p = int(df16["P_SIM16"].isna().sum())
faltas_q = int(df16["Q_SIM16"].isna().sum())
if faltas_p or faltas_q:
    raise ValueError(
        f"Falha de alinhamento por Hora_decimal/k: "
        f"P_SIM16 faltas={faltas_p}, Q_SIM16 faltas={faltas_q}"
    )

df16["E_P_SIM16_kWh"] = df16["P_SIM16"] * DT_H
df16["E_Q_SIM16_kVArh"] = df16["Q_SIM16"] * DT_H

group_cols = ["Mes", "Posto"]

agg16 = df16.groupby(group_cols, dropna=False).agg(
    N_pontos_SIM16=("P_SIM16", lambda s: int(s.notna().sum())),
    Consumo_P_SIM16_kWh=("E_P_SIM16_kWh", "sum"),
    Consumo_Q_SIM16_kVArh=("E_Q_SIM16_kVArh", "sum"),
    DemandaMax_P_SIM16_kW=("P_SIM16", "max"),
    DemandaMax_Q_SIM16_kVAr=("Q_SIM16", "max"),
).reset_index()
...
print("SIM16 usado:", SIM16_FP)
print(agg[["Mes", "Posto", "N_pontos_META", "N_pontos_SIM16", "DemandaMax_P_SIM16_kW"]])

# %% Celula 011 - SIMULACAO 016: tabela de custos (SIM16 - ANO 2047, BESS modular autofinanciado)
# =============================================================================
# Entrada:
#   Simulacao 016 segmentos_ano_2033_2047/01_Integralizacao_Mensal_SIM16_2047.csv
#
# Saida:
#   Simulacao 016 segmentos_ano_2033_2047/Tabela_demanda_consumo_por_posto_SIM16_2047.csv
#
# Demandas contratadas do ano 2047:
#   DCFP contratada com BESS = 5550 kW
#   Alvo operacional fora-ponta do despacho = 5550 kW
#   Permissao para recarga fora-ponta = 4440 kW
#   DCP sem BESS = 3330 kW
#   DCP com BESS = 3330 - (673 + 707) = 1950 kW
#
# Banco de energia modular autofinanciado em 2047:
#   BESS1 instalado em 2028:
#     1 MW / 4,5 MWh nominal, degradado em 2047 para 673 kW / 3788 kWh.
#     Possui excedente nominal de 1500 kWh para suprir energia de peak-shave fora-ponta.
#   BESS2 instalado em 2033:
#     1 MW / 3,0 MWh nominal, degradado em 2047 para 707 kW / 2650 kWh.
#   Os bancos possuem energias diferentes.
#   O BESS1 foi dimensionado com excedente para suprir os picos fora-ponta ate 2047.
#   Os bancos instalados posteriormente possuem somente os 3 MWh necessarios para a ponta.
# =============================================================================
...
SIM16_DIR = WORK_DIR / "Simulacao 016 segmentos_ano_2033_2047"
SIM16_DIR.mkdir(parents=True, exist_ok=True)

IN_FP = SIM16_DIR / f"01_Integralizacao_Mensal_{SIM_TAG}_{ANO_ANALISE}.csv"
OUT_SIM16 = SIM16_DIR / f"Tabela_demanda_consumo_por_posto_{SIM_TAG}_{ANO_ANALISE}.csv"
...
BESS1_P_2047_KW = 673.0
BESS1_E_2047_KWH = 3788.0
BESS1_FATOR_DEGRADACAO_E_2047 = BESS1_E_2047_KWH / BESS1_E_NOMINAL_KWH
BESS1_E_BASE_PONTA_2047_KWH = BESS1_E_BASE_PONTA_NOMINAL_KWH * BESS1_FATOR_DEGRADACAO_E_2047
BESS1_E_EXCEDENTE_FP_2047_KWH = BESS1_E_EXCEDENTE_FP_NOMINAL_KWH * BESS1_FATOR_DEGRADACAO_E_2047
...
BESS2_P_2047_KW = 707.0
BESS2_E_2047_KWH = 2650.0

BESS_P_TOTAL_KW = BESS1_P_2047_KW + BESS2_P_2047_KW
BESS_E_TOTAL_KWH = BESS1_E_2047_KWH + BESS2_E_2047_KWH
BESS_E_NOMINAL_TOTAL_KWH = BESS1_E_NOMINAL_KWH + BESS2_E_NOMINAL_KWH

# Separacao operacional de energia:
# - BESS1 usa somente o excedente degradado para peak-shave fora-ponta;
# - BESS2 fica 100% reservado para load-shift na ponta;
# - durante a ponta, ambos sao liberados ate a reserva minima tecnica.
...
BESS_E_RESERVA_PONTA_KWH = BESS1_E_RESERVA_PONTA_2047_KWH + BESS2_E_RESERVA_PONTA_2047_KWH
BESS_E_DISPONIVEL_FP_KWH = BESS1_E_2047_KWH - BESS1_E_RESERVA_PONTA_2047_KWH
BESS_RESERVA_PONTA_PCT_PONDERADA = 100.0 * BESS_E_RESERVA_PONTA_KWH / BESS_E_TOTAL_KWH

DCP_COM_BESS = max(0.0, DCP_SEM_BESS - BESS_P_TOTAL_KW)

CONTR_SIM16 = {"DCFP": DCFP_COM_BESS, "DCP": DCP_COM_BESS}
...
if not IN_FP.exists():
    curva_fp = SIM16_DIR / "Curva_de_Carga_GERAL_sim_16.csv"
    raise FileNotFoundError(
        f"Nao encontrei {IN_FP}. Execute a Celula 010 antes da Celula 011. "
        f"A Celula 010 corrigida gera este arquivo a partir de {curva_fp.name}; "
...
def tabela_custos(df_in: pd.DataFrame, fonte: str, DCFP: float, DCP: float) -> pd.DataFrame:
    ...
    # Linha de totais: maximo de demanda, soma de energia e soma de R$
    totais = {
        "MES": "TOTAIS",
...
tab16 = tabela_custos(df, SIM_TAG, DCFP=CONTR_SIM16["DCFP"], DCP=CONTR_SIM16["DCP"])
tab16.to_csv(OUT_SIM16, index=False, encoding="utf-8")

print("DONE. Saved (Simulacao 016 segmentos_ano_2033_2047):")
print(" -", OUT_SIM16)
print("\nDemandas contratadas usadas:")
print(f" - {SIM_TAG}: DCFP={CONTR_SIM16['DCFP']:.0f}, DCP={CONTR_SIM16['DCP']:.0f}")
print("\nResumo anual:")
print(tab16[tab16["MES"].isin(["TOTAIS", "CTOTAL", "R$TOTAL", "DCFP", "DCP"])])
print("\nReservas operacionais 2047:")
print(f" - BESS1: reserva fora-ponta={BESS1_RESERVA_PONTA_PCT:.4f}% | reserva ponta={BESS_RESERVA_MIN_PONTA_PCT:.2f}%")
print(f" - BESS2: reserva fora-ponta={BESS2_RESERVA_PONTA_PCT:.4f}% | reserva ponta={BESS_RESERVA_MIN_PONTA_PCT:.2f}%")
print(f" - Energia BESS1 reservada para ponta: {BESS1_E_RESERVA_PONTA_2047_KWH:.1f} kWh")
print(f" - Energia BESS2 reservada para ponta: {BESS2_E_RESERVA_PONTA_2047_KWH:.1f} kWh")
print(f" - Energia disponivel para fora-ponta: {BESS_E_DISPONIVEL_FP_KWH:.1f} kWh")
