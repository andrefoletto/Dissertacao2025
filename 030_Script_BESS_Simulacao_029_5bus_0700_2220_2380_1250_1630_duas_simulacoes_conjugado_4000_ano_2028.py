# -*- coding: utf-8 -*-
# =============================================================================
# 030_Script_BESS_Simulacao_029_5bus_0700_2220_2380_1250_1630_duas_simulacoes_conjugado_4000_ano_2028.py
#
# Regras deste script:
...
#     Simulacao 029 segmentos_pass1_sem_hvac_int_ano_2028
#   PASSO 2 (simulacao completa final):
#     Simulacao 029 segmentos_ano_2028
# =============================================================================
...
DIR_PASS1 = WORK_DIR / "Simulacao 029 segmentos_pass1_sem_hvac_int_ano_2028"
...
def build_circuit_pass1(
    ...
    dss.text(
        ...
        f"kWrated=800 kWhrated=5400 Effcurve=Eff %EffCharge=95 %EffDischarge=95 "
...
DIR_PASS2 = WORK_DIR / "Simulacao 029 segmentos_ano_2028"
...
# =============================================================================
# Entrada:
#   Simulacao 029 segmentos_ano_2028/UFSM_Mon_<KEY>_s*.csv
# Saida:
#   Simulacao 029 segmentos_ano_2028/Monitor_<KEY>_anual.csv
# Remove:
...
# =============================================================================
# Entrada:
#   Simulacao 029 segmentos_ano_2028/EXP_MTR_GERAL_*.csv
# Saida:
#   Simulacao 029 segmentos_ano_2028/EXP_MTR_GERAL_anual.csv
# Remove:
#   EXP_MTR_GERAL_*.csv
# =============================================================================
...
#   Simulacao 029 segmentos_ano_2028/Monitor_<KEY>_anual.csv
# Saidas:
#   Simulacao 029 segmentos_ano_2028/Curva_de_Carga_<KEY>_sim_29.csv
# Remove:
...
#   Simulacao 029 segmentos_ano_2028/EXP_MTR_GERAL_anual.csv
# Saida:
#   Simulacao 029 segmentos_ano_2028/Resumo_EnergyMeter_Geral_Sim29.csv
# =============================================================================
...
# %% Celula 010 - ENERGIA e DEMANDA MAX por mes e posto (SIM29 - ANO 2028, 5 BESS 800 kW / 5,4 MWh cada)
# =============================================================================
# Entradas:
#   META: 00_dados_de_entrada_ano_letivo_2025_15min.csv
#         Usado apenas como mapa anual de Hora_decimal, Mes e Posto.
#   SIM29: curva GERAL com 5 BESS em operacao conjugada para o ano 2028
# Saida:
#   Simulacao 029 segmentos_ano_2028/01_Integralizacao_Mensal_SIM29_2028.csv
#
# Observacao:
...
#   Todas as celulas deste arquivo foram padronizadas para SIM29.
# =============================================================================
...
SIM29_DIR = WORK_DIR / f"Simulacao 029 segmentos_ano_{ANO_ANALISE}"
...
sim29 = clean_cols(sim29)

for name, dfx, req in [
    ("META", meta, ["Hora_decimal", "Mes", "Posto"]),
    (SIM_TAG, sim29, ["Hora_decimal", "P_3f_S", "Q_3f_S"]),
]:
...
sim29["Hora_decimal"] = pd.to_numeric(sim29["Hora_decimal"], errors="coerce")
sim29["P_3f_S"] = pd.to_numeric(sim29["P_3f_S"], errors="coerce")
sim29["Q_3f_S"] = pd.to_numeric(sim29["Q_3f_S"], errors="coerce")
sim29.dropna(subset=["Hora_decimal", "P_3f_S", "Q_3f_S"], inplace=True)
...
df29 = meta.merge(sim29, on="k", how="left").rename(
    columns={"P_3f_S": "P_SIM29", "Q_3f_S": "Q_SIM29"}
)

faltas_p = int(df29["P_SIM29"].isna().sum())
faltas_q = int(df29["Q_SIM29"].isna().sum())
...
agg = agg29.merge(meta_counts, on=group_cols, how="left")
...
# %% Celula 011 - SIMULACAO 029: tabela de custos (SIM29 - ANO 2028, 5 BESS 800 kW / 5,4 MWh cada)
# =============================================================================
# Entrada:
#   Simulacao 029 segmentos_ano_2028/01_Integralizacao_Mensal_SIM29_2028.csv
#
# Saida:
#   Simulacao 029 segmentos_ano_2028/Tabela_demanda_consumo_por_posto_SIM29_2028.csv
#
# Demandas contratadas do ano 2028:
#   DCFP com BESS = 4060 kW  (corte fora-ponta agressivo: 4000 kW * 1,015)
#   DCP sem BESS = 3045 kW
#   DCP com 5 BESS = 3045 - (5 * 800) = 0 kW
#
# Banco de energia:
#   5 BESS de 1 MW / 5,4 MWh, operando a 80% da potencia nominal.
#   Potencia operacional total = 4000 kW.
#   Energia total = 27000 kWh = 15000 kWh de reserva de ponta + 12000 kWh para fora-ponta.
#   Reserva de ponta = 55,6% (15/27).
...
# - A demanda contratada fora-ponta foi reduzida de forma mais agressiva para 4060 kW.
# - Os 5 BESS sao instalados no proprio ano de analise.
# - Cada banco possui 1 MW / 5,4 MWh nominal, mas opera com 800 kW de potencia.
...
# Energia total instalada em 2028: 27 MWh = 15 MWh de reserva de ponta + 12 MWh para fora-ponta.
BESS_E_UNITARIA_KWH = 5400.0
BESS_E_TOTAL_KWH = NUM_BESS * BESS_E_UNITARIA_KWH     # 27000 kWh
BESS_E_RESERVA_PONTA_KWH = 15000.0
BESS_E_DISPONIVEL_FP_KWH = 12000.0
BESS_RESERVA_PONTA_PCT = 100.0 * BESS_E_RESERVA_PONTA_KWH / BESS_E_TOTAL_KWH  # 55,6% = 15/27
...
DCFP_COM_BESS = 4060.0                                  # corte fora-ponta agressivo
...
print("DONE. Saved (Simulacao 029 - 5 BESS - 2028 - conjugado 4000):")
print(" -", OUT_SIM29)
...
print(f" - Energia nominal unitaria dos BESS = {BESS_E_UNITARIA_KWH:.0f} kWh")
