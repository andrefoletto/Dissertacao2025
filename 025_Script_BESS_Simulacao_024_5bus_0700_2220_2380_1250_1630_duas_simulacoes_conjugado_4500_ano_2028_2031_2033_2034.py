# -*- coding: utf-8 -*-
# =============================================================================
# 025_Script_BESS_Simulacao_024_5bus_0700_2220_2380_1250_1630_duas_simulacoes_conjugado_4500_ano_2028_2031_2033_2034.py
#
# Regras deste script:
...
#     Simulacao 024 segmentos_pass1_sem_hvac_int_ano_2034
#   PASSO 2 (simulacao completa final):
#     Simulacao 024 segmentos_ano_2034
# =============================================================================
...
DIR_PASS1 = WORK_DIR / "Simulacao 024 segmentos_pass1_sem_hvac_int_ano_2034"
...
if LIMPAR_SAIDAS_ANTERIORES_PASS1:
    padroes_limpeza_pass1 = [
        ...
        "Curva_de_Carga_BESSM*_IN_sim_24.csv",
        "SOC_por_segmento.csv",
    ]
    removidos = 0
...
def build_circuit_pass1(
    ...
    dss.text('Redirect "UFSM_cargas_medidas_ano_2034.dss"')
    dss.text('Redirect "UFSM_cargas_especiais_ano_2034.dss"')
    dss.text('Redirect "UFSM_cargas_estimadas_com_residuo_ano_2034.dss"')
    ...
    dss.text(
        ...
        f"kWrated=760 kWhrated=5225 Effcurve=Eff %EffCharge=95 %EffDischarge=95 "
...
# %% Celula 003 - PASSO 1: monitores -> Curva_de_Carga_BESSM1_IN_sim_24.csv, BESSM2_IN, BESSM3_IN e BESSM4_IN (SHIFT DIREITA)
# =============================================================================
# Entradas:
#   PASSO1/Monitor_BESSM1_IN_anual.csv
#   PASSO1/Monitor_BESSM2_IN_anual.csv
...
#   PASSO1/Monitor_BESSM4_IN_anual.csv
# Saidas:
#   PASSO1/Curva_de_Carga_BESSM1_IN_sim_24.csv
#   PASSO1/Curva_de_Carga_BESSM2_IN_sim_24.csv
#   PASSO1/Curva_de_Carga_BESSM3_IN_sim_24.csv
#   PASSO1/Curva_de_Carga_BESSM4_IN_sim_24.csv
# Remove:
#   PASSO1/Monitor_BESSM1_IN_anual.csv
#   PASSO1/Monitor_BESSM2_IN_anual.csv
#   PASSO1/Monitor_BESSM3_IN_anual.csv
...
for mon_base in ["BESSM1_IN", "BESSM2_IN", "BESSM3_IN", "BESSM4_IN", "BESSM5_IN"]:
    ...
    out_fp = DIR_PASS1 / f"Curva_de_Carga_{mon_base}_sim_24.csv"
...
for mon_base, prefix in [("BESSM1_IN", "BESSM1"), ("BESSM2_IN", "BESSM2"), ("BESSM3_IN", "BESSM3"), ("BESSM4_IN", "BESSM4"), ("BESSM5_IN", "BESSM5")]:
    in_fp = DIR_PASS1 / f"Curva_de_Carga_{mon_base}_sim_24.csv"
...
#     SOC_15min_BESS_SIM24_2034.csv          (SOC dos cinco bancos a cada 15 min)
#
# Requisitos:
...
BESS1_E_2034_KWH = 5225.0
BESS2_E_2034_KWH = 5225.0
BESS3_E_2034_KWH = 2925.0
BESS4_E_2034_KWH = 2975.0
BESS5_E_2034_KWH = 3000.0
BESS_TOTAL_E_2034_KWH = BESS1_E_2034_KWH + BESS2_E_2034_KWH + BESS3_E_2034_KWH + BESS4_E_2034_KWH + BESS5_E_2034_KWH
...
DIR_PASS2 = WORK_DIR / "Simulacao 024 segmentos_ano_2034"
...
if LIMPAR_SAIDAS_ANTERIORES_PASS2:
    padroes_limpeza = [
        "UFSM_Mon_*_s*.csv",
        "Monitor_*_anual.csv",
        "Curva_de_Carga_*_sim_24.csv",
        "EXP_MTR_GERAL*.csv",
        "Resumo_EnergyMeter_Geral_Sim24.csv",
        "SOC_por_segmento.csv",
        "SOC_15min_BESS_SIM24_2034.csv",
        "01_Integralizacao_Mensal_SIM24_2034.csv",
        "Tabela_demanda_consumo_por_posto_SIM24_2034.csv",
    ]
    removidos = 0
...
def _run_segment_pass2(
    ...
    for step_idx in range(int(n_steps)):
        ...
        soc_m1_kwh = soc_m1_pct / 100.0 * BESS1_E_2034_KWH
        soc_m2_kwh = soc_m2_pct / 100.0 * BESS2_E_2034_KWH
        soc_m3_kwh = soc_m3_pct / 100.0 * BESS3_E_2034_KWH
        soc_m4_kwh = soc_m4_pct / 100.0 * BESS4_E_2034_KWH
        soc_m5_kwh = soc_m5_pct / 100.0 * BESS5_E_2034_KWH
        soc_total_kwh = soc_m1_kwh + soc_m2_kwh + soc_m3_kwh + soc_m4_kwh + soc_m5_kwh
        soc_total_pct_pond = 100.0 * soc_total_kwh / BESS_TOTAL_E_2034_KWH
...
soc_15min_fp = DIR_PASS2 / "SOC_15min_BESS_SIM24_2034.csv"
...
# =============================================================================
# Entrada:
#   Simulacao 024 segmentos_ano_2034/UFSM_Mon_<KEY>_s*.csv
# Saida:
#   Simulacao 024 segmentos_ano_2034/Monitor_<KEY>_anual.csv
# Remove:
...
# =============================================================================
# Entrada:
#   Simulacao 024 segmentos_ano_2034/EXP_MTR_GERAL_*.csv
# Saida:
#   Simulacao 024 segmentos_ano_2034/EXP_MTR_GERAL_anual.csv
# Remove:
#   EXP_MTR_GERAL_*.csv
# =============================================================================
...
#   Simulacao 024 segmentos_ano_2034/Monitor_<KEY>_anual.csv
# Saidas:
#   Simulacao 024 segmentos_ano_2034/Curva_de_Carga_<KEY>_sim_24.csv
# Remove:
...
for in_fp in files:
    ...
    out_fp = DIR_PASS2 / f"Curva_de_Carga_{mon_key}_sim_24.csv"
...
#   Simulacao 024 segmentos_ano_2034/EXP_MTR_GERAL_anual.csv
# Saida:
#   Simulacao 024 segmentos_ano_2034/Resumo_EnergyMeter_Geral_Sim24.csv
# =============================================================================
...
OUT_FP = DIR_PASS2 / "Resumo_EnergyMeter_Geral_Sim24.csv"
...
# %% Celula 010 - ENERGIA e DEMANDA MAX por mes e posto (SIM24 - ANO 2034, BESS modular autofinanciado)
# =============================================================================
# Entradas:
#   META: 00_dados_de_entrada_ano_letivo_2025_15min.csv
#         Usado apenas como mapa anual de Hora_decimal, Mes e Posto.
#   SIM24: curva GERAL com BESS modular autofinanciado em operacao conjugada
#          para o ano 2034.
# Saida:
#   Simulacao 024 segmentos_ano_2034/01_Integralizacao_Mensal_SIM24_2034.csv
#
# Observacao:
#   Esta celula nao compara com SIM2/RGE nem com outra simulacao.
#   Ela apenas integraliza a simulacao conjugada do ano 2034 por mes e posto.
#   Configuracao do despacho:
#     BESS1: instalado em 2028, 1 MW / 5,5 MWh, operando a 80% da potencia,
#            degradado em 2034 por fator 0,95 para 760 kW / 5225 kWh.
...
#            degradado em 2034 por fator 0,975 para 780 kW / 2925 kWh.
#     BESS4: instalado em 2033, 1 MW / 3,0 MWh, operando a 80% da potencia,
#            degradado em 2034 por fator 0,991666667 para 793 kW / 2975 kWh.
#     BESS5: instalado em 2034, 1 MW / 3,0 MWh, operando a 80% da potencia,
#            sem degradacao: 800 kW / 3000 kWh.
#     Alvo fora-ponta = 4703 kW; permissao de recarga fora-ponta = 3658 kW.
#     Em fora-ponta, BESS1 e BESS2 usam reserva de 54,5455% e BESS3/BESS4/BESS5 usam reserva de 100%.
#     Em ponta, os cinco bancos usam reserva minima tecnica de 2%.
#   A curva e lida diretamente pelo sufixo sim_24, sem fallback.
# =============================================================================
...
SIM24_DIR = WORK_DIR / f"Simulacao 024 segmentos_ano_{ANO_ANALISE}"

META_FP = WORK_DIR / "00_dados_de_entrada_ano_letivo_2025_15min.csv"
SIM24_FP = SIM24_DIR / "Curva_de_Carga_GERAL_sim_24.csv"
OUT_FP = SIM24_DIR / f"01_Integralizacao_Mensal_{SIM_TAG}_{ANO_ANALISE}.csv"
...
def gerar_curva_geral_sim24_se_necessario(sim24_fp: Path, dir_pass2: Path) -> None:
    """
    Garante a existencia de Curva_de_Carga_GERAL_sim_24.csv.
    ...
    def _validar_e_gravar_curva(mon: pd.DataFrame, origem: str) -> None:
        ...
        df_out["P_3f_S"] = df_out["P1 (kW)"] + df_out["P2 (kW)"] + df_out["P3 (kW)"]
        df_out["Q_3f_S"] = df_out["Q1 (kvar)"] + df_out["Q2 (kvar)"] + df_out["Q3 (kvar)"]
        df_out[["Hora_decimal", "P_3f_S", "Q_3f_S"]].to_csv(sim24_fp, index=False, encoding="utf-8-sig")
        print("[AUTO] Curva GERAL gerada em:", sim24_fp)

    if sim24_fp.exists():
        curva = pd.read_csv(sim24_fp, encoding="utf-8-sig", sep=None, engine="python")
        if len(curva) == TOTAL_NPTS:
            return
        print(f"[AUTO] Curva GERAL existente tem {len(curva)} linhas; removendo arquivo invalido: {sim24_fp.name}")
        sim24_fp.unlink()
    ...
    if not seg_files:
        raise FileNotFoundError(
            f"Nao encontrei {sim24_fp}. Tambem nao encontrei Monitor_GERAL_anual.csv nem UFSM_Mon_GERAL_s*.csv em {dir_pass2}. "
...
gerar_curva_geral_sim24_se_necessario(SIM24_FP, SIM24_DIR)
if not SIM24_FP.exists():
    raise FileNotFoundError(SIM24_FP)

meta = pd.read_csv(META_FP, dtype=str, encoding="utf-8-sig")
sim24 = pd.read_csv(SIM24_FP, encoding="utf-8-sig", sep=None, engine="python")

meta = clean_cols(meta)
sim24 = clean_cols(sim24)

for name, dfx, req in [
    ("META", meta, ["Hora_decimal", "Mes", "Posto"]),
    (SIM_TAG, sim24, ["Hora_decimal", "P_3f_S", "Q_3f_S"]),
]:
...
sim24["Hora_decimal"] = pd.to_numeric(sim24["Hora_decimal"], errors="coerce")
sim24["P_3f_S"] = pd.to_numeric(sim24["P_3f_S"], errors="coerce")
sim24["Q_3f_S"] = pd.to_numeric(sim24["Q_3f_S"], errors="coerce")
sim24.dropna(subset=["Hora_decimal", "P_3f_S", "Q_3f_S"], inplace=True)

meta["k"] = make_k(meta["Hora_decimal"])
sim24["k"] = make_k(sim24["Hora_decimal"])

meta = meta[(meta["k"] >= 0) & (meta["k"] < TOTAL_NPTS)].copy()
sim24 = sim24[(sim24["k"] >= 0) & (sim24["k"] < TOTAL_NPTS)].copy()

meta = meta[["k", "Mes", "Posto"]].drop_duplicates(subset=["k"], keep="first").copy()
sim24 = sim24[["k", "P_3f_S", "Q_3f_S"]].drop_duplicates(subset=["k"], keep="first").copy()

if len(meta) != TOTAL_NPTS:
    raise ValueError(f"META gerou {len(meta)} amostras anuais de 15 min; esperado {TOTAL_NPTS}.")
if len(sim24) != TOTAL_NPTS:
    raise ValueError(f"SIM24 gerou {len(sim24)} amostras anuais de 15 min; esperado {TOTAL_NPTS}.")

df24 = meta.merge(sim24, on="k", how="left").rename(
    columns={"P_3f_S": "P_SIM24", "Q_3f_S": "Q_SIM24"}
)

faltas_p = int(df24["P_SIM24"].isna().sum())
faltas_q = int(df24["Q_SIM24"].isna().sum())
if faltas_p or faltas_q:
    raise ValueError(
        f"Falha de alinhamento por Hora_decimal/k: "
        f"P_SIM24 faltas={faltas_p}, Q_SIM24 faltas={faltas_q}"
    )

df24["E_P_SIM24_kWh"] = df24["P_SIM24"] * DT_H
df24["E_Q_SIM24_kVArh"] = df24["Q_SIM24"] * DT_H

group_cols = ["Mes", "Posto"]

agg24 = df24.groupby(group_cols, dropna=False).agg(
    N_pontos_SIM24=("P_SIM24", lambda s: int(s.notna().sum())),
    Consumo_P_SIM24_kWh=("E_P_SIM24_kWh", "sum"),
    Consumo_Q_SIM24_kVArh=("E_Q_SIM24_kVArh", "sum"),
    DemandaMax_P_SIM24_kW=("P_SIM24", "max"),
    DemandaMax_Q_SIM24_kVAr=("Q_SIM24", "max"),
).reset_index()
...
print("SIM24 usado:", SIM24_FP)
print(agg[["Mes", "Posto", "N_pontos_META", "N_pontos_SIM24", "DemandaMax_P_SIM24_kW"]])

# %% Celula 011 - SIMULACAO 024: tabela de custos (SIM24 - ANO 2034, BESS modular autofinanciado)
# =============================================================================
# Entrada:
#   Simulacao 024 segmentos_ano_2034/01_Integralizacao_Mensal_SIM24_2034.csv
#
# Saida:
#   Simulacao 024 segmentos_ano_2034/Tabela_demanda_consumo_por_posto_SIM24_2034.csv
#
# Demandas contratadas do ano 2034:
#   DCFP contratada com BESS = 4500 * 1,045 = 4703 kW
#   Alvo operacional fora-ponta do despacho = 4703 kW
#   Permissao para recarga fora-ponta = 3500 * 1,045 = 3658 kW
#   DCP sem BESS = 3000 * 1,045 = 3135 kW
#   DCP com BESS = max(0, 3135 - (760 + 760 + 780 + 793 + 800)) = 0 kW
#
...
#     Em 2034: fator 0,95 -> 760 kW / 5225 kWh.
...
#     1 MW / 3,0 MWh nominal, operando a 80% da potencia nominal em 2031.
#     Em 2034: fator 0,975 -> 780 kW / 2925 kWh.
#   BESS4 instalado em 2033:
#     1 MW / 3,0 MWh nominal, operando a 80% da potencia nominal em 2033.
#     Em 2034: fator 0,991666667 -> 793 kW / 2975 kWh.
#   BESS5 instalado em 2034:
#     1 MW / 3,0 MWh nominal, operando a 80% da potencia nominal: 800 kW / 3000 kWh.
#   Os bancos possuem energias efetivas diferentes em 2034.
#   BESS1 e BESS2 foram dimensionados com excedente para suprir os picos fora-ponta.
#   BESS3/BESS4/BESS5 possuem somente os 3 MWh nominais necessarios para a ponta.
#   Em fora-ponta, BESS1 e BESS2 usam reserva de 54,5455% e BESS3/BESS4/BESS5 usam reserva de 100%.
#   Em ponta, os cinco bancos usam reserva minima tecnica de 2%.
#
# Observacao:
...
SIM24_DIR = WORK_DIR / f"Simulacao 024 segmentos_ano_{ANO_ANALISE}"
SIM24_DIR.mkdir(parents=True, exist_ok=True)

IN_FP = SIM24_DIR / f"01_Integralizacao_Mensal_{SIM_TAG}_{ANO_ANALISE}.csv"
OUT_SIM24 = SIM24_DIR / f"Tabela_demanda_consumo_por_posto_{SIM_TAG}_{ANO_ANALISE}.csv"

# -------------------------
# Demandas contratadas e alvos do despacho
# -------------------------
# Fora-ponta: 4500 * 1,045 = 4703 kW
# Ponta sem BESS: 3000 * 1,045 = 3135 kW
# Ponta com BESS: max(0, 3135 - (760 + 760 + 780 + 793 + 800)) = 0 kW
DCP_SEM_BESS = 3135.0
DCFP_COM_BESS = 4703.0

KW_TARGET_FP = 4703.0
KW_TARGET_LOW_FP = 3658.0
...
BESS1_FATOR_DEGRADACAO_2034 = 0.95
BESS1_P_2034_KW = round(BESS1_P_OPERACIONAL_2028_KW * BESS1_FATOR_DEGRADACAO_2034)
BESS1_E_2034_KWH = BESS1_E_NOMINAL_KWH * BESS1_FATOR_DEGRADACAO_2034
BESS1_E_EXCEDENTE_FP_2034_KWH = BESS1_E_EXCEDENTE_FP_NOMINAL_KWH * BESS1_FATOR_DEGRADACAO_2034
...
BESS2_E_EXCEDENTE_FP_NOMINAL_KWH = 2500.0
BESS2_FATOR_DEGRADACAO_2034 = 0.95
BESS2_P_2034_KW = round(BESS2_P_OPERACIONAL_2028_KW * BESS2_FATOR_DEGRADACAO_2034)
BESS2_E_2034_KWH = BESS2_E_NOMINAL_KWH * BESS2_FATOR_DEGRADACAO_2034
BESS2_E_EXCEDENTE_FP_2034_KWH = BESS2_E_EXCEDENTE_FP_NOMINAL_KWH * BESS2_FATOR_DEGRADACAO_2034

BESS3_ANO_INSTALACAO = 2031
BESS3_P_NOMINAL_KW = 1000.0
BESS3_P_OPERACIONAL_2031_KW = 800.0
BESS3_E_NOMINAL_KWH = 3000.0
BESS3_FATOR_DEGRADACAO_2034 = 0.975
BESS3_P_2034_KW = round(BESS3_P_OPERACIONAL_2031_KW * BESS3_FATOR_DEGRADACAO_2034)
BESS3_E_2034_KWH = BESS3_E_NOMINAL_KWH * BESS3_FATOR_DEGRADACAO_2034

BESS4_ANO_INSTALACAO = 2033
BESS4_P_NOMINAL_KW = 1000.0
BESS4_P_OPERACIONAL_2033_KW = 800.0
BESS4_E_NOMINAL_KWH = 3000.0
BESS4_FATOR_DEGRADACAO_2034 = 0.9916666666666667
BESS4_P_2034_KW = round(BESS4_P_OPERACIONAL_2033_KW * BESS4_FATOR_DEGRADACAO_2034)
BESS4_E_2034_KWH = BESS4_E_NOMINAL_KWH * BESS4_FATOR_DEGRADACAO_2034

BESS5_ANO_INSTALACAO = 2034
BESS5_P_NOMINAL_KW = 1000.0
BESS5_P_OPERACIONAL_2034_KW = 800.0
BESS5_E_NOMINAL_KWH = 3000.0
BESS5_P_2034_KW = BESS5_P_OPERACIONAL_2034_KW
BESS5_E_2034_KWH = BESS5_E_NOMINAL_KWH

BESS_P_TOTAL_KW = BESS1_P_2034_KW + BESS2_P_2034_KW + BESS3_P_2034_KW + BESS4_P_2034_KW + BESS5_P_2034_KW
BESS_E_TOTAL_KWH = BESS1_E_2034_KWH + BESS2_E_2034_KWH + BESS3_E_2034_KWH + BESS4_E_2034_KWH + BESS5_E_2034_KWH
...
# FORA_PONTA:
#   BESS1 e BESS2 reservam 3000/5500 = 54,5455% para a ponta e usam somente o excedente.
#   BESS3, BESS4 e BESS5 reservam 100% para a ponta e nao participam do peak-shave fora-ponta.
# PONTA:
...
BESS1_E_RESERVA_PONTA_2034_KWH = BESS1_E_2034_KWH * BESS1_RESERVA_PONTA_PCT / 100.0
BESS2_E_RESERVA_PONTA_2034_KWH = BESS2_E_2034_KWH * BESS2_RESERVA_PONTA_PCT / 100.0
BESS3_E_RESERVA_PONTA_2034_KWH = BESS3_E_2034_KWH * BESS3_RESERVA_PONTA_PCT / 100.0
BESS4_E_RESERVA_PONTA_2034_KWH = BESS4_E_2034_KWH * BESS4_RESERVA_PONTA_PCT / 100.0
BESS5_E_RESERVA_PONTA_2034_KWH = BESS5_E_2034_KWH * BESS5_RESERVA_PONTA_PCT / 100.0
BESS_E_RESERVA_PONTA_KWH = (
    BESS1_E_RESERVA_PONTA_2034_KWH
    + BESS2_E_RESERVA_PONTA_2034_KWH
    + BESS3_E_RESERVA_PONTA_2034_KWH
    + BESS4_E_RESERVA_PONTA_2034_KWH
    + BESS5_E_RESERVA_PONTA_2034_KWH
)
BESS_E_DISPONIVEL_FP_KWH = (BESS1_E_2034_KWH - BESS1_E_RESERVA_PONTA_2034_KWH) + (BESS2_E_2034_KWH - BESS2_E_RESERVA_PONTA_2034_KWH)

DCP_COM_BESS = max(0.0, DCP_SEM_BESS - BESS_P_TOTAL_KW)

CONTR_SIM24 = {"DCFP": DCFP_COM_BESS, "DCP": DCP_COM_BESS}
...
if not IN_FP.exists():
    curva_fp = SIM24_DIR / "Curva_de_Carga_GERAL_sim_24.csv"
    raise FileNotFoundError(
...
tab24 = tabela_custos(
    df,
    SIM_TAG,
    DCFP=CONTR_SIM24["DCFP"],
    DCP=CONTR_SIM24["DCP"],
)

tab24.to_csv(OUT_SIM24, index=False, encoding="utf-8")

print("DONE. Saved (Simulacao 024 - BESS modular autofinanciado - 2034 - conjugado):")
print(" -", OUT_SIM24)
print("\nDemandas contratadas e alvos usados em 2034:")
...
print(f" - Potencia BESS1 2034 = {BESS1_P_2034_KW:.0f} kW")
print(f" - Potencia BESS2 2034 = {BESS2_P_2034_KW:.0f} kW")
print(f" - Potencia BESS3 2034 = {BESS3_P_2034_KW:.0f} kW")
print(f" - Potencia BESS4 2034 = {BESS4_P_2034_KW:.0f} kW")
print(f" - Potencia BESS5 2034 = {BESS5_P_2034_KW:.0f} kW")
print(f" - Energia nominal total instalada = {BESS_E_NOMINAL_TOTAL_KWH:.0f} kWh")
print(f" - Energia disponivel em 2034 = {BESS_E_TOTAL_KWH:.0f} kWh")
print(f" - Energia BESS1 2034 = {BESS1_E_2034_KWH:.0f} kWh")
print(f" - Energia BESS2 2034 = {BESS2_E_2034_KWH:.0f} kWh")
print(f" - Energia BESS3 2034 = {BESS3_E_2034_KWH:.0f} kWh")
print(f" - Energia BESS4 2034 = {BESS4_E_2034_KWH:.0f} kWh")
print(f" - Energia BESS5 2034 = {BESS5_E_2034_KWH:.0f} kWh")
...
print(f" - Excedente nominal BESS1/BESS2 para fora-ponta = {BESS1_E_EXCEDENTE_FP_NOMINAL_KWH:.0f} kWh")
print(f" - Excedente degradado BESS1 para fora-ponta = {BESS1_E_EXCEDENTE_FP_2034_KWH:.0f} kWh")
print(f" - Excedente degradado BESS2 para fora-ponta = {BESS2_E_EXCEDENTE_FP_2034_KWH:.0f} kWh")
print(f" - Energia reservada para ponta BESS1 = {BESS1_E_RESERVA_PONTA_2034_KWH:.0f} kWh")
print(f" - Energia reservada para ponta BESS2 = {BESS2_E_RESERVA_PONTA_2034_KWH:.0f} kWh")
print(f" - Energia reservada para ponta BESS3 = {BESS3_E_RESERVA_PONTA_2034_KWH:.0f} kWh")
print(f" - Energia reservada para ponta BESS4 = {BESS4_E_RESERVA_PONTA_2034_KWH:.0f} kWh")
print(f" - Energia reservada para ponta BESS5 = {BESS5_E_RESERVA_PONTA_2034_KWH:.0f} kWh")
...
print("\nResumo anual:")
print(tab24[tab24["MES"].isin(["TOTAIS", "CTOTAL", "R$TOTAL", "DCFP", "DCP"])])
