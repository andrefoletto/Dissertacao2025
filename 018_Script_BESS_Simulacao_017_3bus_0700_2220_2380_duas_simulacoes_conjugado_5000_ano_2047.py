# -*- coding: utf-8 -*-
# =============================================================================
# 018_Script_BESS_Simulacao_017_3bus_0700_2220_2380_duas_simulacoes_conjugado_5000_ano_2047.py
#
# Regras deste script:
...
#     Simulacao 017 segmentos_pass1_sem_hvac_int_ano_2047
#   PASSO 2 (simulacao completa final):
#     Simulacao 017 segmentos_ano_2047
# =============================================================================
...
DIR_PASS1 = WORK_DIR / "Simulacao 017 segmentos_pass1_sem_hvac_int_ano_2047"
...
def build_circuit_pass1(
    ...
    dss.text(
        ...
        f"kWrated=673 kWhrated=2946 Effcurve=Eff %EffCharge=95 %EffDischarge=95 "
...
# %% Celula 003 - PASSO 1: monitores -> Curva_de_Carga_BESSM1_IN_sim_17.csv, BESSM2_IN e BESSM3_IN (SHIFT DIREITA)
# =============================================================================
# Entradas:
#   PASSO1/Monitor_BESSM1_IN_anual.csv
#   PASSO1/Monitor_BESSM2_IN_anual.csv
...
#   PASSO1/Curva_de_Carga_BESSM3_IN_sim_17.csv
# Remove:
#   PASSO1/Monitor_BESSM1_IN_anual.csv
#   PASSO1/Monitor_BESSM2_IN_anual.csv
#   PASSO1/Monitor_BESSM3_IN_anual.csv
...
DIR_PASS2 = WORK_DIR / "Simulacao 017 segmentos_ano_2047"
...
# =============================================================================
# Entrada:
#   Simulacao 017 segmentos_ano_2047/UFSM_Mon_<KEY>_s*.csv
# Saida:
#   Simulacao 017 segmentos_ano_2047/Monitor_<KEY>_anual.csv
# Remove:
...
# =============================================================================
# Entrada:
#   Simulacao 017 segmentos_ano_2047/EXP_MTR_GERAL_*.csv
# Saida:
#   Simulacao 017 segmentos_ano_2047/EXP_MTR_GERAL_anual.csv
# Remove:
#   EXP_MTR_GERAL_*.csv
# =============================================================================
...
#   Simulacao 017 segmentos_ano_2047/Monitor_<KEY>_anual.csv
# Saidas:
#   Simulacao 017 segmentos_ano_2047/Curva_de_Carga_<KEY>_sim_17.csv
# Remove:
...
#   Simulacao 017 segmentos_ano_2047/EXP_MTR_GERAL_anual.csv
# Saida:
#   Simulacao 017 segmentos_ano_2047/Resumo_EnergyMeter_Geral_Sim17.csv
# =============================================================================
...
# %% Celula 010 - ENERGIA e DEMANDA MAX por mes e posto (SIM17 - ANO 2047, 3 BESS 1 MW / 3,5 MWh cada, degradados)
# =============================================================================
# Entradas:
#   META: 00_dados_de_entrada_ano_letivo_2025_15min.csv
#         Usado apenas como mapa anual de Hora_decimal, Mes e Posto.
#   SIM17: curva GERAL com 3 BESS em operacao conjugada para o ano 2047
# Saida:
#   Simulacao 017 segmentos_ano_2047/01_Integralizacao_Mensal_SIM17_2047.csv
#
# Observacao:
#   Esta celula nao compara com SIM2/RGE nem com outra simulacao.
#   Ela apenas integraliza a simulacao conjugada com 3 BESS do ano 2047 por mes e posto.
# =============================================================================
...
df17 = meta.merge(sim17, on="k", how="left").rename(
    columns={"P_3f_S": "P_SIM17", "Q_3f_S": "Q_SIM17"}
)

faltas_p = int(df17["P_SIM17"].isna().sum())
faltas_q = int(df17["Q_SIM17"].isna().sum())
...
df17["E_P_SIM17_kWh"] = df17["P_SIM17"] * DT_H
df17["E_Q_SIM17_kVArh"] = df17["Q_SIM17"] * DT_H

group_cols = ["Mes", "Posto"]

agg17 = df17.groupby(group_cols, dropna=False).agg(
...
agg = agg17.merge(meta_counts, on=group_cols, how="left")
...
# %% Celula 011 - SIMULACAO 017: tabela de custos (SIM17 - ANO 2047, 3 BESS 1 MW / 3,5 MWh cada, degradados)
# =============================================================================
# Entrada:
#   Simulacao 017 segmentos_ano_2047/01_Integralizacao_Mensal_SIM17_2047.csv
#
# Saida:
#   Simulacao 017 segmentos_ano_2047/Tabela_demanda_consumo_por_posto_SIM17_2047.csv
#
# Demandas contratadas do ano 2047:
#   DCFP = 5000 kW * (1 + 22 * 0,5%) = 5550 kW
#   DCP sem BESS = 3000 kW * (1 + 22 * 0,5%) = 3330 kW
#   DCP com 3 BESS = 3330 - 3*(800*0,8416666667) = 3330 - 2020 = 1310 kW
#
# Observacao:
...
# - Os 3 BESS foram dimensionados como 1 MW / 3,5 MWh cada no ano inicial.
# - Em 2047, considera-se 84,1666667% da potencia operacional de 800 kW e da energia nominal.
# - O percentual de reserva para ponta e mantido igual ao caso original: 9000/10500 = 85,7142857%.
DCP_BASE_2025 = 3000.0
DCFP_BASE_2025 = 5000.0
CRESC_LINEAR_ANUAL = 0.005

NUM_BESS = 3
FATOR_REMANESCENTE_2047 = 0.8416666666666667

BESS_P_UNITARIA_OPERACIONAL_BASE_KW = 800.0
BESS_E_UNITARIA_NOMINAL_KWH = 3500.0

# Para faturamento de demanda, usa-se a potencia operacional simulada:
# 3 * (800 kW * 0,8416666667) = 2020 kW.
BESS_P_UNITARIA_KW = BESS_P_UNITARIA_OPERACIONAL_BASE_KW * FATOR_REMANESCENTE_2047
BESS_E_UNITARIA_KWH = BESS_E_UNITARIA_NOMINAL_KWH * FATOR_REMANESCENTE_2047

BESS_P_TOTAL_KW = round(NUM_BESS * BESS_P_UNITARIA_KW, 0)  # 2020 kW em 2047
BESS_E_TOTAL_KWH = NUM_BESS * BESS_E_UNITARIA_KWH          # 8837,5 kWh em 2047

# Separacao operacional da energia:
# - mesmo percentual de reserva da ponta do caso original: 9 MWh / 10,5 MWh;
# - o valor absoluto da reserva reduz proporcionalmente com a degradacao energetica.
BESS_RESERVA_PONTA_PCT = 100.0 * 9000.0 / 10500.0
...
DCP_COM_BESS = max(0.0, DCP_SEM_BESS - BESS_P_TOTAL_KW)       # 1310 kW em 2047
...
print("DONE. Saved (Simulacao 017 - 3 BESS - 2047 - conjugado):")
print(" -", OUT_SIM17)
...
print(f" - Potencia remanescente total dos BESS = {BESS_P_TOTAL_KW:.0f} kW")
print(f" - Energia remanescente total dos BESS = {BESS_E_TOTAL_KWH:.0f} kWh")
