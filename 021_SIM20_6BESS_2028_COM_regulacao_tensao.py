# -*- coding: utf-8 -*-
# =============================================================================
# 021_SIM20_6BESS_2028_COM_regulacao_tensao.py
#
# Regras deste script:
...
#     Simulacao 020 COM regulacao tensao_pass1_sem_hvac_int_ano_2028
#   PASSO 2 (simulacao completa final):
#     Simulacao 020 COM regulacao tensao_ano_2028
# =============================================================================
...
# Regulacao Volt-var dos inversores dos BESS
# A potencia ativa continua comandada pelo StorageController.
BESS_KVA = 1000.0
BESS_KVAR_MAX = 600.0
VV_VMIN = 0.98
VV_VMAX = 1.02
...
TARGETS = {
    ...
    "FORA_PONTA": {"kWtarget": 5075, "kWtargetLow": 4060},
}

def add_bess_voltage_regulation(dss: DSS) -> None:
    # Zona morta: Q=0 entre 0.98 pu e 1.02 pu.
    # Fora da faixa, a resposta cresce linearmente ate +/-1 pu de kvarMax.
    dss.text(
        "New XYCurve.VV_BESS npts=6 "
        f"xarray=[0.94 {VV_VMIN:.2f} 0.99 1.01 {VV_VMAX:.2f} 1.06] "
        "yarray=[1.00 0.00 0.00 0.00 0.00 -1.00]"
    )

    # 13.8 kV fase-fase corresponde a 7967.43 V fase-neutro.
    # Um InvControl por banco evita que o OpenDSS distribua a mesma acao
    # reativa sem considerar o ponto de conexao de cada BESS.
    for storage_name, pcc_bus in BESS_PCC_BUSES.items():
        dss.text(
            f"New InvControl.IC_{storage_name} "
            f"DERList=[Storage.{storage_name}] "
            "Mode=VOLTVAR vvc_curve1=VV_BESS "
            "voltage_curvex_ref=rated RefReactivePower=VARMAX "
            f"monBus=[{pcc_bus}.1.2.3] monBusesVbase=[7967.433715] "
            "monVoltageCalc=AVG VoltageChangeTolerance=0.001 "
            "VarChangeTolerance=0.01 deltaQ_factor=0.25 "
            "RateofChangeMode=INACTIVE"
        )

DIR_PASS1 = WORK_DIR / "Simulacao 020 COM regulacao tensao_pass1_sem_hvac_int_ano_2028"
...
def build_circuit_pass1(
    ...
    dss.text('Redirect "UFSM_cargas_medidas_ano_2028.dss"')
    dss.text('Redirect "UFSM_cargas_especiais_ano_2028.dss"')
    dss.text('Redirect "UFSM_cargas_estimadas_com_residuo_ano_2028.dss"')
    ...
    dss.text(
        ...
        f"kWrated=800 kVA={BESS_KVA:.0f} kWhrated=3250 Effcurve=Eff %EffCharge=95 %EffDischarge=95 kvarMax={BESS_KVAR_MAX:.0f} kvarMaxAbs={BESS_KVAR_MAX:.0f} %CutIn=0 %CutOut=0 %PminNoVars=0 VarFollowInverter=No WattPriority=Yes "
    ...
    # Controle Q(V) simultaneo ao controle de potencia ativa.
    add_bess_voltage_regulation(dss)
...
# %% Celula 003 - PASSO 1: monitores -> Curva_de_Carga_BESSM1_IN_sim_20.csv, BESSM2_IN, BESSM3_IN, BESSM4_IN, BESSM5_IN e BESSM6_IN (SHIFT DIREITA)
# =============================================================================
# Entradas:
#   PASSO1/Monitor_BESSM1_IN_anual.csv
#   PASSO1/Monitor_BESSM2_IN_anual.csv
...
#   PASSO1/Monitor_BESSM6_IN_anual.csv
# Saidas:
#   PASSO1/Curva_de_Carga_BESSM1_IN_sim_20.csv
#   PASSO1/Curva_de_Carga_BESSM2_IN_sim_20.csv
#   PASSO1/Curva_de_Carga_BESSM3_IN_sim_20.csv
#   PASSO1/Curva_de_Carga_BESSM4_IN_sim_20.csv
#   PASSO1/Curva_de_Carga_BESSM5_IN_sim_20.csv
#   PASSO1/Curva_de_Carga_BESSM6_IN_sim_20.csv
# Remove:
#   PASSO1/Monitor_BESSM1_IN_anual.csv
#   PASSO1/Monitor_BESSM2_IN_anual.csv
#   PASSO1/Monitor_BESSM3_IN_anual.csv
...
for mon_base in ["BESSM1_IN", "BESSM2_IN", "BESSM3_IN", "BESSM4_IN", "BESSM5_IN", "BESSM6_IN"]:
    ...
    out_fp = DIR_PASS1 / f"Curva_de_Carga_{mon_base}_sim_20.csv"
...
for mon_base, prefix in [("BESSM1_IN", "BESSM1"), ("BESSM2_IN", "BESSM2"), ("BESSM3_IN", "BESSM3"), ("BESSM4_IN", "BESSM4"), ("BESSM5_IN", "BESSM5"), ("BESSM6_IN", "BESSM6")]:
    in_fp = DIR_PASS1 / f"Curva_de_Carga_{mon_base}_sim_20.csv"
...
VOLTAGE_PROBES = {
    "0622": 0.38, "1242": 0.38, "1832": 0.38, "3062": 0.38,
    "3592": 0.38, "3722": 0.38, "3792": 0.38, "4792": 0.22,
    "5092": 0.38, "5152": 0.38,
}

# Registradores de tensao do PASSO 2 (iguais nos scripts SEM e COM regulacao).
# mode=0 e vipolar=yes exportam os modulos V1, V2 e V3 em volts.
VOLTAGE_MONITOR_SPECS = {
    "V_PCC_0700":   {"element": "line.0700_B011", "terminal": 1, "mode": 0},
    "V_PCC_2220":   {"element": "line.2220_B021", "terminal": 1, "mode": 0},
    "V_PCC_2380":   {"element": "line.2380_B031", "terminal": 1, "mode": 0},
    "V_PCC_1250":   {"element": "line.1250_B041", "terminal": 1, "mode": 0},
    "V_PCC_1630":   {"element": "line.1630_B051", "terminal": 1, "mode": 0},
    "V_PCC_1950":   {"element": "line.1950_B061", "terminal": 1, "mode": 0},
    "V_PAC_0016":   {"element": "line.0016_0017", "terminal": 1, "mode": 0},
    "V_CRIT_0622":  {"element": "load.VPROBE_0622", "terminal": 1, "mode": 0},
    "V_CRIT_1242":  {"element": "load.VPROBE_1242", "terminal": 1, "mode": 0},
    "V_CRIT_1832":  {"element": "load.VPROBE_1832", "terminal": 1, "mode": 0},
    "V_CRIT_3062":  {"element": "load.VPROBE_3062", "terminal": 1, "mode": 0},
    "V_CRIT_3592":  {"element": "load.VPROBE_3592", "terminal": 1, "mode": 0},
    "V_CRIT_3722":  {"element": "load.VPROBE_3722", "terminal": 1, "mode": 0},
    "V_CRIT_3792":  {"element": "load.VPROBE_3792", "terminal": 1, "mode": 0},
    "V_CRIT_4792":  {"element": "load.VPROBE_4792", "terminal": 1, "mode": 0},
    "V_CRIT_5092":  {"element": "load.VPROBE_5092", "terminal": 1, "mode": 0},
    "V_CRIT_5152":  {"element": "load.VPROBE_5152", "terminal": 1, "mode": 0},
}
...
MON_SPECS = {
    ...
    "BESSM6_IN":    {"element": "line.B061_B062", "terminal": 1},
    **VOLTAGE_MONITOR_SPECS,
...
DIR_PASS2 = WORK_DIR / "Simulacao 020 COM regulacao tensao_ano_2028"
...
def _build_circuit_pass2(
    ...
    for bus_name, kv_ll in VOLTAGE_PROBES.items():
        dss.text(
            f"New Load.VPROBE_{bus_name} phases=3 bus1={bus_name}.1.2.3 "
            f"conn=wye kv={kv_ll} model=1 kW=0.000001 kvar=0"
        )
    ...
    for key in MON_KEYS:
        ...
        mon_names[key] = mname
        monitor_mode = int(spec.get("mode", 1))
        if monitor_mode == 0:
            dss.text(f"New Monitor.{mname} element={spec['element']} terminal={spec['terminal']} mode=0 vipolar=yes")
        else:
...
# =============================================================================
# Entrada:
#   Simulacao 020 COM regulacao tensao_ano_2028/UFSM_Mon_<KEY>_s*.csv
# Saida:
#   Simulacao 020 COM regulacao tensao_ano_2028/Monitor_<KEY>_anual.csv
# Remove:
...
# =============================================================================
# Entrada:
#   Simulacao 020 COM regulacao tensao_ano_2028/EXP_MTR_GERAL_*.csv
# Saida:
#   Simulacao 020 COM regulacao tensao_ano_2028/EXP_MTR_GERAL_anual.csv
# Remove:
#   EXP_MTR_GERAL_*.csv
# =============================================================================
...
#   Simulacao 020 COM regulacao tensao_ano_2028/Monitor_<KEY>_anual.csv
# Saidas:
#   Simulacao 020 COM regulacao tensao_ano_2028/Curva_de_Carga_<KEY>_sim_20.csv
#   Os Monitor_V_PCC_<BARRA>_anual.csv sao preservados em formato bruto
#   para verificacao da faixa de tensao de 0.98 pu a 1.02 pu.
# Remove:
#   Monitor_<KEY>_anual.csv, exceto os monitores V_PCC
# =============================================================================
...
files = sorted(
    fp for fp in DIR_PASS2.glob("Monitor_*_anual.csv")
    if not fp.name.startswith("Monitor_V_")
)
if not files:
...
for in_fp in files:
    ...
    out_fp = DIR_PASS2 / f"Curva_de_Carga_{mon_key}_sim_20.csv"
...
# %% Celula 009 - PASSO 2: medias trifasicas anuais de tensao
# =============================================================================
# Gera um unico CSV com Hora_decimal e media dos modulos de Va, Vb e Vc em pu:
# seis PCC dos BESS, PAC 0016 e dez barras BT desfavoraveis.
# =============================================================================
...
DT_H = 0.25
TOTAL_NPTS = 35040
APLICAR_SHIFT_TENSAO = True

VOLTAGE_OUTPUTS = {
    "V_PCC_0700":  ("Vmedia_PCC_0700_pu", 13_800.0 / np.sqrt(3.0)),
    "V_PCC_2220":  ("Vmedia_PCC_2220_pu", 13_800.0 / np.sqrt(3.0)),
    "V_PCC_2380":  ("Vmedia_PCC_2380_pu", 13_800.0 / np.sqrt(3.0)),
    "V_PCC_1250":  ("Vmedia_PCC_1250_pu", 13_800.0 / np.sqrt(3.0)),
    "V_PCC_1630":  ("Vmedia_PCC_1630_pu", 13_800.0 / np.sqrt(3.0)),
    "V_PCC_1950":  ("Vmedia_PCC_1950_pu", 13_800.0 / np.sqrt(3.0)),
    "V_PAC_0016":  ("Vmedia_PAC_0016_pu", 13_800.0 / np.sqrt(3.0)),
    "V_CRIT_0622": ("Vmedia_0622_Residuos_pu", 220.0),
    "V_CRIT_1242": ("Vmedia_1242_Solos_pu", 220.0),
    "V_CRIT_1832": ("Vmedia_1832_Remates_pu", 220.0),
    "V_CRIT_3062": ("Vmedia_3062_Biotecnologia_pu", 220.0),
    "V_CRIT_3592": ("Vmedia_3592_Ibama_pu", 220.0),
    "V_CRIT_3722": ("Vmedia_3722_Tambo_pu", 220.0),
    "V_CRIT_3792": ("Vmedia_3792_Laticinios_pu", 220.0),
    "V_CRIT_4792": ("Vmedia_4792_Antena_pu", 127.0),
    "V_CRIT_5092": ("Vmedia_5092_Suinocultura_pu", 220.0),
    "V_CRIT_5152": ("Vmedia_5152_Pomar_pu", 220.0),
}

OUT_FP = DIR_PASS2 / "Tensoes_medias_anuais_COM_regulacao_sim_20.csv"
hora = np.round(np.arange(0.0, 8760.0, DT_H), 10)


def find_voltage_module_column(columns: list[str], phase: int) -> str:
    normalized = {
        re.sub(r"\s+", "", str(col).strip().lower()): str(col)
        for col in columns
    }
    for candidate in (f"v{phase}", f"v{phase}(v)", f"|v{phase}|", f"vmag{phase}"):
        if candidate in normalized:
            return normalized[candidate]
    raise ValueError(
        f"Coluna do modulo de V{phase} nao encontrada. "
        f"Colunas detectadas: {list(columns)}"
    )


df_out = pd.DataFrame({"Hora_decimal": hora})

for mon_key, (column_name, vbase_ln_v) in VOLTAGE_OUTPUTS.items():
    in_fp = DIR_PASS2 / f"Monitor_{mon_key}_anual.csv"
    if not in_fp.exists():
        raise FileNotFoundError(in_fp)
    ...
    if len(mon) != TOTAL_NPTS:
        raise ValueError(f"[{mon_key}] Linhas={len(mon)}; esperado={TOTAL_NPTS}.")

    voltage_cols = [find_voltage_module_column(list(mon.columns), phase) for phase in (1, 2, 3)]
    voltage_phases = mon[voltage_cols].apply(pd.to_numeric, errors="coerce")
    if voltage_phases.isna().any().any():
        raise ValueError(f"[{mon_key}] NaN nos modulos das tensoes de fase.")

    voltage_mean_pu = voltage_phases.mean(axis=1).to_numpy(dtype=float) / vbase_ln_v
    if APLICAR_SHIFT_TENSAO:
        voltage_mean_pu = np.concatenate((voltage_mean_pu[:1], voltage_mean_pu[:-1]))

    df_out[column_name] = voltage_mean_pu
    in_fp.unlink()

df_out.to_csv(OUT_FP, index=False, encoding="utf-8-sig", float_format="%.6f")
print("[OK] Tensoes medias salvas em:", OUT_FP)
print("[INFO] SHIFT de 15 min aplicado:", APLICAR_SHIFT_TENSAO)

# %% Celula 010 - PASSO 2: resumo do EnergyMeter (linha TOTAL)
# =============================================================================
# Entrada:
#   Simulacao 020 COM regulacao tensao_ano_2028/EXP_MTR_GERAL_anual.csv
# Saida:
#   Simulacao 020 COM regulacao tensao_ano_2028/Resumo_EnergyMeter_Geral_Sim20.csv
# =============================================================================
...
OUT_FP = DIR_PASS2 / "Resumo_EnergyMeter_Geral_Sim20.csv"
...
# %% Celula 011 - ENERGIA e DEMANDA MAX por mes e posto (SIM20 - ANO 2028, 6 BESS 1 MW / 3,25 MWh cada)
# =============================================================================
# Entradas:
#   META: 00_dados_de_entrada_ano_letivo_2025_15min.csv
#         Usado apenas como mapa anual de Hora_decimal, Mes e Posto.
#   SIM20: curva GERAL com 6 BESS em operacao conjugada para o ano 2028
# Saida:
#   Simulacao 020 COM regulacao tensao_ano_2028/01_Integralizacao_Mensal_SIM20_2028.csv
#
# Observacao:
#   Esta celula nao compara com SIM2/RGE nem com outra simulacao.
#   Ela apenas integraliza a simulacao conjugada com 6 BESS do ano 2028 por mes e posto.
# =============================================================================
...
SIM20_DIR = WORK_DIR / f"Simulacao 020 COM regulacao tensao_ano_{ANO_ANALISE}"

META_FP = WORK_DIR / "00_dados_de_entrada_ano_letivo_2025_15min.csv"
SIM20_FP = SIM20_DIR / "Curva_de_Carga_GERAL_sim_20.csv"
OUT_FP = SIM20_DIR / f"01_Integralizacao_Mensal_{SIM_TAG}_{ANO_ANALISE}.csv"
...
for fp in [META_FP, SIM20_FP]:
    if not fp.exists():
        raise FileNotFoundError(fp)

meta = pd.read_csv(META_FP, dtype=str, encoding="utf-8-sig")
sim20 = pd.read_csv(SIM20_FP, encoding="utf-8-sig", sep=None, engine="python")

meta = clean_cols(meta)
sim20 = clean_cols(sim20)

for name, dfx, req in [
    ("META", meta, ["Hora_decimal", "Mes", "Posto"]),
    (SIM_TAG, sim20, ["Hora_decimal", "P_3f_S", "Q_3f_S"]),
]:
...
sim20["Hora_decimal"] = pd.to_numeric(sim20["Hora_decimal"], errors="coerce")
sim20["P_3f_S"] = pd.to_numeric(sim20["P_3f_S"], errors="coerce")
sim20["Q_3f_S"] = pd.to_numeric(sim20["Q_3f_S"], errors="coerce")
sim20.dropna(subset=["Hora_decimal", "P_3f_S", "Q_3f_S"], inplace=True)

meta["k"] = make_k(meta["Hora_decimal"])
sim20["k"] = make_k(sim20["Hora_decimal"])

meta = meta[["k", "Mes", "Posto"]].drop_duplicates(subset=["k"], keep="first").copy()
sim20 = sim20[["k", "P_3f_S", "Q_3f_S"]].drop_duplicates(subset=["k"], keep="first").copy()

df20 = meta.merge(sim20, on="k", how="left").rename(
    columns={"P_3f_S": "P_SIM20", "Q_3f_S": "Q_SIM20"}
)

faltas_p = int(df20["P_SIM20"].isna().sum())
faltas_q = int(df20["Q_SIM20"].isna().sum())
if faltas_p or faltas_q:
    raise ValueError(
        f"Falha de alinhamento por Hora_decimal/k: "
        f"P_SIM20 faltas={faltas_p}, Q_SIM20 faltas={faltas_q}"
    )

df20["E_P_SIM20_kWh"] = df20["P_SIM20"] * DT_H
df20["E_Q_SIM20_kVArh"] = df20["Q_SIM20"] * DT_H

group_cols = ["Mes", "Posto"]

agg20 = df20.groupby(group_cols, dropna=False).agg(
    N_pontos_SIM20=("P_SIM20", lambda s: int(s.notna().sum())),
    Consumo_P_SIM20_kWh=("E_P_SIM20_kWh", "sum"),
    Consumo_Q_SIM20_kVArh=("E_Q_SIM20_kVArh", "sum"),
    DemandaMax_P_SIM20_kW=("P_SIM20", "max"),
    DemandaMax_Q_SIM20_kVAr=("Q_SIM20", "max"),
).reset_index()

meta_counts = meta.groupby(group_cols, dropna=False).agg(N_pontos_META=("Posto", "size")).reset_index()
agg = agg20.merge(meta_counts, on=group_cols, how="left")
...
print("SIM20 usado:", SIM20_FP)
print(agg[["Mes", "Posto", "N_pontos_META", "N_pontos_SIM20", "DemandaMax_P_SIM20_kW"]])

# %% Celula 012 - SIMULACAO 020: tabela de custos (SIM20 - ANO 2028, 6 BESS 1 MW / 3,25 MWh cada)
# =============================================================================
# Entrada:
#   Simulacao 020 COM regulacao tensao_ano_2028/01_Integralizacao_Mensal_SIM20_2028.csv
#
# Saida:
#   Simulacao 020 COM regulacao tensao_ano_2028/Tabela_demanda_consumo_por_posto_SIM20_2028.csv
#
# Demandas contratadas do ano 2028:
#   DCFP = 5075 kW
#   DCP sem BESS = 3000 kW * (1 + 3 * 0,5%) = 3045 kW
#   DCP com 6 BESS = max(0, 3045 - (6 x 800)) = 0 kW
#
# Configuracao energetica dos BESS:
#   6 bancos de 1 MW / 3,25 MWh, operando com kWrated=800 kW por banco.
#   Energia total = 6 x 3250 = 19500 kWh.
#   Reserva para ponta = 18000 kWh.
#   Energia disponivel para peak-shave fora-ponta = 1500 kWh.
#   Percentual de reserva para ponta = 18000 / 19500 = 92,3%.
#
# Observacao:
...
SIM20_DIR = WORK_DIR / f"Simulacao 020 COM regulacao tensao_ano_{ANO_ANALISE}"
SIM20_DIR.mkdir(parents=True, exist_ok=True)

IN_FP = SIM20_DIR / f"01_Integralizacao_Mensal_{SIM_TAG}_{ANO_ANALISE}.csv"
OUT_SIM20 = SIM20_DIR / f"Tabela_demanda_consumo_por_posto_{SIM_TAG}_{ANO_ANALISE}.csv"

# -------------------------
# Demandas contratadas
# -------------------------
# Premissa:
# - 2028 e o ano 1 da analise.
# - Dados de carga sao de 2025.
# - Crescimento linear anual = 0,5% sobre a demanda-base de 2025.
# - Em 2028 aplica-se crescimento acumulado de 1,5%.
# - Os 6 BESS sao instalados no proprio ano de analise.
# - Cada banco possui transformador de 1000 kVA, mas opera com kWrated=800 kW e 3250 kWh.
# - Assim, o conjunto reduz a DCP em 6 x 800 = 4800 kW e totaliza 19500 kWh.
DCP_BASE_2025 = 3000.0
DCFP_BASE_2025 = 5000.0
CRESC_LINEAR_ANUAL = 0.005

NUM_BESS = 6
BESS_P_UNITARIA_KW = 800.0      # potencia operacional usada para reduzir a DCP
BESS_E_UNITARIA_KWH = 3250.0

BESS_P_TOTAL_KW = NUM_BESS * BESS_P_UNITARIA_KW       # 4800 kW
BESS_E_TOTAL_KWH = NUM_BESS * BESS_E_UNITARIA_KWH     # 19500 kWh

# Separacao operacional da energia:
# - 18000 kWh reservados para load-shift no horario de ponta;
# - 1500 kWh disponiveis para peak-shave fora-ponta.
BESS_E_RESERVA_PONTA_KWH = 18000.0
BESS_RESERVA_PONTA_PCT = 100.0 * BESS_E_RESERVA_PONTA_KWH / BESS_E_TOTAL_KWH
...
DCP_SEM_BESS = round(DCP_BASE_2025 * fator_linear, 0)        # 3045 kW em 2028
DCFP_COM_BESS = round(DCFP_BASE_2025 * fator_linear, 0)      # 5075 kW em 2028
DCP_COM_BESS = max(0.0, DCP_SEM_BESS - BESS_P_TOTAL_KW)       # 0 kW em 2028

CONTR_SIM20 = {"DCFP": DCFP_COM_BESS, "DCP": DCP_COM_BESS}
...
if not IN_FP.exists():
    raise FileNotFoundError(
        f"Arquivo nao encontrado: {IN_FP}\n"
        f"Execute primeiro a Celula 010 para gerar a integralizacao mensal da {SIM_TAG}."
    )
...
def tabela_custos(df_in: pd.DataFrame, fonte: str, DCFP: float, DCP: float) -> pd.DataFrame:
    ...
    if miss:
        raise ValueError(f"Colunas faltando para {fonte}: {miss}. Disponiveis: {list(df_in.columns)}")
...
tab20 = tabela_custos(
    df,
    SIM_TAG,
    DCFP=CONTR_SIM20["DCFP"],
    DCP=CONTR_SIM20["DCP"],
)

tab20.to_csv(OUT_SIM20, index=False, encoding="utf-8")

print("DONE. Saved (Simulacao 020 - 6 BESS - 2028 - conjugado):")
print(" -", OUT_SIM20)
print("\nDemandas contratadas usadas em 2028:")
print(f" - DCFP = {DCFP_COM_BESS:.0f} kW")
print(f" - DCP sem BESS = {DCP_SEM_BESS:.0f} kW")
print(f" - DCP com 6 BESS = {DCP_COM_BESS:.0f} kW")
print(f" - Potencia operacional total dos BESS = {BESS_P_TOTAL_KW:.0f} kW")
print(f" - Energia nominal total dos BESS = {BESS_E_TOTAL_KWH:.0f} kWh")
print(f" - Energia reservada para ponta = {BESS_E_RESERVA_PONTA_KWH:.0f} kWh")
print(f" - Percentual de reserva para ponta = {BESS_RESERVA_PONTA_PCT:.1f}%")
print(f" - Energia disponivel para peak-shave fora-ponta = {BESS_E_DISPONIVEL_FP_KWH:.0f} kWh")
print("\nResumo anual:")
print(tab20[tab20["MES"].isin(["TOTAIS", "CTOTAL", "R$TOTAL", "DCFP", "DCP"])])
