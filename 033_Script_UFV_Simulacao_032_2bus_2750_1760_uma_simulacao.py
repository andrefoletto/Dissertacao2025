# -*- coding: utf-8 -*-
# =============================================================================
# 033_Script_UFV_Simulacao_032_2bus_2750_1760_uma_simulacao.py
#
# Regras deste script:
...
DIR_PASS2 = WORK_DIR / "Simulacao 032 segmentos"
...
# =============================================================================
# Entrada:
#   Simulacao 032 segmentos/UFSM_Mon_<KEY>_s*.csv
# Saida:
#   Simulacao 032 segmentos/Monitor_<KEY>_anual.csv
# Remove:
...
# =============================================================================
# Entrada:
#   Simulacao 032 segmentos/EXP_MTR_GERAL_*.csv
# Saida:
#   Simulacao 032 segmentos/EXP_MTR_GERAL_anual.csv
# Remove:
#   EXP_MTR_GERAL_*.csv
# =============================================================================
...
# Saidas:
#   Simulacao 032 segmentos/Curva_de_Carga_<KEY>_sim_32.csv
# Remove:
...
for in_fp in files:
    ...
    out_fp = DIR_PASS2 / f"Curva_de_Carga_{mon_key}_sim_32.csv"
...
#   Simulacao 032 segmentos/EXP_MTR_GERAL_anual.csv
# Saida:
#   Simulacao 032 segmentos/Resumo_EnergyMeter_Geral_Sim32.csv
# =============================================================================
...
OUT_FP = DIR_PASS2 / "Resumo_EnergyMeter_Geral_Sim32.csv"
...
# %% Celula 006 - ENERGIA e DEMANDA MAX por mes e posto (SIM2 vs SIM32)
# =============================================================================
# Entradas:
...
#   SIM32: Simulacao 032 segmentos/Curva_de_Carga_GERAL_sim_32.csv
#         (Hora_decimal, P_3f_S, Q_3f_S)
#
# Saida:
#   Simulacao 032 segmentos/01_Integralizacao_Mensal_SIM2_x_SIM32_2025.csv
#
# OBSERVACAO:
#   - P > 0  -> consumo da rede
#   - P < 0  -> injecao na rede
...
SIM32_DIR = WORK_DIR / "Simulacao 032 segmentos"

META_FP = WORK_DIR / "00_dados_de_entrada_ano_letivo_2025_15min.csv"
SIM2_FP = SIM2_DIR / "Curva_de_Carga_GERAL_sim_2.csv"
SIM32_FP = SIM32_DIR / "Curva_de_Carga_GERAL_sim_32.csv"
OUT_FP  = SIM32_DIR / "01_Integralizacao_Mensal_SIM2_x_SIM32_2025.csv"
...
for fp in [META_FP, SIM2_FP, SIM32_FP]:
    if not fp.exists():
...
df32 = meta.merge(sim32, on="k", how="left").rename(columns={"P_3f_S": "P_SIM32", "Q_3f_S": "Q_SIM32"})

# -----------------------------------------------------------------------------
# Separacao entre consumo da rede (P > 0) e injecao na rede (P < 0)
# -----------------------------------------------------------------------------
for dfx, tag in [(df2, "SIM2"), (df32, "SIM32")]:
    pcol = f"P_{tag}"
...
agg32 = df32.groupby(group_cols, dropna=False).agg(
...
agg = agg2.merge(agg32, on=group_cols, how="outer")
...
# %% Celula 007 - SIMULACAO 032: Tabela de custos (SIM2 vs SIM32 - 2025)
# =============================================================================
# Entrada:
#   Simulacao 032 segmentos/01_Integralizacao_Mensal_SIM2_x_SIM32_2025.csv
#
# Saidas (todas em Simulacao 032 segmentos):
#   Tabela_demanda_consumo_por_posto_SIM2_2025.csv
#   Tabela_demanda_consumo_por_posto_SIM32_2025.csv
#   Tabela_demanda_consumo_por_posto_SIM2_vs_SIM32_2025.csv
#
# HIPOTESE ADOTADA:
...
IN_FP     = SIM32_DIR / "01_Integralizacao_Mensal_SIM2_x_SIM32_2025.csv"
OUT_SIM2  = SIM32_DIR / "Tabela_demanda_consumo_por_posto_SIM2_2025.csv"
OUT_SIM32 = SIM32_DIR / "Tabela_demanda_consumo_por_posto_SIM32_2025.csv"
OUT_CMP   = SIM32_DIR / "Tabela_demanda_consumo_por_posto_SIM2_vs_SIM32_2025.csv"

# -------------------------
# DEMANDAS CONTRATADAS POR SIMULACAO
# -------------------------
CONTR_SIM2  = {"DCFP": 4500.0, "DCP": 3000.0}
CONTR_SIM32 = {"DCFP": 4500.0, "DCP": 3000.0}
...
tab32 = tabela_custos(df, "SIM32", DCFP=CONTR_SIM32["DCFP"], DCP=CONTR_SIM32["DCP"])
...
t32 = tab32[tab32["MES"].isin(MESES_ORD)][["MES", "R$TOTAL_MES"]].rename(columns={"R$TOTAL_MES": "R$SIM32"})

cmp = t2.merge(t32, on="MES", how="inner")
cmp["MES"] = pd.Categorical(cmp["MES"], categories=MESES_ORD, ordered=True)
cmp = cmp.sort_values("MES").reset_index(drop=True)

cmp["Economia_R$"] = (cmp["R$SIM2"] - cmp["R$SIM32"]).round(2)
...
print("DONE. Saved (Simulacao 032 segmentos):")
print(" -", OUT_SIM2)
print(" -", OUT_SIM32)
print(" -", OUT_CMP)
...
print(" - SIM32: DCFP=4500, DCP=3000")
