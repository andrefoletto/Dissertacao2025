# -*- coding: utf-8 -*-
# =============================================================================
# 035_Script_UFV_Simulacao_034_4bus_2750_1760_1950_1960_uma_simulacao.py
#
# Regras deste script:
...
#     validado no PVsyst: 1760 modulos de 450 W, 88 strings x 20 modulos,
#     8 inversores x 100 kW, 11 strings por inversor.
#   - Inclui UFV4 ~ 792 kWp em U033 (BT do trafo UFV8004_1000), coerente com o arranjo
#     validado no PVsyst: 1760 modulos de 450 W, 88 strings x 20 modulos,
#     8 inversores x 100 kW, 11 strings por inversor.
# ======================================================================================
...
DIR_PASS2 = WORK_DIR / "Simulacao 034 segmentos"
...
# =============================================================================
# Entrada:
#   Simulacao 034 segmentos/UFSM_Mon_<KEY>_s*.csv
# Saida:
#   Simulacao 034 segmentos/Monitor_<KEY>_anual.csv
# Remove:
...
# =============================================================================
# Entrada:
#   Simulacao 034 segmentos/EXP_MTR_GERAL_*.csv
# Saida:
#   Simulacao 034 segmentos/EXP_MTR_GERAL_anual.csv
# Remove:
#   EXP_MTR_GERAL_*.csv
# =============================================================================
...
# Saidas:
#   Simulacao 034 segmentos/Curva_de_Carga_<KEY>_sim_34.csv
# Remove:
...
for in_fp in files:
    ...
    out_fp = DIR_PASS2 / f"Curva_de_Carga_{mon_key}_sim_34.csv"
...
#   Simulacao 034 segmentos/EXP_MTR_GERAL_anual.csv
# Saida:
#   Simulacao 034 segmentos/Resumo_EnergyMeter_Geral_Sim34.csv
# =============================================================================
...
OUT_FP = DIR_PASS2 / "Resumo_EnergyMeter_Geral_Sim34.csv"
...
# %% Celula 006 - ENERGIA e DEMANDA MAX por mes e posto (SIM2 vs SIM34)
# =============================================================================
# Entradas:
...
#   SIM34: Simulacao 034 segmentos/Curva_de_Carga_GERAL_sim_34.csv
#         (Hora_decimal, P_3f_S, Q_3f_S)
#
# Saida:
#   Simulacao 034 segmentos/01_Integralizacao_Mensal_SIM2_x_SIM34_2025.csv
#
# OBSERVACAO:
#   - P > 0  -> consumo da rede
#   - P < 0  -> injecao na rede
...
SIM34_DIR = WORK_DIR / "Simulacao 034 segmentos"

META_FP = WORK_DIR / "00_dados_de_entrada_ano_letivo_2025_15min.csv"
SIM2_FP = SIM2_DIR / "Curva_de_Carga_GERAL_sim_2.csv"
SIM34_FP = SIM34_DIR / "Curva_de_Carga_GERAL_sim_34.csv"
OUT_FP  = SIM34_DIR / "01_Integralizacao_Mensal_SIM2_x_SIM34_2025.csv"
...
for fp in [META_FP, SIM2_FP, SIM34_FP]:
    if not fp.exists():
...
sim34 = pd.read_csv(SIM34_FP, encoding="utf-8-sig", sep=None, engine="python")

for name, dfx, req in [
    ("META", meta, ["Hora_decimal", "Mes", "Posto"]),
    ("SIM2", sim2, ["Hora_decimal", "P_3f_S", "Q_3f_S"]),
    ("SIM34", sim34, ["Hora_decimal", "P_3f_S", "Q_3f_S"]),
]:
...
sim34["k"] = make_k(sim34["Hora_decimal"])
...
sim34 = sim34[["k", "P_3f_S", "Q_3f_S"]].drop_duplicates(subset=["k"], keep="first").copy()

df2 = meta.merge(sim2, on="k", how="left").rename(columns={"P_3f_S": "P_SIM2", "Q_3f_S": "Q_SIM2"})
df34 = meta.merge(sim34, on="k", how="left").rename(columns={"P_3f_S": "P_SIM34", "Q_3f_S": "Q_SIM34"})

# -----------------------------------------------------------------------------
# Separacao entre consumo da rede (P > 0) e injecao na rede (P < 0)
# -----------------------------------------------------------------------------
for dfx, tag in [(df2, "SIM2"), (df34, "SIM34")]:
    pcol = f"P_{tag}"
...
agg34 = df34.groupby(group_cols, dropna=False).agg(
    N_pontos_SIM34=("P_SIM34", lambda s: int(s.notna().sum())),
    Consumo_P_SIM34_kWh=("E_cons_SIM34_kWh", "sum"),
    Injecao_P_SIM34_kWh=("E_inj_SIM34_kWh", "sum"),
    Consumo_Q_SIM34_kVArh=("E_Q_SIM34_kVArh", "sum"),
    DemandaMax_P_SIM34_kW=("P_cons_SIM34_kW", "max"),
    DemandaMax_Inj_SIM34_kW=("P_inj_SIM34_kW", "max"),
).reset_index()

agg = agg2.merge(agg34, on=group_cols, how="outer")
...
# %% Celula 007 - SIMULACAO 034: Tabela de custos (SIM2 vs SIM34 - 2025)
# =============================================================================
# Entrada:
#   Simulacao 034 segmentos/01_Integralizacao_Mensal_SIM2_x_SIM34_2025.csv
#
# Saidas (todas em Simulacao 034 segmentos):
#   Tabela_demanda_consumo_por_posto_SIM2_2025.csv
#   Tabela_demanda_consumo_por_posto_SIM34_2025.csv
#   Tabela_demanda_consumo_por_posto_SIM2_vs_SIM34_2025.csv
#
# HIPOTESE ADOTADA:
...
WORK_DIR = Path(r"C:\Users\afole\OneDrive\Dissertacao2025")
SIM34_DIR = WORK_DIR / "Simulacao 034 segmentos"
SIM34_DIR.mkdir(parents=True, exist_ok=True)

IN_FP     = SIM34_DIR / "01_Integralizacao_Mensal_SIM2_x_SIM34_2025.csv"
OUT_SIM2  = SIM34_DIR / "Tabela_demanda_consumo_por_posto_SIM2_2025.csv"
OUT_SIM34 = SIM34_DIR / "Tabela_demanda_consumo_por_posto_SIM34_2025.csv"
OUT_CMP   = SIM34_DIR / "Tabela_demanda_consumo_por_posto_SIM2_vs_SIM34_2025.csv"

# -------------------------
# DEMANDAS CONTRATADAS POR SIMULACAO
# -------------------------
CONTR_SIM2  = {"DCFP": 5000.0, "DCP": 3000.0}
CONTR_SIM34 = {"DCFP": 5000.0, "DCP": 3000.0}
...
tab34 = tabela_custos(df, "SIM34", DCFP=CONTR_SIM34["DCFP"], DCP=CONTR_SIM34["DCP"])

tab2.to_csv(OUT_SIM2, index=False, encoding="utf-8")
tab34.to_csv(OUT_SIM34, index=False, encoding="utf-8")

# -------------------------
# Comparativo (economia)
# -------------------------
t2 = tab2[tab2["MES"].isin(MESES_ORD)][["MES", "R$TOTAL_MES"]].rename(columns={"R$TOTAL_MES": "R$SIM2"})
t34 = tab34[tab34["MES"].isin(MESES_ORD)][["MES", "R$TOTAL_MES"]].rename(columns={"R$TOTAL_MES": "R$SIM34"})

cmp = t2.merge(t34, on="MES", how="inner")
cmp["MES"] = pd.Categorical(cmp["MES"], categories=MESES_ORD, ordered=True)
cmp = cmp.sort_values("MES").reset_index(drop=True)

cmp["Economia_R$"] = (cmp["R$SIM2"] - cmp["R$SIM34"]).round(2)
...
print("DONE. Saved (Simulacao 034 segmentos):")
print(" -", OUT_SIM2)
print(" -", OUT_SIM34)
print(" -", OUT_CMP)
...
print(" - SIM34: DCFP=5000, DCP=3000")
