# -*- coding: utf-8 -*-
# =============================================================================
# 033_Script_UFV_Simulacao_032_2bus_2750_1760_uma_simulacao_ano_2028_2033.py
#
# Regras deste script:
...
#   - Curvas de carga da rede do ano de 2033.
#   - Uma simulacao com duas UFVs, sem comparativo com SIM2.
#   - UFV1 instalada em 2028, com degradacao DC das placas no ano de 2033.
#   - UFV2 instalada em 2033, sem degradacao no ano de entrada.
#   - Demandas contratadas: DCFP=5200 kW e DCP=3120 kW.
# =============================================================================

# %% Celula 001 - PASSO UNICO: simulacao segmentada da rede base (monitores + EnergyMeter)
# ======================================================================================
# Saidas:
#   Simulacao 032 segmentos_ano_2028_2033/
...
DIR_PASS2 = WORK_DIR / "Simulacao 032 segmentos_ano_2028_2033"
...
if LIMPAR_SAIDAS_ANTERIORES:
    padroes_limpeza = [
        "UFSM_Mon_*_s*.csv",
        "Monitor_*_anual.csv",
        "Curva_de_Carga_*_sim_32_2033.csv",
        "Curva_de_Carga_*_sim_32.csv",
        "EXP_MTR_GERAL*.csv",
        "Resumo_EnergyMeter_Geral_Sim32_2033.csv",
        "01_Integralizacao_Mensal_SIM32_2033.csv",
        "Tabela_demanda_consumo_por_posto_SIM32_2033.csv",
    ]
    removidos = 0
    for padrao in padroes_limpeza:
        for fp in DIR_PASS2.glob(padrao):
            if fp.is_file():
                fp.unlink()
                removidos += 1
    print(f"[CLEAN] SIM32 2033: {removidos} arquivo(s) antigo(s) removido(s) em {DIR_PASS2}")
...
def _build_circuit_pass2(dss: DSS, posto: str, seg_tag: str) -> dict[str, str]:
    ...
    # UFV1 instalada em 2028 e simulada em 2033.
    # Degradacao DC das placas por 5 anos: 0.995^5.
    FATOR_DEGRADACAO_UFV1_5_ANOS = 0.995 ** 5
    PMPP_INV_ANO_BASE = 99.0
    PMPP_INV_UFV1_2033 = PMPP_INV_ANO_BASE * FATOR_DEGRADACAO_UFV1_5_ANOS

    for i in range(1, 9):
        dss.text(
            f"New PVSystem.UFV8001_INV{i} phases=3 bus1=U013 kv=0.38 "
            f"pmpp={PMPP_INV_UFV1_2033:.3f} kVA=100 pf=1.0 conn=wye "
    ...
    # 11 strings por inversor -> 220 modulos/inversor -> 99.0 kWpdc por inversor
    # UFV2 instalada em 2033; sem degradacao no ano de entrada.
    PMPP_INV_UFV2_2033 = PMPP_INV_ANO_BASE

    for i in range(1, 9):
        dss.text(
            f"New PVSystem.UFV8002_INV{i} phases=3 bus1=U023 kv=0.38 "
            f"pmpp={PMPP_INV_UFV2_2033:.3f} kVA=100 pf=1.0 conn=wye "
            f"%cutin=0.1 %cutout=0.1 "
            f"effcurve=Eff_Huawei100k P-TCurve=P_T_Trina450 "
...
# =============================================================================
# Entrada:
#   Simulacao 032 segmentos_ano_2028_2033/UFSM_Mon_<KEY>_s*.csv
# Saida:
#   Simulacao 032 segmentos_ano_2028_2033/Monitor_<KEY>_anual.csv
# Remove:
...
# =============================================================================
# Entrada:
#   Simulacao 032 segmentos_ano_2028_2033/EXP_MTR_GERAL_*.csv
# Saida:
#   Simulacao 032 segmentos_ano_2028_2033/EXP_MTR_GERAL_anual.csv
# Remove:
#   EXP_MTR_GERAL_*.csv
# =============================================================================
...
# Saidas:
#   Simulacao 032 segmentos_ano_2028_2033/Curva_de_Carga_<KEY>_sim_32_2033.csv
# Remove:
...
for in_fp in files:
    ...
    out_fp = DIR_PASS2 / f"Curva_de_Carga_{mon_key}_sim_32_2033.csv"
...
#   Simulacao 032 segmentos_ano_2028_2033/EXP_MTR_GERAL_anual.csv
# Saida:
#   Simulacao 032 segmentos_ano_2028_2033/Resumo_EnergyMeter_Geral_Sim32_2033.csv
# =============================================================================
...
OUT_FP = DIR_PASS2 / "Resumo_EnergyMeter_Geral_Sim32_2033.csv"
...
# %% Celula 006 - ENERGIA e DEMANDA MAX por mes e posto (SIM32 - 2033)
# =============================================================================
# Entradas:
#   META: 00_dados_de_entrada_ano_letivo_2025_15min.csv
#         (Hora_decimal, Mes, Posto)
#   SIM32: Simulacao 032 segmentos_ano_2028_2033/Curva_de_Carga_GERAL_sim_32_2033.csv
#         (Hora_decimal, P_3f_S, Q_3f_S)
#
# Saida:
#   Simulacao 032 segmentos_ano_2028_2033/01_Integralizacao_Mensal_SIM32_2033.csv
#
# OBSERVACAO:
#   - P > 0  -> consumo da rede
#   - P < 0  -> injecao na rede
...
WORK_DIR = Path(r"C:\Users\afole\OneDrive\Dissertacao2025")
SIM32_DIR = WORK_DIR / "Simulacao 032 segmentos_ano_2028_2033"

META_FP = WORK_DIR / "00_dados_de_entrada_ano_letivo_2025_15min.csv"
SIM32_FP = SIM32_DIR / "Curva_de_Carga_GERAL_sim_32_2033.csv"
OUT_FP = SIM32_DIR / "01_Integralizacao_Mensal_SIM32_2033.csv"
...
# %% Celula 007 - SIMULACAO 032: Tabela de custos (SIM32 - 2033)
# =============================================================================
# Entrada:
#   Simulacao 032 segmentos_ano_2028_2033/01_Integralizacao_Mensal_SIM32_2033.csv
#
# Saida:
#   Simulacao 032 segmentos_ano_2028_2033/Tabela_demanda_consumo_por_posto_SIM32_2033.csv
#
# HIPOTESE ADOTADA:
...
IN_FP = SIM32_DIR / "01_Integralizacao_Mensal_SIM32_2033.csv"
OUT_SIM32 = SIM32_DIR / "Tabela_demanda_consumo_por_posto_SIM32_2033.csv"

# -------------------------
# DEMANDAS CONTRATADAS
# -------------------------
CONTR_SIM32 = {"DCFP": 5200.0, "DCP": 3120.0}
...
print("DONE. Saved (Simulacao 032 segmentos_ano_2028_2033):")
print(" -", OUT_SIM32)

print("\nDemandas contratadas usadas:")
print(" - SIM32: DCFP=5200 kW, DCP=3120 kW")
