# -*- coding: utf-8 -*-
# =============================================================================
# 016_Script_BESS_Simulacao_015_bus_0700_duas_simulacoes_conjugado.py
#
# Regras deste script:
...
#     Simulacao 015 segmentos_pass1_sem_hvac_int
#   PASSO 2 (simulacao completa final):
#     Simulacao 015 segmentos
# =============================================================================
...
TARGETS = {
    ...
    "FORA_PONTA": {"kWtarget": 5000, "kWtargetLow": 4000},
}

DIR_PASS1 = WORK_DIR / "Simulacao 015 segmentos_pass1_sem_hvac_int"
...
def build_circuit_pass1(dss: DSS, soc_init_pct: float, posto: str, seg_tag: str) -> str:
    ...
    dss.text(
        ...
        f"kWrated=800 kWhrated=4000 Effcurve=Eff %EffCharge=95 %EffDischarge=95 "
...
DIR_PASS2 = WORK_DIR / "Simulacao 015 segmentos"
...
# =============================================================================
# Entrada:
#   Simulacao 015 segmentos/UFSM_Mon_<KEY>_s*.csv
# Saida:
#   Simulacao 015 segmentos/Monitor_<KEY>_anual.csv
# Remove:
...
# =============================================================================
# Entrada:
#   Simulacao 015 segmentos/EXP_MTR_GERAL_*.csv
# Saida:
#   Simulacao 015 segmentos/EXP_MTR_GERAL_anual.csv
# Remove:
#   EXP_MTR_GERAL_*.csv
# =============================================================================
...
#   Simulacao 015 segmentos/Monitor_<KEY>_anual.csv
# Saidas:
#   Simulacao 015 segmentos/Curva_de_Carga_<KEY>_sim_15.csv
# Remove:
...
#   Simulacao 015 segmentos/EXP_MTR_GERAL_anual.csv
# Saida:
#   Simulacao 015 segmentos/Resumo_EnergyMeter_Geral_Sim15.csv
# =============================================================================
...
# %% Celula 010 - ENERGIA e DEMANDA MAX por mes e posto (SIM2 vs SIM15)
# =============================================================================
# Entradas:
...
#   SIM15: Simulacao 015 segmentos/Curva_de_Carga_GERAL_sim_15.csv (Hora_decimal, P_3f_S, Q_3f_S)
# Saida:
#   Simulacao 015 segmentos/01_Integralizacao_Mensal_SIM2_x_SIM15_2025.csv
# =============================================================================
...
SIM15_DIR = WORK_DIR / "Simulacao 015 segmentos"
...
OUT_FP  = SIM15_DIR / "01_Integralizacao_Mensal_SIM2_x_SIM15_2025.csv"
...
for fp in [META_FP, SIM2_FP, SIM15_FP]:
    if not fp.exists():
...
for name, dfx, req in [
    ...
    ("SIM15", sim15, ["Hora_decimal","P_3f_S","Q_3f_S"]),
]:
...
df15 = meta.merge(sim15, on="k", how="left").rename(columns={"P_3f_S":"P_SIM15","Q_3f_S":"Q_SIM15"})
...
agg = agg2.merge(agg15, on=group_cols, how="outer")
...
# %% Celula 011 - SIMULACAO 015: Tabela de custos (SIM2 vs SIM15 - 2025) com demandas contratadas diferentes
# =============================================================================
# Entrada:
#   Simulacao 015 segmentos/01_Integralizacao_Mensal_SIM2_x_SIM15_2025.csv
#
# Saidas (todas em Simulacao 015 segmentos):
#   Tabela_demanda_consumo_por_posto_SIM2_2025.csv
#   Tabela_demanda_consumo_por_posto_SIM15_2025.csv
#   Tabela_demanda_consumo_por_posto_SIM2_vs_SIM15_2025.csv   (economia)
# =============================================================================
...
IN_FP    = SIM15_DIR / "01_Integralizacao_Mensal_SIM2_x_SIM15_2025.csv"
OUT_SIM2 = SIM15_DIR / "Tabela_demanda_consumo_por_posto_SIM2_2025.csv"
OUT_SIM15 = SIM15_DIR / "Tabela_demanda_consumo_por_posto_SIM15_2025.csv"
OUT_CMP  = SIM15_DIR / "Tabela_demanda_consumo_por_posto_SIM2_vs_SIM15_2025.csv"

# -------------------------
# DEMANDAS CONTRATADAS POR SIMULACAO
# -------------------------
CONTR_SIM2 = {"DCFP": 5000.0, "DCP": 3000.0}
CONTR_SIM15 = {"DCFP": 5000.0, "DCP": 2200.0}  # 3000 - 800
...
tab15 = tabela_custos(df, "SIM15", DCFP=CONTR_SIM15["DCFP"], DCP=CONTR_SIM15["DCP"])
...
print("DONE. Saved (Simulacao 015 segmentos):")
print(" -", OUT_SIM2)
print(" -", OUT_SIM15)
print(" -", OUT_CMP)
print("\nDemandas contratadas usadas:")
print(" - SIM2: DCFP=5000, DCP=3000")
print(" - SIM15: DCFP=5000, DCP=2200")
print("\nResumo anual:")
print(cmp.tail(1))
