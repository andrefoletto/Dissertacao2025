# -*- coding: utf-8 -*-
# =============================================================================
# 032_Script_UFV_Simulacao_031_1bus_2750_uma_simulacao_ano_2028.py
#
# Regras deste script:
...
#   - Uma simulacao UFV, sem comparativo com SIM2.
...
# ======================================================================================
# Saidas:
#   Simulacao 031 segmentos_ano_2028/
...
DIR_PASS2 = WORK_DIR / "Simulacao 031 segmentos_ano_2028"
...
if LIMPAR_SAIDAS_ANTERIORES:
    padroes_limpeza = [
        "UFSM_Mon_*_s*.csv",
        "Monitor_*_anual.csv",
        "Curva_de_Carga_*_sim_31_2028.csv",
        "Curva_de_Carga_*_sim_31.csv",
        "EXP_MTR_GERAL*.csv",
        "Resumo_EnergyMeter_Geral_Sim31_2028.csv",
        "01_Integralizacao_Mensal_SIM31_2028.csv",
        "Tabela_demanda_consumo_por_posto_SIM31_2028.csv",
    ]
    removidos = 0
    for padrao in padroes_limpeza:
        for fp in DIR_PASS2.glob(padrao):
            if fp.is_file():
                fp.unlink()
                removidos += 1
    print(f"[CLEAN] SIM31 2028: {removidos} arquivo(s) antigo(s) removido(s) em {DIR_PASS2}")
...
# =============================================================================
# Entrada:
#   Simulacao 031 segmentos_ano_2028/UFSM_Mon_<KEY>_s*.csv
# Saida:
#   Simulacao 031 segmentos_ano_2028/Monitor_<KEY>_anual.csv
# Remove:
...
# =============================================================================
# Entrada:
#   Simulacao 031 segmentos_ano_2028/EXP_MTR_GERAL_*.csv
# Saida:
#   Simulacao 031 segmentos_ano_2028/EXP_MTR_GERAL_anual.csv
# Remove:
#   EXP_MTR_GERAL_*.csv
# =============================================================================
...
# Saidas:
#   Simulacao 031 segmentos_ano_2028/Curva_de_Carga_<KEY>_sim_31_2028.csv
# Remove:
...
for in_fp in files:
    ...
    out_fp = DIR_PASS2 / f"Curva_de_Carga_{mon_key}_sim_31_2028.csv"
...
#   Simulacao 031 segmentos_ano_2028/EXP_MTR_GERAL_anual.csv
# Saida:
#   Simulacao 031 segmentos_ano_2028/Resumo_EnergyMeter_Geral_Sim31_2028.csv
# =============================================================================
...
OUT_FP = DIR_PASS2 / "Resumo_EnergyMeter_Geral_Sim31_2028.csv"
...
# %% Celula 006 - ENERGIA e DEMANDA MAX por mes e posto (SIM31 - 2028)
# =============================================================================
# Entradas:
#   META: 00_dados_de_entrada_ano_letivo_2025_15min.csv
#         (Hora_decimal, Mes, Posto)
#   SIM31: Simulacao 031 segmentos_ano_2028/Curva_de_Carga_GERAL_sim_31_2028.csv
#         (Hora_decimal, P_3f_S, Q_3f_S)
#
# Saida:
#   Simulacao 031 segmentos_ano_2028/01_Integralizacao_Mensal_SIM31_2028.csv
#
# OBSERVACAO:
#   - P > 0  -> consumo da rede
#   - P < 0  -> injecao na rede
...
WORK_DIR = Path(r"C:\Users\afole\OneDrive\Dissertacao2025")
SIM31_DIR = WORK_DIR / "Simulacao 031 segmentos_ano_2028"

META_FP = WORK_DIR / "00_dados_de_entrada_ano_letivo_2025_15min.csv"
SIM31_FP = SIM31_DIR / "Curva_de_Carga_GERAL_sim_31_2028.csv"
OUT_FP = SIM31_DIR / "01_Integralizacao_Mensal_SIM31_2028.csv"
...
sim31["Hora_decimal"] = pd.to_numeric(sim31["Hora_decimal"], errors="coerce")
sim31["P_3f_S"] = pd.to_numeric(sim31["P_3f_S"], errors="coerce")
sim31["Q_3f_S"] = pd.to_numeric(sim31["Q_3f_S"], errors="coerce")
sim31.dropna(subset=["Hora_decimal", "P_3f_S", "Q_3f_S"], inplace=True)
...
df31 = meta.merge(sim31, on="k", how="left").rename(
    columns={"P_3f_S": "P_SIM31", "Q_3f_S": "Q_SIM31"}
)

# -----------------------------------------------------------------------------
# Separacao entre consumo da rede (P > 0) e injecao na rede (P < 0)
# -----------------------------------------------------------------------------
df31["P_cons_SIM31_kW"] = df31["P_SIM31"].clip(lower=0)
df31["P_inj_SIM31_kW"] = (-df31["P_SIM31"]).clip(lower=0)

df31["E_cons_SIM31_kWh"] = df31["P_cons_SIM31_kW"] * DT_H
df31["E_inj_SIM31_kWh"] = df31["P_inj_SIM31_kW"] * DT_H
df31["E_Q_SIM31_kVArh"] = df31["Q_SIM31"] * DT_H

group_cols = ["Mes", "Posto"]

agg = df31.groupby(group_cols, dropna=False).agg(
...
# %% Celula 007 - SIMULACAO 031: Tabela de custos (SIM31 - 2028)
# =============================================================================
# Entrada:
#   Simulacao 031 segmentos_ano_2028/01_Integralizacao_Mensal_SIM31_2028.csv
#
# Saida:
#   Simulacao 031 segmentos_ano_2028/Tabela_demanda_consumo_por_posto_SIM31_2028.csv
#
# HIPOTESE ADOTADA:
...
IN_FP = SIM31_DIR / "01_Integralizacao_Mensal_SIM31_2028.csv"
OUT_SIM31 = SIM31_DIR / "Tabela_demanda_consumo_por_posto_SIM31_2028.csv"

# -------------------------
# DEMANDAS CONTRATADAS
# -------------------------
CONTR_SIM31 = {"DCFP": 5075.0, "DCP": 3045.0}
...
tab31 = tabela_custos(
    df,
    "SIM31",
    DCFP=CONTR_SIM31["DCFP"],
    DCP=CONTR_SIM31["DCP"],
)

tab31.to_csv(OUT_SIM31, index=False, encoding="utf-8")

print("DONE. Saved (Simulacao 031 segmentos_ano_2028):")
print(" -", OUT_SIM31)

print("\nDemandas contratadas usadas:")
print(" - SIM31: DCFP=5075 kW, DCP=3045 kW")
