# -*- coding: utf-8 -*-
# =============================================================================
# 034_Script_UFV_Simulacao_033_3bus_2750_1760_1950_uma_simulacao_ano_2028_2033_2036_2047.py
#
# Regras deste script:
...
#   - Curvas de carga da rede do ano de 2047.
#   - Uma simulacao com tres UFVs, sem comparativo com SIM2.
#   - UFV1 instalada em 2028, com degradacao DC das placas no ano de 2047.
#   - UFV2 instalada em 2033, com degradacao DC das placas no ano de 2047.
#   - UFV3 instalada em 2036, com degradacao DC das placas no ano de 2047.
#   - Demandas contratadas: DCFP=5550 kW e DCP=3330 kW.
# =============================================================================

# %% Celula 001 - PASSO UNICO: simulacao segmentada da rede base (monitores + EnergyMeter)
# ======================================================================================
# Saidas:
#   Simulacao 033 segmentos_ano_2028_2033_2036_2047/
...
DIR_PASS2 = WORK_DIR / "Simulacao 033 segmentos_ano_2028_2033_2036_2047"
DIR_PASS2.mkdir(parents=True, exist_ok=True)

# Limpa saidas antigas no inicio da Celula 001.
...
if LIMPAR_SAIDAS_ANTERIORES:
    padroes_limpeza = [
        "UFSM_Mon_*_s*.csv",
        "Monitor_*_anual.csv",
        "Curva_de_Carga_*_sim_33_2047.csv",
        "Curva_de_Carga_*_sim_33.csv",
        "EXP_MTR_GERAL*.csv",
        "Resumo_EnergyMeter_Geral_Sim33_2047.csv",
        "01_Integralizacao_Mensal_SIM33_2047.csv",
        "Tabela_demanda_consumo_por_posto_SIM33_2047.csv",
    ]
    removidos = 0
    for padrao in padroes_limpeza:
        for fp in DIR_PASS2.glob(padrao):
            if fp.is_file():
                fp.unlink()
                removidos += 1
    print(f"[CLEAN] SIM33 2047: {removidos} arquivo(s) antigo(s) removido(s) em {DIR_PASS2}")
...
def _build_circuit_pass2(dss: DSS, posto: str, seg_tag: str) -> dict[str, str]:
    ...
    # Curvas anuais de irradiancia POA e temperatura do modulo - arquivo meteorologico base
    ...
    # === SISTEMA FOTOVOLTAICO UFV1 ~792 kWp (8 x 100 kW) ===
    # Usa shapes anuais (yearly / Tyearly), coerentes com Set Mode=Yearly
    # 1760 modulos x 450 Wp = 792.0 kWp no ano-base
    # 88 strings x 20 modulos
    # 11 strings por inversor -> 220 modulos/inversor -> 99.0 kWpdc por inversor
    # UFV1 instalada em 2028 e simulada em 2047.
    # Degradacao DC das placas por 19 anos: 0.995^19.
    PMPP_INV_ANO_BASE = 99.0
    FATOR_DEGRADACAO_UFV1_19_ANOS = 0.995 ** 19
    PMPP_INV_UFV1_2047 = PMPP_INV_ANO_BASE * FATOR_DEGRADACAO_UFV1_19_ANOS

    for i in range(1, 9):
        dss.text(
            f"New PVSystem.UFV8001_INV{i} phases=3 bus1=U013 kv=0.38 "
            f"pmpp={PMPP_INV_UFV1_2047:.3f} kVA=100 pf=1.0 conn=wye "
    ...
    # === SISTEMA FOTOVOLTAICO UFV2 ~792 kWp (8 x 100 kW) ===
    # Usa shapes anuais (yearly / Tyearly), coerentes com Set Mode=Yearly
    # 1760 modulos x 450 Wp = 792.0 kWp no ano-base
    # 88 strings x 20 modulos
    # 11 strings por inversor -> 220 modulos/inversor -> 99.0 kWpdc por inversor
    # UFV2 instalada em 2033 e simulada em 2047.
    # Degradacao DC das placas por 14 anos: 0.995^14.
    FATOR_DEGRADACAO_UFV2_14_ANOS = 0.995 ** 14
    PMPP_INV_UFV2_2047 = PMPP_INV_ANO_BASE * FATOR_DEGRADACAO_UFV2_14_ANOS

    for i in range(1, 9):
        dss.text(
            f"New PVSystem.UFV8002_INV{i} phases=3 bus1=U023 kv=0.38 "
            f"pmpp={PMPP_INV_UFV2_2047:.3f} kVA=100 pf=1.0 conn=wye "
            f"%cutin=0.1 %cutout=0.1 "
            f"effcurve=Eff_Huawei100k P-TCurve=P_T_Trina450 "
    ...
    # === SISTEMA FOTOVOLTAICO UFV3 ~792 kWp (8 x 100 kW) ===
    # Usa shapes anuais (yearly / Tyearly), coerentes com Set Mode=Yearly
    # 1760 modulos x 450 Wp = 792.0 kWp no ano-base
    # 88 strings x 20 modulos
    # 11 strings por inversor -> 220 modulos/inversor -> 99.0 kWpdc por inversor
    # UFV3 instalada em 2036 e simulada em 2047.
    # Degradacao DC das placas por 11 anos: 0.995^11.
    FATOR_DEGRADACAO_UFV3_11_ANOS = 0.995 ** 11
    PMPP_INV_UFV3_2047 = PMPP_INV_ANO_BASE * FATOR_DEGRADACAO_UFV3_11_ANOS

    for i in range(1, 9):
        dss.text(
            f"New PVSystem.UFV8003_INV{i} phases=3 bus1=U033 kv=0.38 "
            f"pmpp={PMPP_INV_UFV3_2047:.3f} kVA=100 pf=1.0 conn=wye "
            f"%cutin=0.1 %cutout=0.1 "
            f"effcurve=Eff_Huawei100k P-TCurve=P_T_Trina450 "
...
# =============================================================================
# Entrada:
#   Simulacao 033 segmentos_ano_2028_2033_2036_2047/UFSM_Mon_<KEY>_s*.csv
# Saida:
#   Simulacao 033 segmentos_ano_2028_2033_2036_2047/Monitor_<KEY>_anual.csv
# Remove:
...
# =============================================================================
# Entrada:
#   Simulacao 033 segmentos_ano_2028_2033_2036_2047/EXP_MTR_GERAL_*.csv
# Saida:
#   Simulacao 033 segmentos_ano_2028_2033_2036_2047/EXP_MTR_GERAL_anual.csv
# Remove:
#   EXP_MTR_GERAL_*.csv
# =============================================================================
...
# Saidas:
#   Simulacao 033 segmentos_ano_2028_2033_2036_2047/Curva_de_Carga_<KEY>_sim_33_2047.csv
# Remove:
...
for in_fp in files:
    ...
    out_fp = DIR_PASS2 / f"Curva_de_Carga_{mon_key}_sim_33_2047.csv"
...
#   Simulacao 033 segmentos_ano_2028_2033_2036_2047/EXP_MTR_GERAL_anual.csv
# Saida:
#   Simulacao 033 segmentos_ano_2028_2033_2036_2047/Resumo_EnergyMeter_Geral_Sim33_2047.csv
# =============================================================================
...
OUT_FP = DIR_PASS2 / "Resumo_EnergyMeter_Geral_Sim33_2047.csv"
...
# %% Celula 006 - ENERGIA e DEMANDA MAX por mes e posto (SIM33 - 2047)
# =============================================================================
# Entradas:
#   META: 00_dados_de_entrada_ano_letivo_2025_15min.csv
#         (Hora_decimal, Mes, Posto)
#   SIM33: Simulacao 033 segmentos_ano_2028_2033_2036_2047/Curva_de_Carga_GERAL_sim_33_2047.csv
#         (Hora_decimal, P_3f_S, Q_3f_S)
#
# Saida:
#   Simulacao 033 segmentos_ano_2028_2033_2036_2047/01_Integralizacao_Mensal_SIM33_2047.csv
#
# OBSERVACAO:
#   - P > 0  -> consumo da rede
#   - P < 0  -> injecao na rede
...
WORK_DIR = Path(r"C:\Users\afole\OneDrive\Dissertacao2025")
SIM33_DIR = WORK_DIR / "Simulacao 033 segmentos_ano_2028_2033_2036_2047"

META_FP = WORK_DIR / "00_dados_de_entrada_ano_letivo_2025_15min.csv"
SIM33_FP = SIM33_DIR / "Curva_de_Carga_GERAL_sim_33_2047.csv"
OUT_FP = SIM33_DIR / "01_Integralizacao_Mensal_SIM33_2047.csv"
...
for fp in [META_FP, SIM33_FP]:
    if not fp.exists():
        raise FileNotFoundError(fp)

meta = pd.read_csv(META_FP, dtype=str, encoding="utf-8-sig")
sim33 = pd.read_csv(SIM33_FP, encoding="utf-8-sig", sep=None, engine="python")

for name, dfx, req in [
    ("META", meta, ["Hora_decimal", "Mes", "Posto"]),
    ("SIM33", sim33, ["Hora_decimal", "P_3f_S", "Q_3f_S"]),
]:
...
sim33["Hora_decimal"] = pd.to_numeric(sim33["Hora_decimal"], errors="coerce")
sim33["P_3f_S"] = pd.to_numeric(sim33["P_3f_S"], errors="coerce")
sim33["Q_3f_S"] = pd.to_numeric(sim33["Q_3f_S"], errors="coerce")
sim33.dropna(subset=["Hora_decimal", "P_3f_S", "Q_3f_S"], inplace=True)

meta["k"] = make_k(meta["Hora_decimal"])
sim33["k"] = make_k(sim33["Hora_decimal"])

meta = meta[["k", "Mes", "Posto"]].drop_duplicates(subset=["k"], keep="first").copy()
sim33 = sim33[["k", "P_3f_S", "Q_3f_S"]].drop_duplicates(subset=["k"], keep="first").copy()

df33 = meta.merge(sim33, on="k", how="left").rename(
    columns={"P_3f_S": "P_SIM33", "Q_3f_S": "Q_SIM33"}
)

# -----------------------------------------------------------------------------
# Separacao entre consumo da rede (P > 0) e injecao na rede (P < 0)
# -----------------------------------------------------------------------------
df33["P_cons_SIM33_kW"] = df33["P_SIM33"].clip(lower=0)
df33["P_inj_SIM33_kW"] = (-df33["P_SIM33"]).clip(lower=0)

df33["E_cons_SIM33_kWh"] = df33["P_cons_SIM33_kW"] * DT_H
df33["E_inj_SIM33_kWh"] = df33["P_inj_SIM33_kW"] * DT_H
df33["E_Q_SIM33_kVArh"] = df33["Q_SIM33"] * DT_H

group_cols = ["Mes", "Posto"]

agg = df33.groupby(group_cols, dropna=False).agg(
    N_pontos_SIM33=("P_SIM33", lambda s: int(s.notna().sum())),
    Consumo_P_SIM33_kWh=("E_cons_SIM33_kWh", "sum"),
    Injecao_P_SIM33_kWh=("E_inj_SIM33_kWh", "sum"),
    Consumo_Q_SIM33_kVArh=("E_Q_SIM33_kVArh", "sum"),
    DemandaMax_P_SIM33_kW=("P_cons_SIM33_kW", "max"),
    DemandaMax_Inj_SIM33_kW=("P_inj_SIM33_kW", "max"),
).reset_index()
...
# %% Celula 007 - SIMULACAO 033: Tabela de custos (SIM33 - 2047)
# =============================================================================
# Entrada:
#   Simulacao 033 segmentos_ano_2028_2033_2036_2047/01_Integralizacao_Mensal_SIM33_2047.csv
#
# Saida:
#   Simulacao 033 segmentos_ano_2028_2033_2036_2047/Tabela_demanda_consumo_por_posto_SIM33_2047.csv
#
# HIPOTESE ADOTADA:
...
WORK_DIR = Path(r"C:\Users\afole\OneDrive\Dissertacao2025")
SIM33_DIR = WORK_DIR / "Simulacao 033 segmentos_ano_2028_2033_2036_2047"
SIM33_DIR.mkdir(parents=True, exist_ok=True)

IN_FP = SIM33_DIR / "01_Integralizacao_Mensal_SIM33_2047.csv"
OUT_SIM33 = SIM33_DIR / "Tabela_demanda_consumo_por_posto_SIM33_2047.csv"

# -------------------------
# DEMANDAS CONTRATADAS
# -------------------------
CONTR_SIM33 = {"DCFP": 5550.0, "DCP": 3330.0}
...
def tabela_custos(df_in: pd.DataFrame, fonte: str, DCFP: float, DCP: float) -> pd.DataFrame:
    ...
    # Custos TE brutos e abatimentos TE
...
tab33 = tabela_custos(
    df,
    "SIM33",
    DCFP=CONTR_SIM33["DCFP"],
    DCP=CONTR_SIM33["DCP"],
)

tab33.to_csv(OUT_SIM33, index=False, encoding="utf-8")

print("DONE. Saved (Simulacao 033 segmentos_ano_2028_2033_2036_2047):")
print(" -", OUT_SIM33)

print("\nDemandas contratadas usadas:")
print(" - SIM33: DCFP=5550 kW, DCP=3330 kW")
