# -*- coding: utf-8 -*-
# =============================================================================
# 024_Script_BESS_Simulacao_023_4bus_0700_2220_2380_1250_duas_simulacoes_conjugado_4500_ano_2028.py
#
# Regras deste script:
...
#     Simulacao 023 segmentos_pass1_sem_hvac_int_ano_2028
#   PASSO 2 (simulacao completa final):
#     Simulacao 023 segmentos_ano_2028
# =============================================================================
...
TARGETS = {
    ...
    "FORA_PONTA": {"kWtarget": 4568, "kWtargetLow": 3553},
}

DIR_PASS1 = WORK_DIR / "Simulacao 023 segmentos_pass1_sem_hvac_int_ano_2028"
...
def build_circuit_pass1(
    ...
    dss.text(
        ...
        f"kWrated=800 kWhrated=4250 Effcurve=Eff %EffCharge=95 %EffDischarge=95 "
...
DIR_PASS2 = WORK_DIR / "Simulacao 023 segmentos_ano_2028"
...
# =============================================================================
# Entrada:
#   Simulacao 023 segmentos_ano_2028/UFSM_Mon_<KEY>_s*.csv
# Saida:
#   Simulacao 023 segmentos_ano_2028/Monitor_<KEY>_anual.csv
# Remove:
...
# =============================================================================
# Entrada:
#   Simulacao 023 segmentos_ano_2028/EXP_MTR_GERAL_*.csv
# Saida:
#   Simulacao 023 segmentos_ano_2028/EXP_MTR_GERAL_anual.csv
# Remove:
#   EXP_MTR_GERAL_*.csv
# =============================================================================
...
#   Simulacao 023 segmentos_ano_2028/Monitor_<KEY>_anual.csv
# Saidas:
#   Simulacao 023 segmentos_ano_2028/Curva_de_Carga_<KEY>_sim_23.csv
# Remove:
...
#   Simulacao 023 segmentos_ano_2028/EXP_MTR_GERAL_anual.csv
# Saida:
#   Simulacao 023 segmentos_ano_2028/Resumo_EnergyMeter_Geral_Sim23.csv
# =============================================================================
...
# %% Celula 010 - ENERGIA e DEMANDA MAX por mes e posto (SIM23 - ANO 2028, 4 BESS 800 kW / 4,25 MWh cada)
# =============================================================================
# Entradas:
#   META: 00_dados_de_entrada_ano_letivo_2025_15min.csv
#         Usado apenas como mapa anual de Hora_decimal, Mes e Posto.
#   SIM23: curva GERAL com 4 BESS em operacao conjugada para o ano 2028
# Saida:
#   Simulacao 023 segmentos_ano_2028/01_Integralizacao_Mensal_SIM23_2028.csv
#
# Observacao:
#   Esta celula nao compara com SIM2/RGE nem com outra simulacao.
#   Ela apenas integraliza a simulacao conjugada com 4 BESS do ano 2028 por mes e posto.
#   A curva e procurada com sufixo sim_23 ou sim_023.
#   Todas as celulas deste arquivo foram padronizadas para SIM23.
# =============================================================================
...
CURVAS_CANDIDATAS = [
    SIM23_DIR / "Curva_de_Carga_GERAL_sim_23.csv",
    SIM23_DIR / "Curva_de_Carga_GERAL_sim_023.csv",
]
SIM23_FP = next((fp for fp in CURVAS_CANDIDATAS if fp.exists()), None)
...
if SIM23_FP is None:
    msg = "Nenhuma curva GERAL encontrada. Arquivos testados:\n" + "\n".join(str(fp) for fp in CURVAS_CANDIDATAS)
    raise FileNotFoundError(msg)

for fp in [META_FP, SIM23_FP]:
    if not fp.exists():
...
agg23 = df23.groupby(group_cols, dropna=False).agg(
...
agg = agg23.merge(meta_counts, on=group_cols, how="left")
...
# %% Celula 011 - SIMULACAO 023: tabela de custos (SIM23 - ANO 2028, 4 BESS 800 kW / 4,25 MWh cada)
# =============================================================================
# Entrada:
#   Simulacao 023 segmentos_ano_2028/01_Integralizacao_Mensal_SIM23_2028.csv
#
# Saida:
#   Simulacao 023 segmentos_ano_2028/Tabela_demanda_consumo_por_posto_SIM23_2028.csv
#
# Demandas contratadas do ano 2028:
#   DCFP com BESS = 4568 kW  (corte fora-ponta agressivo: 4500 kW * 1,015)
#   DCP sem BESS = 3045 kW
#   DCP com 4 BESS = max(0, 3045 - (4 * 800)) = 0 kW
#
# Banco de energia:
#   4 BESS de 1 MW / 4,25 MWh, operando a 80% da potencia nominal.
#   Potencia operacional total = 3200 kW.
#   Energia total = 17000 kWh.
#   Reserva de ponta = 70,6%.
...
# - Em 2028 aplica-se crescimento acumulado de 1,5% para a ponta sem BESS.
# - A demanda contratada fora-ponta foi reduzida de forma mais agressiva para 4568 kW.
# - Os 4 BESS sao instalados no proprio ano de analise.
# - Cada banco possui 1 MW / 4,25 MWh nominal, mas opera com 800 kW de potencia.
...
BESS_P_UNITARIA_KW = BESS_P_NOMINAL_UNITARIA_KW * BESS_FATOR_OPERACAO_P
BESS_E_UNITARIA_KWH = 4250.0
BESS_RESERVA_PONTA_PCT = 70.6

BESS_P_TOTAL_KW = NUM_BESS * BESS_P_UNITARIA_KW       # 3200 kW
BESS_E_TOTAL_KWH = NUM_BESS * BESS_E_UNITARIA_KWH     # 17000 kWh
...
DCFP_COM_BESS = 4568.0                                  # corte fora-ponta agressivo
...
tab23.to_csv(OUT_SIM23, index=False, encoding="utf-8")

print("DONE. Saved (Simulacao 023 - 4 BESS - 2028 - conjugado 4500):")
print(" -", OUT_SIM23)
print("\nDemandas contratadas usadas em 2028:")
print(f" - DCFP = {DCFP_COM_BESS:.0f} kW")
print(f" - DCP sem BESS = {DCP_SEM_BESS:.0f} kW")
print(f" - DCP com 4 BESS = {DCP_COM_BESS:.0f} kW")
...
print("\nResumo anual:")
print(tab23[tab23["MES"].isin(["TOTAIS", "CTOTAL", "R$TOTAL", "DCFP", "DCP"])])
