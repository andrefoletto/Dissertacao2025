# -*- coding: utf-8 -*-
# =============================================================================
# 027_Script_BESS_Simulacao_026_2bus_0700_2220_duas_simulacoes_conjugado_4000.py
#
# Regras deste script:
...
#     Simulacao 026 segmentos_pass1_sem_hvac_int
#   PASSO 2 (simulacao completa final):
#     Simulacao 026 segmentos
# =============================================================================
...
STORAGE_NAMES = ["BESSM1", "BESSM2"]
STORAGE_CTRL = "C_BESSM12"
SOC_INICIAL = 50.0
...
DIR_PASS1 = WORK_DIR / "Simulacao 026 segmentos_pass1_sem_hvac_int"
...
def build_circuit_pass1(
    ...
    # HVAC externo (obrigatorio) para ambos
    ...
    dss.text(
        ...
        f"kWrated=800 kWhrated=8000 Effcurve=Eff %EffCharge=95 %EffDischarge=95 "
    ...
    dss.text(
        ...
        "ElementList=[BESSM1, BESSM2] MonPhase=AVG "
...
for i, row in segments.iterrows():
    ...
    print(
        ...
        f"{mon_fps['BESSM1_IN'].name} | {mon_fps['BESSM2_IN'].name}"
    )
...
# %% Celula 002 - PASSO 1: emenda monitores BESSM1_IN e BESSM2_IN
# =============================================================================
# Entrada:
...
for mon_base in ["BESSM1_IN", "BESSM2_IN"]:
...
# %% Celula 003 - PASSO 1: monitores -> Curva_de_Carga_BESSM1_IN_sim_26.csv e BESSM2_IN (SHIFT DIREITA)
# =============================================================================
# Entradas:
#   PASSO1/Monitor_BESSM1_IN_anual.csv
#   PASSO1/Monitor_BESSM2_IN_anual.csv
# Saidas:
#   PASSO1/Curva_de_Carga_BESSM1_IN_sim_26.csv
#   PASSO1/Curva_de_Carga_BESSM2_IN_sim_26.csv
# Remove:
#   PASSO1/Monitor_BESSM1_IN_anual.csv
#   PASSO1/Monitor_BESSM2_IN_anual.csv
# =============================================================================
...
for mon_base in ["BESSM1_IN", "BESSM2_IN"]:
    ...
    out_fp = DIR_PASS1 / f"Curva_de_Carga_{mon_base}_sim_26.csv"
...
for mon_base, prefix in [("BESSM1_IN", "BESSM1"), ("BESSM2_IN", "BESSM2")]:
    in_fp = DIR_PASS1 / f"Curva_de_Carga_{mon_base}_sim_26.csv"
...
DIR_PASS2 = WORK_DIR / "Simulacao 026 segmentos"
...
for i, row in segments.iterrows():
    ...
    print(
        f"[OK] {seg_tag}: hora_ini={hora_ini:.2f}h steps={n_steps} | "
        f"M1 {soc_ini_m1:.3f}% -> {soc_final_m1:.3f}% | "
        f"M2 {soc_ini_m2:.3f}% -> {soc_final_m2:.3f}% | meter={mtr_fp.name}"
    )
...
# =============================================================================
# Entrada:
#   Simulacao 026 segmentos/UFSM_Mon_<KEY>_s*.csv
# Saida:
#   Simulacao 026 segmentos/Monitor_<KEY>_anual.csv
# Remove:
...
# =============================================================================
# Entrada:
#   Simulacao 026 segmentos/EXP_MTR_GERAL_*.csv
# Saida:
#   Simulacao 026 segmentos/EXP_MTR_GERAL_anual.csv
# Remove:
#   EXP_MTR_GERAL_*.csv
# =============================================================================
...
#   Simulacao 026 segmentos/Monitor_<KEY>_anual.csv
# Saidas:
#   Simulacao 026 segmentos/Curva_de_Carga_<KEY>_sim_26.csv
# Remove:
...
for in_fp in files:
    ...
    out_fp = DIR_PASS2 / f"Curva_de_Carga_{mon_key}_sim_26.csv"
...
#   Simulacao 026 segmentos/EXP_MTR_GERAL_anual.csv
# Saida:
#   Simulacao 026 segmentos/Resumo_EnergyMeter_Geral_Sim26.csv
# =============================================================================
...
OUT_FP = DIR_PASS2 / "Resumo_EnergyMeter_Geral_Sim26.csv"
...
# %% Celula 010 - ENERGIA e DEMANDA MAX por mes e posto (SIM2 vs SIM26)
# =============================================================================
# Entradas:
...
#   SIM26: Simulacao 026 segmentos/Curva_de_Carga_GERAL_sim_26.csv (Hora_decimal, P_3f_S, Q_3f_S)
# Saida:
#   Simulacao 026 segmentos/01_Integralizacao_Mensal_SIM2_x_SIM26_2025.csv
# =============================================================================
...
SIM26_DIR = WORK_DIR / "Simulacao 026 segmentos"

META_FP = WORK_DIR / "00_dados_de_entrada_ano_letivo_2025_15min.csv"
SIM2_FP = SIM2_DIR / "Curva_de_Carga_GERAL_sim_2.csv"
SIM26_FP = SIM26_DIR / "Curva_de_Carga_GERAL_sim_26.csv"
OUT_FP = SIM26_DIR / "01_Integralizacao_Mensal_SIM2_x_SIM26_2025.csv"
...
for fp in [META_FP, SIM2_FP, SIM26_FP]:
    if not fp.exists():
...
sim26 = pd.read_csv(SIM26_FP, encoding="utf-8-sig", sep=None, engine="python")

for name, dfx, req in [
    ("META", meta, ["Hora_decimal", "Mes", "Posto"]),
    ("SIM2", sim2, ["Hora_decimal", "P_3f_S", "Q_3f_S"]),
    ("SIM26", sim26, ["Hora_decimal", "P_3f_S", "Q_3f_S"]),
]:
...
sim26["k"] = make_k(sim26["Hora_decimal"])
...
sim26 = sim26[["k", "P_3f_S", "Q_3f_S"]].drop_duplicates(subset=["k"], keep="first").copy()

df2 = meta.merge(sim2, on="k", how="left").rename(columns={"P_3f_S": "P_SIM2", "Q_3f_S": "Q_SIM2"})
df26 = meta.merge(sim26, on="k", how="left").rename(columns={"P_3f_S": "P_SIM26", "Q_3f_S": "Q_SIM26"})

df2["E_P_SIM2_kWh"] = df2["P_SIM2"] * DT_H
df2["E_Q_SIM2_kVArh"] = df2["Q_SIM2"] * DT_H
df26["E_P_SIM26_kWh"] = df26["P_SIM26"] * DT_H
df26["E_Q_SIM26_kVArh"] = df26["Q_SIM26"] * DT_H
...
agg26 = df26.groupby(group_cols, dropna=False).agg(
    N_pontos_SIM26=("P_SIM26", lambda s: int(s.notna().sum())),
    Consumo_P_SIM26_kWh=("E_P_SIM26_kWh", "sum"),
    Consumo_Q_SIM26_kVArh=("E_Q_SIM26_kVArh", "sum"),
    DemandaMax_P_SIM26_kW=("P_SIM26", "max"),
    DemandaMax_Q_SIM26_kVAr=("Q_SIM26", "max"),
).reset_index()

agg = agg2.merge(agg26, on=group_cols, how="outer")
...
# %% Celula 011 - SIMULACAO 026: Tabela de custos (SIM2 vs SIM26 - 2025) com demandas contratadas diferentes
# =============================================================================
# Entrada:
#   Simulacao 026 segmentos/01_Integralizacao_Mensal_SIM2_x_SIM26_2025.csv
#
# Saidas (todas em Simulacao 026 segmentos):
#   Tabela_demanda_consumo_por_posto_SIM2_2025.csv
#   Tabela_demanda_consumo_por_posto_SIM26_2025.csv
#   Tabela_demanda_consumo_por_posto_SIM2_vs_SIM26_2025.csv   (economia)
# =============================================================================
...
WORK_DIR = Path(r"C:\Users\afole\OneDrive\Dissertacao2025")
SIM26_DIR = WORK_DIR / "Simulacao 026 segmentos"
SIM26_DIR.mkdir(parents=True, exist_ok=True)

IN_FP = SIM26_DIR / "01_Integralizacao_Mensal_SIM2_x_SIM26_2025.csv"
OUT_SIM2 = SIM26_DIR / "Tabela_demanda_consumo_por_posto_SIM2_2025.csv"
OUT_SIM26 = SIM26_DIR / "Tabela_demanda_consumo_por_posto_SIM26_2025.csv"
OUT_CMP = SIM26_DIR / "Tabela_demanda_consumo_por_posto_SIM2_vs_SIM26_2025.csv"

CONTR_SIM2 = {"DCFP": 5000.0, "DCP": 3000.0}
CONTR_SIM26 = {"DCFP": 4000.0, "DCP": 1400.0}  # 3000 - 2*800
...
print("DONE. Saved (Simulacao 026 segmentos):")
print(" -", OUT_SIM2)
print(" -", OUT_SIM26)
print(" -", OUT_CMP)
print("\nDemandas contratadas usadas:")
print(" - SIM2: DCFP=5000, DCP=3000")
print(" - SIM26: DCFP=4000, DCP=1400")
print("\nResumo anual:")
print(cmp.tail(1))
...
# -*- coding: utf-8 -*-
# =============================================================================
# 028_Script_BESS_Simulacao_027_3bus_0700_2220_2380_duas_simulacoes_conjugado_4000.py
#
# Regras deste script:
#   - Sem acentos em comentarios e strings pt-br.
#   - Celulas independentes: cada celula define suas variaveis e funcoes locais.
...
# Pastas mantidas:
#   PASSO 1 (sem HVAC interno):
#     Simulacao 027 segmentos_pass1_sem_hvac_int
#   PASSO 2 (simulacao completa final):
#     Simulacao 027 segmentos
# =============================================================================
...
DIR_PASS1 = WORK_DIR / "Simulacao 027 segmentos_pass1_sem_hvac_int"
DIR_PASS1.mkdir(parents=True, exist_ok=True)
os.chdir(WORK_DIR)
...
def build_circuit_pass1(
    ...
    dss.text(
        "New Storage.BESSM1 phases=3 bus1=B013 kv=0.38 "
        f"kWrated=800 kWhrated=6333 Effcurve=Eff %EffCharge=95 %EffDischarge=95 "
        f"%reserve={reserve_pct} dispmode=external"
    )
    dss.text(
        "New Storage.BESSM2 phases=3 bus1=B023 kv=0.38 "
...
DIR_PASS2 = WORK_DIR / "Simulacao 027 segmentos"
DIR_PASS2.mkdir(parents=True, exist_ok=True)
...
# =============================================================================
# Entrada:
#   Simulacao 027 segmentos/UFSM_Mon_<KEY>_s*.csv
# Saida:
#   Simulacao 027 segmentos/Monitor_<KEY>_anual.csv
# Remove:
#   UFSM_Mon_<KEY>_s*.csv
# =============================================================================
...
# =============================================================================
# Entrada:
#   Simulacao 027 segmentos/EXP_MTR_GERAL_*.csv
# Saida:
#   Simulacao 027 segmentos/EXP_MTR_GERAL_anual.csv
# Remove:
#   EXP_MTR_GERAL_*.csv
# =============================================================================
...
#   Simulacao 027 segmentos/Monitor_<KEY>_anual.csv
# Saidas:
#   Simulacao 027 segmentos/Curva_de_Carga_<KEY>_sim_27.csv
# Remove:
#   Monitor_<KEY>_anual.csv
# =============================================================================
...
#   Simulacao 027 segmentos/EXP_MTR_GERAL_anual.csv
# Saida:
#   Simulacao 027 segmentos/Resumo_EnergyMeter_Geral_Sim27.csv
# =============================================================================
...
# %% Celula 010 - ENERGIA e DEMANDA MAX por mes e posto (SIM2 vs SIM27)
# =============================================================================
# Entradas:
#   META: 00_dados_de_entrada_ano_letivo_2025_15min.csv  (Hora_decimal, Mes, Posto)
#   SIM2:  Simulacao 002/Curva_de_Carga_GERAL_sim_2.csv      (Hora_decimal, P_3f_S, Q_3f_S)
#   SIM27: Simulacao 027 segmentos/Curva_de_Carga_GERAL_sim_27.csv (Hora_decimal, P_3f_S, Q_3f_S)
# Saida:
#   Simulacao 027 segmentos/01_Integralizacao_Mensal_SIM2_x_SIM27_2025.csv
# =============================================================================
...
WORK_DIR = Path(r"C:\Users\afole\OneDrive\Dissertacao2025")
SIM2_DIR = WORK_DIR / "Simulacao 002"
SIM27_DIR = WORK_DIR / "Simulacao 027 segmentos"

META_FP = WORK_DIR / "00_dados_de_entrada_ano_letivo_2025_15min.csv"
SIM2_FP = SIM2_DIR / "Curva_de_Carga_GERAL_sim_2.csv"
SIM27_FP = SIM27_DIR / "Curva_de_Carga_GERAL_sim_27.csv"
OUT_FP = SIM27_DIR / "01_Integralizacao_Mensal_SIM2_x_SIM27_2025.csv"
...
for fp in [META_FP, SIM2_FP, SIM27_FP]:
    if not fp.exists():
        raise FileNotFoundError(fp)
...
for name, dfx, req in [
    ("META", meta, ["Hora_decimal", "Mes", "Posto"]),
    ("SIM2", sim2, ["Hora_decimal", "P_3f_S", "Q_3f_S"]),
    ("SIM27", sim27, ["Hora_decimal", "P_3f_S", "Q_3f_S"]),
]:
    miss = [c for c in req if c not in dfx.columns]
    if miss:
        raise ValueError(f"Missing columns in {name}: {miss}. Available: {list(dfx.columns)}")
...
df2 = meta.merge(sim2, on="k", how="left").rename(columns={"P_3f_S": "P_SIM2", "Q_3f_S": "Q_SIM2"})
df27 = meta.merge(sim27, on="k", how="left").rename(columns={"P_3f_S": "P_SIM27", "Q_3f_S": "Q_SIM27"})
...
agg = agg2.merge(agg27, on=group_cols, how="outer")
meta_counts = meta.groupby(group_cols, dropna=False).agg(N_pontos_META=("Posto", "size")).reset_index()
agg = agg.merge(meta_counts, on=group_cols, how="left")
...
# %% Celula 011 - SIMULACAO 027: Tabela de custos (SIM2 vs SIM27 - 2025) com demandas contratadas diferentes
# =============================================================================
# Entrada:
#   Simulacao 027 segmentos/01_Integralizacao_Mensal_SIM2_x_SIM27_2025.csv
#
# Saidas (todas em Simulacao 027 segmentos):
#   Tabela_demanda_consumo_por_posto_SIM2_2025.csv
#   Tabela_demanda_consumo_por_posto_SIM27_2025.csv
#   Tabela_demanda_consumo_por_posto_SIM2_vs_SIM27_2025.csv   (economia)
# =============================================================================
...
IN_FP = SIM27_DIR / "01_Integralizacao_Mensal_SIM2_x_SIM27_2025.csv"
OUT_SIM2 = SIM27_DIR / "Tabela_demanda_consumo_por_posto_SIM2_2025.csv"
OUT_SIM27 = SIM27_DIR / "Tabela_demanda_consumo_por_posto_SIM27_2025.csv"
OUT_CMP = SIM27_DIR / "Tabela_demanda_consumo_por_posto_SIM2_vs_SIM27_2025.csv"

CONTR_SIM2 = {"DCFP": 5000.0, "DCP": 3000.0}
CONTR_SIM27 = {"DCFP": 4000.0, "DCP": 600.0}  # 3000 - 2*400
...
tab2 = tabela_custos(df, "SIM2", DCFP=CONTR_SIM2["DCFP"], DCP=CONTR_SIM2["DCP"])
tab27 = tabela_custos(df, "SIM27", DCFP=CONTR_SIM27["DCFP"], DCP=CONTR_SIM27["DCP"])

tab2.to_csv(OUT_SIM2, index=False, encoding="utf-8")
tab27.to_csv(OUT_SIM27, index=False, encoding="utf-8")

t2 = tab2[tab2["MES"].isin(MESES_ORD)][["MES", "R$TOTAL_MES"]].rename(columns={"R$TOTAL_MES": "R$SIM2"})
t27 = tab27[tab27["MES"].isin(MESES_ORD)][["MES", "R$TOTAL_MES"]].rename(columns={"R$TOTAL_MES": "R$SIM27"})

cmp = t2.merge(t27, on="MES", how="inner")
cmp["MES"] = pd.Categorical(cmp["MES"], categories=MESES_ORD, ordered=True)
cmp = cmp.sort_values("MES").reset_index(drop=True)

cmp["Economia_R$"] = (cmp["R$SIM2"] - cmp["R$SIM27"]).round(2)
cmp["Economia_%"] = np.where(cmp["R$SIM2"] == 0, np.nan, (cmp["Economia_R$"] / cmp["R$SIM2"] * 100).round(3))

tot = {
    "MES": "TOTAIS",
    "R$SIM2": float(cmp["R$SIM2"].sum()),
    "R$SIM27": float(cmp["R$SIM27"].sum()),
    "Economia_R$": float(cmp["Economia_R$"].sum()),
    "Economia_%": (np.nan if cmp["R$SIM2"].sum() == 0 else float(cmp["Economia_R$"].sum() / cmp["R$SIM2"].sum() * 100)),
}
cmp = pd.concat([cmp, pd.DataFrame([tot])], ignore_index=True)
...
print("DONE. Saved (Simulacao 027 segmentos):")
print(" -", OUT_SIM2)
print(" -", OUT_SIM27)
print(" -", OUT_CMP)
print("\nDemandas contratadas usadas:")
print(" - SIM2: DCFP=5000, DCP=3000")
print(" - SIM27: DCFP=4000, DCP=600")
print("\nResumo anual:")
print(cmp.tail(1))
...
# -*- coding: utf-8 -*-
# =============================================================================
# 029_Script_BESS_Simulacao_028_4bus_0700_2220_2380_1250_duas_simulacoes_conjugado_4000.py
#
# Regras deste script:
#   - Sem acentos em comentarios e strings pt-br.
#   - Celulas independentes: cada celula define suas variaveis e funcoes locais.
...
# Pastas mantidas:
#   PASSO 1 (sem HVAC interno):
#     Simulacao 028 segmentos_pass1_sem_hvac_int
#   PASSO 2 (simulacao completa final):
#     Simulacao 028 segmentos
# =============================================================================
...
STORAGE_NAMES = ["BESSM1", "BESSM2", "BESSM3", "BESSM4"]
STORAGE_CTRL = "C_BESSM1234"
SOC_INICIAL = 50.0
...
DIR_PASS1 = WORK_DIR / "Simulacao 028 segmentos_pass1_sem_hvac_int"
DIR_PASS1.mkdir(parents=True, exist_ok=True)
os.chdir(WORK_DIR)
...
def build_circuit_pass1(
    ...
    dss.text(
        "New Storage.BESSM1 phases=3 bus1=B013 kv=0.38 "
        f"kWrated=800 kWhrated=5500 Effcurve=Eff %EffCharge=95 %EffDischarge=95 "
        f"%reserve={reserve_pct} dispmode=external"
    )
    dss.text(
        "New Storage.BESSM2 phases=3 bus1=B023 kv=0.38 "
    ...
    dss.text(
        f"New StorageController.{STORAGE_CTRL} Element=line.0015_0016 terminal=1 "
        "ElementList=[BESSM1, BESSM2, BESSM3, BESSM4] MonPhase=AVG "
        f"ModeDischarge=PeakShave ModeCharge=PeakShaveLow "
        f"kWtarget={kWtarget} kWtargetLow={kWtargetLow}"
    )
...
def run_one_segment_pass1(
    ...
    print(
        ...
        f"hora_ini={hora_q:.2f}h -> Hour={hour_int} Sec={sec_int} "
        f"DT={DT_H:.2f}h n_steps={int(n_steps)} | "
        f"SOC_ini_M1={float(soc_ini_m1):.3f}% | SOC_ini_M2={float(soc_ini_m2):.3f}% | SOC_ini_M3={float(soc_ini_m3):.3f}% | SOC_ini_M4={float(soc_ini_m4):.3f}%"
    )
...
for i, row in segments.iterrows():
    ...
    print(
        ...
        f"{mon_fps['BESSM1_IN'].name} | {mon_fps['BESSM2_IN'].name} | {mon_fps['BESSM3_IN'].name} | {mon_fps['BESSM4_IN'].name}"
    )
...
for mon_base in ["BESSM1_IN", "BESSM2_IN", "BESSM3_IN", "BESSM4_IN"]:
    out_fp = DIR_PASS1 / f"Monitor_{mon_base}_anual.csv"
    files = sorted(DIR_PASS1.glob(f"UFSM_Mon_{mon_base}_s*.csv"))
    if not files:
        raise FileNotFoundError(f"Nenhum UFSM_Mon_{mon_base}_s*.csv em {DIR_PASS1}")
...
# %% Celula 003 - PASSO 1: monitores -> Curva_de_Carga_BESSM1_IN_sim_28.csv, BESSM2_IN, BESSM3_IN e BESSM4_IN (SHIFT DIREITA)
# =============================================================================
# Entradas:
#   PASSO1/Monitor_BESSM1_IN_anual.csv
#   PASSO1/Monitor_BESSM2_IN_anual.csv
...
#   PASSO1/Monitor_BESSM4_IN_anual.csv
# Saidas:
#   PASSO1/Curva_de_Carga_BESSM1_IN_sim_28.csv
#   PASSO1/Curva_de_Carga_BESSM2_IN_sim_28.csv
#   PASSO1/Curva_de_Carga_BESSM3_IN_sim_28.csv
#   PASSO1/Curva_de_Carga_BESSM4_IN_sim_28.csv
# Remove:
#   PASSO1/Monitor_BESSM1_IN_anual.csv
#   PASSO1/Monitor_BESSM2_IN_anual.csv
#   PASSO1/Monitor_BESSM3_IN_anual.csv
...
for mon_base in ["BESSM1_IN", "BESSM2_IN", "BESSM3_IN", "BESSM4_IN"]:
    in_fp = DIR_PASS1 / f"Monitor_{mon_base}_anual.csv"
    out_fp = DIR_PASS1 / f"Curva_de_Carga_{mon_base}_sim_28.csv"
...
for mon_base, prefix in [("BESSM1_IN", "BESSM1"), ("BESSM2_IN", "BESSM2"), ("BESSM3_IN", "BESSM3"), ("BESSM4_IN", "BESSM4")]:
    in_fp = DIR_PASS1 / f"Curva_de_Carga_{mon_base}_sim_28.csv"
...
DIR_PASS2 = WORK_DIR / "Simulacao 028 segmentos"
DIR_PASS2.mkdir(parents=True, exist_ok=True)
...
for i, row in segments.iterrows():
    ...
    print(
        ...
        f"M2 {soc_ini_m2:.3f}% -> {soc_final_m2:.3f}% | "
        f"M3 {soc_ini_m3:.3f}% -> {soc_final_m3:.3f}% | "
        f"M4 {soc_ini_m4:.3f}% -> {soc_final_m4:.3f}% | meter={mtr_fp.name}"
    )
...
# =============================================================================
# Entrada:
#   Simulacao 028 segmentos/UFSM_Mon_<KEY>_s*.csv
# Saida:
#   Simulacao 028 segmentos/Monitor_<KEY>_anual.csv
# Remove:
#   UFSM_Mon_<KEY>_s*.csv
# =============================================================================
...
# =============================================================================
# Entrada:
#   Simulacao 028 segmentos/EXP_MTR_GERAL_*.csv
# Saida:
#   Simulacao 028 segmentos/EXP_MTR_GERAL_anual.csv
# Remove:
#   EXP_MTR_GERAL_*.csv
# =============================================================================
...
#   Simulacao 028 segmentos/Monitor_<KEY>_anual.csv
# Saidas:
#   Simulacao 028 segmentos/Curva_de_Carga_<KEY>_sim_28.csv
# Remove:
#   Monitor_<KEY>_anual.csv
# =============================================================================
...
for in_fp in files:
    mon_key = in_fp.stem.replace("Monitor_", "").replace("_anual", "")
    out_fp = DIR_PASS2 / f"Curva_de_Carga_{mon_key}_sim_28.csv"
...
#   Simulacao 028 segmentos/EXP_MTR_GERAL_anual.csv
# Saida:
#   Simulacao 028 segmentos/Resumo_EnergyMeter_Geral_Sim28.csv
# =============================================================================
...
IN_FP = DIR_PASS2 / "EXP_MTR_GERAL_anual.csv"
OUT_FP = DIR_PASS2 / "Resumo_EnergyMeter_Geral_Sim28.csv"
...
# %% Celula 010 - ENERGIA e DEMANDA MAX por mes e posto (SIM2 vs SIM28)
# =============================================================================
# Entradas:
#   META: 00_dados_de_entrada_ano_letivo_2025_15min.csv  (Hora_decimal, Mes, Posto)
#   SIM2:  Simulacao 002/Curva_de_Carga_GERAL_sim_2.csv      (Hora_decimal, P_3f_S, Q_3f_S)
#   SIM28: Simulacao 028 segmentos/Curva_de_Carga_GERAL_sim_28.csv (Hora_decimal, P_3f_S, Q_3f_S)
# Saida:
#   Simulacao 028 segmentos/01_Integralizacao_Mensal_SIM2_x_SIM28_2025.csv
# =============================================================================
...
WORK_DIR = Path(r"C:\Users\afole\OneDrive\Dissertacao2025")
SIM2_DIR = WORK_DIR / "Simulacao 002"
SIM28_DIR = WORK_DIR / "Simulacao 028 segmentos"

META_FP = WORK_DIR / "00_dados_de_entrada_ano_letivo_2025_15min.csv"
SIM2_FP = SIM2_DIR / "Curva_de_Carga_GERAL_sim_2.csv"
SIM28_FP = SIM28_DIR / "Curva_de_Carga_GERAL_sim_28.csv"
OUT_FP = SIM28_DIR / "01_Integralizacao_Mensal_SIM2_x_SIM28_2025.csv"
...
for fp in [META_FP, SIM2_FP, SIM28_FP]:
    if not fp.exists():
        raise FileNotFoundError(fp)

meta = pd.read_csv(META_FP, dtype=str, encoding="utf-8-sig")
sim2 = pd.read_csv(SIM2_FP, encoding="utf-8-sig", sep=None, engine="python")
sim28 = pd.read_csv(SIM28_FP, encoding="utf-8-sig", sep=None, engine="python")

for name, dfx, req in [
    ("META", meta, ["Hora_decimal", "Mes", "Posto"]),
    ("SIM2", sim2, ["Hora_decimal", "P_3f_S", "Q_3f_S"]),
    ("SIM28", sim28, ["Hora_decimal", "P_3f_S", "Q_3f_S"]),
]:
    miss = [c for c in req if c not in dfx.columns]
    if miss:
        raise ValueError(f"Missing columns in {name}: {miss}. Available: {list(dfx.columns)}")
...
meta["k"] = make_k(meta["Hora_decimal"])
sim2["k"] = make_k(sim2["Hora_decimal"])
sim28["k"] = make_k(sim28["Hora_decimal"])

meta = meta[["k", "Mes", "Posto"]].drop_duplicates(subset=["k"], keep="first").copy()
sim2 = sim2[["k", "P_3f_S", "Q_3f_S"]].drop_duplicates(subset=["k"], keep="first").copy()
sim28 = sim28[["k", "P_3f_S", "Q_3f_S"]].drop_duplicates(subset=["k"], keep="first").copy()

df2 = meta.merge(sim2, on="k", how="left").rename(columns={"P_3f_S": "P_SIM2", "Q_3f_S": "Q_SIM2"})
df28 = meta.merge(sim28, on="k", how="left").rename(columns={"P_3f_S": "P_SIM28", "Q_3f_S": "Q_SIM28"})

df2["E_P_SIM2_kWh"] = df2["P_SIM2"] * DT_H
df2["E_Q_SIM2_kVArh"] = df2["Q_SIM2"] * DT_H
df28["E_P_SIM28_kWh"] = df28["P_SIM28"] * DT_H
df28["E_Q_SIM28_kVArh"] = df28["Q_SIM28"] * DT_H
...
agg28 = df28.groupby(group_cols, dropna=False).agg(
    N_pontos_SIM28=("P_SIM28", lambda s: int(s.notna().sum())),
    Consumo_P_SIM28_kWh=("E_P_SIM28_kWh", "sum"),
    Consumo_Q_SIM28_kVArh=("E_Q_SIM28_kVArh", "sum"),
    DemandaMax_P_SIM28_kW=("P_SIM28", "max"),
    DemandaMax_Q_SIM28_kVAr=("Q_SIM28", "max"),
).reset_index()

agg = agg2.merge(agg28, on=group_cols, how="outer")
meta_counts = meta.groupby(group_cols, dropna=False).agg(N_pontos_META=("Posto", "size")).reset_index()
agg = agg.merge(meta_counts, on=group_cols, how="left")
...
# %% Celula 011 - SIMULACAO 028: Tabela de custos (SIM2 vs SIM28 - 2025) com demandas contratadas diferentes
# =============================================================================
# Entrada:
#   Simulacao 028 segmentos/01_Integralizacao_Mensal_SIM2_x_SIM28_2025.csv
#
# Saidas (todas em Simulacao 028 segmentos):
#   Tabela_demanda_consumo_por_posto_SIM2_2025.csv
#   Tabela_demanda_consumo_por_posto_SIM28_2025.csv
#   Tabela_demanda_consumo_por_posto_SIM2_vs_SIM28_2025.csv   (economia)
# =============================================================================
...
WORK_DIR = Path(r"C:\Users\afole\OneDrive\Dissertacao2025")
SIM28_DIR = WORK_DIR / "Simulacao 028 segmentos"
SIM28_DIR.mkdir(parents=True, exist_ok=True)

IN_FP = SIM28_DIR / "01_Integralizacao_Mensal_SIM2_x_SIM28_2025.csv"
OUT_SIM2 = SIM28_DIR / "Tabela_demanda_consumo_por_posto_SIM2_2025.csv"
OUT_SIM28 = SIM28_DIR / "Tabela_demanda_consumo_por_posto_SIM28_2025.csv"
OUT_CMP = SIM28_DIR / "Tabela_demanda_consumo_por_posto_SIM2_vs_SIM28_2025.csv"

CONTR_SIM2 = {"DCFP": 5000.0, "DCP": 3000.0}
CONTR_SIM28 = {"DCFP": 4000.0, "DCP":   0.0}  # 3000 - 3*800
...
tab2 = tabela_custos(df, "SIM2", DCFP=CONTR_SIM2["DCFP"], DCP=CONTR_SIM2["DCP"])
tab28 = tabela_custos(df, "SIM28", DCFP=CONTR_SIM28["DCFP"], DCP=CONTR_SIM28["DCP"])

tab2.to_csv(OUT_SIM2, index=False, encoding="utf-8")
tab28.to_csv(OUT_SIM28, index=False, encoding="utf-8")

t2 = tab2[tab2["MES"].isin(MESES_ORD)][["MES", "R$TOTAL_MES"]].rename(columns={"R$TOTAL_MES": "R$SIM2"})
t28 = tab28[tab28["MES"].isin(MESES_ORD)][["MES", "R$TOTAL_MES"]].rename(columns={"R$TOTAL_MES": "R$SIM28"})

cmp = t2.merge(t28, on="MES", how="inner")
cmp["MES"] = pd.Categorical(cmp["MES"], categories=MESES_ORD, ordered=True)
cmp = cmp.sort_values("MES").reset_index(drop=True)

cmp["Economia_R$"] = (cmp["R$SIM2"] - cmp["R$SIM28"]).round(2)
cmp["Economia_%"] = np.where(cmp["R$SIM2"] == 0, np.nan, (cmp["Economia_R$"] / cmp["R$SIM2"] * 100).round(3))

tot = {
    "MES": "TOTAIS",
    "R$SIM2": float(cmp["R$SIM2"].sum()),
    "R$SIM28": float(cmp["R$SIM28"].sum()),
    "Economia_R$": float(cmp["Economia_R$"].sum()),
    "Economia_%": (np.nan if cmp["R$SIM2"].sum() == 0 else float(cmp["Economia_R$"].sum() / cmp["R$SIM2"].sum() * 100)),
}
cmp = pd.concat([cmp, pd.DataFrame([tot])], ignore_index=True)
...
print("DONE. Saved (Simulacao 028 segmentos):")
print(" -", OUT_SIM2)
print(" -", OUT_SIM28)
print(" -", OUT_CMP)
print("\nDemandas contratadas usadas:")
print(" - SIM2: DCFP=5000, DCP=3000")
print(" - SIM28: DCFP=4000, DCP=0")
print("\nResumo anual:")
print(cmp.tail(1))

# -*- coding: utf-8 -*-
# =============================================================================
# 030_Script_BESS_Simulacao_029_5bus_0700_2220_2380_1250_1630_duas_simulacoes_conjugado_4000.py
#
# Regras deste script:
#   - Sem acentos em comentarios e strings pt-br.
#   - Celulas independentes: cada celula define suas variaveis e funcoes locais.
...
# Pastas mantidas:
#   PASSO 1 (sem HVAC interno):
#     Simulacao 029 segmentos_pass1_sem_hvac_int
#   PASSO 2 (simulacao completa final):
#     Simulacao 029 segmentos
# =============================================================================
...
DIR_PASS1 = WORK_DIR / "Simulacao 029 segmentos_pass1_sem_hvac_int"
DIR_PASS1.mkdir(parents=True, exist_ok=True)
os.chdir(WORK_DIR)
...
def build_circuit_pass1(
    ...
    dss.text(
        "New Storage.BESSM1 phases=3 bus1=B013 kv=0.38 "
        f"kWrated=800 kWhrated=5000 Effcurve=Eff %EffCharge=95 %EffDischarge=95 "
        f"%reserve={reserve_pct} dispmode=external"
    )
    dss.text(
        "New Storage.BESSM2 phases=3 bus1=B023 kv=0.38 "
...
DIR_PASS2 = WORK_DIR / "Simulacao 029 segmentos"
DIR_PASS2.mkdir(parents=True, exist_ok=True)
...
# =============================================================================
# Entrada:
#   Simulacao 029 segmentos/UFSM_Mon_<KEY>_s*.csv
# Saida:
#   Simulacao 029 segmentos/Monitor_<KEY>_anual.csv
# Remove:
#   UFSM_Mon_<KEY>_s*.csv
# =============================================================================
...
# =============================================================================
# Entrada:
#   Simulacao 029 segmentos/EXP_MTR_GERAL_*.csv
# Saida:
#   Simulacao 029 segmentos/EXP_MTR_GERAL_anual.csv
# Remove:
#   EXP_MTR_GERAL_*.csv
# =============================================================================
...
#   Simulacao 029 segmentos/Monitor_<KEY>_anual.csv
# Saidas:
#   Simulacao 029 segmentos/Curva_de_Carga_<KEY>_sim_29.csv
# Remove:
#   Monitor_<KEY>_anual.csv
# =============================================================================
...
#   Simulacao 029 segmentos/EXP_MTR_GERAL_anual.csv
# Saida:
#   Simulacao 029 segmentos/Resumo_EnergyMeter_Geral_Sim29.csv
# =============================================================================
...
# %% Celula 010 - ENERGIA e DEMANDA MAX por mes e posto (SIM2 vs SIM29)
# =============================================================================
# Entradas:
#   META: 00_dados_de_entrada_ano_letivo_2025_15min.csv  (Hora_decimal, Mes, Posto)
#   SIM2:  Simulacao 002/Curva_de_Carga_GERAL_sim_2.csv      (Hora_decimal, P_3f_S, Q_3f_S)
#   SIM29: Simulacao 029 segmentos/Curva_de_Carga_GERAL_sim_29.csv (Hora_decimal, P_3f_S, Q_3f_S)
# Saida:
#   Simulacao 029 segmentos/01_Integralizacao_Mensal_SIM2_x_SIM29_2025.csv
# =============================================================================
...
WORK_DIR = Path(r"C:\Users\afole\OneDrive\Dissertacao2025")
SIM2_DIR = WORK_DIR / "Simulacao 002"
SIM29_DIR = WORK_DIR / "Simulacao 029 segmentos"

META_FP = WORK_DIR / "00_dados_de_entrada_ano_letivo_2025_15min.csv"
SIM2_FP = SIM2_DIR / "Curva_de_Carga_GERAL_sim_2.csv"
SIM29_FP = SIM29_DIR / "Curva_de_Carga_GERAL_sim_29.csv"
OUT_FP = SIM29_DIR / "01_Integralizacao_Mensal_SIM2_x_SIM29_2025.csv"
...
for fp in [META_FP, SIM2_FP, SIM29_FP]:
    if not fp.exists():
        raise FileNotFoundError(fp)

meta = pd.read_csv(META_FP, dtype=str, encoding="utf-8-sig")
sim2 = pd.read_csv(SIM2_FP, encoding="utf-8-sig", sep=None, engine="python")
sim29 = pd.read_csv(SIM29_FP, encoding="utf-8-sig", sep=None, engine="python")

for name, dfx, req in [
    ("META", meta, ["Hora_decimal", "Mes", "Posto"]),
    ("SIM2", sim2, ["Hora_decimal", "P_3f_S", "Q_3f_S"]),
    ("SIM29", sim29, ["Hora_decimal", "P_3f_S", "Q_3f_S"]),
]:
    miss = [c for c in req if c not in dfx.columns]
    if miss:
        raise ValueError(f"Missing columns in {name}: {miss}. Available: {list(dfx.columns)}")
...
meta["k"] = make_k(meta["Hora_decimal"])
sim2["k"] = make_k(sim2["Hora_decimal"])
sim29["k"] = make_k(sim29["Hora_decimal"])

meta = meta[["k", "Mes", "Posto"]].drop_duplicates(subset=["k"], keep="first").copy()
sim2 = sim2[["k", "P_3f_S", "Q_3f_S"]].drop_duplicates(subset=["k"], keep="first").copy()
sim29 = sim29[["k", "P_3f_S", "Q_3f_S"]].drop_duplicates(subset=["k"], keep="first").copy()

df2 = meta.merge(sim2, on="k", how="left").rename(columns={"P_3f_S": "P_SIM2", "Q_3f_S": "Q_SIM2"})
df29 = meta.merge(sim29, on="k", how="left").rename(columns={"P_3f_S": "P_SIM29", "Q_3f_S": "Q_SIM29"})

df2["E_P_SIM2_kWh"] = df2["P_SIM2"] * DT_H
df2["E_Q_SIM2_kVArh"] = df2["Q_SIM2"] * DT_H
df29["E_P_SIM29_kWh"] = df29["P_SIM29"] * DT_H
df29["E_Q_SIM29_kVArh"] = df29["Q_SIM29"] * DT_H
...
agg29 = df29.groupby(group_cols, dropna=False).agg(
...
agg = agg2.merge(agg29, on=group_cols, how="outer")
meta_counts = meta.groupby(group_cols, dropna=False).agg(N_pontos_META=("Posto", "size")).reset_index()
agg = agg.merge(meta_counts, on=group_cols, how="left")
...
# %% Celula 011 - SIMULACAO 029: Tabela de custos (SIM2 vs SIM29 - 2025) com demandas contratadas diferentes
# =============================================================================
# Entrada:
#   Simulacao 029 segmentos/01_Integralizacao_Mensal_SIM2_x_SIM29_2025.csv
#
# Saidas (todas em Simulacao 029 segmentos):
#   Tabela_demanda_consumo_por_posto_SIM2_2025.csv
#   Tabela_demanda_consumo_por_posto_SIM29_2025.csv
#   Tabela_demanda_consumo_por_posto_SIM2_vs_SIM29_2025.csv   (economia)
# =============================================================================
...
IN_FP = SIM29_DIR / "01_Integralizacao_Mensal_SIM2_x_SIM29_2025.csv"
OUT_SIM2 = SIM29_DIR / "Tabela_demanda_consumo_por_posto_SIM2_2025.csv"
OUT_SIM29 = SIM29_DIR / "Tabela_demanda_consumo_por_posto_SIM29_2025.csv"
OUT_CMP = SIM29_DIR / "Tabela_demanda_consumo_por_posto_SIM2_vs_SIM29_2025.csv"

CONTR_SIM2 = {"DCFP": 5000.0, "DCP": 3000.0}
CONTR_SIM29 = {"DCFP": 4000.0, "DCP":   0.0}  # 3000 - 4*000
...
tab2 = tabela_custos(df, "SIM2", DCFP=CONTR_SIM2["DCFP"], DCP=CONTR_SIM2["DCP"])
tab29 = tabela_custos(df, "SIM29", DCFP=CONTR_SIM29["DCFP"], DCP=CONTR_SIM29["DCP"])

tab2.to_csv(OUT_SIM2, index=False, encoding="utf-8")
tab29.to_csv(OUT_SIM29, index=False, encoding="utf-8")

t2 = tab2[tab2["MES"].isin(MESES_ORD)][["MES", "R$TOTAL_MES"]].rename(columns={"R$TOTAL_MES": "R$SIM2"})
t29 = tab29[tab29["MES"].isin(MESES_ORD)][["MES", "R$TOTAL_MES"]].rename(columns={"R$TOTAL_MES": "R$SIM29"})

cmp = t2.merge(t29, on="MES", how="inner")
cmp["MES"] = pd.Categorical(cmp["MES"], categories=MESES_ORD, ordered=True)
cmp = cmp.sort_values("MES").reset_index(drop=True)

cmp["Economia_R$"] = (cmp["R$SIM2"] - cmp["R$SIM29"]).round(2)
cmp["Economia_%"] = np.where(cmp["R$SIM2"] == 0, np.nan, (cmp["Economia_R$"] / cmp["R$SIM2"] * 100).round(3))

tot = {
    "MES": "TOTAIS",
    "R$SIM2": float(cmp["R$SIM2"].sum()),
    "R$SIM29": float(cmp["R$SIM29"].sum()),
    "Economia_R$": float(cmp["Economia_R$"].sum()),
    "Economia_%": (np.nan if cmp["R$SIM2"].sum() == 0 else float(cmp["Economia_R$"].sum() / cmp["R$SIM2"].sum() * 100)),
}
cmp = pd.concat([cmp, pd.DataFrame([tot])], ignore_index=True)
...
print("DONE. Saved (Simulacao 029 segmentos):")
print(" -", OUT_SIM2)
print(" -", OUT_SIM29)
print(" -", OUT_CMP)
print("\nDemandas contratadas usadas:")
print(" - SIM2: DCFP=5000, DCP=3000")
print(" - SIM29: DCFP=4000, DCP=0")
print("\nResumo anual:")
print(cmp.tail(1))
